...
_bmad-code-review, three-layer adversarial pass (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against `origin/main...HEAD`, verified against source before triage. Final: 8 patch (all applied), 8 defer, 0 dismissed. (1 decision-needed — dead `page_position`/`nextPage` scaffolding — resolved by BigDev 2026-08-20: remove now. 1 patch re-scoped to defer on closer analysis during implementation — see the `page` upper-bound item below — resolved by BigDev 2026-08-20: defer to 11a.3.) All applied patches verified: `apps/public`/`packages/contracts`/`packages/i18n` typecheck + lint + full test suite (1150 tests) + `astro check` + live `pii-scrape` CI gate script all pass post-patch.

- [x] [Review][Patch] Dead `page_position` i18n key (en+hi) and unused `MembersLabels.nextPage` field — removed [packages/i18n/locales/en/members.json; packages/i18n/locales/hi/members.json; apps/public/src/lib/members-render.ts; apps/public/src/pages/members.astro]. **Patched.** Both keys/fields deleted; `MembersLabels` no longer carries `nextPage`. **Decision (BigDev, 2026-08-20): remove now** — house convention is no scaffolding for hypothetical future use; 11a.3 re-adds these when it actually builds a real "next" link.

- [x] [Review][Patch] `/members` throws on every single request — i18n token-format mismatch [apps/public/src/pages/members.astro:76]. `tr('invalid_request_body')` is called with no params, but the en/hi template is `"...at most {{max}} entries..."` (double-brace) while `packages/i18n/src/resolver.ts:33`'s `TOKEN` regex is single-brace `/\{(\w+)\}/g`, which matches the inner `{max}` and requires a `max` param — `t()` throws `missing interpolation param 'max'` unconditionally, before pagination is even checked. Reproduced directly: `"{{max}}".replace(TOKEN, …)` → matches `{max}`, param missing, throws. No test in the diff exercises this path — `members-render.test.ts` feeds a hand-built `MembersLabels` fixture directly, bypassing `t()` entirely, and the integration scrape spec never issues a real HTTP request to `/members`. **Patched.** Locale templates now use single-brace `{max}` (matching the house TOKEN convention); `members.astro` passes `{ max: PUBLIC_PAGE_SIZE_MAX }` to `tr(...)`; `buildMembersRejectionView` no longer does its own `.replace('{{max}}', …)` post-processing, since `t()` now does the interpolation correctly. Verified: full `apps/public`/`packages/contracts` test suites (1150 tests) + typecheck + `astro check` + lint all pass.

- [x] [Review][Patch] `/members` caches its 400-rejection response with the same public/long-TTL header as valid content [apps/public/src/pages/members.astro:89-97]. `Astro.response.status = rejection.status` is set conditionally, but `Astro.response.headers.set('Cache-Control', 'public, max-age=60, s-maxage=300')` runs unconditionally below it — a shared cache will store and replay a 400 for a malformed `?page=`/`?limit=` probe for up to 5 minutes, weakening the "the rejection is the signal" anti-scraping posture the pagination module itself documents (a repeat probe within the cache window never reaches the origin). **Patched.** The header block is now conditional: accepted requests keep the `edge_cacheable` header; rejections get `Cache-Control: no-store`. Verified the `pii-scrape` gate's cache-policy leg still passes — it does a whole-file textual scan, and the accepted-path header (which appears first in source) is what it matches.

- [x] [Review][Patch] `/blog/[postId]` 404 skips the new Cache-Control header entirely, and the new gate leg can't see it [apps/public/src/pages/blog/[postId].astro:46-60]. The `if (!post) return new Response(null, { status: 404 })` early return (line 49) happens before `Astro.response.headers.set('Cache-Control', …)` (line 59, added by this story's AC5 work). `checkCachePolicyReconciliation`/`detectCacheSignal` scan the whole file's source text for the call's presence, not per-branch, so this surface reads as compliant while its 404 path ships with no header at all — the exact "absence read as default is fine" hazard this story's own gate leg exists to catch, reintroduced on a branch the gate is structurally blind to. **Patched.** The (static, `post`-independent) header set now runs before the `if (!post)` branch, and the 404 `Response` explicitly carries `headers: Astro.response.headers` — a `new Response(...)` return does not otherwise inherit `Astro.response.headers`.

- [x] [Review][Patch] `labels.invalidLink` (rejection-flow copy, "Open the directory") is reused for the valid, non-error "back to first page" link [apps/public/src/pages/members.astro:144-146]. There is no dedicated label for this link; it only works today because the current copy happens to be generic enough. Any future edit sharpening the 400-specific wording (a plausible edit — the story explicitly discusses tuning this copy) silently corrupts an unrelated, non-error UI element. **Patched.** Added a dedicated `back_to_start` i18n key (en+hi) and `MembersLabels.backToStart`; the page-2+ link now uses it instead of `invalidLink`.

- [x] [Review][Defer] `page` has no upper bound in `parsePageParams` — only `limit` is capped [apps/public/src/lib/pagination.ts:92-159] — deferred, pre-existing (re-scoped from the original "previous-page link" framing on closer analysis: suppressing the "previous" link would contradict `members-render.test.ts`'s deliberate, passing `'page 2 offers a previous link back to page 1'` test — backward navigation always pointing at a valid earlier page is defensible design, not a bug; a "next" link is the one that asserts unverified future content, and that one already stays suppressed). `?page=999999999` is accepted (200) and renders a plausible pagination chain on a directory with zero rows. Not exploitable for real data extraction today — `hasMembers` is hard-coded `false`, no DB read uses `page`/`offset` yet. **Decision (BigDev, 2026-08-20): defer to 11a.3** — closing this needs a new decidable rejection reason (mirroring `limit_above_cap`) and a ceiling number chosen against a real row count, which 11a.3's roster query is the natural place to pick. Trigger: Story 11a.3 wiring the real roster query.

- [x] [Review][Patch] AC2's "reused, not re-decided" FR-91 cap claim is false — no real shared constant links the two surfaces [apps/public/src/lib/pagination.ts:24-35; packages/contracts/src/_common/pagination.ts:1-12]. `PUBLIC_PAGE_SIZE_MAX = 50` in `apps/public` is documented as "re-exported here rather than re-decided so the two surfaces cannot drift into two different 'the FR-91 cap'," citing `packages/contracts/src/_common/pagination.ts` — but that file exports no such constant, only an inline `.max(50)` Zod literal in `PaginationQuery`. There is no import connecting the two. The only guarding test (`pagination.test.ts: expect(PUBLIC_PAGE_SIZE_MAX).toBe(50)`) is tautological against the same hardcoded literal and would not fail if the two caps ever drifted apart — precisely the "two different FR-91 caps" failure the comment claims is prevented. **Patched.** `_common/pagination.ts` now exports `PUBLIC_SURFACE_PAGE_SIZE_CAP = 50`, used by both its own `PaginationQuery` schema and re-exported (imported, not re-declared) as `apps/public`'s `PUBLIC_PAGE_SIZE_MAX`. `pagination.test.ts` now asserts identity against the imported binding, not a second hardcoded literal.

- [x] [Review][Patch] `checkPaginationBinding`'s regex is satisfied by literal source text, including inside a comment [packages/contracts/src/public-pages/gate.ts:358-380]. `/parsePageParams\s*\(/.test(source)` runs against raw, non-comment-stripped source — a comment mentioning `parsePageParams(` (e.g. copied from another file's docstring) would satisfy the leg without the page ever calling it. The leg's own docstring discloses the "calls it but ignores the result" residual but not this one. **Patched.** Added `stripCommentsForBindingCheck` (same strip pattern already proven in `authenticated-fragment.test.ts`) and the leg now matches against comment-stripped source.

- [x] [Review][Patch] `cachePolicySatisfied('edge_cacheable', …)` never checks for `max-age`/`s-maxage` presence [packages/contracts/src/public-pages/gate.ts:270-278]. `shared = /(^|[\s,])public([\s,]|$)/.test(value) && !/no-store/.test(value)` — a page declaring `cache_policy: edge_cacheable` and literally setting `Cache-Control: public` with no TTL at all would satisfy the gate, even though the epic AC's stated intent is an actual "edge cache TTL." Not exploited today (every shipped page sets explicit `max-age`/`s-maxage`), but the check doesn't enforce it. **Patched.** Added a `hasTtl` check requiring a `max-age=`/`s-maxage=` token; verified against all 8 shipped routes via the live `pii-scrape` gate — still green (every `edge_cacheable` route already sets an explicit TTL).

- [x] [Review][Defer] `detectCacheSignal`/`cachePolicySatisfied('redirect', …)` use first-match, non-comment-stripped regexes [packages/contracts/src/public-pages/gate.ts:258-267] — deferred, pre-existing pattern in this new gate mechanism, unexploited by any page in this diff (no page uses conditional Cache-Control branches or an `Astro.redirect(` in dead code/comments today).

- [x] [Review][Defer] `page-weight.mjs` per-route attribution joins emitted chunks to routes by bare basename, not full route path [apps/public/scripts/page-weight.mjs] — deferred, pre-existing. Two routes sharing a filename in different directories (e.g. two `index.astro`) would silently collapse into one attribution. Explicitly caveated in-diff as informational/non-enforcing (D5(a)); no basename collision exists in the current route set.

- [x] [Review][Defer] Offset precision loss for extreme `page` values [apps/public/src/lib/pagination.ts:92-159] — deferred, pre-existing. `parsePageParams` allows `page` up to `Number.MAX_SAFE_INTEGER`; `offset = (page-1)*limit` can then exceed safe-integer range. Inert today — no DB read consumes `offset` yet (`hasMembers` is hard-coded `false`) — becomes live once 11a.3 wires the roster query.

- [x] [Review][Defer] `?raw` matrix-inlining (Trap 3) is proven only within the vitest/dev toolchain via a relative-fs re-read, not against a built `dist/` artifact [apps/public/tests/matrix.server.test.ts] — deferred, pre-existing. The Debug Log records the actual `dist/` build behavior as checked via a manually-run, reverted probe, not by any test that ships and runs in CI.

- [x] [Review][Defer] `AuthenticatedFragment`'s cache-safety test is a comment-stripped literal-token scan, defeatable by string concatenation, a wrapping helper function, or reading via `Astro.locals.user`/`.viewer` (only `.session`/`.member` are on the fixed token list) [apps/public/tests/authenticated-fragment.test.ts] — deferred, pre-existing. The surrounding prose's absolute claim ("for *any* request") overstates what a textual scan establishes.

- [x] [Review][Defer] AC3's specified test method (SSR output identical with/without arbitrary cookies/headers) was replaced with the static scan above, without being logged in the story's own "raised, not absorbed" deviation section [Story §"⚠ TWO THINGS RAISED"] — deferred, pre-existing. The substitution is a defensible engineering call (`.astro` files aren't unit-render-testable here) but the story's own convention calls for logging it there, not only justifying it inline in Completion Notes.

- [x] [Review][Defer] AC9's Debug Log table records one planted mutation (`outputForVerdict`'s `visible` guard removed) as covering multiple separately-enumerated detection routes at once [Story Dev Agent Record, Task 9 Debug Log] — deferred, pre-existing. The underlying `matrix.server.test.ts` unit tests ARE independently planted per case; this is specifically a Debug Log recording gap, not a test-suite gap — but it's the exact "one fixture tripping several checks" anti-pattern AC9's own language warns against.
