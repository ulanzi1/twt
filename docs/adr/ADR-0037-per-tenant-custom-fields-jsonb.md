...
- **Widen the guard so the epic's own worked example passes.** `epics.md:3603` gave "alternate ID number" as the canonical use case; it is **Tier-2** by direct analogy to §2.7's classification of the eHRMS ID. Rejected: widening means putting an un-blind-indexed, government-adjacent identifier in plaintext JSONB on the certified PII-free `members` table — a worse outcome than an unbuilt example. ⚠ **ESCALATION 2 CLOSED 2026-08-07, Decision 2026-08-07-084** — the Trustee Panel ruled to rewrite the epic's example (`epics.md:3603` now reads "cadre grade", Tier-3) rather than relax this guard. A Tier-2 blind-index-host story was drafted as a stub and deliberately not commissioned; this rejection stands, unrevisited.
...
1. **The epic's own worked example does not build.** "Alternate ID number" is Tier-2 and is refused. ESCALATION 2 owns it. ⚠ **CLOSED 2026-08-07, Decision 2026-08-07-084** — the epic's example was rewritten to "cadre grade" (Tier-3); this guard was not relaxed.
2. **The three §1.7 limit classes bind only this story's write paths.** §1.7 says they bind *"every JSONB write path … no code path bypasses them"* — they currently bind **none** of the repo's ~20 other JSONB columns. This story lands `limits.ts` as the destination and says so in that module's header rather than letting a future reader assume coverage. **ESCALATION 3.** ⚠ **CLOSED 2026-08-07, Decision 2026-08-07-084** — accepted as a named, gated deferral with a concrete re-trigger; not mechanized.
3. **§1.7's "write-rate limit when approached" on the GIN ceiling is not built.** What ships is the observed signal. ESCALATION 3. ⚠ **CLOSED 2026-08-07** — same ruling, re-trigger already concrete (unchanged).
4. **Claims and pools are unhosted.** A real FR-54 coverage gap, gated (D7).
5. **No member-facing renderer.** Values are written through the API only. **ESCALATION 5.** ⚠ **GRAMMAR GAP CLOSED 2026-08-07** — `ux-design-specification.md` §12 gained Group E, Pattern 12 (Dynamic Field Rendering), via `bmad-ux`. No renderer is built by this closure; the pattern governs whichever future surface builds one. Values remain API-only today.
