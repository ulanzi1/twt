...
- [x] **Task 7 — Tests (AC1–AC12, incl. **AC10**)** ⚠ **v1.1: the header range read `(AC1–AC10)`, which excluded **AC11** and **AC12** — the very ACs this task's own subtasks test, and AC11 is named in its next sentence. **AC10** was an ORPHAN AC: ⛔ no task tagged it, and the only `AC10` match in the task list was this range notation, ⛔ not a tag.** ⚠ **RE-OPENED by code review 2026-09-20 (chunk 4):** AC7's note has no end-to-end test, AC11's `effectiveOutcome()` arm has none at any layer, checklist family 2 (two-connection races) is unproven for every new write, the P1/P3/P4 gate matrix is mostly absent, and several ticked lines exercise the D1-reversed window or the un-built halves of Tasks 5–6. The Dev Agent Record's test counts (domain 3427 · api 1317 · contracts 1149 · events 36 · 7,046) are ⚠ stale after the D1/D3/D4 patches and are ⛔ not verified by this review (the reviewers are read-only and the specs need the live DB). (AC1–AC10) — execute on `twt-test-pg` `:5433`; "written but not run" is ⛔ not
  - ✅ **TASK 7 CLOSED 2026-09-22.** ⭐ Every `[Review][Patch]` bullet under it is ticked or explicitly scoped, and the header's own v1.1 corrections stand. ⭐⭐ **THE TEST TOTAL IS ATTESTED, per package, against `twt-test-pg` on `:5433`:** domain **3,488** (+1 skipped) · api **1,365** · contracts **1,191** · mobile **551** · admin **495** · i18n **110** · events **36** = **7,236**. ⚠ The Dev Record's `7,046` and v1.0's `7,116`/`7,122` are superseded; `7,164` was the 2026-09-22 mid-pass figure and is superseded too. ⚠⚠ **WHAT TASK 7 DID ⛔ NOT ACHIEVE, stated plainly:** ⛔ **not** every listed sub-item — bullet 918 (the admin UI halves) is **partially** closed and says which parts stand, and several bullets record their own residuals (the AI-6-3 decoy's `current_check` leg; `resource_locator` on the read line; the member-route `correctionNeeded`/`memberEditable` pairing; the VPA never-echo plaintext). ⭐ Each is named where it belongs rather than swept into a tick ([[feedback_closure_language_precision]]). ⚠ **AND THE GO-LIVE FENCE IS UNAFFECTED:** `-236` consequence 4 stands — 6.18 must ⛔ not go live until 6.20 lands or its compare source changes. Task 7 makes the story MERGEABLE, ⛔ not shippable.
      attested
...
- [x] [Review][Patch] AC8's highlight is not delivered on its three client surfaces — the District Admin console's `console-name-difference-flag` renders only `approvedWithDifference` and drops the packet's `differenceReasons`; the Pariwar Admin card prints `approved with a name difference: {reasons.join(', ')}` as hard-coded English with RAW codes (`bank_shortened_name`) where `t.nameCheck.reasons` has the labels; the R9 screen has no badge at all and shows the difference only after the Tier-1 disclosure, through the panel's `.some(verdict === 'clerical_difference')` fallback (no `passing`). Render the reason via the label table on all three; retire the R9 fallback once chunk 2's R9 flag exists. [auditor+edge+blind]
  - ⚠⚠ **CORRECTION 2026-09-22 (v1.8) — this box was ticked with its R9 THIRD UN-BUILT, and is now TRUE.** The DA console and the Pariwar Admin card were fixed; the **R9 screen was ⛔ not**. Chunk 2's API half landed (`name_difference_reasons` on BOTH R9 reads — `getQueue` and `getPanel`), but ⛔ **nothing in `apps/admin` rendered it** (`grep name_difference_reasons apps/admin/src` → `PendingCaseCard.tsx` only). So an R9 voter could learn an accepted difference ONLY by opening the Tier-1 names disclosure — the exact opposite of AC8's *"rides those surfaces' own reads"*. ⭐ Found by WRITING bullet 929's R9 test, ⛔ not by reading: the test went red. ⭐ **Now built:** `R9CasePanel` renders `r9-name-difference-flag` (both the open-session and the open-form views) and `R9VotingPage` badges each queue row (`r9-queue-name-difference-<id>`), through the SAME label table (`t.nameCheck.reasons`, code fallback, ⛔ never `undefined`). Tests: `apps/admin/tests/r9-case-panel.test.tsx`, red first (4 failing), then green; each asserts ⛔ no names fetch. ⛔ The tick is left as it was and this note is appended rather than the bullet rewritten ([[feedback_closure_language_precision]]).
...
- [x] [Review][Patch] Tidy — `hooks.ts`'s `/** POST an approve / deny / escalate decision … */` doc-block now sits above `useRecordHelplineNomineeBank` (`usePostVerifierDecision` lost its own); claim-verification `i18n-en.ts`'s `// Story 6.11 — reason-code display labels` now precedes the new `nameCheck` block; `BankDetailsCard` hand-copies `IFSC_RE`/`ACCOUNT_RE` though `NOMINEE_BANK_IFSC_REGEX` is exported from contracts (the account-number regex is not — export it); the mobile note hard-codes `500` twice where the admin card imports `NAME_DIFFERENCE_NOTE_MAX_CHARS`; and `VerifierConsoleRoute` needs a branch for chunk 2's `unavailable` state or a transient error still reads "bank details missing". [blind+edge+auditor]
  - ✅ **CLOSED 2026-09-22 — and ⛔ not all of it was tidy.** ⭐ **The duplicated validation regexes were a DRIFT HAZARD, ⛔ not duplication as a style problem:** `BankDetailsCard` hand-copied both `IFSC_RE` and `ACCOUNT_RE`. `NOMINEE_BANK_IFSC_REGEX` was already a wire constant it ignored; the account one was module-private in contracts and is now exported as `NOMINEE_BANK_ACCOUNT_NUMBER_REGEX`. ⚠ The day the boundary widens or narrows the range, a copied rule makes the console refuse numbers the server accepts — or accept numbers it will reject after the operator has typed everything on a bereavement call. Same shape for the mobile note, which hard-coded `500` **twice** where `NAME_DIFFERENCE_NOTE_MAX_CHARS` exists: a change at the boundary would have **truncated a family's note client-side, at the old length, before the server ever saw it**. ⭐ The two misplaced comments are moved to the things they describe (the Story 6.11 hooks banner sat above the 6.18 hooks; the `reasonCodes` label comment sat above the `nameCheck` block). ⚠ **The `unavailable` item was already done** — `VerifierConsoleRoute` branches on `!available` in BOTH `approveBlockedReason` and the badge, so a transient failure no longer reads as *"bank details missing"*; re-derived at source, ⛔ not taken on trust.
...
- [x] [Review][Patch] Tests that PIN a defect must be rewritten WITH the fix, not after — (a) `nominee-name-check-return-loop.spec.ts` "a claim under correction is EXCLUDED from the commit set" drives the claim to `state_trustee_approved` and calls `returnToDistrictAdmin` — under D1 that throws `ClaimNotReturnableError`, and the commit exclusion is then testable only from a reachable setup (a return at `state_trustee_freeze`); it also has NO positive control (nothing shows an identical claim WITHOUT a return row IS committed, so it would pass if the commit did nothing at all). Add the D1 pin (a return from `state_trustee_approved` is refused) and the rest of the returnable matrix — only `verifier_approved` and `state_trustee_freeze` are exercised today, not `reversed`, `denied` or `verifier_review`. (b) the AC9 sentinel test asserts the exact payload key set `[accounts, actor, from_state, nominee_declaration_token, to_state, trigger]`, `checkedBase` in the events test has no display field, and `seedNomineeNameCheck` (`_helpers.ts`, ~8 callers) passes `actorId` but no display — D3-A breaks all three; nothing reads a recorded check back and asserts `checkedByActorDisplay !== ''`, so the `?? ''` defect is neither pinned nor covered, and "a missing display name blocks the check" has no domain leg. (c) the AC8 read tests use a passing check only — add the mixed `[clerical_difference, does_not_match]` case, a stale check, `underCorrection` `true` after a return and `false` after resubmission (it is asserted only as `false`, once), so chunk 1's P3 and chunk 2's unified-predicate patch have something under them. [edge+auditor]
  - ✅ **CLOSED 2026-09-22 — (a) and (b); (c) was already covered.** **(a)** the D1 pin and a reachable commit-exclusion setup landed in an earlier pass; what remained was the **returnable MATRIX**, and ⛔ only the two states that ALLOW a return were ever exercised — ⭐ a window tested ⛔ only where it is open says ⛔ nothing about where it must be shut. Added `reversed` (the third member, previously untested — an appeal reversal puts the claim back in front of the Pariwar Admin, so leaving it out would make a reversal a one-way door). ⚠⚠ **AND I FOUND I HAD DUPLICATED AN EXISTING TEST:** a `verifier_review`/`denied` refusal case already existed. ⭐ I deleted my duplicate rather than leave two tests asserting the same thing, and folded in the one thing mine added — ⭐ that ⛔ **NOTHING was written**, the half a throw-assertion alone ⛔ cannot see: a refusal that still opened a row would strand the claim under a correction record nothing can clear. **(b)** the DOMAIN leg now reads a recorded check BACK and asserts `checkedByActorDisplay` is the named human **and** non-empty as its own assertion — ⚠ the `?? ''` defect produced exactly that value, and ⛔ nothing that never reads the field back can see it. **(c)** the mixed / stale / `underCorrection` AC8 variants are covered by the panel residual (bullet below) and the gate matrix (802). **PROVEN:** widening `TRUSTEE_RETURNABLE_STATES` fails three tests; narrowing it so `reversed` drops out fails the new one.
...
- [x] [Review][Patch] The AC4/AC6 gate matrix is mostly absent, and the P4 test overclaims — Task 7 ticks *"P1/P3/P4 refused without a current passing check and without two accounts"*. Present: P1 no-check + `does_not_match`; P3 stale only; P4 accounts-deleted only. Absent: P1 with no accounts (AC6's *"a claim filed without them waits"*); P3 with no check / no accounts / `does_not_match`; P4 with no check, stale or `does_not_match`; the ONE-account case at any gate and at `recordNomineeNameCheck` (the AC3 test uses `accounts: []`, which fails earlier on "no live accounts"). The P4 test's `expect(claimState).not.toBe('state_trustee_approved')` passes for any other state (assert equality with the prior state), and its promised *"no orphaned session outcome, no metadata row"* is never queried (read `claim_r9_voting_sessions.outcome` and the trustee-decision rows). Pin the two spec'd non-gates — `resolveEscalation` not gated, `commitCycleFreeze` carrying no check — and, for `-226` cl.5's *"no … escalation"*, that a recorded `does_not_match` mints no `claim.verifier_escalated` (only denial is pinned). [auditor+edge+blind]
  - ✅ **CLOSED 2026-09-22 — the tick said *"P1/P3/P4 refused without a current passing check and without two accounts"* and the truth was THREE CELLS OF FIFTEEN.** ⭐ Now **TABLE-DRIVEN**, so a missing cell is a missing ROW rather than a test nobody happened to write: five deficiencies (no check · no accounts · **ONE account** · stale · `does_not_match`) × P1 and P3, each asserting the TYPED error **and** the claim state as **EQUALITY** with the prior state. ⚠ The one-account case had ⛔ no test at any gate — the existing `accounts: []` test fails EARLIER, on a different guard, so cl.7's actual rule was never reached. ⭐ **P4 gained three cells** (never-checked, stale, `does_not_match`) — and P4 is the ⛔ only gate catching an APPEAL REVERSAL or a D5 post-approval correction, so the untested cells were precisely the ones it exists for. ⚠ **Its `not.toBe('state_trustee_approved')` is now EQUALITY** (the old form passes for a claim that moved anywhere else), and its promised *"no orphaned session outcome, no metadata row"* — ⛔ never queried — now reads `claim_r9_voting_sessions.outcome` and the trustee-decision phases. ⭐⭐ **THE TWO SPEC'D NON-GATES ARE PINNED AS DELIBERATE ABSENCES:** `resolveEscalation` ungated (with the gate proven to still bite DOWNSTREAM, which is what makes the absence safe) and `commitCycleFreeze` carrying ⛔ no check of its own, with a STALE check between vote and commit showing the window is covered by staleness and ⛔ not by re-checking. ⭐ And cl.5's third limb: a `does_not_match` mints ⛔ **no** `claim.verifier_escalated` — ⛔ only the denial limb was pinned before. **PROVEN:** removing the two-account guard fails exactly the 4 accounts cells; disabling P4 fails exactly its 4; ADDING the gate to `resolveEscalation` — the mistake a future author reading *"every approving path"* would make — fails the non-gate test.
- [x] [Review][Patch] Domain-level coherence, distinct ranks and the return's note are unpinned — pairs with chunk 1's domain-revalidation patch, which needs its tests: the events test only ever accepts well-formed payloads (never `clerical_difference` with a null reason, or `matches` with a reason), "requires EXACTLY two" tests only length 1 (`[rank 1, rank 1]` is never tried), and `returnToDistrictAdmin` is tested only with `reasonCode: null`, never `other` + a null `rationaleCiphertext`. No contracts unit test exists for `RecordNomineeNameCheckRequest`'s `superRefine`/strictness — the "one load-bearing control" (cl.5) is tested only over HTTP in chunk 5. [edge+auditor]
  - ✅ **CLOSED 2026-09-22 — one premise stale, one gap REAL and now recorded.** ⚠ *"No contracts unit test exists for `RecordNomineeNameCheckRequest`"* is **STALE**: `packages/contracts/tests/nominee-name-check-request.test.ts` has **ten** cases covering the `superRefine` and strictness. ⭐ The genuine gap was the **EVENT PAYLOAD** schema, which is a DIFFERENT schema from the request and is what is written to `events_log` FOREVER — a payload only the HTTP boundary refuses is one a direct domain caller, a backfill or a future second writer can still persist. Added: `clerical_difference` with a NULL reason, a reason on **both** `matches` and `does_not_match`, and ⭐ **DUPLICATE ranks** — the old *"requires EXACTLY two"* tried ⛔ only LENGTH 1, and `[rank 1, rank 1]` has length two while being two verdicts about one account and ⛔ none about the other. ⚠⚠ **AND THE RETURN-NOTE TEST FOUND A REAL GAP I EXPECTED TO PASS:** `returnToDistrictAdmin` accepts a return with ⛔ **NO** note. `-227` cl.10 sends a claim back *"with Note"* and the CONTRACT enforces it (its own comment cites cl.10) — the DOMAIN writer ⛔ never re-checks the rationale. ⇒ UNREACHABLE over HTTP today, so ⛔ not a live defect, but defence-in-depth that is missing; **current behaviour is PINNED in a named test** (which also asserts the stored rationale is really NULL, ⛔ not an invented empty string) and the gap is **recorded in `deferred-work.md`** ([[feedback_trace_reachability_before_escalating]]). ⭐ I did ⛔ **not** change the domain writer's validation unilaterally.
...
- [x] [Review][Patch] AC7's note has no end-to-end test, and the contract limits are untested — every writer fixture passes `nameDifferenceNoteCiphertext: null`, the AC9 test inserts the note by direct `insert`, the column test proves only Drizzle metadata (4 tests; never the DB), so no test writes a non-null note through the writer and reads it back via AC2; `nameDifferenceNote` `.max(500)` / `.trim().min(1)` (`nominee-bank.ts:76`) is untested; and `correctionNeeded` is claimed REQUIRED but every parse just adds `correctionNeeded: false` — nothing parses without it (making it optional keeps the suite green). `nomineeNameCheck` in the verifier-console fixture likewise has no absence/strictness test for *"carries no name and no note"*. [auditor+edge+blind]
  - ✅ **CLOSED 2026-09-22 — both claims were made and ⛔ NEITHER was tested.** `nameDifferenceNote` carries `.trim().min(1).max(500)` and ⛔ nothing exercised any of the three. Now: OPTIONAL (absence first-class — `-226` cl.2 PERMITS a note, it does ⛔ not oblige one); ⛔ **BLANK REFUSED** — ⭐ absent and blank are ⛔ NOT the same, and a blank would show the District Admin an empty note block where an explanation should be, which reads as an explanation WITHHELD; the 500 boundary asserted on **BOTH** sides; and TRIMMED, read off the parsed OUTPUT — ⚠ a `.max()` without a `.trim()` would fail 500 characters plus the trailing newline a mobile keyboard adds for free. ⭐⭐ **And `correctionNeeded` — claimed REQUIRED, never asserted:** every parse supplied it, so `.optional()` kept the suite green. ⚠ It is what the FILER-FACING banner reads: a response that could omit it leaves a client rendering `undefined` as *"nothing to correct"* — telling a family their details are fine when the District Admin has said they are not. `memberEditable` is pinned as required too (conflating the pair produced a cruel screen once already). ⚠ **My first draft asserted against `RecordNomineeBankResponse`** — the WRITE echo, which carries ⛔ no such field; re-derived at source and corrected to `NomineeBankStatusResponse`. **PROVEN:** dropping `.trim().min(1)` fails two; making `correctionNeeded` optional fails the third.
- [x] [Review][Patch] Two assertions cannot fail, and the AC9 title claims more than it scans — `expect(dump).not.toContain('Asha')` (AC8 read test): no fixture anywhere plants "Asha" (the seeded ciphertexts are `enc:v1:holder-N`), so that half proves nothing (plant it, or delete it); the AC9 sentinel test says *"both names and a note"* but plants a holder name, a note, an account number and an IFSC as raw ciphertext-column strings — ⛔ no nominee name (no `member_nominees` rows; `tokenFor` runs over an empty declaration) — and the describe title says *"no log, event, audit line or error body"* while only `events_log` is scanned; because the writer never decrypts, it can only prove ciphertext strings are not copied into the stream. Plant the nominee name, retitle to what it scans, and leave the decrypted-plaintext / log / audit / error-body legs to a chunk-5 API test with real plaintext (model: `nominee-bank.spec.ts:150-160`). [blind+auditor+edge]
  - ✅ **CLOSED 2026-09-22.** The `'Asha'` half and the nominee-name plant landed in earlier passes; what remained was the **TITLE**, which claimed more than the body scans. It read *"reach NO log, event, audit line or error body"* while the test queries ⛔ only `events_log` — and ⭐ this layer ⛔ **NEVER DECRYPTS**, so what it can prove is that stored CIPHERTEXT strings are not copied into the stream, ⛔ not that a decrypted plaintext stays out of a log, an audit line or an error body. Those are three different claims about three different sinks. ⇒ re-titled to *"reach no EVENT PAYLOAD (this file scans `events_log`)"*, with the decrypted-plaintext legs NAMED where they now live — the API AC9 tests with SEVEN real envelopes. ⭐ Between them the claim the old title made is covered; ⛔ neither file makes it alone.
...
- [x] [Review][Patch] Migration-level assertions for 0116–0118 are thinner than family 5 — no new table, so RLS/FK/partial-unique are unchanged, but nothing asserts the new column via `information_schema`, nor that `correction_return` and `returned_for_correction` exist in `pg_enum` (they are exercised only implicitly by the return-loop inserts, and the live specs self-skip without `DATABASE_URL`). Add one live schema assertion for the column + both enum labels; the phase↔outcome pairing stays the recorded chunk-1 defer. [edge+auditor]
  - ✅ **CLOSED 2026-09-22 — asserted against the CATALOG, ⛔ never inferred.** `information_schema` for the 0116 column (TEXT, and ⭐ **NULLABLE**, which is the RULING — `-226` cl.2 permits a note and does ⛔ not oblige one; a NOT NULL column would have forced every filer to explain a difference they may not have, on a death claim), and `pg_enum` for BOTH labels on the **RIGHT TYPES** — `correction_return` on the PHASE enum and `returned_for_correction` on the OUTCOME enum. ⚠ They are different concepts: adding one without the other leaves the return half-writable. ⭐ Named types, ⛔ not "some enum somewhere": a label on the wrong type passes a label-existence check and fails at the one write that uses it. ⚠ Enum labels ⛔ cannot be removed in Postgres, and these are written ⛔ only on the Pariwar Admin's return — the least travelled path in the story, and the one a bereaved family is waiting on. Plus a ⛔ **NON-VACUITY** test: a column and a label that do ⛔ not exist must come back EMPTY, so a malformed catalog query ⛔ cannot pass for the wrong reason.
...
- [x] [Review][Patch] ⚠⚠ **v1.1 — UN-TICKED; only the P3 third landed.** ✅ `cycle-freeze.spec.ts` has the
  - ✅ **CLOSED 2026-09-22 — all three ZERO-greps answered.** **(1)** the opt-out is threaded through `verifier-decision.spec.ts`'s `seedClaim` (`'passing' | 'accounts_only' | 'none'`), so P1's two refusal states are reachable at last — and both 409 CODES are pinned: `verifier_decision.nominee_name_check_required` and `verifier_decision.bank_details_required`, as **DISTINCT** codes, ⛔ not a shared *"cannot approve"*. ⭐ What was untested is the **TRANSLATION**: the domain refuses these cases (802 covers that), but an unmapped error reaches the console as *"something went wrong"*, which trains an operator to read a RULING as a glitch. ⭐ With a ⛔ non-vacuity leg — a DENY on the SAME un-approvable claim still succeeds (cl.6/cl.7). ⚠ **Two real requirements I hit on the way:** the route needs a DISPLAY NAME (D3) or it answers `admin.display_name_missing` and the 409 under test is ⛔ never reached, and every deny needs a RATIONALE. **(2)** the console packet's nominee section — `accountsComplete`, `currentAndPassing`, `differenceReasons` — now has three tests where `grep` returned ZERO. ⭐⭐ Including the case the FIELD'S OWN DOC singles out: a MIXED `[clerical_difference, does_not_match]` yields `[]`, because a section that reported the clerical reason there would let the console render the **REASSURING** *"approved with a name difference"* badge on a claim that must be SENT BACK — one payout destination belonging to somebody else, shown as a tidy footnote. Plus ⛔ NO name anywhere in the section (Trap 4, on the packet every console load carries). **PROVEN:** forcing `passing = true` fails the mixed case. ⛔ **NOT closed:** the AI-6-3 decoy was ⛔ not extended to `current_check`.
...
- [x] [Review][Patch] **The AC9 HTTP leg is vacuous, and the AC5 filer test asserts against rows the fixture deleted** — "the audited read line carries NO name and NO note" seeds through `seedNomineeNameCheck`, whose accounts hold `'enc:v1:holder-N'` — ⛔ not decryptable envelopes, so the GET decrypts to `unreadable`, no plaintext name ever exists, and `not.toContain('holder-1')` / `'enc:v1'` cannot fail (an audit sink logging the decrypted holder name, nominee name or note would still pass). No note is planted anywhere in `apps/api/tests`; it resets `td.auditSink.events` before the GET only and never scans the POST's lines (`admin_claim.nominee_name_checked` / `_rejected`), a 4xx/409 body, or captured logs. The nominee-bank AC5 test (`nominee-bank.spec.ts`) POSTs `'Asha Devi'` / `'Ravi Kumar'`, then `seedNomineeNameCheck` DELETEs and re-INSERTs the accounts as `enc:v1:holder-N`, then asserts `not.toContain('Asha')` — the names are gone before the assertion; its body carries one boolean, so the `denied`/`rejected` substring checks are near-tautologies; and nothing asserts the claim's state after `does_not_match`, so *"never denied"* is unproven. The Dev Record's *"REAL seeded plaintexts rather than vacuously"* holds for account number `999888777666`, IFSC `SBIN0009999` and mobile `9876543210` — ⛔ not VPA (never set; `vpa_ciphertext` is absent from the INSERT) and ⛔ not address (never planted): `not.toContain('vpa')` / `'address'` are key-name checks. Model: `nominee-bank.spec.ts:150-160`. Plant decryptable holder + nominee names, a note, a VPA and an address; scan every response body, error body, audit line and log. [auditor+edge+blind]
  - ✅ **CLOSED 2026-09-22 — the test was vacuous in the exact way described, and its closure note overclaimed it.** ⭐ The fixture seeded the literal `'enc:v1:holder-N'`, ⛔ not decryptable envelopes ⇒ the GET decrypted to `unreadable`, **⛔ no plaintext name ever existed in the process**, and `not.toContain('holder-1')` / `'enc:v1'` ⛔ cannot fail — an audit sink logging the decrypted holder name, nominee name or note would have PASSED it. ⚠ The `vpa` and `address` legs were worse: ⛔ neither column was ever populated, so both were **KEY-NAME checks wearing a plaintext check's clothes**. ⭐ **Rewritten with SEVEN real envelopes the handler genuinely decrypts** — distinctive strings that ⛔ cannot appear by coincidence — and a ⭐⭐ **non-vacuity block asserting the read really DID decrypt them** before asserting the audit trail carries none. ⭐ **A SECOND test scans the WRITE path's own audit lines**, which the old one ⛔ never did (it reset the sink before the GET only) — and the write is the call carrying a HUMAN JUDGEMENT about those names, so it is the likelier place to log them *"for context"*; it also asserts the line DOES carry the non-PII reason code, since a line recording nothing would pass everything else. **PROVEN:** adding a holder name to the read's audit context fails it by name. ⛔ **NOT closed:** the `nominee-bank.spec.ts` AC5 half (the fixture that deletes the rows it later asserts against).
- [x] [Review][Patch] AC7's note has no end-to-end API test — `grep nameDifferenceNote|name_difference_note` in `apps/api/tests` matches only `nameDifferenceNoteCiphertext: null` seeds (`nominee-accounts.spec.ts`, `suspended-member-reachability.spec.ts`); `seedAccountsWithNames` never sets the column. Nothing exercises: a helpline bank POST with a note → the stored ciphertext → the AC2 read returning `name_difference_note` → an `unreadable` note. The handler's note-decrypt path is unexercised at every layer (the admin card/panel tests are against mocks). [edge+auditor]
  - ✅ **CLOSED 2026-09-22, and the path was unexercised at EVERY layer exactly as the bullet said.** ⭐ The WRITE half is in `nominee-bank-helpline.spec.ts`: a helpline POST carrying a note → stored as **ciphertext** (asserted ⛔ NOT plaintext) → **per-account**, with rank 2 NULL → and ⛔ **not** in the audit trail. ⭐ The READ half is in `nominee-name-check.spec.ts`: the note comes **BACK** on the AC2 read as `readable`, rank 2 reads `null` (⛔ not an empty-string "readable"), plus an **UNREADABLE** case proving a corrupt envelope is its own state — ⛔ never a blank, ⛔ never a crash, and ⭐ the holder name on the SAME ROW still reads, so one bad field ⛔ does not poison the row. ⚠ **WHY IT IS SPLIT, recorded rather than glossed:** the helpline spec's `seedClaimAt` creates a claim against a bare member UUID with ⛔ no `members` row and ⛔ no posting, so the AC2 read's district target is `null` and it 403s for EVERY role — the carried `deferred-work.md` residual. ⛔ Neither file claims to cover the path alone; each names the other. `seedAccountsWithNames` gained an optional `note1`. **PROVEN:** dropping the note in the handler fails BOTH read tests by name.
- [x] [Review][Patch] Attribution and audit pairing are unpinned over HTTP (checklist family 8; pairs with D3-A and chunk 2's audit patches) — the fixture passes `actorId: randomUUID(), actor: 'operator'` and no display name; the local `authenticate()` in `nominee-name-check.spec.ts` creates the account with email and password only (⚠ inference: so the 201 happy path returns `''` attribution today, and blocking on a missing name will turn it into an error); no test asserts a non-empty `checked_by_actor_display` on GET after POST, "a missing display name blocks the check", `admin_claim.nominee_name_checked` / `_rejected` existing, their ordering relative to commit, or `resource_locator` on the read line (it asserts only `lines.length >= 1`). Copy the model: `verifier-decision.spec.ts:711,732` ("NULL display_name BLOCKS every verb", "snapshot survives a rename"). ⚠ The 201 test cannot tell whether the POST wrote anything: `seedNomineeNameCheck` first records a passing `matches`, then the POST records `matches` and asserts `passing === true` — a no-op or a returned pre-existing check satisfies every assertion; use a claim with no seeded check, a distinct verdict (`clerical_difference` + reason), and assert a new event and a persisted re-GET. When D3-A lands, this fixture, the events-test `checkedBase` and `_helpers.seedNomineeNameCheck` break together. [auditor+edge+blind]
  - ✅ **CLOSED 2026-09-22.** ⚠ **One premise was already stale:** `authenticate()` in this spec takes a display name and defaults to one, so the *"returns `''` today"* inference no longer held — re-derived at source, ⛔ not taken on trust. ⭐⭐ **THE 201 HAPPY PATH ⛔ COULD NOT TELL WHETHER THE POST WROTE ANYTHING**, exactly as described: it seeded a PASSING `matches`, POSTed `matches`, then asserted `passing === true` ⇒ a no-op handler, or one returning the pre-existing check, satisfied every assertion. ⭐ Replaced with a claim carrying ⛔ **no** seeded check (asserted null first), a **DISTINCT** verdict (`clerical_difference` + `married_name`), a persisted **re-GET**, and **exactly ONE** `claim.nominee_name_checked` event — ⛔ not zero (a no-op) and ⛔ not two (a double write). ⭐⭐ **And D3's attribution is pinned both ways:** the recorded check carries the NAMED human's display string, and an admin with ⛔ **no** display name is refused at the boundary with the exact `409 admin.display_name_missing` — ⚠ pinned as the EXACT code rather than a set of plausible statuses, because a status-set assertion would have passed for a 403 from the permission chain, a completely different failure that also happens to be a refusal — with ⛔ **nothing** written. ⛔ **NOT closed:** the `resource_locator` assertion on the read line, and the audit lines' ordering relative to commit.
- [x] [Review][Patch] AC3's boundary is half-tested over HTTP — present: `clerical_difference` without a reason → 400, a reason on `matches` → 400, a 201 for a `district_admin`. Absent: 409 stale declaration token, 409 stale `account_updated_at`, 409 one account, 409 wrong state, `does_not_match` → 201 with the state unmoved, duplicate ranks → 400, and two concurrent POSTs (family 2 — chunk 4's gap, again at HTTP); the second 400 test asserts the status only, not the error code; chunk 2's `ClaimStreamConcurrencyError` → 500 is unpinned; both 400 tests call `seedNomineeNameCheck` first, so the 400 path is never shown to fire before the 409 path. [auditor+edge]
  - ✅ **CLOSED 2026-09-22 — ⭐ including the VACUITY, which was the sharper half.** Added: **409 stale declaration token**, **409 stale `account_updated_at`** (the D5 chain over HTTP), **409 no accounts** — ⚠ the ⛔ ONLY POST test in the file that does ⛔ not seed first, which is why the accounts-required guard had ⛔ never been the thing under test — **400 duplicate ranks**, and ⭐⭐ **`does_not_match` → 201 with the claim state UNMOVED**, re-read fresh, which is `-226` cl.5 asserted over HTTP: a 4xx there would be the system ACTING on a mismatch. ⭐⭐ **THE ORDERING PROOF the two old 400 tests ⛔ cannot give:** both conditions held AT ONCE (a stale token AND a clerical difference with ⛔ no reason) ⇒ the answer must be the **400**, with a ⛔ non-vacuity leg showing the same stale token alone really does give 409. ⭐ A shared `assertCode` now pins the **CODE**, ⛔ not the bare status. ⚠ **MY FIRST DRAFT READ THE WRONG ENVELOPE** — `body.code` is `undefined` for every response (it is `{ error: { code } }`) — so had I written `toBeDefined()` it would have passed uniformly; the codes and the shape are read off real responses and that is recorded in the helper. ⛔ **NOT closed:** the two concurrent POSTs (family 2 at HTTP) and `ClaimStreamConcurrencyError` → 500.
- [x] [Review][Patch] The helpline correction is still unproven over HTTP — chunk 2's HIGH (the handler's `editable` pre-check) has no test that detects it or its fix: `nominee-bank-helpline.spec.ts:271` (`state_trustee_freeze`, no return) and `nominee-bank.spec.ts:439` (member route, `verifier_approved`, no return) assert `nominee_bank.not_collectable` where refusal is correct both TODAY and AFTER the fix, so they are ⛔ not pins. No API test seeds `reversed` (the helpline spec's `SeedTarget` is only `intake_converged | verifier_approved | state_trustee_freeze`); none combines a live return, or a `does_not_match`, with a helpline POST; none shows the member POST refused while `correctionNeeded` is true (chunk 3's banner patch); none covers the return-row half of `correctionNeeded` or its clearing after correction + a matching re-check. Add all, and `reversed` to the driver. [edge+auditor]
  - ✅ **CLOSED 2026-09-22 — and the review's diagnosis was exactly right: the two existing tests are ⛔ NOT PINS.** Both assert `nominee_bank.not_collectable` where refusal is correct **before AND after** chunk 2's fix — ⭐ a test that passes on both sides of the change it guards is ⛔ not guarding it. **The pin is the POSITIVE case**, and there are now three: a LIVE RETURN unlocks the helpline correction at `state_trustee_freeze` **and** at `reversed`, and the DA's `does_not_match` **ALONE** unlocks it — ⚠ the second half of the `underCorrection` DISJUNCTION, which had ⛔ nothing over HTTP and is AC5's own worked example. ⭐ Each carries a NEGATIVE CONTROL first (the same call, same state, refused before the governance record exists), so a success ⛔ cannot be explained by the state having been writable all along. ⭐ **`reversed` added to the seed driver** — ⛔ no API test could reach it, and it is one of the two states the exception exists FOR; the route runs through a real denial and an appeal reversal (⚠ `claim.state_trustee_denied`'s payload is `.strict()` and carries ⛔ no `reason_code` — the reason lives on the decision ROW). ⭐ And the STATUS read now reports `correctionNeeded` from **BOTH** halves, with a ⛔ non-vacuity leg showing it is FALSE first. **PROVEN:** reverting the third branch to its pre-6.18 shape fails all three positive pins — ⚠ while the two pre-existing refusal tests stay GREEN, which is the review's point demonstrated. ⛔ **NOT closed:** *"the member POST refused while `correctionNeeded` is true"* — `memberEditable` lives on the MEMBER route, ⛔ not this one, so it belongs with `nominee-bank.spec.ts`; the scope is stated in the test rather than overclaimed.
- [x] [Review][Patch] AC12 has no HTTP test and the mobile client legs have no test at all — no API test refuses a Devanagari name on any of the four bound routes (member bank, helpline bank + correction, nominee declare, life-events nominee change), and none accepts a hyphenated/initialled name (the admin card refuses Devanagari in the CLIENT and never reaches the server); the three other output schemas the regex comment says must stay ungated are never fed Devanagari. `git status` shows no `apps/mobile/tests` change while `nominee-review.tsx` and `NomineeForm.tsx` changed, and the Dev Record reports 541 mobile tests unchanged across v0.8 and v0.9: the inline "please enter in English" message, the note input and the `correction_needed` banner are untested, and chunk 3's real-`t()` leg for `nominee.bank.holder_english`, `.note`, `.note_help`, `.correction_needed` and `nominees.name_english` does not exist — the stub rule is MET for admin and UNMET for mobile ([[feedback_stub_must_call_not_transcribe]]); the `i18n-parity` CI script checks only non-empty `hi`, not that the keys are used. [edge+auditor]
  - ✅ **CLOSED 2026-09-22.** ⭐⭐ **THE SERVER-SIDE RULE WAS ASSERTED ⛔ NOWHERE** — the admin card refuses Devanagari in the CLIENT and so ⛔ never reaches the server, which means a client-only guard was the ⛔ only thing tested, and a client-only guard is a guard a SECOND client does ⛔ not have. Now: a Devanagari holder name is a **400 at the boundary** with ⛔ nothing persisted. ⭐⭐ **And the half that matters MORE had ⛔ no test either:** the gate ACCEPTS `A. Devi`, `Mary-Anne Fernandes`, `Priya D'Souza`, `Ravi Kumar Singh` — ⚠ a gate that merely REFUSES is easy; one that rejected a real bank account's name would trap a helpline operator in a loop they ⛔ cannot escape, on a call with a bereaved family, over a name that is CORRECT. ⭐ **And the distinction cl.9 actually draws is pinned:** the NOTE is ⛔ **NOT** script-gated — the holder name must be English because it matches a bank record, but the note is prose ABOUT that name, and demanding English there would mean a grieving family ⛔ cannot explain themselves at all. ⚠ The two fields sit in the same payload, so a future edit applying `EnglishScriptName` "consistently" to both would pass every other test in the file. ⭐⭐ **THE MOBILE `t()` LEG NOW EXISTS** (`nominee-name-copy-resolves.test.ts`): the five `claim` keys and `nominees.name_english` resolve through the **REAL** `t()` in BOTH locales — ⛔ never transcribed, because a hard-coded expected STRING is a SECOND COPY of the copy and passes when the key is deleted. Plus ⭐ the two locales must DIFFER (`i18n-parity` accepts any non-empty `hi`, so an untranslated copy-paste passes it) and ⭐⭐ the SCREEN must still ASK for each key — ⚠ without that, deleting a `t()` call leaves every resolution assertion green. **PROVEN both ways:** removing a key from `hi/claim.json` fails the resolution legs; removing the `t()` CALL while the key still resolves fails the drift leg. **PROVEN at the boundary:** dropping `EnglishScriptName` from the contract fails the 400 test.
- [x] [Review][Patch] Three admin test files PIN defects chunks 1–3 ordered fixed — write them WITH the fixes or the current tests fail or lock the defect in: `pending-case-card-return.test.tsx` asserts the Return button exists at `state_trustee_approved` in `voted_pending_commit` (the D1 dead end; a guaranteed 409 after D1), asserts `expect(badge).toContain('married_name')` (the raw code — the fix renders the label; the panel test asserts "A married name", so the two surfaces disagree), has NO negative for a bare Return click, and its comment *"the card never receives a name … this pins that"* precedes `not.toMatch(/Devi|Asha|Kumar/)` — the test itself says the input has no names, so it cannot fail; `helpline-bank-details.test.tsx` drives `recorded` as a controlled prop and asserts the form is GONE once recorded (`queryByTestId('helpline-bank-submit')` is null) — which pins the "no re-entry after the names view" defect D4-A ordered fixed, and cannot see D4's "derive `recorded` from a server read"; `nominee-name-check-panel.test.tsx` pins `name-check-none` (the never-checked vs stale conflation), uses `passing: true` only for the AC8 flag (no mixed `[clerical_difference, does_not_match]`, no stale check), uses ONE testid `name-check-resubmitted` for two states, and never asserts the `does_not_match` approve-blocked reason. Props (`canCheck`, `under_correction`, `resubmitted`, `recorded`) are all hand-set, so none of the real derivations is tested; `onSubmit` is `mockResolvedValue(undefined)` — no rejected submit, double submit or post-submit state; the "no hint / no score / no pairing" checks are word blacklists visibly tuned around the verdict `<select>` labels — a highlight, colour class, `aria-label`, `data-*` attribute or icon that pairs a mismatching account with a nominee passes every one; add a structural check (no element carries both a nominee testid and an account testid, no highlight class). [edge+auditor+blind]
  - ✅ **CLOSED 2026-09-22 — the ONE residual file v1.1 named.** ⭐⭐ **The MIXED `[clerical_difference, does_not_match]` case is the one most likely to be got wrong and it had ⛔ no test:** one account is an accepted clerical difference, the other does ⛔ not match at all. A panel that read *"there IS an approved difference"* and rendered the reassuring state would tell a District Admin the claim is fine when one of the two payout destinations belongs to SOMEBODY ELSE. ⭐ `-226` cl.6 — a `does_not_match` on ANY account sends the claim back, and it is ⛔ never outweighed by the other account being fine. ⭐ **The STALE state**, with its non-vacuity pair — ⚠ and my first draft had the WIRE SHAPE WRONG: `current_check` and `latest_check_is_stale` are **MUTUALLY EXCLUSIVE**, ⛔ not two flags on one record; re-derived at source. The distinction it guards: saying *"no check has been recorded"* for a stale one **ERASES a colleague's work**. ⭐⭐ **And the one-testid-two-states trap v1.1 named:** `name-check-current` renders for a PASSING check AND a SENDING-BACK one, so `getByTestId` alone ⛔ cannot tell a reassurance from an instruction — the two things a District Admin must ⛔ never confuse. The test asserts the CONTENT DIFFERS. **PROVEN:** dropping `passing` from the difference-flag condition fails the mixed case; disabling the stale branch fails the stale one.
...
- [x] [Review][Patch] The admin UI halves the story requires have no test — `grep canApprove | accountsComplete | console-name-difference-flag | approve-blocked` over `apps/admin/tests` returns only `verifier-console.test.tsx:93` (a default `nomineeNameCheck` in `PRESENT_PACKET`). Untested: *"approve disabled until AC4 holds"* incl. the "1" shortcut (the Dev Record calls it an "unguarded second entrance"), "bank details missing" on the DA console (AC6), the DA console's AC8 flag, the `VerifierConsoleRoute` error-code table, `useNomineeNameCheck`'s refetch-on-focus, `R9CasePanel` (no test touches it, so the R9 disclosure and the R9 half of AC8 have none), the form reset when `canApprove` flips, `HelplineClaimPage` ↔ `BankDetailsCard` (bankRecorded reset, names-read error, step-up — `helpline-claim-page.test.tsx` and `helpline-console.test.tsx` are unchanged), `BankDetailsCard` with `accounts: []` / an anonymized holder / a re-render across claims, panel verdicts cleared when `account_updated_at` or the claim changes, and a bare Return refused / the code forced to `other`. The PA-card names disclosure and the returned-claims list are un-built (D3, D4) — their tests are owed with them. [edge+auditor]
  - ⏳ **PARTIALLY CLOSED 2026-09-22 — the APPROVE GATE, which was the sharpest item; the rest stands.** ⭐ `grep canApprove | approve-blocked` returned ONE line (a default in an unrelated fixture), so the gate this story exists to put on the console was asserted ⛔ nowhere — including the entrance the component's own comment calls *"an unguarded second entrance a keyboard-first console makes easy to forget"*. Now covered: the BUTTON disabled **with its specific reason**, the `"1"` shortcut not opening the form, an **ALREADY-OPEN approve form CLOSING** when the gate shuts (⭐ the one that ⛔ only shows up over time — `outcome` is local state and survives, so a District Admin who opened the form and then recorded `does_not_match` kept it submittable), the OTHER shortcuts still working (cl.6/cl.7 — a claim is ⛔ never refused over a name, so deny/escalate must stay reachable), and a positive control. ⚠⚠ **AND A CORRECTION TO MY OWN TEST, measured rather than assumed:** I first titled entrance 2 *"the shortcut RESPECTS the gate"*. It ⛔ cannot prove that — removing `&& canApprove` from the key handler changes ⛔ **nothing observable**, because entrance 3's effect closes the form regardless. ⭐ Verified by removing that guard ALONE (suite stayed green) and then BOTH (two tests fail). ⇒ the two guards are **REDUNDANT BY CONSTRUCTION**; the shortcut's is defence-in-depth, correct and worth keeping, and ⛔ not independently observable. The test now claims ⛔ only what it proves. ⛔ **STILL OPEN:** the DA console's AC6 *"bank details missing"* and AC8 flag, the `VerifierConsoleRoute` error-code table, `useNomineeNameCheck`'s refetch-on-focus, `R9CasePanel` (touched by ⛔ no test at all), the form reset on `account_updated_at` change, and `BankDetailsCard`'s empty/anonymized/re-render cases.
  - ✅ **CLOSED 2026-09-22 (v1.8) — THE REST OF IT, and ⭐ one item was a MISSING FEATURE, ⛔ not a missing test.** Each item below was proved **red-capable by a planted regression** (run, then reverted byte-identical), ⛔ not only green:
    - **The DA console, MOUNTED** — new `verifier-console-route-name-check.test.tsx` renders the real `VerifierConsoleRoute` (router + `api/client` faked function-by-function; real hooks, real QueryClient, real components). **AC8:** `console-name-difference-flag` shows the reason LABELS from the packet with ⛔ no names fetch. **AC6:** accounts missing ⇒ approve disabled + `bankDetailsMissing` as the reason. **The blocker table:** `statusUnavailable` ⛔ is not `bankDetailsMissing` (plant: dropped the `!available` branch ⇒ red), `approveBlocked`, and the positive control. ⚠ **PINNED AS IT IS:** the "you sent it back — have it corrected" reason (`approveBlockedSentBack`) appears ⛔ **only once the names disclosure has been opened** — the packet carries `currentAndPassing`, ⛔ not the verdicts, so before opening, a claim the District Admin themselves sent back reads *"record the check"*. Recorded, ⛔ not changed: widening the packet is a contract change this bullet does not ask for.
    - **`nameCheckErrorMessage`** — the table row by row (9 codes), a CODE beating the status (step-up on a 403 ⛔ is not "no permission"), a non-`ApiError` ⇒ generic; plus, mounted, a stale-token 409 on the WRITE shows `errorStale` and ⛔ never the decision table's wording, and a 403 on the READ shows `errorForbidden`.
    - **`useNomineeNameCheck`'s refetch-on-focus** — mounted on TanStack's OWN defaults (⛔ not the app's cache-disabled client, which would make the test vacuous); a focus cycle re-reads the session (the NON-VACUITY control) and ⛔ never the names. Plant: `refetchOnWindowFocus: true` ⇒ red.
    - **`R9CasePanel`** — ⭐ touched by ⛔ no test before. On-demand (⛔ no fetch until pressed; one fetch for THIS claim), both columns, ⛔ no verdict control (`-226` cl.3), the open-form view too, a failed read says so. ⚠⚠ **The AC8 half went RED and was a missing FEATURE** — see the correction note on the "AC8's highlight is not delivered on its three client surfaces" bullet.
    - **The panel's selection reset** — a corrected account (new `account_updated_at`) clears every verdict and reason and the next Record REFUSES rather than sending the old judgement; a new declaration token clears them too; and ⛔ an identical refetch does NOT wipe what is being typed (the reset is keyed, ⛔ not per-render). Plant: effect deps `[]` ⇒ 2 red.
    - **`BankDetailsCard`** — an ANONYMIZED nominee renders as removed-at-request and ⛔ never "could not be read"; nothing on file ⇒ entry form and ⛔ no names grid even when a payload is passed; a CHANGE OF CLAIM clears all four typed fields and closes a re-entered correction form with its reason. Plant: effect deps `[]` ⇒ 2 red. ⚠ *"an anonymized HOLDER"* is ⛔ **unreachable**: `holder_name` is `ReadableName` (readable | unreadable) in `packages/contracts/src/claims/nominee-name-check.ts` — only the NOMINEE side has `anonymized` — so the reachable case was tested instead.
    - **The Pariwar Admin card's D3 disclosure** — its button had ⛔ never been pressed by a test: ⛔ no fetch until pressed, then both names + the filer's note, ⛔ no verdict control.
    - ⭐ `HelplineClaimPage` ↔ `BankDetailsCard` (bankRecorded reset, names-read error, step-up) was already closed by the earlier 2026-09-22 passes — see bullets 734/735/737 — and is ⛔ not re-claimed here. The returned-claims list is `correction-queue.test.tsx`.
    - **Counts:** `@twt/admin` **499 → 539** (+40: R9 10 · route 22 · panel 3 · bank 4 · PA card 1), 45/45 files. Admin typecheck + lint clean; `microcopy:check`, `tokens:check-theme`, `pii:check` green; `pnpm turbo run typecheck lint` 40/40, `pnpm turbo run test` 37/37 (⚠ unchanged packages served from turbo's cache — ⛔ nothing outside `apps/admin` changed, so the live-DB suites were ⛔ not re-run in this pass).
...
- [x] [Review][Patch] The read-count test cannot see the under-count — `verifier-console.spec.ts` asserts `readCount <= VERIFIER_CONSOLE_MAX_READS`, importing the constant (nothing hard-coded, so raising it to 14 breaks nothing), but `readCount` is the handler's own self-reported counter: the uncounted `getMemberNomineeDeclarationRefs` read is exactly what it cannot see, before and after chunk 2's fix. The doc-block's own remedy needs an actual query counter. [edge]
  - ✅ **CLOSED 2026-09-22 — with an ACTUAL query counter, exactly as the doc-block's own remedy asked.** The budget tests asserted `readCount <= VERIFIER_CONSOLE_MAX_READS` importing the constant, so ⛔ nothing was hard-coded — but `readCount` is the handler's **OWN SELF-REPORT**, and ⭐ a self-report can ⛔ never detect its own omission. The new test wraps the pg client the drizzle tx runs on and counts the statements **Postgres actually receives**. ⚠⚠ **I GOT THE FIRST VERSION WRONG AND THE CORRECTION IS RECORDED IN THE TEST:** asserting `actual <= VERIFIER_CONSOLE_MAX_READS` compares two different things — that constant bounds **top-level source reads** (fan-out WIDTH), ⛔ not raw SQL, and a single top-level read legitimately issues several statements through nested helpers. 27 statements against a budget of 14 would have been a **defect report for a defect that does ⛔ not exist**. ⇒ it measures the property that IS true and IS load-bearing: the real statement count must ⛔ **NOT GROW** when ROWS are added. ⭐⭐ **AND IT FOUND THAT THE TWO PRE-EXISTING no-N+1 TESTS WERE VACUOUS:** their fixture inserted `death_certificate` **twice** (collapsed by the unique `(claim_case_id, document_type)` constraint) and omitted `byte_size`, which is **NOT NULL** — so BOTH statements threw and a bare `.catch(() => {})` swallowed them. ⇒ ⛔ no rows were ever added and `expect(after).toBe(before)` compared a claim to ITSELF: it would have passed against **any** N+1. ⚠ Found by planting an N+1 in the new test and watching it ⛔ not fail. Both fixtures now use DISTINCT document types, supply `byte_size`, and assert the rows really landed — ⛔ no bare catch, because a failing fixture must FAIL the test rather than quietly turn it into a tautology. **PROVEN:** an uncounted per-document read fails the new test by name (*"rows grew the REAL statement count 27 → 29"*).
- [x] [Review][Patch] Closure honesty — the Dev Agent Record overclaims — (a) the never-echo set is *"asserted against REAL seeded plaintexts"* (VPA and address are not — see the AC9 patch); (b) the fixture is "REAL" (see above); (c) Task 7's `[x]` lines (now re-opened) for *"P1/P3/P4 refused …"*, *"whole return loop … and that a return starts no appeal flow"* and *"PII sentinels incl. the note"* are domain-level, partial or vacuous; (d) the counts (domain 3427 · api 1317 · contracts 1149 · admin 466 · mobile 541 · events 36 · 7,046) cannot be verified from the diff, which contains no run output; (e) the Task 6 completion note says "R9 screen ⛔ NOT BUILT" while the v0.9 changelog says "R9-panel name disclosure" and the tree shows the R9 panel built and the Pariwar Admin card NOT — inverted. Correct the record when the patches land. [auditor]
  - ✅ **CLOSED 2026-09-22 — the record is CORRECTED IN PLACE and the original notes are ⛔ NOT rewritten** ([[feedback_closure_language_precision]], [[feedback_record_unattested_no_backfill]]). ⭐ **(d) THE COUNTS ARE NOW ATTESTED**, by running each package against `twt-test-pg` on **:5433**: domain **3,462** · api **1,339** · contracts **1,186** · mobile **545** · admin **486** · i18n **110** · events **36** = **7,164**. ⚠ The record's `7,046`, and v1.0's `7,116`/`7,122`, are ALL superseded — ⛔ none was ever re-earned on the correct database, which is the un-attested-total residual v1.1 recorded. **(a)/(b)** the never-echo claim was ⛔ false for the ADDRESS in a way nobody had noticed: `address_ciphertext` is NULLABLE and the fixture never populated it, so the assertion was passing against a NULL column and could ⛔ **never have failed** — now really seeded, plaintext asserted absent, and PROVEN able to fail. ⚠ The **VPA** leg is still ⛔ not asserted against a real plaintext and is ⛔ not claimed to be. **(c)** already un-ticked by v1.1. **(e)** the inverted Task 6 note is left STANDING, with the inversion stated beside it — both halves are built now, so the sentence is stale as well as wrong, and deleting it would erase the record of the error.
- [x] [Review][Patch] Tidy — three specs (`verifier-console-shape`, `verifier-console`, `verifier-decision`) carry the added `// Story 6.18 …` comment block indented 8 spaces instead of 4; `driveClaimState`'s union type has an unused `'verification_in_progress'`; a stray double blank line before `seedNominee`; the mobile-echo test asserts `not.toContain('address')` with only a mobile column seeded (vacuous). [blind+auditor]
  - ✅ **CLOSED 2026-09-22 — three cosmetics and ⭐ one assertion that could ⛔ never fail.** The three 8-space comment blocks are re-indented; `driveClaimState`'s union drops `'verification_in_progress'`, which ⛔ no caller ever passed (a union member no call site uses claims the helper supports a path it has ⛔ never been run down); the stray double blank line is gone. ⭐⭐ **THE REAL ONE:** `expect(res.body).not.toContain('address')` ran against a fixture that seeded ⛔ **no address at all** — `address_ciphertext` is NULLABLE — while its own comment claimed *"the helper seeds a REAL encrypted mobile, so this assertion is not vacuous"*, ⚠ true of the mobile and ⛔ FALSE of the address. Now seeded for real, with the **plaintext** asserted absent beside the field name. ⭐ Proven twice over: an extra response key is refused by the `.strict()` response schema outright (a stronger guard than assumed), so the probe had to smuggle the address through an EXISTING field — and the assertion fired.
...
**Code review 2026-09-22 — production/non-test code, `0bc7c936..HEAD` (chunk 1 of 2 by file class; the test-file half of this same range is a follow-up pass).** Scope: the 10 non-test files changed since the prior review round's patches landed — `hooks.ts`, `NomineeNameCheckPanel.tsx`, `i18n-en.ts`, `BankDetailsCard.tsx`, `HelplineClaimPage.tsx`, `HelplineConsoleShell.tsx`, mobile `nominee-review.tsx`, `microcopy.yaml`, `nominee-bank.ts`, `check.ts` (44-file/6,536-line range chunked down to these 10; the other 34 are test files). Layers: Blind Hunter (diff text only, no repo access), Edge Case Hunter (diff + repo read access), Acceptance Auditor (diff + this story file). **0 decisions, 5 patches, 5 defer, 5 dismissed.** The Acceptance Auditor found the diff fully consistent with the story's ACs and decisions — every hunk maps onto an already-recorded "code review 2026-09-22" / validate-pass finding above (see 933/944–949), no overclaim, no new AC violation, D3 untouched and correctly so. Several claims were re-verified directly in the tree rather than taken on trust: the `reach`-regex regression was confirmed with a live regex test; the microcopy gate was re-run and passed green over the newly-globbed files; the `ApiError`-missing-import concern was checked against the file and is a false positive (already imported, pre-existing).

- [x] [Review][Patch] Unguarded `stepUpRequired` races across the intake/bank-save flows — `BankDetailsCard`'s Save button is disabled only by `pending`, never by `stepUpRequired`, so an operator can re-submit while elevation is outstanding; a second in-flight `recordBank.mutateAsync` resolving after the operator's step-up succeeds can re-flip `stepUpRequired` back to `true` right after it was cleared; and because `stepUpRequired` lives in the parent (not scoped by claim id) while `BankDetailsCard` is `key`'d per claim, a stale resolution after the operator switches claims can still flip the flag against the wrong claim's step-up state. Fix: pass `stepUpRequired` down to gate the Save button, and/or tie `submitBank`'s continuation to the claim id it was issued for. [`apps/admin/src/modules/helpline-claims/HelplineClaimPage.tsx:263-281`] [blind+edge]
  - ✅ **CLOSED 2026-09-22 — both halves.** `filedClaimCaseIdRef` (kept current by an effect) lets `submitBank`'s continuation compare the claim it was issued for against the claim now showing, and skips both `setStepUpRequired` calls on a mismatch. `BankDetailsCard`'s `pending` prop is now `recordBank.isPending || stepUpRequired`, so the Save button (and every field) stays disabled for the whole time the panel is up, not just while the mutation is in flight. Typecheck clean (`@twt/admin`, forced/non-cached).
- [x] [Review][Patch] Contradictory ARIA semantics on the "saved" success message — the save-success `Text` combines `accessibilityRole="alert"` (conventionally assertive/interrupting) with `accessibilityLiveRegion="polite"`, directly contradicting the adjacent comment's own stated intent ("`saved` is polite — must not interrupt"). `role="status"` (implicitly polite) matches the comment's reasoning; `role="alert"` does not. [`apps/mobile/app/(claim)/nominee-review.tsx:388-391`] [blind]
  - ✅ **CLOSED 2026-09-22.** Changed to `accessibilityRole="text"` (⛔ not `"status"` — that RN role does not exist in this codebase's usage; `"text"` is the pairing used everywhere else here for a polite announcement, e.g. `(signup)/nominees.tsx`, `(life-events)/index.tsx`). `notice` (assertive) is unchanged. Typecheck clean.
- [x] [Review][Patch] `microcopy.yaml`'s tightened `reach` tone-guide regex narrows coverage beyond its stated fix — verified directly (`node -e` regex test): `(could|did|does)(n.?t|\s+not)\s+reach\s+(the\s+)?\b(target|goal|amount|total)\b` correctly fixes the telephony false positive ("we couldn't reach a nominee's phone") but now FAILS to match "didn't reach its target" / "did not reach our goal" / "couldn't reach their target" — possessive-determiner phrasing the old, unanchored pattern caught and this one silently drops. Widen the optional-article group to include possessives, or relax the strict "the" requirement. [`microcopy.yaml:79`] [blind, verified]
  - ✅ **CLOSED 2026-09-22.** Article group widened to `((the|its|our|their)\s+)?`. Re-verified with the same regex test: all three possessive cases now match, the telephony false positive still does not. `microcopy:check` re-run live afterward — still green (0 findings) over the full scope, including the `claim.json`/mobile files this pattern applies to.
- [x] [Review][Patch] The mobile "saved" announcement has no time to be heard — `NomineeReviewScreen`'s submit handler calls `setSubmit('saved')` immediately followed, with no delay, by `router.push('/(claim)/acknowledgement')` (pre-existing navigation, untouched by this diff) — so the `accessibilityLiveRegion="polite"` just added to that message is asserted in source but the screen navigates away before a screen reader has a chance to announce it. Fix: hold the "saved" state for a short delay before navigating, announce imperatively, or move the confirmation onto the acknowledgement screen. [`apps/mobile/app/(claim)/nominee-review.tsx:219-225`] [edge, verified]
  - ✅ **CLOSED 2026-09-22.** A named `SAVED_ANNOUNCEMENT_DELAY_MS` (1200 ms) constant now separates `setSubmit('saved')` from `router.push`, awaited before navigating. ⚠ **RESIDUAL, not claimed closed:** no test pins this delay or the announcement timing itself — `nominee-review-announcements.test.ts` (family 13(d), closed earlier) asserts the live region exists on every state, not that navigation waits for it; a test-pass finding is owed if the test-file half of this review round covers it.
- [x] [Review][Patch] `check.ts`'s three-way route classification (`COVERAGE_SET` / `NON_ADJUDICATION_ROUTES` / `ENROLMENT_OWED`) has no duplicate-across-list check — a route file mistakenly listed in two of the three sets (e.g. both "no chain needed" and "owed enrolment") is silently absorbed by the `Set` union inside `unclassifiedRouteFiles()` rather than flagged, undermining the gate's own "every file classified exactly once" design intent. [`scripts/claim-adjudication-human-actor-invariant/check.ts:190-203`] [edge]
  - ✅ **CLOSED 2026-09-22.** New `duplicateClassifiedFiles()` finds any file appearing more than once across the three lists and fails the gate on it, wired into `main()` alongside the existing unclassified-file check. Gate re-run live afterward — still passes (0 duplicates today; the 3 `ENROLMENT_OWED` entries remain correctly single-classified). ⚠ No probe-duplicate test was added to prove the new check can fail (the earlier `claims.zzz-probe.routes.ts` proof at 944–945 covers the sibling unclassified-file check, not this one) — owed if the test-file half of this review round covers `check.ts`'s own test file.
- [x] [Review][Defer] `claims.helpline.routes.ts` — the very route this diff's feature depends on, an explicitly state-independent write per D4's third branch — remains in `ENROLMENT_OWED`, not enrolled in the human-actor-invariant gate; a live, unguarded security-relevant route ships in the same range that documents the gap. Matches the story's own already-recorded decision to track rather than close this debt here (see 944–945 above) — flagged again for visibility since the gap is live, even though openly recorded rather than hidden. [`scripts/claim-adjudication-human-actor-invariant/check.ts:158-176`] — deferred, pre-existing (the route itself shipped earlier in this story; this diff only documents the gap) [blind]
- [x] [Review][Defer] `COVERAGE_FLOOR = 8` checks only array length, not entry identity — deleting a real `COVERAGE_SET` entry while adding an unrelated placeholder entry keeps the floor check green. [`scripts/claim-adjudication-human-actor-invariant/check.ts:213-219`] — deferred, pre-existing, minor hardening gap in an already-recorded control [blind]
- [x] [Review][Defer] `unclassifiedRouteFiles()` uses a flat, non-recursive `readdirSync` — a future `*.routes.ts` file added in a subdirectory would silently escape classification. Verified: no subdirectories exist under `apps/api/src/modules/claims` today, so no current impact. [`scripts/claim-adjudication-human-actor-invariant/check.ts:190-196`] — deferred, pre-existing, no current impact [blind]
- [x] [Review][Defer] `unclassifiedRouteFiles()` has no try/catch around `readdirSync` — a missing/renamed `apps/api/src/modules/claims` directory would crash the whole gate script uncaught instead of reporting a scoped failure. [`scripts/claim-adjudication-human-actor-invariant/check.ts:190-196`] — deferred, pre-existing, low likelihood [edge]
- [x] [Review][Defer] Multiple validation errors on the mobile nominee-review screen (holder-name, IFSC, VPA) can each independently carry `accessibilityRole="alert"` + `liveRegion="assertive"` — if more than one condition is true at once, several assertive announcements could compete with no debounce/ordering. [`apps/mobile/app/(claim)/nominee-review.tsx:223-269`] — deferred, minor polish, needs UX judgment on the right roll-up strategy [blind]

_Dismissed as noise or handled elsewhere (5, not listed): "`ApiError` used with no import shown" — false positive, already imported at `HelplineClaimPage.tsx:24` (pre-existing, outside this diff's hunks) and the new code mirrors the existing intake pattern at the same file; "new locale globs turned on with no evidence of a clean run" — verified empirically, `microcopy:check` was run live and passes green over the full current scope including the newly-globbed `claim.json` files and both mobile screens; "redundant local aliasing of shared regex constants" in `BankDetailsCard.tsx` — the local `IFSC_RE`/`ACCOUNT_RE` are immutable const aliases of the imported values, no actual drift risk exists today; "self-attested prose reasons not machine-checked" in `check.ts`'s three classification lists — by design, matches this codebase's established recorded-debt-with-reasoning convention ([[project_enum_mint_authority_delegated]] and siblings); "removing the `result === null` step-up-panel guard not proven safe" — the Edge Case Hunter confirmed `canSubmit` still independently requires `result === null` for the intake-submit button, so removing the display guard on the step-up panel does not reopen the intake-submit gate._

**Code review 2026-09-22 — test-file half, `0bc7c936..HEAD~1` (chunk 2 of 2; the production-code half above is closed).** 31 files, ~5,900 diff lines, chunked in two by the story's own established chunk-4/5 split: **A** — `packages/domain/tests` + `packages/contracts/tests` + `scripts/microcopy/claim.test.ts` (20 files); **B** — `apps/api/tests` + `apps/admin/tests` + `apps/mobile/tests` (11 files). Same three layers per chunk (Blind Hunter — diff file only, sandboxed by instruction; Edge Case Hunter — diff + repo access; Acceptance Auditor — diff + this story), re-derived against live `HEAD` source rather than taken on trust. **0 decisions, 30 patches, 6 defer, ~4 dismissed.** The Acceptance Auditor traced every "code review 2026-09-22" closure claim in both chunks against the actual production source (not diff prose) and found them genuinely proven, not merely asserted — including confirming the admin tests are NOT stale relative to the separately-merged production-code patch commit. One substantive gap surfaced, already known: **AC11's `return_to_district_admin` action still has no HTTP-level test** — `cycle-freeze.spec.ts` isn't touched by this diff at all; matches the story's own "whole-story test map" as an already-recorded, not-yet-closed item, so it's recorded here as `[Defer]` rather than a new finding this diff should have closed.

- [x] [Review][Patch] `sameMs` in the D5 currency test doesn't test a genuine same-millisecond collision — it reuses the file's two pre-existing, second-apart `UPDATED_1`/`UPDATED_2` constants rather than two edits that actually land in the same millisecond, so it can't distinguish the claimed property from the already-covered "identical stamps" case. [`packages/domain/tests/claim/nominee-name-check-events.test.ts:412-426`] [blind]
  - ✅ **CLOSED 2026-09-22.** `sameMs` now moves one timestamp 500 microseconds later (`new Date` silently truncates sub-ms precision on parse), a genuinely different moment that collides only after truncation — with a non-vacuity check that the pre-truncation strings really differ. Live suite: 37/37 green.
- [x] [Review][Patch] `isNomineeNameCheckCurrent` has no test for "a rank is present in the live accounts but absent from the recorded check, at EQUAL array length" — a length-only implementation would pass every existing test here and still be wrong. [`packages/domain/tests/claim/nominee-name-check-events.test.ts:384-391`] [edge]
  - ✅ **CLOSED 2026-09-22.** New test replaces rank 2 with rank 3 at equal length; the `!recorded` branch (`check.accounts.find(...)` returning `undefined`) is what fails it.
- [x] [Review][Patch] `stripComments`'s non-vacuity check only proves `//` line-comments are stripped; the `/* … */` block-comment branch of the same function is never exercised. [`packages/domain/tests/claim/nominee-name-no-comparison-fence.test.ts:131-135`] [edge]
  - ✅ **CLOSED 2026-09-22.** Added the block-comment non-vacuity case beside the line-comment one.
- [x] [Review][Patch] `FORBIDDEN_PATTERNS`' positive-control `SPECIMENS` exercise only ONE alternative per multi-way regex alternation — `soundex`, `toUpperCase() ===`, `canonicaliseName`, `Intl.Collator`, and the quoted-literal form of `trim() === [a-z]` are all untested alternatives that could rot unnoticed behind the one alternative that IS tested. [`packages/domain/tests/claim/nominee-name-no-comparison-fence.test.ts:64-73,509-522`] [blind+edge]
  - ✅ **CLOSED 2026-09-22 — AND IT FOUND A REAL GAP IN THE PATTERN ITSELF.** A new exhaustive-alternative test covers every named alternative across all seven patterns. ⚠⚠ Writing the `trim() === "priya sharma"` specimen (a QUOTED literal) proved the pattern's own `trim\(\)\s*===\s*[a-z]` alternative required a BARE lowercase letter right after `===` — a quoted name-shaped literal slipped past every alternative. Widened to `trim\(\)\s*===\s*(['"][a-z]|[a-z])`. ⚠ First attempt (`['"a-z]`) was too broad — it false-positived on `actorDisplay.trim() === ''` (a legitimate blank-string check, `nominee-name-check-persist.ts:109`); corrected to require a quote FOLLOWED BY a letter. Verified against the real fenced files before and after. Live suite: 7/7 green.
- [x] [Review][Patch] The migration spec's enum-type discrimination is a substring match (`.toContain('phase')` / `.toContain('outcome')`) rather than the exact catalog type name — a future unrelated type merely containing that substring would satisfy "the RIGHT enum type" check. [`packages/domain/tests/integration/claim/nominee-name-check-migration.spec.ts:65-66`] [blind+edge]
  - ✅ **CLOSED 2026-09-22.** Tightened to `.toBe('state_trustee_decision_phase')` / `.toBe('state_trustee_decision_outcome')` — the exact catalog type names, re-verified against the actual migration SQL. Live suite: 3/3 green.
- [x] [Review][Patch] "THE ONE EXACT-COUNT PIN IN THE REPO" (comment) is inaccurate — the rewritten vocabulary-diff test's two-equality check (`live.filter(∉baseline)` + `baseline.filter(∉live)`) also pins the exact total count just as rigidly as the removed `toHaveLength(N)` calls did. Fix the comment's claim, not a behavior bug. [`packages/domain/tests/claim/dpdpa-consent-events.test.ts:123-131`] [blind]
  - ✅ **CLOSED 2026-09-22.** Comment corrected: this file owns the one literal `toHaveLength(N)` FORM (which is what made six-file maintenance expensive); the exact-count PROPERTY is pinned twice by design, for different reasons.
- [x] [Review][Patch] A dead/redundant assertion in the D3 attribution test — `checkedByActorDisplay.trim()).not.toBe('')` immediately follows `.toBe('Anita Kumari (District Admin)')` on the same value, so it cannot independently fail here; the justification given (a future refactor + a different fixture) describes a property of some OTHER test. [`packages/domain/tests/integration/claim/nominee-name-check-return-loop.spec.ts`] [blind]
  - ✅ **CLOSED 2026-09-22.** Redundant assertion removed. ⚠ Checked, not assumed: the API-layer sibling test carries the IDENTICAL redundant pair on the SAME literal actor display, so it is not actually a "different fixture" as the old comment claimed either — left alone there as out of this patch's file scope, recorded honestly rather than papered over.
- [x] [Review][Patch] The "note is TRIMMED" test's second half (the 500-char + trailing-newline boundary case) only checks `.safeParse(...).success`, not the parsed value — weaker than the pattern the SAME test establishes one line earlier. [`packages/contracts/tests/claims-nominee-bank.test.ts:46-55`] [blind]
  - ✅ **CLOSED 2026-09-22.** Now uses `.parse(...)` and asserts the parsed value equals the 500-char string with the trailing newline genuinely gone.
- [x] [Review][Patch] `nameDifferenceNote` is tested for absence (`undefined`) but never for an explicit `null` — the schema is `.optional()` not `.nullable()`, so a client sending literal `null` (a common JSON idiom) is untested. [`packages/contracts/tests/claims-nominee-bank.test.ts:211-215`] [edge]
  - ✅ **CLOSED 2026-09-22.** New test pins that an explicit `null` is refused. Contracts suite: 20/20 green.
- [x] [Review][Patch] The tenant-isolation RETURN-denial test asserts only `.rejects.toThrow()`, unlike its sibling record-check test which also confirms no partial write landed — a rejection that still wrote a `correction_return` row from the wrong session would go undetected. [`packages/domain/tests/integration/claim/nominee-name-check-tenant-isolation.spec.ts:167-175`] [edge]
  - ✅ **CLOSED 2026-09-22.** After both rejected attempts, re-enters Pariwar A's own scope and queries `claim_state_trustee_decisions` directly for a live `correction_return`-phase row — none exists. Live suite: 4/4 green.
- [x] [Review][Patch] `r9-voting.spec.ts`'s `P4_DEFICIENCIES` table is missing the "ONE account only (cl.7)" cell that the parallel P1/P3 deficiency tables both have — an R9 approval on a claim with exactly one bank account is untested at this gate specifically. [`packages/domain/tests/integration/claim/r9-voting.spec.ts:344-386`] [edge]
  - ✅ **CLOSED 2026-09-22.** New cell deletes account rank 2 after setup; `assertNomineeNameCheckForApproval`'s `liveAccounts.length !== 2` guard (verified to run BEFORE the check lookup) is what fires. Live suite: 22/22 green.
- [x] [Review][Patch] The "two concurrent CHECKS both land" positive control never queries `events_log` to confirm TWO distinct events were appended — it only confirms neither write was rejected, which would also be true of a silent no-op on the loser. [`packages/domain/tests/integration/claim/nominee-name-check-write-concurrency.spec.ts:451-491`] [edge+auditor]
  - ✅ **CLOSED 2026-09-22.** Added a direct `events_log` count query for `claim.nominee_name_checked` on the claim — asserts exactly 2. Live suite: 5/5 green.
- [x] [Review][Patch] The microcopy allow-list suppression tests never prove the suppression is scoped to the exact hex literal rather than the whole LINE — a second, unrelated hex sharing a line with `#B00020`/`#1E8E3E` could be silently suppressed too, since the checker's `isAllowed` matches anywhere on the line. [`scripts/microcopy/claim.test.ts:226-236`] [edge]
  - ⚠ **PARTIALLY CLOSED 2026-09-22 — investigated, a real gate-design conflict found, PINNED rather than fixed.** Writing the test confirmed the finding: `checkMagicNumberColors` calls `isAllowed(file, line, config)` with no match-position, so it IS whole-line for this one finding family. A first fix (pass `matchRange`, mirroring `checkVocabulary`'s already-correct call) broke a DIFFERENT, pre-existing, INTENTIONALLY line-level allow-list entry (`primary_color|secondary_color|accent_color` on `AddPariwarForm.tsx`) — that entry's `pattern` deliberately matches a nearby FIELD NAME, not the hex itself, so it has zero position overlap with the hex it means to allow. ⇒ **REVERTED** the `lib.ts` change; making `checkMagicNumberColors` position-aware needs a per-entry opt-in or rewriting the older entry, both gate-DESIGN calls with blast radius beyond this story. The test now PINS today's real, line-level behaviour honestly instead, so a future fix cannot regress it further un-noticed. **Recorded as owed in `deferred-work.md`.**
- [x] [Review][Patch] `claim.test.ts`'s "does NOT flag `donor` on a code file" test compares two inputs it calls "the very same string" that are NOT the same (`donorRow`, an identifier, vs. the standalone token `"donor"`) — it may be passing for a reason unrelated to the `includeMemberOnly` property it claims to isolate. [`scripts/microcopy/claim.test.ts`] [blind]
  - ✅ **CLOSED 2026-09-22.** Kept the identifier case (now correctly explained: `\bdonor\b` already fails between "donor" and "Row" regardless of the flag) and added a genuinely isolating pair — the identical standalone word `donor` in both a code file and the copy register, differing only in `includeMemberOnly`. Full microcopy suite: 366/366 green.
- [x] [Review][Patch] Missing positive controls on the cross-tenant POST-denial tests — neither `nominee-bank-helpline.spec.ts` nor `nominee-name-check.spec.ts` pairs its "wrong tenant POST is refused" test with a matching "identical POST succeeds for the rightful tenant" control, unlike their own GET-denial tests one section up. A route that 404s for EVERY caller would pass either test just as well as one that correctly discriminates by tenant. [`apps/api/tests/integration/claims/nominee-bank-helpline.spec.ts:520-540`, `apps/api/tests/integration/claims/nominee-name-check.spec.ts:~1430-1470`] [blind+edge]
  - ✅ **CLOSED 2026-09-22, both files.** `nominee-bank-helpline.spec.ts`: added an owner POST on a SEPARATE claim (so it can't disturb the intruder test's own "nothing written" assertion), asserting 201 — needed `elevateClaimFile(owner)` first, found by running it live. `nominee-name-check.spec.ts` already had a "POSITIVE CONTROL" describe block covering the two GET denials; added the missing POST leg to it, reusing `postCheck`/`seedNomineeNameCheck`. Both files' live suites green (18/18, 39/39).
- [x] [Review][Patch] Weak reachability checks — `expect(res.statusCode).not.toBe(404)` where a specific `.toBe(200)`/`.toBe(201)` was checkable and IS used elsewhere in the same files; a 500 on the rightful tenant's own call would still pass. [`apps/api/tests/integration/claims/nominee-bank-helpline.spec.ts:505-520,748`] [blind+edge]
  - ✅ **CLOSED 2026-09-22.** Tightened to `.toBe(200)`.
- [x] [Review][Patch] Mismatched title: "the filer's NOTE survives POST → ciphertext → the AC2 read, and rides nowhere else" does not test the AC2-read step at all (a comment inside the test explains the read is deferred to `nominee-name-check.spec.ts` because this fixture can't produce a resolvable posting district) — retitle to match what this specific test proves. [`apps/api/tests/integration/claims/nominee-bank-helpline.spec.ts:415-469`] [blind]
  - ✅ **CLOSED 2026-09-22.** Retitled to name the sibling file as the AC2 read-back's actual owner.
- [x] [Review][Patch] Two tests bypass the file's own `assertCode` discipline (its docstring explicitly warns against a bare status check) — the AC3 400-vs-409 precedence test and the duplicate-ranks test both assert only `res.statusCode`, so an unrelated 400 would satisfy either. [`apps/api/tests/integration/claims/nominee-name-check.spec.ts:~941-973,~1029-1052`] [blind+edge]
  - ✅ **CLOSED 2026-09-22.** Both now use `assertCode(res, 400, 'request.validation')` — the actual code the Fastify/Zod schema-validation path emits (`middleware/error-mapping`), re-derived at source. Live suite: 6/6 green on the targeted tests.
- [x] [Review][Patch] The AC9 "the WRITE path leaks nothing either" test's title claims parity with the GET test's comprehensive PII list, but its `PT` fixture only carries `holder1`/`nominee`/`note` — omitting `holder2`, `account`, `ifsc`, `vpa`, `mobile`, `address` — so a leak of the account number or IFSC into the write-path audit line would go undetected despite the title's implied parity. [`apps/api/tests/integration/claims/nominee-name-check.spec.ts:1219-1267`] [blind]
  - ✅ **CLOSED 2026-09-22.** `PT` widened to the full 9-key set matching the GET test, reusing the same real-envelope fixture shape (`seedAccountsWithNames`'s hardcoded account/IFSC, `seedNominee`'s hardcoded mobile/address). Live suite: 2/2 green.
- [x] [Review][Patch] Weak matcher `/married|difference/i` on a specific clerical-reason assertion — loose enough to match incidental nearby copy rather than the correct text for that specific account. [`apps/admin/tests/nominee-name-check-panel.test.tsx:205`] [blind]
  - ✅ **CLOSED 2026-09-22.** Tightened to the exact rendered label pair, `'Clerical difference (accept with a reason) — A married name'`.
- [x] [Review][Patch] Inconsistent rigor between two key-checks in the same file — `CLAIM_KEYS` requires the exact call-site syntax `t('key')` in source; `COMMON_KEYS` only checks the bare string appears ANYWHERE in the file (could match a comment or an unrelated identifier), despite the file's stated purpose being to prevent exactly that kind of drift. [`apps/mobile/tests/unit/nominee-name-copy-resolves.test.ts:55-88`] [blind+edge]
  - ✅ **CLOSED 2026-09-22.** `COMMON_KEYS` check now requires the exact `t('key')` call-site syntax, matching `CLAIM_KEYS`.
- [x] [Review][Patch] A "two different states under one testid" test asserts only `sendingBack !== passing` on raw `textContent` — satisfied by any incidental difference (verdict order, a timestamp fragment) without confirming the sending-back state actually renders the correction instruction the comment says matters. [`apps/admin/tests/nominee-name-check-panel.test.tsx:234-250`] [blind]
  - ✅ **CLOSED 2026-09-22.** Added specific-content assertions: the sending-back render contains the "Does not match — send back for correction" verdict label; the passing render never does.
- [x] [Review][Patch] `helpline-claim-page.test.tsx`'s step-up tests always vary HTTP status AND error code together (403+`step_up_required`, 409+`not_collectable`) — nothing isolates which one the panel is actually gated on; a 403 with a DIFFERENT code could wrongly show the panel, untested either way. [`apps/admin/tests/helpline-claim-page.test.tsx:190-236`] [edge]
  - ✅ **CLOSED 2026-09-22 — confirmed correct, not just newly covered.** New test: a 403 with a non-step-up code (`claim.forbidden`) shows its raw message and ⛔ NOT the step-up panel — proving the gate really is on the CODE, not the bare status. Live suite: 9/9 green.
- [x] [Review][Patch] Family 13(d) `role="status"` coverage in the panel test checks only 3 of the panel's 5 announced states (loading, current, sent-back-hint) — `name-check-stale` and `name-check-none` are untested for the same attribute. [`apps/admin/tests/nominee-name-check-panel.test.tsx:293-345`] [edge]
  - ✅ **CLOSED 2026-09-22 — AND IT FOUND A REAL SOURCE GAP.** `name-check-stale` already carried `role="status"` in source (untested only); `name-check-none` did ⛔ NOT — unlike its siblings `name-check-current`/`name-check-stale`. Fixed in `NomineeNameCheckPanel.tsx` and both states are now tested. Live suite: 31/31 green.
- [x] [Review][Patch] The "sent-back hint" is proven to APPEAR on a `does_not_match` verdict but never proven to CLEAR after the operator corrects the verdict back to `matches`/`clerical_difference` — a stale hint after a correction is untested. [`apps/admin/tests/nominee-name-check-panel.test.tsx:333-345`] [edge]
  - ✅ **CLOSED 2026-09-22 — confirmed correct.** New test flips the verdict `does_not_match` → `matches` and confirms the hint disappears; it did, on the first run — a genuine new-coverage patch, not a bug find.
- [x] [Review][Patch] The AC12 Devanagari-refusal test only puts the non-English name on account rank 1 — rank 2 is always the default English fixture, so the English-script gate being applied to BOTH ranks is unproven. [`apps/api/tests/integration/claims/nominee-bank-helpline.spec.ts:347-360`] [edge]
  - ✅ **CLOSED 2026-09-22.** New sibling test puts the Devanagari name on rank 2 instead; refused 400, nothing persisted. Live suite: 18/18 green.
- [x] [Review][Patch] `verifier-console.spec.ts`'s `nomineeSection` tests cover "no accounts" and "check recorded" (passing/mixed) but never "accounts complete, no check EVER recorded" — the exact state that gates `verifier_decision.nominee_name_check_required` elsewhere. [`apps/api/tests/integration/claims/verifier-console.spec.ts:882-968`] [edge]
  - ✅ **CLOSED 2026-09-22.** `seedClaim` extended to forward optional `seedNomineeNameCheck` opts (default unchanged for all 22 existing call sites); new test uses `{ accountsOnly: true }`. Live suite: 20/20 green.
- [x] [Review][Patch] "Deny still succeeds on an un-approvable claim" is tested only for the `bank_details_required` blocker, not the `nominee_name_check_required` blocker — whether deny is wrongly gated specifically when the blocker is a missing name check is unproven. [`apps/api/tests/integration/claims/verifier-decision.spec.ts:796-872`] [edge]
  - ✅ **CLOSED 2026-09-22.** New sibling test uses `seedClaim(..., 'accounts_only')`; deny still succeeds (201). Live suite: 29/29 green.
- [x] [Review][Patch] `nominee-name-copy-resolves.test.ts`'s own stated premise — that `t()` THROWS on a missing key, which is what makes every other check in the file non-vacuous — is asserted in comments but never actually demonstrated with a failing-key call. [`apps/mobile/tests/unit/nominee-name-copy-resolves.test.ts:1-88`] [edge]
  - ✅ **CLOSED 2026-09-22.** New test confirms `t()` throws for a missing key in both the `claim` namespace and the default namespace.
- [x] [Review][Patch] Only `saved`/`notice` are checked for the EXACT live-region politeness value (`polite` vs `assertive`) in the announcements test; the other three anchors (`holder_english`, `ifsc_error`, `vpa_invalid`) are only checked for attribute PRESENCE, so a message carrying the wrong politeness would go undetected on those three. [`apps/mobile/tests/unit/nominee-review-announcements.test.ts:58-104`] [edge]
  - ✅ **CLOSED 2026-09-22.** All three now assert the exact `accessibilityLiveRegion="assertive"` value (re-derived from source, matching their nature as validation errors), not just presence.

**Verification (2026-09-22, all patches together):** `@twt/admin` + `@twt/mobile` + `@twt/contracts` + `@twt/domain` + `@twt/api` typecheck (forced, non-cached) clean. Full package test suites, each run from its own directory/config: `@twt/domain` 279 files / 3491 tests green (1 pre-existing skip); `@twt/contracts` 68/68 files, 1192/1192 tests; `@twt/admin` 43/43 files, 499/499 tests; `@twt/mobile` 36/36 files, 552/552 tests; `apps/api` claims integration (serial, live DB) 17/17 files, 234/234 tests; `scripts/microcopy` 14/14 files, 366/366 tests. 29 of 30 patches cleanly closed; 1 (the microcopy line-vs-position scoping patch) investigated, found a real conflicting design constraint, safely reverted, and pinned as an honest recorded gap rather than silently dropped or force-applied.
- [x] [Review][Defer] **HIGH — AC11's `return_to_district_admin` cycle-freeze action still has no HTTP-level test.** `cycle-freeze.spec.ts` is not among the 11 files this diff touches; the 400 codes (`no reason_code`, `other` without rationale), the 409 codes (`not_returnable`, `awaiting_correction`, `bank_details_required`, `nominee_name_check_required`), the metadata-only 201, the `admin_cycle_freeze.returned` audit line and cross-Pariwar on this specific action remain unproven over HTTP. Already recorded in this story's own "whole-story test map" (above) — this diff neither closes it nor claims to. [`apps/api/tests/integration/claims/cycle-freeze.spec.ts`] — deferred, pre-existing, already tracked [auditor]
  - ⏳ **PARTLY CLOSED — STALE WHEN WRITTEN (re-checked 2026-09-22).** The return loop has been driven over HTTP since `0bc7c936` (before this review's range began): both 400s, the metadata-only 201, the `admin_cycle_freeze.returned` audit line, 409 `awaiting_correction`, 409 `exclusion_conflict`, cross-Pariwar, and the approve-side `nominee_name_check_required` / `bank_details_required` 409s — 22/22 green. ⛔ **Still open:** 409 `cycle_freeze.not_returnable` has no test anywhere. HIGH → LOW. Detail in `deferred-work.md`.
  - ✅ **FULLY CLOSED 2026-09-22.** The residual `not_returnable` test is written (a return on a `verifier_review` claim → 409 + `details.state`, plus the four absences: no state move, no event, no decision row, no audit line), proved red-capable by a planted regression, and `cycle-freeze.spec.ts` is 23/23. ⛔ Nothing remains open under this item — AC11 is now proven over HTTP end to end.
- [x] [Review][Defer] Stale "Nth claim event" ordinals remain in 5 sibling `describe()` titles (`ground-inspection-events`, `nominee-bank-events`, `dpdpa-consent-events`, `shepherd-events`, `r9-outcome-events`.test.ts) even though this same review pass diagnosed and retitled the identical pattern in `claim-reversed-event.test.ts` one file over. Cosmetic (titles only, not assertions). — deferred, low priority, mechanical when next touched [blind]
- [x] [Review][Defer] The statement-counting test's instrumentation assumes every statement `assembleVerifierConsole` issues is routed through the ONE intercepted `client.query` reference — if Drizzle holds a separately-bound reference on some code path, undercounting would be silent. Needs Drizzle-internals investigation, not a mechanical fix. [`apps/api/tests/integration/claims/verifier-console.spec.ts:1527-1630`] — deferred, needs investigation [blind]
- [x] [Review][Defer] Duplicated, near-identical document-insert setup between two adjacent tests in the same file — a future edit to one and not the other could reintroduce the exact silent-vacuity bug (a bare `.catch()` swallowing a failed insert) this diff just fixed elsewhere in the file. [`apps/api/tests/integration/claims/verifier-console.spec.ts:1593-1613,1646-1675`] — deferred, refactor larger than a one-line patch [blind]
- [x] [Review][Defer] `appeal_stage1_reviewed` with `decision: 'upheld'` (the sibling of `'reversed'`) is never seeded in the AC5 correction-unlock tests — whether an upheld denial wrongly unlocks a bank correction is unproven either way. [`apps/api/tests/integration/claims/nominee-bank-helpline.spec.ts:137-153`] — deferred, needs a new seed variant, not urgent [edge]
- [x] [Review][Defer] `nominee-review-announcements.test.ts`'s anchor-matching regex is non-greedy to the first `</Text>` and could mis-attribute a live-region check if a message were ever nested inside another `<Text>`. Speculative — no current nesting in the screen. [`apps/mobile/tests/unit/nominee-review-announcements.test.ts:36-42`] — deferred, speculative, flag on the next edit to that screen [edge]

_Dismissed as noise or handled elsewhere (3, not listed): the "vacuous-by-construction" entrance-2 shortcut test in `verifier-decision.test.tsx` — the Blind Hunter flagged it as kept-despite-disclosure, but the Acceptance Auditor traced it and confirmed the test's own comment already states, honestly and narrowly, only the property it CAN prove (having already corrected an earlier overclaim in-comment) — this is the correct handling of an unfalsifiable sub-case, not a defect; the "5/5 runs" empirical comment in `write-concurrency.spec.ts` — re-read in context it is disclosed as an observation, not asserted as a guarantee, and the race spec is explicitly KEPT because the ordering is unspecified, so the comment is accurately hedged already; two "implementation-path claims stated as fact but unverifiable from this diff" (a claim about which code branch a length guard exercises, and a claim about a sibling test file's behavior outside this diff's scope) — both are about comment accuracy referencing content this diff doesn't include, not check-able defects in the diff itself._
...
- ⚠ **CLOSURE-HONESTY CORRECTION, 2026-09-22 (code review bullet "the Dev Agent Record overclaims").
  ⭐ The notes below were ⛔ NOT rewritten — they are the record of what their author believed at the
  time, and [[feedback_closure_language_precision]] keeps them. ⭐ Each correction is stated here,
  once, against the tree:**
  - **(a)/(b) the never-echo set is ⛔ NOT "asserted against REAL seeded plaintexts" as claimed.** The
    VPA leg still is not; the ADDRESS leg was ⛔ not either until 2026-09-22 — `address_ciphertext`
    is NULLABLE and the fixture never populated it, so `expect(body).not.toContain('address')` was
    passing against a NULL column and could ⛔ never have failed. ⭐ Now really seeded, with the
    PLAINTEXT asserted absent and the assertion proven able to fail.
  - **(c) Task 7's ticked lines were partial, domain-level or vacuous.** ⭐ Already un-ticked by the
    v1.1 validate pass; the ones since closed are recorded against their own bullets, ⛔ not here.
  - **(d) the counts were unverifiable from the diff, and they were WRONG.** ⭐ **ATTESTED
    2026-09-22** by running each package against `twt-test-pg` on **:5433**: domain **3,462** ·
    api **1,339** · contracts **1,186** · mobile **545** · admin **486** · i18n **110** ·
    events **36** = **7,164**. ⚠ The record's `7,046` and v1.0's `7,116`/`7,122` are all superseded;
    ⛔ none of them was ever re-earned on the correct database.
  - **(e) the Task 6 note IS inverted relative to the tree, and it is left standing below.** The
    tree had the **R9 panel BUILT** and the **Pariwar Admin card NOT** — the opposite of what the
    note says. ⭐ Both are built now (the two `[x]` bullets in chunk 3), so the sentence is ⛔ not
    merely wrong, it is stale; it stays as the record of the inversion.
...
- ✅ **2026-09-22 (v1.8) — the last open box, bullet 929, CLOSED; story → `review`.** The admin halves
  are now tested through the real route and the real hooks, not only as pure components. Writing the R9
  test found that **AC8's R9 flag had never been rendered** (both R9 reads carried the codes; no admin
  code read them). The flag is now built on the R9 panel and queue. See bullet 929's closure and the
  correction under the "AC8 on three surfaces" bullet. ⚠ The go-live fence (`-236` consequence 4) stands.
...
> |---|---|---|---|---|---|---|---|---|
| 2026-09-22 | 1.6 | **TASK 7 CLOSED — the last eight bullets, and ⭐ the test total ATTESTED at 7,236.** ⭐⭐ **THE PATTERN THIS PASS KEPT FINDING is tests that were GREEN while proving ⛔ nothing,** and the last eight were no different. **(824)** `nameDifferenceNote` carries `.trim().min(1).max(500)` and ⛔ NOTHING exercised any of the three; `correctionNeeded` was documented REQUIRED and every parse supplied it, so `.optional()` kept the suite green — ⚠ it is what the FILER-FACING banner reads, and a client rendering `undefined` as *"nothing to correct"* tells a family their details are fine when the District Admin has said they are not. **(849)** the 0116 column and both 0117/0118 enum labels were exercised ⛔ only IMPLICITLY — now asserted against the CATALOG, on the **right types**, with a ⛔ non-vacuity leg. ⚠ Enum labels ⛔ cannot be removed in Postgres and these are written ⛔ only on the Pariwar Admin's return, the least travelled path in the story. **(920)** the panel's MIXED `[clerical_difference, does_not_match]` case had ⛔ no test — ⭐ a panel that read *"there IS an approved difference"* would tell a District Admin the claim is fine when one of the two payout destinations belongs to **somebody else**. **(874)** three ZERO-greps answered: P1's two 409 codes as DISTINCT codes (⭐ the domain refuses these cases; what was untested is the TRANSLATION — an unmapped error reaches the console as *"something went wrong"* and trains an operator to read a RULING as a glitch), and the console packet's `accountsComplete`/`currentAndPassing`/`differenceReasons`. **(825)** the AC9 describe claimed *"no log, event, audit line or error body"* while scanning ⛔ only `events_log` — re-titled to what it scans, with the decrypted-plaintext legs NAMED where they live. **(798)** the returnable MATRIX: ⛔ only the states that ALLOW a return were ever exercised. ⚠⚠ **FOUR CORRECTIONS TO MY OWN WORK, each caught by running rather than reading:** I **duplicated an existing test** (deleted mine, folded in its one addition); I asserted against the **wrong schema** (`RecordNomineeBankResponse` vs `NomineeBankStatusResponse`); I had the **stale wire shape backwards** (`current_check` and `latest_check_is_stale` are MUTUALLY EXCLUSIVE); and a fixture's `seedClaim` seeded the very accounts my *"no accounts"* test asserted absent. ⭐⭐ **EVERY new assertion was PROVEN ABLE TO FAIL** — dropping `.trim().min(1)`, making `correctionNeeded` optional, widening AND narrowing `TRUSTEE_RETURNABLE_STATES`, disabling the stale branch, dropping `passing` from the difference flag, forcing `passing = true` in the packet. ✅ **ATTESTED per package on `:5433`:** domain **3,488** · api **1,365** · contracts **1,191** · mobile **551** · admin **495** · i18n **110** · events **36** = **7,236**; typecheck 20/20, lint 20/20. ⚠ **NOT ACHIEVED:** bullet 918 stays OPEN (the DA console's AC6/AC8 flags, the error-code table, refetch-on-focus, `R9CasePanel`), and four residuals are recorded against their own bullets. ⚠⚠ **THE GO-LIVE FENCE IS UNAFFECTED** — `-236` consequence 4: 6.18 must ⛔ not go live until 6.20 lands. Task 7 makes the story MERGEABLE, ⛔ not shippable. | BigDev + Claude |
| 2026-09-22 | 1.5 | **THE TWO REMAINING USER-VISIBLE DEFECTS, PLUS TIDY AND THE RECORD (734, 744, 912, 749, 920, 916).** ⭐⭐ **THE STEP-UP DEFECT WAS TWO BUGS HIDING EACH OTHER** and fixing either alone would have left the operator stuck: ⛔ only the INTAKE set `stepUpRequired`, AND the shell gated the panel on `result === null` while the bank save is reachable ⛔ ONLY once a result exists ⇒ **the panel was STRUCTURALLY unreachable** for a route that sits behind the same `stepUp` preHandler. A raw *"step up required"* string, five minutes into typing two account numbers on a bereavement call, with ⛔ no way out. Both fixed and **each half proven to fail independently**; a second test pins that suppressing THAT code ⛔ cannot silence any other error. ⭐⭐ **ANNOUNCEMENTS — and the bullet UNDERSTATED it.** Re-derived at source first: most of the admin side already carried `role="status"`, so ⛔ only two named items were open. ⚠ But `ifsc_error`, `vpa_invalid`, `saved` and `notice` on the same member screen carried ⛔ **no accessibility role at all** — including **the submit outcome**, the single most important thing to announce on a claim screen. All now announce, `saved` **polite** and `notice` **assertive**. Tests assert **`role="status"` itself**, ⛔ not a testid, because a testid proves a node exists and proves ⛔ nothing about whether anyone is told. ⚠ My first sent-back test seeded the read and FAILED — the hint is form-driven; recorded. ⭐ **TIDY WAS ⛔ NOT ALL TIDY:** `BankDetailsCard` hand-copied BOTH validation regexes and the mobile note hard-coded `500` **twice** — ⚠ a copied rule DRIFTS, and the mobile one would have **truncated a family's note client-side at the old length before the server ever saw it**. `NOMINEE_BANK_ACCOUNT_NUMBER_REGEX` is now exported and both call sites import it. ⭐⭐ **AND ONE ASSERTION COULD ⛔ NEVER HAVE FAILED:** `not.toContain('address')` ran against a fixture that seeded ⛔ no address — `address_ciphertext` is NULLABLE — while its own comment claimed it was *"not vacuous"* (true of the mobile, ⛔ FALSE of the address). Seeded for real, plaintext asserted, ⭐ and proven twice: the `.strict()` response schema refuses an extra key outright, so the probe had to smuggle the address through an EXISTING field. ⭐⭐ **THE TEST TOTAL IS ATTESTED AT LAST** — run per package against `twt-test-pg` **:5433**: domain **3,462** · api **1,339** · contracts **1,186** · mobile **545** · admin **486** · i18n **110** · events **36** = **7,164**. ⚠ `7,046`, `7,116` and `7,122` are ALL superseded; ⛔ none was ever re-earned on the correct database. The Dev Agent Record's overclaims are corrected IN PLACE with the originals ⛔ NOT rewritten, including the inverted Task 6 note, which stays standing beside its correction. ⚠ **The VPA never-echo leg is still ⛔ not asserted against a real plaintext, and is ⛔ not claimed to be.** ✅ Live: domain 3,462 · api 1,339 · admin 486 · mobile 545, typecheck 20/20, lint 20/20, microcopy green. ⚠ **17 bullets remain** — the API/HTTP block, the domain gate matrix, and the admin-UI coverage. | BigDev + Claude |
...
### File List — fourth pass (2026-09-22, v1.8): bullet 929's remainder + the R9 AC8 flag

**NEW (2):**

- `apps/admin/tests/r9-case-panel.test.tsx` — 10 tests; `R9CasePanel` + `R9VotingPage` (AC2 on demand, AC8, D3).
- `apps/admin/tests/verifier-console-route-name-check.test.tsx` — 22 tests; the mounted District Admin console
  (AC4/AC6 blocker table, AC8, the error table, no refetch on focus).

**MODIFIED (5):**

- `apps/admin/src/modules/r9-voting/R9CasePanel.tsx` — renders `r9-name-difference-flag` from the panel's own read.
- `apps/admin/src/modules/r9-voting/R9VotingPage.tsx` — badges each queue row from the queue's own read.
- `apps/admin/tests/nominee-name-check-panel.test.tsx` — +3 (the selection reset).
- `apps/admin/tests/helpline-bank-details.test.tsx` — +4 (anonymized nominee, nothing on file, change of claim ×2).
- `apps/admin/tests/pending-case-card-return.test.tsx` — +1 (the D3 disclosure), `api/client` faked for it.
- this story; `_bmad-output/implementation-artifacts/sprint-status.yaml` (`in-progress → review`).

## Change Log

| Date | Version | Description | Author |
|---|---|---|---|
| 2026-09-22 | 1.8 | **THE LAST OPEN BOX — bullet 929 (the admin UI halves) CLOSED; status `in-progress → review`.** ⭐ +40 admin tests (499 → 539), every new group proved red-capable by a planted regression. ⭐⭐ **One item was a MISSING FEATURE, ⛔ not a missing test:** the R9 screen never rendered AC8's flag although both R9 reads have carried `name_difference_reasons` since the 2026-09-20 review — so the "AC8 on three surfaces" bullet was ticked with its R9 third un-built. Now rendered on the R9 panel and queue; correction note appended under that bullet, ⛔ the tick not rewritten. ⚠ Pinned, ⛔ not changed: the DA console's "you sent it back" approve-blocked reason needs the names disclosure opened. ⚠⚠ **The go-live fence is UNAFFECTED** — `-236` consequence 4: ⛔ not live until 6.20 lands or the compare source changes. | Dev (Claude Opus 5) |
