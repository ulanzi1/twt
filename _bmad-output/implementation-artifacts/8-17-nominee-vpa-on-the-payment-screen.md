...
| ⭐ It **already renders unmasked coordinates** — holder, bank, **full account number**, IFSC | `pay.tsx:473-480` (`FieldRow` ×4), inside the **else-branch of the ternary at `:448`** | ⭐ read |
...
the ternary at `:448`**, ⛔ never in the decrypt-failure branch.
...
- [ ] **Task 3 — The screen + the label** (AC1, AC2, AC6) — a `FieldRow` beside `ifsc_label` at
      `pay.tsx:473-480`, ⭐ **inside the `:448` ternary's else-branch**; ⛔ omit the row when absent;
