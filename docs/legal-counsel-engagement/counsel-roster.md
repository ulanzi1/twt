...
| `nda_signature_status` | **`signed` 2026-06-21** · held in **trustee-owned OneDrive Personal Vault** — ⚠ per-document locator ⛔ OWED | ⚠ `-157` cl.2. Executed **off-repo by design** (`engagement-letter-template.md` §14 step 5); ⛔ the required `engagement-ledger.md` §5 reference path is **missing**. ⛔ Un-attested until it lands. ⚠ The sheet's A2.1 box was **unticked** — date only |
| `engagement_letter_signature_status` | **`signed` 2026-06-21** · **trustee-owned OneDrive Personal Vault** — ⚠ locator + counsel-side archive path ⛔ OWED | ⚠ `-157` cl.2, as above. ⚠ §14 also requires **Panel chair + ≥1 further trustee + a trust-side witness**; ⛔ whether those were obtained is ⛔ not recorded |
