...
- [x] [Review][Patch] Fail-soft isolation is broken: an attested-history read failure silently discards the already-resolved missed-cycle list (and a raw Postgres error inside `listMemberMissedCycles` can poison the shared scope tx, forcing the same full wipe via `HISTORY_EMPTY`) — contradicts the code's own comment ("an unresolvable passbook must never suppress the member's own missed cycles") and AC4. Reproduced directly. **Fixed**: `resolveHistory` now wraps the attested-history read in its own try/catch, returning `{ rows: [], totalInr: 0, missedCycles }` on failure instead of propagating to the outer `HISTORY_EMPTY` catch. New regression test added (`contribution-history.test.ts`: "an attested-history failure must NOT empty already-resolved missed cycles"). [`apps/api/src/modules/member-pool/handlers.ts:186-198,608-707`]
- [x] [Review][Patch] `personalEventRequestForCycle` — documented as "the ONE place" the D4 UUID mapping happens — is dead code; `MissedCycleSection.tsx`/`PersonalEventAssertion.tsx` independently inline the same mapping instead of calling it. Functionally equivalent today but can silently diverge. **Fixed**: `personalEventRequestForCycle` narrowed to take `cycleId: string | undefined` directly (absorbing the undefined-branch too) and wired into `PersonalEventAssertion.tsx`'s `onPress`; `missed-cycle-section.test.ts` updated to match the new signature plus one added case for the `undefined` branch. [`apps/mobile/components/yogdaan-bahi/missed-cycles.ts`, `apps/mobile/components/member-status/PersonalEventAssertion.tsx`]
- [x] [Review][Patch] ~~`MAX_MISSED_CYCLE_ROWS` is unused~~ — **CORRECTED DURING IMPLEMENTATION: false positive, all three review layers missed this.** This is a deliberate, established repo-wide pattern, not an oversight: the sibling `MAX_CONTRIBUTION_HISTORY_ROWS` (`history.ts:140`) is likewise never referenced by its own `.limit(500)`, with the identical rationale spelled out at its call site (`history.ts:300-301`) — "the forced-pagination invariant gate accepts a literal for a fixed bound but flags a named-const reference." Wiring the constant into the SQL would have TRIPPED the `domain-invariants:check` gate. Left unchanged; verified the gate still passes. No action taken. [`packages/domain/src/contribution/facts.ts:719,815`]
- [x] [Review][Patch] Stale test-file citation in JSDoc — cites non-existent `missed-cycle-rows.spec.ts`; the actual pinning test is `apps/jobs/tests/missed-cycle-visibility-live.test.ts`. **Fixed.** [`packages/domain/src/contribution/facts.ts` — `listMemberMissedCycles` JSDoc]
- [x] [Review][Patch] `coveredFromSql(scope.pariwarId)` is invoked twice in the same SQL statement; the second clause (`<= at`) already implies the first (`IS NOT NULL`), making it superfluous while doubling an expression to keep in sync. **Fixed**: hoisted into a single `covered` CTE, cross-joined once; both WHERE arms read the same computed value. Verified against the real `twt-test-pg:5433` live-DB test (`missed-cycle-visibility-live.test.ts`, real worker → sweep → `alert.closed` → this read). [`packages/domain/src/contribution/facts.ts:800-820`]
- [x] [Review][Patch] `toDate(row.closed_at) as Date` re-introduces the exact cast pattern the adjacent comment names as "the 10.24 live bug" — `toDate` can return `null`; the assertion silently discards that possibility. **Fixed**: replaced the cast with an explicit null-check that throws a descriptive error (caught by `resolveMissedCycles`'s existing fail-soft), instead of silently manufacturing an invalid `Date`. [`packages/domain/src/contribution/facts.ts:823-830`]
- [x] [Review][Patch] Per-pool/per-cycle context resolution inside `resolveMissedCycles`'s `for...of` loop is fully sequential (one DB round-trip per distinct pool) even though lookups across distinct pools are independent. **Fixed**: distinct pool contexts and cycle freeze instants are now pre-fetched CONCURRENTLY via `Promise.all` before a single synchronous pass over `missed` — ordering and per-row omission logic are unchanged. Verified against the live-DB test. [`apps/api/src/modules/member-pool/handlers.ts:733-812`]
- [x] [Review][Patch] (resolved from Decision) `resolveMissedCycles`'s catch-all conflates "zero missed cycles" with "the read failed" with no distinguishing signal. **Fixed**: bumped `log.warn` → `log.error`, matching every other fail-soft catch in this module (`:130`, `:164`, `:~700`) — no new metrics pipeline invented, consistent with this module's existing observability convention. [`apps/api/src/modules/member-pool/handlers.ts:808-813`]
- [x] [Review][Patch] (resolved from Decision) Trim per-row `missed_cycle.entry_a11y` copy — it currently restates the full `missed_cycle.section_note` disclaimer verbatim on every row; rely on the section-level note and shorten the per-row string. **Fixed** in both locales — `entry_a11y` is now purely structural ("Cycle {cycle}, Pool {pool}, Reference {reference}"); the per-row disclaimer is still spoken once via `state_a11y` on the state line, and once more at the section level via `section_note`. `microcopy:check` + `i18n:check-parity` both green. [`packages/i18n/locales/en/contribution.json`, `packages/i18n/locales/hi/contribution.json`]

**Dismissed (7):** `cycleRef ?? poolCanonicalIdentifier` empty-string gap (speculative, not reachable per code-access review layers) · `router.push(... as never)` typed-route bypass (pre-existing codebase convention, see `YogdaanBahiRow.tsx:65`) · import ordering in `YogdaanBahi.tsx` (cosmetic, lint would catch a real violation) · `friction-budget.md` self-authored disposition (process observation, out of code-review scope) · `MissedCycleRow`/`MissedCycleEntry` naming similarity (no concrete failure scenario) · `HISTORY_EMPTY` live-call-site question (resolved — confirmed live at `handlers.ts:195`, central to the Patch finding above) · (resolved from Decision) `missed-cycle-non-aggravation.test.ts`'s lexical-scan fence left as first-pass — consistent with the codebase's existing `no-ingest-path.test.ts` precedent.

**Verification run after applying patches:** `microcopy:check`, `i18n:check-parity`, `domain-invariants:check`, `friction:check` all green; `tsc --noEmit` clean on `@twt/domain`/`@twt/api`/`@twt/mobile`; `@twt/contracts` full suite (870 tests) green; `@twt/domain` full non-DB suite (1598 tests) green; `@twt/mobile` full suite (242 tests, incl. updated `missed-cycle-section.test.ts`) green; `apps/api` `contribution-history.test.ts` + `missed-cycle-non-aggravation.test.ts` (17 + 7 tests, incl. new regression test) green; live-DB `apps/jobs/tests/missed-cycle-visibility-live.test.ts` against real `twt-test-pg:5433` green (validates the `coveredFromSql` CTE consolidation end-to-end).
