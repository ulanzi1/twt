...
describe('ClaimDpdpaConsentRecordedPayloadSchema (the 24th claim event)', () => {
  it('is registered as a claim event type + bound in the payload-schema map', () => {
    ...
    expect(CLAIM_EVENT_TYPES).toHaveLength(35); // Story 6.21a added the 35th (claim.death_certificate_reviewed — an identity annotation, D5); Story 6.20 added the 33rd + 34th (claim.nominee_determination_recorded, claim.nominee_lock_released — both identity annotations, D4 / AC2); Story 6.18 added the 32nd (claim.nominee_name_checked)
