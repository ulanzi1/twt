...
- [ ] [Review][Patch] **⭐⭐ THE HEADLINE — the `t()`-through proof is CIRCULARLY DEFERRED between this story and 11b.2b, and its stated ground is FALSE** [`packages/ui/tests/contribution-list/presenter.test.ts:193-197`]
...
- [ ] [Review][Patch] **The presenter's exhaustiveness throw SERIALIZES the member's name parts into its message — and the mobile catch logs that message** [`packages/ui/src/contribution-list/presenter.ts` (`default:` arm) → `apps/mobile/components/contributor-list/PoolContributorList.tsx:129-131`]
...
- [ ] [Review][Patch] **The presenter's and view-model's stated design rationale is now FALSE — derivation is EAGER over the whole list, not per visible row** [`packages/ui/src/contribution-list/presenter.ts` header; `packages/ui/src/contribution-list/view-model.ts` header]
