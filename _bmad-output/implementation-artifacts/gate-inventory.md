...
| `pnpm ci:local` | ✅ Active as merge gate. Mirrors all 14 ci.yml jobs sequentially. Integration jobs need `DATABASE_URL` on `:5433`. |
| Pre-push hook | ✅ Active. `.githooks/pre-push` runs `ci:local` before every push. |
| AI-4 open | ⚠️ Enforcement-floor attestation — ratify `ci:local` + pre-push as sanctioned merge gate via ADR (gate-ran status not yet auditable). |
| AI-5 open | ⚠️ Enforcement-floor hardening — `timeout-minutes`, `cache-dependency-path`, pin `tsx` across 1.16x jobs + `ci:local`. |
...
| `benefit-mechanism` check (a): rule-record tag scan | 1.16d | No rule records in DB (`clause_versions` + `apps/api/src/modules/rules/` land at Story 2.3). Scan finds zero rows → pass. | **Story 2.3** — rule registry lands; tag scan flips enforcing. |
...
| `schema-diff` + `benefit-mechanism` → full teeth at rule-registry | 1.16c/d | Story 2.3 | `clause_versions` table + rule records exist; both tag scan and non-add guard enforce against real data. |
| TS-literal seed extractor | 1.16d | Story 2.3 | Deferred extractor for benefit-mechanism TS-literal cross-check (see deferred-work). |
