...
| The **catch-up seam**, named and typed as absent (**D8**). | **Building catch-up.** R7(D) `catch_up_required` / R7(E)/(F) `complete_all` have no mechanism. Decision **2026-08-07-086** names 10.23 as its dependency but states in terms: *"its own scope was NOT expanded or redefined by this entry."* ⛔ Out of scope to **build**, but **Escalation 6's invariant blocks `done`** until every package this story imposes has a satisfiable completion path. |
...
satisfiable through a ratified system workflow before 10.23 is `done`.* A successor catch-up story is
...
6. ⛔ **BLOCKING DISCHARGE CONDITION — catch-up has no mechanism and no owner, and this story creates
   ...
   > binds the **flag flip** — where the harm actually begins — rather than story closure alone.
...
- [x] ⛔ **Escalation 6 is a DISCHARGE INVARIANT, not a `deferred-work.md` row.** Record it as an entry
      ...
      and closing it is not, until the property holds.** Record the flag as the enforcement mechanism
...
they can discharge and cannot** (Escalation 6). That is why Escalation 6 blocks `done` and the other
eight do not.
...
**All 14 ACs satisfied; all 12 tasks complete. The story is `review`, NOT closed — Escalation 6's
discharge invariant is undischarged and the AC14 flag is OFF.**
...
#### ⛔ The condition on `done` — this flip is NOT a closure
...
**flag flip** as well as closure. **Merging with the flag off is permitted.** Enablement authority is
the Trustee Panel's exclusively (Decision `2026-08-07-089`); naming the owner is an ownership
assignment, **not** a discharge.
