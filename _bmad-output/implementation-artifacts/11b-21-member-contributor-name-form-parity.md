...
- [ ] **Task 0 — Re-verify the baseline** (all ACs)
  ...
  - [ ] Re-run the routing note §7 commands. Confirm the five `return null` arms (`:1208`, `:1215`,
...
- [ ] **Task 1 — GOVERNANCE COMMIT, before any code** (AC0)
  ...
  - [ ] In the same entry, record: `-168` cl.5 superseded (the throw); `-169` cl.8 superseded in part
  ...
  - [ ] Annotate `-222`, `-221`, `-168`, `-169` and `-172` with a pointer to `-224`, as annotations only
  ...
  - [ ] `epics.md`: add `### Story 11b.21: …` after the 11b.20 section and before `## Epic 12`, using
        ...
        cl.4.
  - [ ] Annotate **FR-24** at `epics.md:68` (*"First-name + last-initial only"*) and the PRD's §FR-24
  ...
  - [ ] Sprint row `11b-21-…` → `in-progress`; annotate its row comment and the 2026-09-18d ledger
  ...
  - [ ] Commit as `governance(11b.21): -224 — seven engineering calls; section the story; row → in-progress`.
...
- [ ] **Task 3 — Contract** (D2, AC2, AC10)
  ...
  - [ ] Update `contributions.test.ts`: fixtures, every existing decoy kept, and the old field names
...
- [ ] **Task 4 — API handler** (D3, D4, D7, AC1, AC2, AC7, AC10)
  - [ ] Read the mode once via `resolvePoolNamePresentationModeForRequest(tx, pariwarId)`. This is the
        house pattern: `handlers.ts:486` (`resolveDriveList`), `:754` (`resolveDriveDetail`).
  - [ ] Delete the `getCurrentMemberStates` call and the `representable` filter (D7). Map over
        `confirmed` directly.
  ...
  - [ ] Remove the now-unused `splitFirstNameLastInitial` import (`:81`).
  ...
  - [ ] Tests: the unit rewrite; the `pool-contributors-rtbf.spec.ts` inversions (AC10); live-DB legs
...
- [ ] **Task 5 — `packages/ui` presenter** (D1, D5, AC3, AC4, AC10)
  - [ ] Change the input/output kinds per AC3, widen the namespace literal, and add the placeholder ref
        ...
        "deceased" (Trap 8).
  - [ ] Update the presenter, `ANTI-WIDENING` and no-list-iteration tests per AC10 and Trap 8. Add the
        AC4 cross-namespace one-key test (in `packages/i18n/tests/`) and prove its teeth.
- [ ] **Task 6 — Mobile** (D6, AC5, AC6, AC10)
  ...
  - [ ] Change `usePoolContributorsQuery` to the new key and `gcTime: 0`, with a comment pointing at
        ...
        says).
  - [ ] Add the one-time startup `removeQueries` of the old exact key after restore (D6), with a test.
  - [ ] Update `contributor-list-render.test.ts` pins per AC10. Add the three-rows test, the real-`t()`
        placeholder leg in both locales, and the query-options test (key + `gcTime: 0`).
  - [ ] Add `PoolContributorList.tsx` to `pool-name-form.test.ts`'s `CONSUMERS` and run it (Trap 8).
- [ ] **Task 7 — Annotate the stale prose** (Trap 7; AC3's view-model annotation). Use the form
      `⚠ SUPERSEDED 2026-09-?? by Story 11b.21 / -222: …`
  - [ ] `packages/contracts/src/public-pages/sahyog-vivran.ts` (`:538-548`, `:569`) and
  ...
  - [ ] en `sahyog-vivran.json` `$comment.contributor_unnamed` (the only one naming `11b-21`): record D1
  ...
  - [ ] `packages/ui`: `view-model.ts:45,64-65,79`; `presenter.ts:23-27` (*"wire row is `{ firstName,
  ...
  - [ ] Files that locate the moved helpers in `public-pages/handlers.ts`:
        `apps/public/src/lib/sahyog-vivran-message-block.ts:72,76` and
        `packages/domain/src/encryption/README.md:97`.
  - [ ] `apps/api/src/modules/kyc/bounded-decrypt.ts:56` and `tests/unit/bounded-decrypt.test.ts:123`
        (comments naming `ConfirmedContributorRow`).
  - [ ] ⭐ Finish with `grep -rn "11b-21\|lastInitial\|last-initial\|OMITTED ENTIRELY\|public-pages/handlers" packages apps`.
...
- [ ] **Task 8 — Records** (AC5, AC6, AC8, AC9)
  ...
  - [ ] `deferred-work.md`: the four re-marks in AC8.
  - [ ] `friction-budget.md`: add the 11b.21 disposition (AC9).
