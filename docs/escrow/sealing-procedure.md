...
Each step is concrete enough that ≥2 trustees with the prerequisites can execute it without judgment calls. Mechanism-specific instructions are committed in [ADR-0001 credential-escrow-mechanism](../adr/adr-0001-credential-escrow-mechanism.md) (ratified 2026-06-05): sealing medium = physical sealed envelopes with tamper-evident seals; custodial locations = trustee residences (prod-credential) + bank safe deposit box (audit-mirror-credential) + bank safe deposit box at separate branch (high-sensitivity-tier-credential).
...
3. The `verification_check_signature` is computed at this step (it's part of what's sealed); per [ADR-0001](../adr/adr-0001-credential-escrow-mechanism.md) the signature = the tamper-evident seal-tape integrity reference (both sealing trustees sign across the seal-tape with date inscribed; the signed seal-tape is the verification artifact verified at quorum-open).

### 2.4 Apply the sealing mechanism

1. Per [ADR-0001](../adr/adr-0001-credential-escrow-mechanism.md): the credential payload is printed or written onto paper; the paper is placed in a physical envelope; the envelope flap is sealed with tamper-evident seal-tape; both sealing trustees sign across the seal-tape with their initials and the date. The signed seal-tape is the tamper-evidence — any post-seal access to the envelope contents requires breaking the tape, which destroys the trustee signatures verifiably.
...
1. Per [ADR-0001](../adr/adr-0001-credential-escrow-mechanism.md): transmission is in-person physical transfer by both sealing trustees to the custodial location appropriate to the envelope class. For `prod-credential` envelopes: trustees deliver to the designated trustee residence holding active prod envelopes; the receiving trustee signs receipt acknowledgment recorded in `escrow-ledger.md`. For `audit-mirror-credential` envelopes: both sealing trustees physically present themselves at the bank safe deposit box (Branch A); both place the envelope inside the box together; the bank-issued deposit-record slip is the custodial receipt + recorded in `custodial_location_reference`. For `high-sensitivity-tier-credential` envelopes: same procedure at the geographically-separate bank safe deposit box (Branch B per ADR-0001 commitment 2).
