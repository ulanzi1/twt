...
- [x] Mint the token at spawn. ⚠⛔ **CORRECTED AT REVIEW 2026-09-04 — THIS TASK NAMED THE WRONG FILE.** It read *"in `packages/domain/src/pool/spawn.ts` — the insert at `:454`"*; `spawn.ts:454` is inside the `pool.spawned` **EVENT PAYLOAD** object, and `spawn.ts` contains ⛔ **no `insert(pools)` at all** — the only production one is `packages/domain/src/pool/project.ts:200`. ⇒ executed LITERALLY this task would have put the token into the event payload and left `pools.public_token` NULL → `23502`. ⭐ The dev correctly minted in `project.ts` instead. ⚠ Recorded rather than silently fixed because the dev agent works from the TASKS list, ⛔ not the ACs ([[feedback_spec_edits_must_propagate_to_tasks]]) — the insert at `project.ts:200` uses **raw
...
- [x] Build the entry as a **sibling** of `<ActiveContributionCard />` in `apps/mobile/app/(tabs)/index.tsx`,
      ...
      self-suppress (`:31`), and an `onPress` handler (`:42`). ⚠⛔ **CORRECTED AT REVIEW 2026-09-04:** this task said `Pressable`; `ViewContributorsEntry.tsx` in fact uses a **tamagui `<Button>`**, which is `styled(View)` and ⛔ NOT a `Pressable`. ⭐ The distinction is ⛔ not pedantic — RN supplies `accessible={true}` on `Pressable` by DEFAULT and ⛔ NOT on a `View`, so following this task as written is precisely how the family-13(a) defect (label never announced) got in. ⛔ **Beside the card, ⛔ NOT inside it**
...
> Code review 2026-09-03 (`bmad-code-review`) — 3 parallel layers (Blind Hunter, Edge Case Hunter,
> Acceptance Auditor). Acceptance Auditor: **AC0–AC6 all satisfied, Traps 1–5 all defused, no
> Critical/High.** Load-bearing-invariant families 1/2/3/4/5/6/11/12/13 all covered by test or
> construction. The items below are the surviving Medium/Low findings.
>
> **Disposition 2026-09-03:** 5 patches applied (P1, P2, P4, P5, P6); 1 patch (`mint-on-every-event`)
> reclassified to defer after the proposed one-liner proved unsound; 4 defers recorded in
> `deferred-work.md`. `lint` + `typecheck` green for @twt/{public,domain,mobile,api}; @twt/public 458
> unit + @twt/mobile 436 unit green. ⚠ P4/P5/P6 are live-DB specs — written and typecheck/collect
> clean, **not executed here (no database in the review session)**; run `integration-tests` before merge.

- [x] [Review][Patch] Stale MMKV-persisted active-contribution cache renders a dead `SahyogVivranEntry` on app upgrade — `PersistQueryClientProvider` (`apps/mobile/components/Provider.tsx:18`, `persistOptions` has **no `buster`**, `maxAge` 7d) rehydrates a pre-11b.10 `{ assigned: true }` card verbatim (no Zod re-parse), so `data.sahyogVivranToken` is `undefined` while `data.assigned` is `true`. `SahyogVivranEntry` deliberately did **not** guard on the token ("NOT a second precondition"), so it rendered and `sahyogVivranUrl(undefined, locale)` → `Linking.openURL('…/sahyog-vivran/undefined?lang=…')` → public 404. **✅ Applied 2026-09-03:** added `|| !data.sahyogVivranToken` to the suppression guard; rationale comment records the persisted-cache rehydration path; the fence test's guard regex updated. [apps/mobile/components/sahyog-vivran/SahyogVivranEntry.tsx] Source: blind+edge.
- [x] [Review][Patch] `isSahyogDriveResponse` not extended to validate the new required `publicToken` — a malformed/truncated upstream body passed the hand-written guard, then `toDisplayRow` calls `driveHref(row.publicToken, search)` → `encodeURIComponent(undefined)` === `"undefined"`, so **every `/sahyog` row linked to `/sahyog-vivran/undefined`** instead of the module falling to its outage state. Same class as the 2026-08-27 `fundingOutcome` / empty-name hardening of this exact guard. **✅ Applied 2026-09-03:** added `typeof r['publicToken'] === 'string' && r['publicToken'].length > 0` to the per-row check. [apps/public/src/lib/sahyog.server.ts] Source: blind (+edge).
- [x] [Review][Defer] `mintPoolPublicToken()` runs on every pool event, not just genesis [packages/domain/src/pool/project.ts:229] — **reclassified patch → defer during implementation.** It sits in the unconditional `insert(pools).values({…})` in `projectPoolState`; on every non-spawn transition a fresh CSPRNG token is generated and then discarded by the `onConflictDoUpdate.set` arm — a wasted `randomBytes(16)` per transition (~4 in a pool's life) and a ~2⁻¹²⁸ chance a discarded value collides on the non-arbiter `pools_public_token_uq` and aborts the projection. The proposed one-liner (`...(existing.length === 0 ? { publicToken: … } : {})`) is **unsound**: Postgres runs `ExecConstraints` (NOT NULL) on the proposed tuple before the ON CONFLICT arbiter, so omitting the column on a non-genesis event raises `23502` and breaks every `spawned→live→closed→settled` step. A sound fix needs a split genesis/transition write path — not worth it for the cost. Comment added at the call site; carried in `deferred-work.md`. Source: edge.
- [x] [Review][Patch] "GLOBAL unique index" test did not actually test globalness — the duplicate-rejection test seeded both colliding tokens in `PARIWAR_A`; a composite `(pariwar_id, public_token)` index would reject that identically. **✅ Applied 2026-09-03:** the test now seeds the shared token into `PARIWAR_A` **and** `PARIWAR_B` (as the Docker superuser, before app scope), so only a `public_token`-alone unique index makes it pass. [packages/domain/tests/integration/pool/public-token.spec.ts] Source: auditor.
- [x] [Review][Patch] No redelivery-idempotency test for the token at spawn — nothing drove a later `projectPoolState` for the same `poolId` and asserted `publicToken` is byte-identical; a regression adding `publicToken` to the `onConflictDoUpdate.set` arm would pass every other test. **✅ Applied 2026-09-03:** new test projects a spawn, reads the address, drives the pool to `live` (the DO UPDATE arm), asserts the token is unchanged. [packages/domain/tests/integration/pool/public-token.spec.ts] Source: auditor.
- [x] [Review][Patch] Byte-identical-404 equality omitted headers — the wrong-token vs no-such-drive test compared `statusCode` and `body` but not response headers (its own comment promised "body or headers"). **✅ Applied 2026-09-03:** added a full header-set comparison (minus per-response volatile headers) to the equality assertion. [apps/api/tests/integration/public-pages/sahyog-vivran.spec.ts] Source: auditor.
- [x] [Review][Defer] `SahyogVivranEntry` lock-step suppression mirrors only one of the card's two paths [apps/mobile/components/sahyog-vivran/SahyogVivranEntry.tsx] — deferred, pre-existing. The entry gates on `!data.assigned`; `ActiveContributionCard` also `return null`s when `derivePoolProgressCardViewModel` throws on a corrupt/impossible progress read. In that rare case the entry renders with no card above it — an orphaned button; the link itself still resolves (token is independent of the progress figures). Cosmetic, rare defensive path.
- [x] [Review][Defer] Migration 0114 `SET NOT NULL` has no transitional `DEFAULT` [packages/domain/migrations/0114_pool-public-address-token.sql] — deferred, pre-existing. During a rolling deploy an old instance projecting `pool.spawned` INSERTs without `public_token` → 23502 → that spawn fails until new code is everywhere (self-heals via the forward-retrying saga). Harmless if migrations gate traffic before new code serves; the migration header covers the backfill/InitPlan traps but is silent on this window. Note for the deploy runbook.
- [x] [Review][Defer] `driveToken` param schema gives a 400/404 split for malformed addresses [packages/contracts/src/public-pages/sahyog-vivran.ts] — deferred, pre-existing. Whitespace (`.trim()` → empty → `.min(1)` fails) or a >64-char value → 400 at the schema boundary; an unknown well-formed token → 404. Not an enumeration oracle: every real token is 22-char base64url and always passes the schema, so real drives are uniformly 404 on a wrong token; and the doc-block only commits to not regex-matching the token's own shape.

> ⭐⭐ **SECOND INDEPENDENT CODE REVIEW — 2026-09-04 (`bmad-code-review`, a different LLM).** 3 parallel
> layers (Blind Hunter · Edge Case Hunter · Acceptance Auditor) over `ff6e546..worktree`, plus a
> **live-DB execution pass the first review could not run**.
>
> ⚠⛔ **THE FIRST PASS RECORDED P4/P5/P6 AS "NOT EXECUTED — no database in the review session".** A test
> Postgres (`twt-test-pg`, `:5433`) **was** reachable. Executing them: `public-token.spec.ts` **12/12
> green** (P4 + P5 now attested), and **P6 FAILS DETERMINISTICALLY** — it can never pass as written.
> ⇒ the row was flipped to `done` over a red spec.
>
> ⭐ **EXECUTED THIS PASS:** `typecheck` 19/19 green · domain `public-token.spec.ts` 12/12 · api
> `sahyog-drive` 19/19 + `rate-limit-key` green · api `sahyog-vivran.spec.ts` **31/32, 1 FAILED** ·
> mobile `sahyog-vivran-entry` 12/12 · public `sahyog-render`/`sahyog-vivran-client`/`drive-link-a11y`
> 57/57. Migration 0114's backfill verified empirically against 1192 pre-existing rows: all 22-char
> base64url, **all distinct** ⇒ the InitPlan-hoist trap did not fire.
>
> ⛔⛔ **AC1 IS NOT SATISFIED.** Its central enumeration claim is falsified by a **live two-request
> walk**, proven end-to-end this pass (D-A below). Two live defence documents state the opposite.

- [x] [Review][Decision] ✅ **RESOLVED 2026-09-04 BY ESCALATION — BigDev ruled option (d): ROUTED TO THE TRUSTEE PANEL.** ⛔ **Resolved via explicit escalation, ⛔ NOT "closed"** ([[feedback_closure_language_precision]]) — **AC1 remains NOT SATISFIED** and this story is held at `in-progress` until the Panel answers. ⭐ Artefacts committed: `_bmad-output/planning-artifacts/trustee-panel-routing-note-2026-09-04-11b10-listed-drive-discoverability.md` (the ask, with Appendix A in plain words) and `.decision-log.md#decision-2026-09-04-186` (the author-committed FACTUAL correction; ⛔ it corrects a claim **Story 11b.10 itself made**, ⛔ never a Panel answer — `-184` (A)/(B)/cl.2 all stand and are ⛔ NOT edited in place). ⚠ The three over-strong documents WERE corrected in the same pass (`routes.ts` ×3, `login-wall.spec.ts`, the matrix `D4-linkage` rider) — ⭐ that removes a claim **proven false**, a record-keeping obligation, and ⛔ pre-empts nothing: the corrected sentences stay true under every option the note offers. ⚠ The note SURFACES `-184` cl.5's option (d) (disclosure reduction) as the only lever that reduces the HARM rather than the FINDABILITY, ⛔ observationally and ⛔ without requesting it ([[feedback_gap_analysis_observational]]); cl.5 is ⛔ NOT treated as re-opened. ⛔ `limits.search` untouched in both directions (Trap 5, 11b.3a AC2). ⬇ The finding that forced the escalation: ⛔⛔ **OPTION (a) DOES ⛔ NOT CLOSE THE WALK, AND WAS ⛔ NOT IMPLEMENTED.** BigDev ruled option (a) (*drop `publicToken` from the wire, build `driveHref` server-side*); ⚠ verifying the data path BEFORE editing showed the option was mis-framed by the reviewer who offered it, so ⛔ nothing was changed and the decision returns to BigDev. **TWO facts kill it:** (1) `apps/public` has ⛔ NO database access — it `fetch`es the API over HTTP (`apps/public/src/lib/sahyog.server.ts:117`, `API_ORIGIN`) ⇒ *"build the href server-side"* ⛔ cannot avoid the token crossing that boundary; the token would simply travel as a pre-built href string instead of a field. (2) ⭐⭐ **DECISIVE — `poolCode` IS A PUBLIC SEARCH BOX ON THE RENDERED PAGE**, ⛔ not merely a JSON query parameter: `apps/public/src/pages/sahyog.astro:410` renders `<input name="poolCode">` and `sahyog.server.ts:112` forwards it ⇒ `GET /sahyog?poolCode=P-2026-08-001` returns **HTML containing `<a href="/sahyog-vivran/{token}">`**. The walk therefore runs entirely through the rendered page and ⛔ never touches the wire shape at all. ⇒ ⭐⭐ **THE DEEPER TRUTH, which reframes the whole finding: for LISTED (`closed`/`settled`) drives the token ⛔ CANNOT bound discovery, BY CONSTRUCTION — D3 *requires* publishing the link.** The token's real security value is for `live` drives, which are ⛔ unlisted. That is a property of the RATIFIED DESIGN, ⛔ not an implementation defect — which is why (a) was never capable of fixing it. ⭐ The remaining genuine options are: **(b) accept and STATE it** (amend the three defence documents to claim only what is true); **(c) gate or remove the `poolCode` filter** — closes the TARGETED lookup only, bulk paging still harvests tokens, and it is ⛔ pre-existing surface outside this story's ruled scope; **(d) escalate as a routing note** to the panel that ratified `2026-09-03-184` (B), since the claim at issue is the one it ruled on. ⚠ ⛔ **`limits.search` is ⛔ NOT an option in either direction** (Trap 5, 11b.3a AC2). ⬇ Original finding, with the live proof:  Build `driveHref` server-side inside the index render so the token never crosses the API boundary; the walk cost returns to unbounded guessing, which is the property AC1 actually claims. ⚠ Touches the contract (`sahyog-drive.ts`), the handler, the domain read's field-pick, `sahyog.astro` + `sahyog-render.ts`, **and** the two artefacts that pin the wire shape by identity — `public-vs-private-matrix.yaml` and `scrape-test.spec.ts`'s exact nine-field set. ⛔ The ratified D3 per-row link and `limits.search` are UNTOUCHED. ⬇ Original finding: ⛔⛔ **THE SEQUENTIAL WALK IS NOT CLOSED — "no sequence left to walk" is FALSE for `closed`/`settled` drives.** This story added `publicToken` to the **public** `/sahyog-drive` index row (`packages/contracts/src/public-pages/sahyog-drive.ts:121`; serialized at `apps/api/src/modules/public-pages/handlers.ts:404`). The index already carries a **pre-existing** exact-match filter on the *sequential* identifier — `poolCode` (`packages/contracts/src/public-pages/sahyog-drive.ts:195` → `packages/domain/src/pool/public-read.ts:594`). Joining them yields an unauthenticated **identifier → token → four decrypted Tier-1 bank fields** walk in **two requests**, at the same `limits.search` tier. ⭐ **PROVEN LIVE this pass:** `GET /api/v1/p/{pariwar}/public-pages/sahyog-drive?poolCode=P-2026-08-b765ec` → `200 {"items":[{…,"publicToken":"tok-077e0e83-…"}]}`. Pre-11b.10 the walk was 1 request; it is now 2 — the cost class is unchanged. `public-read.ts:455` even warns *"⛔ do not add a filter on it [publicToken] … a token filter would turn this index into an ORACLE"* while the identifier filter beside it already supplies that capability. ⚠ The *one-click* exposure is D3-ratified and correctly disclosed five times; **this residual is disclosed nowhere**. Options: (a) drop `publicToken` from the wire and build `driveHref` server-side inside the index render; (b) gate/remove the `poolCode` filter (⛔ pre-existing, out of this story's ruled scope); (c) accept and **state it** in the story record + the three defence documents. ⛔ Not patchable without your call — (a) and (b) both touch ratified surface. Source: blind+auditor, verified by live execution.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** ✅ **RULED BY BigDev 2026-09-04 — add `actorId` and emit the pair NOW.** `rotatePoolPublicToken` gains an `actorId` parameter and writes the `events_log` append + audit row in the SAME tx as the update. ⬇ Original finding: **Family 8 REAL GAP — `rotatePoolPublicToken` writes no audit, no event, and no `updated_at`** [packages/domain/src/pool/public-token.ts:90-100]. The one mutation whose *purpose* is withdrawing a leaked public address leaves zero trace; `pools.updatedAt` is `defaultNow()` with no `$onUpdate` and no trigger, so even the row's timestamp does not move. The doc-block reasons at length about the route/permission-key question and the CDN TTL but never about the audit seam, and `deferred-work.md` does not carry it. ⇒ whoever wires the ruled "routing note" route inherits an audit-less privileged mutation. ⛔ Not an unambiguous patch: the function takes no `actorId`, so emitting the pair needs a signature change (and a decision between an `events_log` append vs an audit row). Options: (a) add `actorId` + emit the pair now; (b) document the obligation on the function so the route story cannot miss it; (c) carry as a defer. Source: edge+auditor.
- [x] [Review][Defer] ✅ **RULED BY BigDev 2026-09-04 — DEFER with the constraint named.** ⛔ Family 5 stays OPEN and is recorded as such, ⛔ not closed. ⬇ Original finding: **Family 5 REAL GAP — no DB CHECK mirrors the token's app-layer shape** [packages/domain/migrations/0114_pool-public-address-token.sql; packages/domain/src/schema/pools.ts]. The app pins `^[A-Za-z0-9_-]{22}$` in *tests only*; the column accepts `''`, whitespace, a 5000-char value, or `+`/`/`/`=` (which would then need percent-encoding in a path segment — a property `mintPoolPublicToken`'s doc-block says must never be silently depended on). Family 5 requires the mirror. ⚠⛔ **BUT A LIVE CHECK OF THE TEST DB SHOWS THE OBVIOUS FIX WOULD BREAK THE SUITE:** of 2702 rows, **1510 do not match that regex** — every fixture seeds `seed-<uuid>` / `tok-<uuid>` tokens (`apps/api/tests/integration/public-pages/sahyog-drive.spec.ts` and siblings). Adding `CHECK (public_token ~ '^[A-Za-z0-9_-]{22}$')` requires migrating the whole fixture convention first. Options: (a) a looser CHECK that still closes the real hazards (`length BETWEEN 16 AND 64 AND public_token ~ '^[A-Za-z0-9_-]+$'`) — passes today's fixtures; (b) the strict CHECK + a fixture sweep; (c) defer with the constraint named. Source: edge, complication found by live DB inspection.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** **P6 can never pass — the byte-identical-404 header equality compares a per-request counter** [apps/api/tests/integration/public-pages/sahyog-vivran.spec.ts:448-456]. `stableHeaders()` strips `date`/`keep-alive`/`request-id`/`x-request-id` but leaves `x-ratelimit-remaining`, which decrements on every request against the same key. Observed live: expected `99997`, received `99998`. ⭐ Fix must preserve the security property, not just delete the header: if the two paths consumed *different* budget that difference would itself be the oracle — so strip `x-ratelimit-*` from the deep-equal **and** assert both responses consumed exactly one unit. Also strengthen the arms: both currently use non-existent tokens (`tok-${randomUUID()}` vs `tok-P-2026-09-${…}`), so the "real drive, wrong token" case is not actually contrasted against "no such drive". Source: auditor+blind, **confirmed by execution**.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04** — amended to the VERIFIED fact and to name the open escalation, in all three documents plus a fourth found in the sweep (`routes.ts:309` the rate-limit note, `routes.ts` CONTROL 7 both in the numbered list and in THE ENUMERATION BOUND, `login-wall.spec.ts:237`, and the matrix `D4-linkage` rider whose heading no longer reads an unqualified "CLOSED BY STORY 11b.10"). ⭐ Each says what IS true — the address bounds discovery for UNLISTED (`live`) drives and for archived drives past the page horizon, and ⛔ cannot for listed ones because the index publishes the link — and routes the consequence. ⛔ None of them resolves it. **Correct the falsified enumeration claim in the live defence documents** [apps/api/src/modules/public-pages/routes.ts:309; apps/api/tests/integration/public-pages/login-wall.spec.ts:237]. Both assert *"there is ⛔ no sequence left to walk"* / *"NO SEQUENCE TO WALK"*. Per D-A that is true only for `live` drives (unlisted) and drives past the page horizon. ⭐ Amend, ⛔ do not delete — the same discipline the story applied to the clause it replaced. Whatever D-A resolves to, the prose must match it. Source: auditor+blind.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** **Family 13(a) REAL GAP — the new mobile entry's label is never announced** [apps/mobile/components/sahyog-vivran/SahyogVivranEntry.tsx:79-87]. `accessibilityRole="link"` + `accessibilityLabel` + `accessibilityHint` sit on a tamagui `<Button>`, which is `styled(View, …)` (`@tamagui/button/dist/esm/Button.native.js:23`); `@tamagui/web`'s `createComponent.native.js` contains **zero** occurrences of `accessible` (verified). An RN `View` is not an accessibility element without `accessible={true}`, so the inner `Text` is focused and the label, role and hint are ⛔ not announced. ⭐ Worked example already in-repo: `apps/mobile/components/panchayat/PinnedItem.tsx:107` (`accessible={true}` + label together) with the mechanism stated at `:42`. Fix: add `accessible={true}`. ⚠ This is the un-mechanized family the checklist warns decays, on the epic before 11b.8 makes it launch-blocking. Source: edge+auditor.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** **Family 13 — WCAG 2.5.3 Label in Name on the new `/sahyog` link** [packages/i18n/locales/{en,hi}/sahyog-drive.json]. Visible text is `link.view_drive` ("View drive" / "अभियान देखें"); the `aria-label` is `link.view_drive_a11y` ("View the full details of drive {code}" / "अभियान {code} का पूरा विवरण देखें"). The `aria-label` overrides the visible label and does ⛔ not contain it as a contiguous substring in **either** locale ⇒ speech-input users saying the visible label cannot activate the link. Fix by making the a11y string a superset that *starts with* the visible label — e.g. `"View drive {code} — full details"` / `"अभियान देखें — {code} का पूरा विवरण"` — and add `expect(a11y).toContain(visible)` to `apps/public/tests/sahyog-drive-link-a11y.test.ts`, which currently asserts only that `{code}` interpolates. Source: edge.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** **AI-10-1 (family 11) — the `UNASSIGNED` conjunction has no stated member meaning** [apps/api/src/modules/member-pool/handlers.ts:655-661]. `readPoolPublicToken(...) === null → return UNASSIGNED` suppresses the member's **entire** contribution card — CTA, UPI reference, amount, progress meter — not just the new entry, and is documented only as "lock-step" rendering hygiene. AI-10-1 requires one sentence about the *member's* experience, checked against the Niyamavali. ⭐ The branch is unreachable in practice and the story should say **why**: the runtime role `twt_app` holds only `INSERT, SELECT, UPDATE` on `pools` (⛔ no `DELETE`, verified live) and the token is read in the *same* scope-tx that already resolved the pool ⇒ the row cannot vanish between the two reads. The story argues unreachability from the NOT NULL constraint, which is the weaker ground. Source: edge (REAL GAP) + auditor (construction, noted omission).
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** Both token accessors return `''` rather than `null` for an empty-string token [packages/domain/src/pool/public-token.ts — `rotatePoolPublicToken`, `readPoolPublicToken`]. `rows[0]?.publicToken ?? null` coalesces only null/undefined, so `''` (which `NOT NULL` permits and the unique index accepts) flows through every `=== null` guard, reaches `AssignedContributionCard`'s `z.string().min(1)` and 500s `/member/active-contribution` — while `/sahyog` catches the same row via the new `publicToken.length > 0` guard, so the two consumers disagree. Unreachable today (no writer emits `''`), but it is the hazard D-C's CHECK would close. Fix: `?? null` → `|| null` in both. Source: edge.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** The "ZERO NULL TOKENS" assertion passes vacuously on an unexpected result shape [packages/domain/tests/integration/pool/public-token.spec.ts:306]. `expect((rows.rows ?? (nulls as …)[0]?.n ?? 0).toBe(0)` defaults to `0` whenever the row shape is not what it expects ⇒ the assertion cannot fail for the reason it exists. Assert the row is present first, then its value. Source: blind.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** The drive-link a11y test inspects only the first of two identical link blocks [apps/public/tests/sahyog-drive-link-a11y.test.ts; apps/public/src/pages/sahyog.astro]. The `<a href={col.hrefOf(row)}>` block is duplicated verbatim in the **active** and **archive** tables (2 occurrences, verified), but the test slices from `PAGE.indexOf(...)` ⇒ a regression stripping `aria-label` from the archive table — the larger of the two — is caught by nothing. Assert over both occurrences. Source: blind.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** The link's accessible name escapes `MatrixField` visibility gating [apps/public/src/pages/sahyog.astro (`aria-label={col.a11yOf?.(row)}`); apps/public/src/lib/surface-fields.ts (`driveLinkA11yLabel: null`)]. The cell *value* renders through `<MatrixField>`, but the accessible name is interpolated straight into the DOM and is mapped to `null` in `SAHYOG_DRIVE_ROW_FIELD_IDS`, so the surface-field derivation and the scrape leg are blind to it. Its content is `poolCanonicalIdentifier` — a matrix-governed field. If the matrix suppresses that field the `<td>` renders empty while the `aria-label` announces the drive code anyway. `hrefOf` is gated by `isVisible('drive_href')`; `a11yOf` is gated by nothing. Source: blind.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** SSR guard length-checks `publicToken` but not `poolCanonicalIdentifier` [apps/public/src/lib/sahyog.server.ts:185-196]. The new check asserts `typeof === 'string' && length > 0`; the identifier immediately above is still `typeof === 'string'` only. An empty identifier passes and `driveLinkA11y('')` renders "View the full details of drive " on every affected row — N links sharing one accessible name, the exact defect the label exists to prevent. Mirror the check. Source: edge.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** The 503 outage page announces a `<dt>` with no definition [apps/public/src/lib/sahyog-vivran-render.ts; apps/public/src/pages/sahyog-vivran/[driveToken].astro]. `buildSahyogVivranOutageView()` sets `poolCanonicalIdentifier: ''` → `null` → `MatrixField` emits nothing, but the "Drive code" `<dt>` still renders ⇒ a screen reader announces a term with no value. Not echoing the address is right; the orphaned `<dt>` is the unhandled consequence — suppress the pair together. Source: edge.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** Unhandled promise rejection on the outbound link [apps/mobile/components/sahyog-vivran/SahyogVivranEntry.tsx:90]. `void Linking.openURL(...)` — rejects if no handler exists or the OS refuses the URL. Two precedents exist and the diff copied the weaker one: `LockInClockWidget.tsx:62-67` wraps the identical call in `try/catch` with a documented reason; `(auth)/terminated.tsx:94` does not. Adopt the `LockInClockWidget` shape. Source: edge.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** `driveHref` re-reads the raw `lang` query value instead of the resolved locale [apps/public/src/lib/sahyog-render.ts]. The page validates `langParam` for its own locale resolution, then `driveHref` reads the parameter again straight from the URL and re-emits it into all 25 row links (percent-encoded, so not an injection). `/sahyog?lang=<2KB string>` is reflected into every link on an `s-maxage` page. Use the already-resolved locale. Source: blind.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** Add the rotation rider for the **index** cache [packages/domain/src/pool/public-token.ts]. The doc-block covers the drive page's own five-minute `s-maxage=300` edge window but not `/sahyog`'s: after a rotation every warm PoP keeps serving index rows whose `driveHref` points at the withdrawn token, so the page this story made linkable 404s from a *link* for up to five minutes. State it in the same rider. Source: edge.
- [x] [Review][Patch] ✅ **APPLIED 2026-09-04.** Governance-prose corrections (bundled) — (a) **Task 1** points the mint at `packages/domain/src/pool/spawn.ts:454`, which is inside the `pool.spawned` **event payload**; `spawn.ts` has no `insert(pools)` at all (the only production one is `project.ts:200`). Executed literally it would have left `public_token` NULL → `23502`; the dev correctly minted in `project.ts`. ⚠ [[feedback_spec_edits_must_propagate_to_tasks]] — the dev agent works from the Tasks list. (b) **Task 6** says to follow `ViewContributorsEntry.tsx` *"exactly … `Pressable` + `onPress`"*; that component uses a tamagui `<Button>`, and the mis-description is part of why the family-13(a) gap above was easy to miss. (c) **Task 4**'s ✅ covers work that correctly did not occur (no token key was added to `SAHYOG_VIVRAN_FIELD_IDS`). (d) Completion Notes and `sprint-status.yaml` say the D3 exposure sentence appears in **five** places; the `deferred-work.md` closure entry says **four** (five code locations is correct). (e) Stale `/sahyog-vivran/[poolCanonicalIdentifier]` route addresses remain in prose this story otherwise swept — `apps/public/src/lib/surface-fields.ts:396,400`, `apps/public/src/lib/sahyog-vivran-render.ts:1`, plus `packages/domain/src/schema/pariwar_nominee_bank_masking_schedule.ts:51`, `apps/api/src/modules/nominee-bank-masking/handlers.ts:137`, `apps/admin/src/modules/nominee-bank-masking/i18n-en.ts:14`. Source: auditor.
- [x] [Review][Defer] Rotation has no reachable operational path [packages/domain/src/pool/public-token.ts] — deferred, pre-existing (ruled). D1 accepts permanent public access on the strength of rotation being *"the ONLY remedy"*, but the story rules out a route and a permission key (v39→v40 is a governance act) and the migration header rules out direct SQL as the fallback. AC2 is satisfied literally (rotation exists and its isolation is tested); the operational path is owed as a **routing note**. ⛔ Not this story's edit — but the gap between "the remedy exists" and "the remedy is reachable" should be named in the routing note, not left implicit. Source: blind.
- [x] [Review][Defer] Third deploy-ordering direction: new app ↔ old API [packages/contracts/src/contributions/active-contribution-card.ts] — deferred, pre-existing. `sahyogVivranToken` is a **required** `z.string().min(1)` and the mobile SDK Zod-parses the response (`packages/api-client/src/index.ts:539-546`), so a new build against a not-yet-deployed API fails the parse and the **whole** My Pool card disappears — not just the entry. The review recorded the reverse direction (P1, stale MMKV) and the migration direction (0114) but not this one. An `.optional()` field with the same component guard would be strictly safer. Source: blind+auditor.
- [x] [Review][Defer] The public drive URL carries no Pariwar segment while the lookup is Pariwar-scoped [apps/mobile/lib/public-site.ts:60-62; packages/domain/src/pool/sahyog-vivran-read.ts:387] — deferred, pre-existing. `sahyogVivranUrl` emits `/sahyog-vivran/{token}?lang=`; the Astro page resolves `ACTIVE_PARIWAR_ID` and the read ANDs `eq(pools.pariwarId, pariwarId)`. The token is *globally* unique so it would resolve on its own, but the predicate 404s it ⇒ a member of any Pariwar ≠ the public site's gets a rendered card, a rendered entry and a dead link. Unreachable under the single-Pariwar v1 posture, which nothing in the diff asserts. Source: edge.
- [x] [Review][Defer] A rotated token survives in the 7-day MMKV-persisted card [apps/mobile/components/sahyog-vivran/SahyogVivranEntry.tsx; apps/mobile/components/Provider.tsx:18] — deferred, pre-existing. P1's guard catches a *missing* token; it cannot catch a *stale* one. After a rotation, an offline or pre-refetch cold start opens the withdrawn address and 404s — the dead link the guard exists to prevent, in the one scenario where the address was deliberately withdrawn. Not fully closable client-side; at minimum bump `persistOptions.buster` when the card shape changes. Source: edge.
- [x] [Review][Defer] Migration 0114 rolling-deploy window — **already carried** from the first pass; re-confirmed this pass in both directions (old code + new schema → `23502` on spawn; new code + un-migrated schema → `42703` on `/sahyog` and the member card, which is ⛔ not the fail-soft `null` arm the handler comments assume). Source: blind+edge.

> ⛔ **DISMISSED THIS PASS (5):** (1) *"`mintPoolPublicToken` runs on every pool event"* — correctly
> reclassified to defer by the first pass; the `ExecConstraints`-before-arbiter reasoning is sound.
> (2) *"the FORCE-RLS backfill matches zero rows and then `SET NOT NULL` aborts"* — **empirically
> disproven**: 1192 pre-existing rows were backfilled with distinct tokens. (3) *"`card.poolId`
> comparison is vacuous"* — false positive; `poolId` **is** on the card
> (`active-contribution-card.ts:93`). (4) The 503 blank drive-code — intentional and already flagged
> by the story. (5) `/sahyog-vivran/` with no segment returning a non-empty JSON 404 body — names no
> drive, so not an oracle. Plus *"`hrefOf`/`a11yOf` set-iff is not type-enforced"* — real but asserted
> by test; a discriminated-union refactor is beyond this story.
