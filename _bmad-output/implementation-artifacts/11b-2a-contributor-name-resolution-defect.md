...
- [ ] [Review][Decision] **The erasure guarantee ENDS AT THE WIRE — an RTBF'd contributor keeps rendering from the MMKV-persisted query cache for up to 7 days, in plaintext, and survives sign-out**
...
- [ ] [Review][Patch] **The new batched lifecycle read at step (6a) is the ONE step in the pipeline with no degradation path — any failure tells an assigned member "you have no live pool"** [`apps/api/src/modules/member-pool/handlers.ts:319-322`]
...
- [ ] [Review][Patch] **The per-row `getMemberKycProfile` catch cannot distinguish a transaction-wide abort from a per-row anomaly — one transient fault logs N FALSE integrity anomalies against real members** [`apps/api/src/modules/member-pool/handlers.ts:349-359`]
...
- [ ] [Review][Patch] **The RTBF integration spec HARD-CODES the erasure sentinel instead of importing it — the exact drift class this story's own AC3 forbids, in the one test that proves the leak is closed** [`apps/api/tests/integration/contributions/pool-contributors-rtbf.spec.ts:42`]
...
- [ ] [Review][Patch] **Two stacked JSDoc blocks on `anonymizeCiphertextOnly`; the first describes the OPPOSITE function and contradicts the code beneath it** [`apps/api/tests/integration/contributions/pool-contributors-rtbf.spec.ts:255-272`]
...
- [ ] [Review][Patch] **`spawns no more workers than there are items` is TAUTOLOGICAL — it cannot fail** [`apps/api/tests/unit/bounded-decrypt.test.ts:47-58`]
...
- [ ] [Review][Decision] **`missed_cycle.state_a11y` was re-worded to assert NON-CONSEQUENCE, which the system's own contribution-discipline model contradicts** [`packages/i18n/locales/{en,hi}/contribution.json`]
