...
- [x] [Review][Patch] `vpa`'s `.max(255)` is narrower than the write-side collection regex permits — a legitimately-collected VPA of 256–321 chars would fail response serialization (a live 500, contradicting AC2/Trap3's "never a 500") [`packages/contracts/src/contributions/nominee-accounts.ts` — `vpa: z.string().min(1).max(255).optional()`] — **fixed:** raised to `.max(321)` matching `NOMINEE_BANK_VPA_REGEX`'s actual max (256 + `@` + 64), with the rationale recorded inline
- [x] [Review][Patch] The new render-fence test's "no hand-written label" regex (`/["'>]\s*UPI ID\s*["'<]/`) only catches the string adjacent to a quote or `>`/`<` — a template literal (`` `UPI ID` ``) or JSX-expression concatenation would slip past it undetected [`apps/mobile/tests/unit/pay-screen-vpa-render.test.ts`] — **fixed:** the character classes were extended to also match a backtick, so a template-literal form of the label is caught too
...
| 2026-09-12 | 1.1 | ⭐ **CODE REVIEW (3-layer: Blind Hunter, Edge Case Hunter, Acceptance Auditor) — 2 patches applied, status → `done`.** Acceptance Auditor independently re-ran every gate and found zero AC/Trap/checklist violations. Two real, confirmed gaps survived triage: **(1)** the new `vpa` field's `.max(255)` was narrower than `NOMINEE_BANK_VPA_REGEX`'s actual collectible max (321 chars — 256-char handle + `@` + a 64-char PSP token), so a legitimately-collected long VPA would fail response serialization on this exact route (`fastify-type-provider-zod`'s `response: { 200: NomineeAccountsResponse }`) — a live 500 contradicting AC2/Trap3's own "never a 500" guarantee. Fixed: raised to `.max(321)`, rationale recorded inline. **(2)** `pay-screen-vpa-render.test.ts`'s "no hand-written label" regex only matched the string adjacent to a quote or `>`/`<`, missing a template-literal form. Fixed: character classes extended to also match a backtick. ⭐ **Six other raised findings were dismissed as noise after verification**, not merely waved through: an empty-string decrypt path that cannot occur (`decryptNomineeBankFieldSoft` converts empty plaintext to the sentinel before it ever reaches this field); both-accounts-decrypted and the audit-lock serialization cost, which are Trap 4's explicitly ratified, disclosed price of AC4 rather than defects; a cosmetic double-checked task box in the sibling `11b-17` doc; a stylistic `== null` check; and a speculative `vpaPresent`/`vpa` cross-field schema refinement already covered by construction today. Both fixes verified: contract tests (14/14), the new render-fence test (7/7), and full `typecheck` across `@twt/contracts`/`@twt/mobile`/`@twt/api` all green. | BigDev + Claude |
