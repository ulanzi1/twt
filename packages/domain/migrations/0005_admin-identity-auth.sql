...
--
-- TABLE ORDER: `users` is created first (everything else has a FK → users.id).
-- FKs on admin_credentials/webauthn_credentials/recovery_codes/step_up_otps are
-- still added via ALTER TABLE (drizzle-kit convention) to keep the snapshot
-- consistent; admin_sessions carries its FK inline for structural clarity.
--
...
--   3. BEFORE UPDATE triggers for `users` + `admin_credentials` — reuse the
--      `set_updated_at()` function defined in migration 0003.
--   4. Non-negative CHECK constraints on `failed_attempts` (admin_credentials) and
--      `attempts` (step_up_otps) — lockout/rate-limit counters must never go negative.
--   5. Composite partial index on step_up_otps(user_id, expires_at) WHERE
--      consumed_at IS NULL — the hot verify-path lookup.
--   6. Index on admin_sessions(user_id) — the FR-56 suspension DELETE path.
--
...
-- TABLES + ENABLE RLS + FKs + indexes + policies); the GRANT/FORCE/TRIGGER
-- hand-supplements are invisible to `drizzle-kit check`, matching 0002/0003/0004.
-- Re-running 0005 is a no-op (drizzle consults drizzle.__drizzle_migrations).
--
...
CREATE TABLE "admin_credentials" (
	...
	"failed_attempts" integer DEFAULT 0 NOT NULL CONSTRAINT "admin_credentials_failed_attempts_chk" CHECK (failed_attempts >= 0),
...
CREATE TABLE "step_up_otps" (
	...
	"consumed_at" timestamp with time zone,
	"attempts" integer DEFAULT 0 NOT NULL CONSTRAINT "step_up_otps_attempts_chk" CHECK (attempts >= 0),
	"created_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE INDEX "step_up_otps_verify_idx" ON "step_up_otps" ("user_id", "expires_at") WHERE consumed_at IS NULL;--> statement-breakpoint
...
ALTER TABLE "step_up_otps" FORCE ROW LEVEL SECURITY;--> statement-breakpoint
-- (3) BEFORE UPDATE triggers — reuse set_updated_at() defined in migration 0003.
