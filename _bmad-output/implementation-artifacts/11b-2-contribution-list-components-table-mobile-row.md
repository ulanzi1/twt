...
_Independent 3-layer adversarial review over the **whole stacked range `80e0d12..HEAD`** — all three stories at once. ⭐ Its subject is the SEAMS BETWEEN THE STORIES, ⛔ not a re-run of the pass that closed this row. **2 decision-needed, 12 patch, 2 deferred, 1 dismissed.** ⛔ NO ROW IS FLIPPED by the writing of this section._

**Findings owned by 11b.2 (the `@twt/ui` presenter). The full cross-story list lives in all three story files, routed by owner.**

- [x] [Review][Patch] **⭐⭐ THE HEADLINE — the `t()`-through proof is CIRCULARLY DEFERRED between this story and 11b.2b, and its stated ground is FALSE** [`packages/ui/tests/contribution-list/presenter.test.ts:193-197`]
  — this file records that AC2 *"reads the locale JSON FROM DISK and therefore asserts **AROUND** `t()`"*, calls that a known limitation, and routes it as deferred work with **"11b.2b — which CAN call `t()` — as the named trigger"** (`deferred-work.md:191-205`, item `11b.2 (vi)`). 11b.2b then shipped `done` **without** it: `apps/mobile/tests/unit/contributor-list-render.test.ts:13-17` records a real `t()` resolution as what *"THIS HARNESS CANNOT PROVE … (no mount)"* and points **back** at this story's AC2 as the owner. ⛔ The loop closes on nothing. ⚠⚠ **The "no mount" ground is FALSE, and the counter-example is a sibling file in the same directory under the same mount-free harness:** `apps/mobile/tests/unit/panchayat-noticeboard-render.test.ts:21` imports `t` from `@twt/i18n` and `:141` calls `t(key, undefined, { locale, namespace: 'noticeboard' })` — real resolver, real namespace, in the third slot. ⇒ **Family 10 REAL GAP** (a **constructible** obligation recorded as not-constructible — the inverse of the family), triaged at AC severity. ⭐ **Why five prior single-story passes all missed it:** each read a well-formed pointer to a live sibling obligation, and neither could check the sibling. This is exactly the decay 11b.2's own AC6 orders be recorded for item (ii) (*"8.3 D11's re-trigger fired at 11a.1 and was not acted on"*) — applied now to this story's own deferral, whose trigger has been **spent unrecorded**. Fix: add the assertion in 11b.2b on the noticeboard precedent, AND amend item `11b.2 (vi)` in `deferred-work.md` so the spent trigger leaves a trace.
- [x] [Review][Patch] **The presenter's exhaustiveness throw SERIALIZES the member's name parts into its message — and the mobile catch logs that message** [`packages/ui/src/contribution-list/presenter.ts` (`default:` arm) → `apps/mobile/components/contributor-list/PoolContributorList.tsx:129-131`]
  — the arm throws `` `unhandled display-name kind: ${JSON.stringify(exhaustive)}` ``, and `exhaustive` is the **whole `displayName` operand**, name fields included. The render layer's catch justifies including `error.message` on the ground that *"every resolver throw carries the param NAME, key, namespace and locale, ⛔ never a param VALUE (`resolver.ts:39,59,64`, verified)"* — ⚠ that verification is sound **for the i18n resolver only**, and the same `try` also calls `deriveContributionRowViewModel`. ⇒ the stated safety ground is **narrower than the catch it justifies**. ⚠ **Latent, not live:** the adapter hardcodes `kind:'name'` and the wire row is `.strict()` with `firstName: z.string().min(1)`, so today no third kind can arrive — the file says so itself four lines above, which means the two comments also disagree with each other. The first producer that can emit a third kind (11b.3's Astro path is named as a candidate) turns a dev log into a name leak on the one surface whose entire subject is PII shielding ([[project_anonymous_diagnostic_log_convention]] — the signal is the ACTION, never the subject). ⚠ FENCE 4 in `contributor-list-render.test.ts` "proves" no member data reaches the log by grepping the **catch source text** for `firstName|lastInitial|label`, so it passes while the property fails. Fix: throw the KIND, never the operand.
- [x] [Review][Patch] **The presenter's and view-model's stated design rationale is now FALSE — derivation is EAGER over the whole list, not per visible row** [`packages/ui/src/contribution-list/presenter.ts` header; `packages/ui/src/contribution-list/view-model.ts` header]
  — both headers justify the per-row shape and the "allocation-light" constraint with *"the consumer is a WINDOWING render layer that calls this once per visible row on every scroll frame"*. ⭐ **The adapter shipped in the SAME diff explicitly retracts it:** `apps/mobile/components/contributor-list/contribution-row-input.ts` — *"The shipped consumer calls this EAGERLY, once per row, inside a memoized `.map()` … ⛔ not once per visible row per scroll frame, which is what this doc-block claimed until the second code review."* The two `@twt/ui` headers were never updated. ⇒ on the 11b.3 public render these headers are written for (documented as the ~10,000-member case), virtualization no longer bounds derivation cost — the memo runs the presenter **and** a `t()` resolution for every row on every locale change and every poll result, which is the opposite of what the header promises. ⛔ The per-row SHAPE requirement (Trap 1) is unaffected and still correct; it is only the RATIONALE that is stale. Correct the two headers to the adapter's already-corrected wording.
- [x] [Review][Defer] **`ContributionListI18nRef.namespace` is a CLOSED literal type, contradicting the doc-block beside it** [`packages/ui/src/contribution-list/view-model.ts`] — deferred, trigger-bound
  — the type is `readonly namespace: 'contribution'` while the doc immediately above says *"The namespace is carried per-REF anyway — … 11b.3 may add a second namespace."* ⇒ the per-ref carrying buys nothing until the type is widened, and `i18n-keys.ts`'s `satisfies Record<string, ContributionListI18nRef>` will **reject the first such addition**, in a file 11b.3 is told is reuse-only. ⛔ Deferred rather than patched deliberately: widening the type now is a change with no consumer ([[feedback_no_premature_package]] in spirit), and the closed literal is the CORRECT type for today's single namespace. **Trigger: Story 11b.3**, at the moment it adds a ref outside `contribution`.
