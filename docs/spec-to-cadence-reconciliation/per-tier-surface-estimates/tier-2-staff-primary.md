...
**Cadence basis (§5 assumption override):** 80 hr/week NET + AI-assisted per `estimation-methodology.md §2 row 2` (D-03-resolved Tasks 7+8 review; ratification pending Task 9 ≥2-trustee co-sign per Decision 2026-06-04-016). 1 AI-cadence month = 346 hr per methodology §2. Note: Story 0.11 synthesis currently at `done` in sprint-status development_status (operator-console design requirements are committed); per Tasks 7+8 review P-11 verification: Story 0.11 development_status flipped at the prior code-review-closure pass.
...
- Epic 4 Trustee tooling + R9 voting (Story 4.1 simple 1 + 4.2 medium 2 + 4.3 medium 2 + 4.4 medium 2 + 4.5 complex 4 + 4.6 complex 4): ~15 pts (Epic 4 full = 26 pts; Tier-2 share ~60%)
- Epic 6 Verifier Console + State-Trustee approval (Story 6.1 medium 2 + 6.3 complex 4 + 6.11 complex 4 + 6.13 complex 4): ~14 pts
- Epic 10 helpdesk operator + admin consoles (Stories 10.1 simple 1 + 10.2 medium 2 + 10.3 complex 4 + 10.4 medium 2 + 10.5 medium 2): ~11 pts
- Epic 13 field-worker dispatch (Stories 13.3 medium 2 + 13.4 medium 2 + 13.5 medium 2): ~6-12 pts depending on share of Epic 13 (full Epic 13 = 19 pts; Tier-2 share ~30-60%)
- Epic 9 HQ Finance disbursement + State-Trustee approval surface (share of 9.8 complex 4 + 9.9 complex 4 + 9.10 complex 4 + 9.11 complex 4): ~12-16 pts

Total: ~58-68 story-points (refinement of prior `~60`; itemization confirms the prior estimate is within ±15% which is acceptable for a diagnostic row).

**Derivation (diagnostic view — NOT summed with epic-aggregation rows):** ~60 story-points × 4 hr/pt = 240 hr base. Complexity (+130% per §3: multi-party state machine + external telephony + multi-tenant RLS): 240 × 2.30 = 552 hr. CI/ADR overhead: 35% per methodology §4(f) Tier-2 staff-console aggregation → 552 × 1.35 = 745 hr ÷ 346 hr/month = 2.15 months computed midpoint. **Assumption-catalogue adjustment (per Tasks 7+8 review P-03):** computed midpoint 2.15 → operational midpoint 0.90 on the grounds that (i) Trustee-tooling governance workflows are rule-driven and AI-scaffolded efficiently — these are state-machine-CRUD patterns with extensive open-source prior art, not novel design; (ii) Story 0.11 operator-shadowing synthesis provides empirical design anchoring (routing-policy categories + SLA targets + helpline-call-to-ticket workflow already validated), reducing UX iteration cycles by an estimated 40-50%; (iii) WCAG AA acceptable-gap carve-out per UX §8 reduces accessibility testing overhead (no hard-blocker); (iv) the +130% complexity multiplier in §3 already partially compensates these efficiencies — the residual adjustment from 2.15 → 0.90 may be optimistic and is flagged for Month-3 re-attestation re-evaluation. Medium-band asymmetric formula per methodology §3 (factor = 0.5; 1 + factor = 1.5): floor = 0.90 / 1.5 = 0.60; ceiling = 0.90 × 1.5 = 1.35. Medium-band ratio 1.35 ÷ 0.60 = 2.25 ✓ — formula consequence per P-13 tautological-band disclaimer.
