...
| `governance-boundary` (FR-58C feature-flag capability bar) | 10.8 | `pnpm governance-boundary:check` / `scripts/governance-boundary/check.ts` | **Two legs.** (a) *Conformance* — the domain flag registry (`FLAG_DEFAULTS`) and the capability bar (`governance_boundary.yaml`) admit exactly the same keys in BOTH directions, with a `count` cross-check. (b) **⟵ LOAD-BEARING** *source scan* — no feature-flag evaluation reaches inside a governance module (`audit` / `rbac` / `consent` / `validity-service` / `contribution` / `scripts`; roots read from the bar's own `prohibited` list so scan scope and documented prohibitions cannot drift). Three AST detection routes (module specifier, named symbol incl. aliases, `featureFlags.*` property access) so an `import * as` cannot defeat it. Makes the seven `epics.md:3516-3522` prohibitions structurally impossible rather than documented. Prohibition (e) (freeze-table rows) is enforced at *admission* by the bar parser instead — no import exists to scan for. Invariant scan — no `fetch-depth:0`. | ✅ Active and enforcing (landed green-with-teeth; both legs proven to FAIL on live planted violations) |
