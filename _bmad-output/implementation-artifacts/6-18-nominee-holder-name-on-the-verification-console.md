...
- [x] [Review][Patch] ✅ **CLOSED v1.4 — `tests/integration/claim/nominee-name-check-return-concurrency.spec.ts`**
  ...
  **SYMMETRIC** — proved with the route submitted first, because a guard implemented on only one writer would
  ...
  ⚠ **Residual, ⛔ not claimed as closed:** `recordNomineeNameCheck` and the helpline bank-correction write have
  ⛔ no own-committing race yet. This spec covers the RETURN loop (AC11), which is where the cross-writer
  exclusion invariants live.
