...
**Cadence basis (§5 assumption override):** 80 hr/week NET + AI-assisted per `estimation-methodology.md §2 row 2` (D-03-resolved Tasks 7+8 review; ratification pending Task 9 ≥2-trustee co-sign per Decision 2026-06-04-016). 1 AI-cadence month = 346 hr per methodology §2. Note: decomposition into per-surface rows deferred (aggregate diagnostic row maintained per worksheet §4 design).

**Surface-count itemization (per Tasks 7+8 review P-16):** replacing prior `~N pts` round-bucket prose with explicit story-point breakdown per methodology §6 round-number-forbidden discipline:
- Epic 3 signup + KYC + member-directory + profile (Stories 3.1 simple 1 + 3.2 medium 2 + 3.3a complex 4 + 3.3b complex 4 + 3.4 medium 2 + 3.5 medium 2 + 3.6 medium 2 + 3.7 medium 2 + 3.8 complex 4 + 3.9 medium 2 + 3.10 complex 4 + 3.11 very-complex 7): 36 pts (refinement of "~30" estimate; full Epic 3 sum is 35 pts per worksheet §7)
- Epic 8 My Pool + Yogdaan Bahi + contribution loop (Stories 8.1 simple 1 + 8.2 medium 2 + 8.3 simple 1 + 8.4 complex 4 + 8.5 medium 2 + 8.6 medium 2 + 8.7 medium 2 + 8.8 medium 2 + 8.9 medium 2 + 8.10 complex 4 + 8.11 complex 4 + 8.12 very-complex 7): 33 pts (Epic 8 full sum per worksheet §7 — "~25 pts" prose underestimate)
- Epic 6 Ravi-mode claim filing share (Story 6.2 complex 4 + Story 6.5 complex 4): 8 pts ✓
- Epic 9 Sunita-mode nominee console (Story 9.1 medium 2 + share of 9.8 complex 4 + share of 9.9 complex 4): ~8-10 pts
- Epic 11a + 11b Shradhanjali + member-directory: Epic 11a 12 pts (full) + Epic 11b 24 pts (full) = 36 pts total; Tier-1 share approximately half = ~18 pts ✓

Total: ~36 + ~33 + ~8 + ~10 + ~18 = ~105 story-points (revised from prior `~89`; the prior estimate undercounted Epic 3 + Epic 8 by aggregating sub-stories). Worksheet §7 Epic-aggregation rows remain the deterministic source for §8 totaling; the Tier-1 row is diagnostic.

**Derivation (diagnostic view — NOT summed with epic-aggregation rows):** ~105 story-points × 4 hr/pt = 420 hr base. NFR-20 WCAG 2.1 AA testing overhead: UI testing conducted by fellow members per project execution model; a11y-compliant code generated by AI; overhead reduced from methodology §4 safety-critical +100% to +30% effective premium: 420 × 1.30 = 546 hr. CI/ADR overhead: 45% (UX-DR3 friction-budget gate + FR-74 PII density + Story 1.10 audit-log per methodology §4(f) Tier-1 row) → 546 × 1.45 = 792 hr ÷ 346 hr/month = 2.29 months computed midpoint. **Assumption-catalogue adjustment (per Tasks 7+8 review P-03):** computed midpoint 2.29 → operational midpoint 1.35 on the grounds that (i) NFR-20 testing premium is substantially reduced by the fellow-member testing model (testing capacity is not Solo Builder time); (ii) AI generates accessibility patterns from day one rather than retrofitting them post-implementation (eliminates the a11y-rework cycles that typical Tier-1 estimates absorb); (iii) UX design is anchored by Stories 0.8/0.9/0.10 synthesis which reduces design iteration cycles by an estimated 30-40% (the synthesis itself surfaces critical patterns that would otherwise emerge as post-implementation rework). The 1.35 midpoint reflects these AI-cadence + fellow-tester multipliers. Medium-band asymmetric formula per methodology §3 (factor = 0.5; 1 + factor = 1.5): floor = 1.35 / 1.5 = 0.90; ceiling = 1.35 × 1.5 = 2.025 ≈ 2.02. Medium-band ratio 2.02 ÷ 0.90 = 2.244 ≈ 2.25 ✓ (2dp rounding artifact; computed 2.025 ÷ 0.90 = 2.25 exactly) — formula consequence per P-13 tautological-band disclaimer.
