...
- [ ] **Task 5 — Filing** ⚠ **RE-OPENED by code review 2026-09-20 (D4 = A):** the member half and the note end-to-end are built; the HELPLINE half is ⛔ NOT — see `### Review Findings` D4. (AC6, AC7) — member app, the note end-to-end, the filer message, and the HELPLINE bank-entry surface + two-names view
  ...
  - [ ] `helpline-claims`: bank-details section on `recordHelpline`, both-names view, optional note,
        required before the page completes. ⚠ **RE-OPENED (D4 = A):** built only for the session that
        just filed the intake; still owed — `recorded` derived from a SERVER read of the accounts on
        file, re-entry after the names view, `correctionReason`, the `correctionNeeded` banner, a
        step-up path on the bank save, and completion of a legacy claim filed without accounts.
...
- [ ] **Task 6 — The DA and Pariwar Admin surfaces** ⚠ **RE-OPENED by code review 2026-09-20 (D4 = A):** the District Admin console strip and the R9 disclosure are built; the Pariwar Admin card's names disclosure and the District Admin's returned-claims list are ⛔ NOT. (AC2, AC3, AC5, AC8)
...
- **The nominee declaration can be rewritten after the member's death** (`-227`'s "NOT cover" list, item 2; the 2026-09-19 note's E2). ✅ **NOW ROUTED — a genuine Panel question — and ✅ RULED the same day by `2026-09-20-233`: the declared nominee CANNOT be changed after the member has died** (the build is **Task 4e**, ⏳ proposed; follow-ups V W X ANSWERED by `-234` — which makes the build far larger and changes 6.18's own compare source; Y Z AA open): `trustee-panel-routing-note-2026-09-20-6-18-nominee-change-after-death.md` (⛔ not sent, ⛔ not committed). The hole is real in the code as written: the only guard in the nominee and life-events handlers is `{withdrawn, anonymized}` (death is not one), the handler then `replaceMemberNominees` unconditionally, `POST /member/nominees` needs only a member session, and the new name check compares two values the filer may control.
...
- **Task 6 — DA console ✅, PA card ✅, R9 screen ⛔ NOT BUILT.** `<NomineeNameCheckPanel>` mounted
  ...
  only when a District Admin chooses to look. `VERIFIER_CONSOLE_MAX_READS` 11 → 13 with the
