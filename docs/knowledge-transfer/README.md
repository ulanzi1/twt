...
| Native-stack validation framework (~2-week prototype on RN + Tamagui of three named patterns × three test devices × P1-P6 pass criteria; ratify-or-pivot decision per FM-2 tiered escalation per UX §Phase-0 P0-5 + epics line 564 + 688 + 693 + AR-49 + architecture §Launch Gate Risks P0-5 + architecture §PoC Validation Pending) | Story 0.14 | `docs/native-stack-validation/` (author-committed 2026-06-02 per Decision 2026-06-02-014; framework includes README + experiment-protocol + device-procurement-roster + measurement-template (54-cell measurement matrix pre-staged with `_PENDING-MEASUREMENT_` literal) + pass-criteria-evaluation-framework + ratify-decision-template + pivot-evaluation-decision-tree (F1-F5 fail-criteria response paths) + engagement-ledger; AC-named research artifact at `_bmad-output/research/p0-5-native-stack-validation.md` per epics line 936. Trustee Panel experiment-scope + device-procurement-budget ratification + Solo Builder three-device procurement + ~2-week prototype build under CNG workflow + measurement collection + ratify-or-pivot decision + ≥1-trustee acknowledgement (distinct from prior Stories ≥2-trustee quorum per BigDev decision authority per UX spec line 845) pending Tasks 7-11. UX §Phase-0 P0-5 + epics line 564 + 688 + 693 + AR-49 + architecture §Launch Gate Risks P0-5 row at architecture line 4784 + architecture §PoC Validation Pending P0-5 commitment discharge path per `docs/native-stack-validation/README.md` §9 disjoint-anchor discipline — SIXTH Phase-0 portfolio distinct from bus-factor-of-one mitigation + loop-node operational-responsiveness + empathy field-work + spec-to-cadence-funding-reconciliation + legal-counsel-concurrent-review portfolios. ADR-NNNN-native-mobile-stack-ratify slot at `adr-index.md` line 52 populates from `slot-reserved-pre-write` to substantive content at Story 0.14 Task 11. Cross-Story coupling: Story 0.14 Task 7 device-procurement budget cross-coupled with Story 0.12 contract-help-path; Story 0.14 Task 11 closure unblocks Epic 1 Story 1.1 line 990 precondition + Story 0.10 P0-2c PRECONDITION-2 prototype-operability) |
| Architectural launch-gate inventory framework (15-row roster: 12 architecture §Launch Gate Risks verbatim from architecture lines 4778-4788 + 3 Gap Analysis conditional-escalation candidates from lines 4802-4815 + 4817-4840 + 1 reserved; closure-criteria-rubric + target-date-rationale-template with relative-to-fact-trigger discipline + monthly-review-cadence-protocol with ≥2-trustee quorum + escalation-protocol with 5-trigger / 5-outcome vocabulary; trustee-side governance surface aggregating prior six Phase-0 portfolios' closures for Phase 1 launch readiness signal arming per AR-49 + epics line 689 + PRD §12 Phase 0 line 1467 + Sprint Change Proposal Item 17) | Story 0.15 | `docs/launch-gate-inventory/` (author-committed 2026-06-03 per Decision 2026-06-03-015; framework includes README + inventory-roster + closure-criteria-rubric + target-date-rationale-template + monthly-review-cadence-protocol + escalation-protocol + engagement-ledger; Trustee Panel ≥2-trustee inventory ratification pending Story 0.15 Task 8 (uniform threshold across inventory-ratification + per-row-closure + final-sign-off, distinct from Story 0.14's ≥1-trustee BigDev-decision-authority pattern); first monthly trustee-panel review pending Task 9; per-row closure events as prior Phase-0 portfolio Stories' Tasks 7-11 close + Legal Counsel returns + Trust formation activities pending Task 10; all-rows-closed-or-deferred final sign-off + Phase 1 launch readiness signal armed + annual re-attestation cadence operational pending Task 11. AR-49 + epics line 689 + PRD §12 Phase 0 line 1467 + Sprint Change Proposal Item 17 discharge path per `docs/launch-gate-inventory/README.md` §9 disjoint-anchor discipline — SEVENTH Phase-0 portfolio distinct from bus-factor-of-one mitigation + loop-node operational-responsiveness + empathy field-work + spec-to-cadence-funding-reconciliation + legal-counsel-concurrent-review + native-stack-validation portfolios; unique among the seven portfolios in that it is an **aggregation surface** consuming the prior six portfolios' closures rather than discharging an independent property. New ADR slots at `adr-index.md` Section L (≥6 slots: slippage-tolerance per-gate-class; trust-formation decomposition strategy; conditional-escalation predicate materialization criteria; monthly-review cadence integration; emergency-review trigger threshold; ≥2-trustee quorum fallback). Closure of Story 0.15 does NOT contribute to the 30-day-takeover joint discharge in §10 below) |
