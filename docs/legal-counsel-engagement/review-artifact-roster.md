...
| `actual_submission_date` | ✅ **2026-08-24** — submitted by **in-person handover of the printed draft**. ⚠ **50 days past the 2026-07-05 target.** ⭐ **The AC-1 first-artifact obligation is DISCHARGED — LATE.** ⛔ Not *"closed"* and ⛔ not back-dated: the breach occurred, is recorded, and the obligation is now met ([[feedback_closure_language_precision]]) |
| `sla_target_return_date` | **2026-09-07** — **standard tier** (5–10 business days per §4); ⛔ **not** expedited, ⛔ no surge pricing incurred. ⭐ Recorded at the **outer bound** (10 biz days) so the breach clock is ⛔ not started prematurely; the **earliest** return under tier is **2026-08-31** |
...
| `integration_status` | ✅ **`awaiting-counsel-return`** — flipped from `pending-submission` **2026-08-24** on the actual submission event. ⭐ **The first genuine lifecycle transition this engagement has recorded** |
...
⭐⭐ **SUBMITTED 2026-08-24 — Task 10 event.** In-person handover of the printed draft; counsel **acknowledged the same day**, inside the §11 two-business-hour window ⇒ ⛔ **no acknowledgment-SLA breach**, ⛔ no §8 row owed. Standard tier; return due **2026-09-07** (earliest 2026-08-31).
⚠ **In-person handover leaves ⛔ no transmission timestamp** — so the **acknowledgment is the evidentiary anchor** for this submission, ⛔ not a send record. ⚠ It is also, by default, the **interim** exchange mechanism: the counsel-side encrypted-document-exchange ADR (`adr-index.md` Section K row #2) is ⛔ still unauthored, so ⛔ no contemplated channel existed to use.
...
⭐⭐ **2026-08-28 — COUNSEL DELIVERED THE TWO DISCLOSURE CLAUSES. ⛔⛔ THIS IS ⛔ NOT THE RETURN, AND `integration_status` IS ⛔ DELIBERATELY NOT FLIPPED.** Counsel supplied, in writing, the **Public Disclosure of Member Information** and **Public Disclosure of Nominee and Claim Information** clauses that `2026-08-28-160` cl.4(b) required and that sheet **A2.1** committed him to draft. ⇒ ⭐ **cl.4(b) / A2.1 are DISCHARGED.**
⛔ **What has ⛔ NOT happened, stated plainly** ([[feedback_closure_language_precision]]): the **sixteen original clauses remain unreviewed**, and **Annex questions 1–7 remain unanswered**. A **clause delivery** and a **review return** are ⛔ different acts, and only the first has occurred. ⇒ `integration_status` stays **`awaiting-counsel-return`**, `actual_return_date` stays `<PENDING-TASK-11>`, and ⛔ **no `engagement-ledger.md` §7 Return-receipt is opened** — §7 has been empty **by construction** since the engagement began, and ⛔ a partial delivery is not what opens it.
⚠ **SLA note, recorded so the clock is read correctly:** the delivery arrived **2026-08-28**, ⛔ **before** the tier's earliest return date (**2026-08-31**). ⛔ It is ⛔ not an early *return* — the return window is unaffected and **2026-09-07 still stands**.
⭐ **INTEGRATED into a NEW draft, ⛔ v0.1 left untouched:** `handover/TWT-Terms-and-Conditions-DRAFT-v0.2-for-counsel-review.docx` — counsel's text reproduced **verbatim** as clauses **14** and **15** (verified byte-for-byte), former 14–16 renumbered **16–18**. ⛔ **v0.1 is deliberately NOT overwritten**: it is the artefact physically handed over on 2026-08-24 and the evidentiary record of what counsel actually received.
⚠⭐ **AND ONE THING WAS CHANGED THAT WAS ⛔ NOT COUNSEL'S TO HAVE CHANGED SILENTLY — SO IT IS FLAGGED, ⛔ NOT BURIED.** v0.1 clause 13 read *"Consent for each distinct publication of your information is recorded separately"* — **the per-subject consent gate `-160` cl.3 superseded as a mechanism**. Left as written it would have **contradicted counsel's own clause 14 two paragraphs later**. It is replaced with data-class routing (member's own → cl.14; nominee's → cl.15; family's → the family's own consent) and **put back to counsel as new Annex question 8**. ⛔ Recorded as an **author amendment awaiting counsel confirmation**, ⛔ not as counsel's drafting.
⚠ **THREE further questions raised to counsel in v0.2 (Annex 9–11):** (9) clause 15 binds the **Nominee** via the **Claim Terms** — ⛔ a separate instrument that **does not exist as a drafted document**; does it belong here, there, or both? (10) the DPDP alignment `-160` cl.9 defers to pre-launch, now with concrete text to align. (11) confirmation that **family-owned** information sits **outside** clause 14. |
...
⭐⭐ **v0.2 (2026-08-28) — THREE TRUSTEE ANSWERS INCORPORATED, AND ONE OF THEM CORRECTED A FACTUAL ERROR IN v0.1's OWN FRAMING.**
⛔ **v0.1 Q2 WITHDRAWN, ⛔ not answered — the question was WRONG ON THE FACTS.** It asked about acceptance taken *"at a point of acute grief"*. ⚠ **The committed source said the opposite and had already been read**: `prd.md:63` records the nominee *"Files a claim ~1 month after death (**grief eases the rush**)"*. ⇒ the premise was contradicted by the very document the draft was assembled from. **BigDev also notes the term is ordinary practice** — insurers require the same. ⇒ the grief framing is **struck from clause 2**, replaced with *"Claims are typically brought some months after the death"*, and ⛔ **the question is not re-put.**
✅ **v0.1 Q3 ANSWERED AND PROMOTED FROM A DISCLAIMER TO A RULE.** BigDev: *"Not allowed… Banking name must match nominee name."* ⇒ clause 3 now **requires both accounts to be the nominee's own** with the holder name matching, and the Trust **may refuse or halt** a claim on mismatch. ⛔ v0.1's *"the Trust does not verify that an account belongs to you"* is **removed** — it contradicted the rule.
⚠⛔ **BUT THE RULE IS ⛔ UNENFORCED IN THE SUBSTRATE, AND THERE IS ⛔ NOTHING TO ENFORCE IT AGAINST — verified at `e3257b9`.** `contracts/src/claims/nominee-bank.ts:56` types `accountHolderName` as free text (`z.string().trim().min(1).max(200)`), stored as **Tier-1 envelope ciphertext** (`schema/claim_nominee_bank_accounts.ts:61`); there is ⛔ **no validation** anywhere, and ⛔ **no nominee/claimant name column on `claims`** to compare it to. ⇒ ⭐ the match is presently a **human check at claim review**, ⛔ not a system control — and an automated check would face the **same per-row-DEK problem** that blocked name search at Story 11b.1 (D2). ⛔ **A term that does legal work while nothing enforces it must not be recorded as enforced.** ⇒ carried as **new Annex Q8** (what must the Trust do on mismatch?) and owed as an engineering item.
✅ **v0.1 Q6 PARTLY ANSWERED — retention is SETTLED.** BigDev: RBI circulars mandate preserving transactional and account records up to **10 years**, and the DPDP Act 2023 does not require erasure where retention is necessary for **compliance with a law in force**. ⇒ clause 12 now states the retention position. ⭐⭐ **AND A DISTINCTION ADDED THAT THE ANSWER DOES ⛔ NOT ITSELF MAKE: RETENTION IS ⛔ NOT PUBLICATION.** A duty to **keep** a record is ⛔ not a licence to **display** it — what is public is governed by clause 9's masking, what is held is governed by clause 12. ⭐ This is the same line `2026-08-28-160` cl.10(f) draws (*"complete bank details remain available in the protected internal record"*). ⛔ **Still open:** the nominee's **export and correction** rights, and whether the clause-8 authority may be **withdrawn after settlement**.
⚠ **v0.1 Q4 (overpayment) was ⛔ NOT answered** — BigDev's third answer was labelled "Q4" but its content (RBI retention / DPDPA exception) answers **Q6**. Applied to Q6; ⛔ Q4 is ⛔ not treated as answered and is carried as **Annex Q2** in v0.2.
⭐⭐ **AND Q10 IS EXPANDED INTO ITS OWN ANNEX B, at BigDev's request.** It is a **scoping question about the Trust's own consent screen**, ⛔ not about this drafting: a four-row table naming each claim-time box and what now carries it — (a) claim-time processing **unaffected**; (b) Sahyog Vivran → contributor names are **other Members'** data (their cl.14), verifiers are staff, nominee bank is **cl.8**; (c) In Memoriam → the deceased Member's own name is **cl.14**; (d) Sahyog Drive → **cl.14**, box being removed by Story 11b.9. ⇒ the question put is whether boxes (b) and (c) retain **any** subject other than **family-authored, family-owned content**, and if not whether they should be **re-scoped or retired**. ⭐ With a third part the Trust states as its own position: **a box presented as controlling something it no longer controls is worse than no box** — it invites a family to believe they have a choice the terms have already settled. ⛔ Deliberately ⛔ NOT self-answered: `-160` cl.3 superseded the gate as a **mechanism** without ruling which boxes survive, and that is Story 11b.3's and 11b.6's to settle. |
