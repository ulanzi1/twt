...
- **FR-74: Public-vs-Private matrix codified (§35.1).** Public (no auth): Member Directory (first-name + last-initial + school + district + designation), Sahyog Drive listings, Sahyog Vivran (per-claim story + verifier names hyperlinked + contributor count + first-name + last-initial contributors), In Memoriam, Niyamavali w/ version diff, public Blog, About/Founders/Team, Contact. Members-only: full member lookup, nominee bank/IFSC during active alert window only, contribution history. Never public: trust account ledger, partner commissions, internal expenses, full mobile/email/address/DOB, raw photo. CI scrape-test asserts no PII leak.
  ...
  - ⛔ **RECONCILED 2026-09-04 — a THIRD correction, to the `Members-only` entry above. ⚠ It is a RECORD-KEEPING correction: the change it records was ⭐ properly Trustee-ratified and is ⛔ not in question; what never happened is this annotation.**
  - **(3) `nominee bank/IFSC` is SUPERSEDED for the PUBLIC Sahyog Vivran surface** (`2026-08-28-165` clauses 1–2, **Trustee-ratified**, completing `2026-08-28-164` cl.3 under `2026-08-28-160` cl.10): **four** named `(surface, field)` pairs — `nominee_account_holder_name`, `nominee_account_number`, `nominee_ifsc`, `nominee_vpa` — are declared **`tier: public`** on `sahyog-vivran`. ⇒ this clause's *"Members-only: nominee bank/IFSC **during active alert window only**"* no longer describes that surface, and this clause's **`Public (no auth)`** enumeration of what Sahyog Vivran carries is **incomplete** — it lists story / verifier names / contributor count / contributor name form, and ⛔ predates the bank fields entirely. ⛔ **Scoped to the PUBLIC `sahyog-vivran` surface ONLY.**
  - ⭐ **THE `Members-only` CLASSIFICATION STILL DESCRIBES THE MEMBER SURFACES ACCURATELY, and ⛔ that half is NOT superseded.** The donor payment path (`contracts/contributions/nominee-accounts.ts`, Story 9.9) returns the details ⛔ only for the member's OWN assigned pool and ⛔ only while it is `live` — `payment/handlers.ts` answers `{ available: false }` when there is no live pool. ⇒ *"during active alert window only"* is ⭐ implemented, on the member side.
  - ⛔⛔ **SUPERSEDED WITHIN A DAY — RE-ANNOTATED 2026-09-04 (`2026-09-04-190`, Trustee-ratified).** Annotation (3) above is **STALE AS WRITTEN** and is kept only so the sequence is legible. ⭐ The Panel has **REMOVED the banking coordinates from the public surface**: `nominee_account_number`, `nominee_ifsc`, `nominee_bank_name` and `nominee_branch` are ⛔ **no longer `tier: public`** ⇒ `-165`'s allowlist goes **4 pairs → 1**. ⭐ The ONLY public residue is the **nominee's NAME** (`nominee_account_holder_name`), rendered as **"Nominee Name"** — ⛔ never *"Account holder"*. ⇒ **this clause's original *"Members-only: nominee bank/IFSC during active alert window only"* is once again VERY NEARLY ACCURATE**, and a **logged-in member sees the COMPLETE banking information** (`-190` cl.3). ⚠ The masking window code is **RETAINED** by ruling (*"we may use it in future"*) but now has ⛔ **NO public consumer** — ⛔ do not describe it as a live control.
  - ⚠⛔ **AND THE CONSEQUENCE IS AN INVERSION, RECORDED HERE BECAUSE ⛔ NOTHING ELSE RECORDS IT:** for a **finished** drive the member app shows these fields to ⛔ **nobody**, while anyone holding the public link sees them **in full** and — under `D8-default` **FAIL-OPEN** (`2026-09-02-179` cl.1) — **indefinitely**. ⇒ on this data class **the public is LESS restricted than a member**. ⛔ Whether the Panel intended that consequence was ⛔ NOT recorded anywhere and was **ROUTED**, ⛔ not assumed — `trustee-panel-routing-note-2026-09-04-11b3a-member-vs-public-bank-detail-inversion.md` (`#decision-2026-09-04-188`). ⭐⭐ **ANSWERED 2026-09-04 (`-189` cl.3): a member must see MORE than the public and ⛔ NEVER less. ⇒ CLOSED by `-190` cl.1+3 — by LOWERING THE PUBLIC, ⛔ not by widening the member.**
...
> - **One-slice-one-surface story discipline** — each story modifies API OR admin UI OR mobile UI (contract-first via `packages/contracts/`); the next story consumes the previous story's contract. This bounds story file-churn for solo-build dev-agent context windows. **Named exception (first established by Story 10.8 and independently confirmed by Story 10.12):** a `[PRIMITIVE]` registry whose only viable population mechanism is administrator-authored data — not downstream code — may include the minimal administrator authoring surface that is intrinsic to the primitive's existence. The UI must exist solely to author or maintain that primitive and must not evolve into an independent product surface. The API and that authoring surface constitute one economic unit rather than two independent surfaces. This does not change the story's classification to `[SURFACE]`; the story's primary deliverable remains the primitive (schema, validation, governance, and enforcement).
...
> **Dev Notes (added 2026-06-03 per Decision 2026-06-03-015):** Story 0.15 framework author-committed 2026-06-03 at `docs/launch-gate-inventory/` per Decision 2026-06-03-015 — README (11 sections + 14 structural invariants + disjoint anchor §9 establishing Story 0.15 as the SEVENTH Phase-0 portfolio + 30+ domain glossary terms) + inventory-roster (15 rows: Rows 1-11 architecture §Launch Gate Risks verbatim from architecture lines 4778-4788 + Rows 12-14 architecture §Gap Analysis conditional-escalation candidates from architecture lines 4802-4815 + 4817-4840 with verbatim predicates + Row 15 reserved) + closure-criteria-rubric (objective evidence vocabulary 11 artifact types + testable signal vocabulary 5 types + weak-closure rejection examples 8 patterns + per-disposition closure-evidence requirements + multi-link rows) + target-date-rationale-template (relative-to-fact triggers preferred per [[feedback_architecture_vs_adr_boundary]] + sequenced gates pattern + decomposed gates pattern + conditional candidates pattern + annual re-attestation + worked examples for Rows 1/7/11/12) + monthly-review-cadence-protocol (≥2-trustee quorum + monthly cadence until close-or-defer-with-ADR + Tier C quarterly fallback + meeting-minutes schema + emergency review triggers 4 types + annual re-attestation walk-through) + escalation-protocol (5 escalation triggers + 5 outcomes vocabulary `accept-slippage`/`revise-target-with-rationale`/`decompose-to-sub-gates`/`transfer-ownership`/`reframe-disposition` + escalation log row schema + repeat-escalation root-cause inquiry + Phase 1 launch readiness signal) + engagement-ledger (§1+§2+§9+§10+§11 substantive at author-commit; §3-§8 with `<PENDING-TASK-N>` placeholders). Rows 1-2 (A-13 backup engineer retainer + P0-3 Spec-to-Cadence Reality Check) at `current_status = closed` per closure-status-aggregation discipline (framework-leg discharge via Decisions 2026-05-30-006 + 2026-06-01-012 + 2026-06-01-012-amend-1 supersession entries); Rows 3-11 at `current_status = open` (substantive closure events pending per prior Phase-0 Stories' Tasks 7-11 + Legal Counsel returns + Trust formation activities); Rows 12-14 at `current_status = conditional-escalation-pending-predicate` per architecture §Gap Analysis observations. Tasks 8-11 `_AWAITING EXTERNAL ACTION_` — Trustee Panel ≥2-trustee inventory ratification (Task 8 uniform threshold across inventory-ratification + per-row-closure + final-sign-off, distinct from Story 0.14's ≥1-trustee BigDev-decision-authority pattern) + first monthly trustee-panel review within 4 weeks of ratification + ongoing monthly cadence (Task 9) + per-row closure events as prior Phase-0 portfolio Stories' Tasks 7-11 close + Legal Counsel returns per Story 0.13 + Trust formation activities per Trustee Panel direct ownership (Task 10) + all-rows-closed-or-deferred final sign-off + Phase 1 launch readiness signal armed + annual re-attestation cadence operational (Task 11). 6 deferred-ADR slots added to `docs/knowledge-transfer/adr-index.md` Section L (slippage-tolerance per-gate-class + trust-formation decomposition strategy + conditional-escalation predicate materialization criteria + monthly-review meeting cadence integration + emergency-review trigger threshold calibration + ≥2-trustee quorum fallback under trustee-availability gaps). Architecture-vs-UX/epics P0-4 numbering divergence (architecture line 4783 P0-4 = "Empty/Skeleton/Error Inventory" UX deliverable vs UX/epics P0-4 = legal counsel onboarding = Story 0.13 discharge) documented in inventory-roster.md Row 6 notes + engagement-ledger §9 per [[feedback_architecture_vs_prd_boundary]] — architecture remains source-of-truth for row labels; divergence is documented not silently corrected. Story 0.15 constitutes the SEVENTH Phase-0 portfolio distinct from bus-factor-of-one mitigation (Stories 0.1-0.6) + loop-node operational-responsiveness (Story 0.7) + empathy field-work (Stories 0.8-0.11) + spec-to-cadence-funding-reconciliation (Story 0.12) + legal-counsel-concurrent-review (Story 0.13) + native-stack-validation (Story 0.14). Story 0.15 is unique among the seven portfolios in that it is an **aggregation surface** — closure consumes the prior six portfolios' closures rather than discharging an independent property. Per [[feedback_closure_language_precision]]: framework-leg (Tasks 1-7) = **Closed by [edit]**; Tasks 8-11 legs = **Resolved via explicit deferral**.
...
> ⛔ **RECONCILED 2026-08-24 (AI-11a-1(b), FIRST INSTANCE — Story 11b.1's AUTHORING PASS) — ⭐ FOUR OF THIS STORY'S ACCEPTANCE CRITERIA DESCRIBE THINGS THE SUBSTRATE CANNOT DO. THREE ARE NEW FINDINGS OF THIS PASS; ⛔ NOTHING HAD RECORDED THEM.**
>
> ⚠ **Clauses (1)-(4) are the four defective ACs. ⭐ Clause (5) is a GRAIN conflict with the UX spec, and clause (6) is a RULING taken in this session that binds 11b.2/11b.3 — ⛔ neither is an AC defect, and both are recorded here because this is the pass that found them.**
>
> ⚠ **This is the vehicle AI-11a-1(b) exists for** (`epic-11a-retro-2026-08-23.md:381`): *"when a story-authoring pass reconciles a defective AC, the reconciliation is written back into `epics.md` … so the correction lives in the document the next author reads."* ⛔ Annotated, ⛔ never rewritten. Every claim below was checked by reading the named file at `a231ca7`.
>
> **(1) ⛔⛔ THE VISIBILITY-MATRIX PARSER MECHANICALLY REJECTS THE DECEASED MEMBER'S NAME ON THIS SURFACE — IN ANY FORM, INCLUDING first-name + last-initial.** `packages/contracts/src/public-pages/matrix.ts:376-395` permits **EXACTLY ONE** Tier-1-PII-at-`public` field **matrix-wide**, scoped at the root ON PURPOSE (*"a per-surface check would permit one exception on EVERY surface, which is a general door wearing the costume of an exception"*). ⭐ **The one is already spent** on `member-directory.member_name`, whose own `scope:` states it *"does NOT reach In Memoriam or Sahyog Vivran"*. ⚠ **And both doors are shut:** the same gate is fail-closed the other way — *"a rendered field this file does not declare is a leak"* — and the check has ⛔ **no `renders:` exemption**, so the field cannot even be pre-declared unbuilt. ⇒ ⛔ **the AC's *"per-pool entries show … nominee family identifier"* and its name-bearing search leg are UNBUILDABLE without a Panel ruling.** ⭐ The SUBSTRATE is not the blocker: `resolvePoolIdentity()` already returns `deceasedFirstName` + `deceasedLastInitial`. ⇒ **Story 11b.1 D1.**
>
> **(2) ⛔⛔ *"SEARCH … BY DECEASED MEMBER'S NAME"* HAS NO SUBSTRATE, AND NEVER HAS.** `packages/domain/src/schema/member_kyc_profiles.ts:70-78` holds `name_ciphertext` as **Tier-1 envelope** ciphertext with ⛔ **no blind-index column of any kind**; `member/search-read.ts:41-44` supports **exactly** `memberId | mobileBlindIndex | pariwar`. ⚠ Envelope encryption gives each name **its own DEK**, so there is no ciphertext equality to match on. ⇒ answering this AC means **decrypting every candidate row on every request, on an UNAUTHENTICATED route** — ⭐ the exact amplification `DIRECTORY_DECRYPT_CONCURRENCY = 8` was introduced to bound at 11a.3, one order of magnitude worse, with the edge cache **structurally unable** to absorb it (every query string is a fresh key). ⇒ **Story 11b.1 D2.**
>
> **(3) ⛔ NO CONSENT TYPE COVERS THIS SURFACE.** The `consent_type` pgEnum (`packages/domain/src/schema/consent_records.ts:102-127`) carries **two** publication values — `sahyog_vivran_publication` (11b.3) and `in_memoriam_listing` (11b.6), both minted by **Story 6.9** (migration 0058). ⛔ **Neither names Sahyog Drive.** ⚠ The epic-level **C-5** ruling requires *"each 11b surface … DECLARES ITS OWN CONSENT GATE"* while naming only those two — ⇒ **11b.1 was assigned a gate that does not exist**, and the omission is in C-5's own text. ⚠ Minting one now is ⛔ not free: **6.9 has already shipped**, so every claim already captured carries no such consent ⇒ a **retroactive consent gap** on day one. ⇒ **Story 11b.1 D4.**
>
> **(4) ⚠ COUNSEL'S DPDPA CLEARANCE FOR THIS SURFACE IS HELD, BY NAME.** `2026-08-24-157` cl.3(a): *"11b.1 and 11b.6 publish DECEASED members and their FAMILIES. ⛔ The families accepted no Terms of Service."* ⇒ ⛔ **this surface may be BUILT; it may ⛔ NOT be PUBLISHED.** Counsel's revisit is due **2026-09-07**. ⚠ ⛔ **AND THE PHRASING RULE BINDS HERE:** write *"counsel has not reviewed X"* — ⛔ **never** *"counsel is not engaged"*, which is **FALSE and has been since 2026-06-21** (`2026-08-24-158`).
>
> **(5) ⚠ THE EPIC AC AND THE UX SPEC DESCRIBE TWO DIFFERENT TABLES, AT DIFFERENT GRAIN.** This AC specifies a **POOL index** (*"per-pool entries"*). `ux-design-specification.md:1158` specifies a **CONTRIBUTION-level** 10-column table — `Donation ID | Member ID | HRMS | Donor Name | School | District | Block | Pool | Late Teacher | Date`. ⛔ **Three of those ten have NO substrate whatsoever** (verified: no `donation_id` anywhere in `packages/`; no HRMS field on any member table; and `Member ID` on a public wire is what 11a.3's handler refuses in terms — *"⛔ No `member_id`"*, because a per-member permalink is an enumeration primitive). ⚠ ⭐ **AND TWO OF ITS COLUMN LABELS ARE MICROCOPY-GATE PROHIBITED**: `microcopy.yaml:42` bars *"donor"* (→ *colleague / सम्मानित साथी*) and `:48` bars *"Late Teacher"* (→ *Deceased Member*). ⇒ the contribution-level table is **11b.2's components on 11b.3's host**, ⛔ not this story, and the UX inventory owes an amendment. ⇒ **Story 11b.1 D5.**
>
> **(6) ✅ RULED (BigDev, 2026-08-24) — ⛔ A CONTRIBUTOR'S NAME IS NEVER REMOVED BECAUSE THEY DIED, AND ⚠ RTBF IS ⛔ NOT THE SAME RULE. ⭐ THIS BINDS 11b.2 AND 11b.3, ⛔ NOT 11b.1.** Raised at this authoring pass and ruled in the same session. **The ruling:** *"a contribution made while alive stays in the record with that name on it — dying does not un-give it."* ⇒ ⛔ **no death-derived predicate may filter, mask or anonymize a CONTRIBUTOR row**, on any surface, at any tier. ⚠ ⭐ **Recorded despite the answer being obvious, because TODAY IT HOLDS BY ACCIDENT:** `resolveMemberDisplayName(state, name)` — the ONE seam every contributor display path routes through, whose own header names *"Sahyog Drive contributor lists"* as its future consumer — takes a **`MemberLifecycleState`**, and death is an **overlay** ⇒ ⭐ **that seam is blind to death BY CONSTRUCTION.** ⛔⛔ **And that is the failure mode THIS EPIC creates:** C-5's sharp edge instructs authors to ADD the `account-frozen` conjunct to predicates lacking it (because 11a.3 wrongly *published* a deceased member) — ⚠ **the same correction applied to a CONTRIBUTOR read silently DELETES dead contributors from the historical record**, and ⛔ nothing catches it. ⇒ ⭐ **the right conjunct in the wrong read.** ⚠⛔ **THE CARVE-OUT, ⛔ not an exception to negotiate:** *"history is never rewritten"* is **ALREADY FALSE for RTBF** and correctly so — `resolveMemberDisplayName` resolves an `anonymized` member to *"an anonymous member"* **regardless of any residual name** (Story 3.12 defense-in-depth), and ⚠ **a deceased member IS reachable**: `member.rtbf_anonymized` reaches `anonymized` from **every** lifecycle state, and death touches ⛔ none of them. ⇒ **TWO RULES, ⛔ neither subsuming the other: death ⇒ the name STAYS (a historical fact nobody asked to remove) · RTBF ⇒ the name GOES via the existing branch (a legal obligation the member exercised).** ⛔ Do not write a predicate treating them as one case, and ⛔ **do not restate this as *"contribution history is immutable"*** — it is not, and that sentence implements the wrong thing in the RTBF direction. ⇒ **11b.2 and 11b.3 each carry this into their OWN AC and Tasks lists** ([[feedback_spec_edits_must_propagate_to_tasks]]). ⚠ ⛔ It settles ⛔ **nothing** about the name FORM, which stays **unruled**.
>
> ⭐ **WHAT THE FOUR CONVERGE ON, AND IT IS A REAL SURFACE:** a **pool-level transparency index** — Active (`closed`) + Archive (`settled`), paginated on the reused FR-91 primitive, filterable by **district + date range + pool code** (all three queryable with ⛔ **zero decrypts**), each row carrying the **confirmed-only** contribution count (Story 9.5) and Story 7.8's Pool-Reality #2 framing — publishing ⛔ **no member PII at all**. ⚠ **The name is ONE matrix row and ONE `resolvePoolIdentity()` call away** when the Panel and counsel answer.
>
> ⚠ **AND THE THREE PRIOR ANNOTATIONS ON THIS STORY ARE ⛔ UNDISTURBED:** the 2026-08-19 name-form ruling still binds (contributor and donor name forms here remain **unruled**); the 2026-08-23 `members.state`/anti-enumeration legs still bind; and SD-2's **disposition (c)** still stands — search ships **public-tier-only**, the authenticated tier **deferred**. ⚠ ⛔ **None of them is a ruling on (1)–(5) above**, and ⛔ none may be read as one.
>
> ✅ **DISPOSITIONS (BigDev, 2026-08-24) — ALL RULED, and ⭐ TWO OF THEM CHANGED WHAT THIS AC MEANS.** **(1) → D1(b):** the matrix's exactly-one Tier-1-at-`public` rule is **WIDENED BY RULING** and the deceased member's name **RENDERS**, ⭐ **in FULL — ⛔ NOT first-name + last-initial (see D10 below, which SUPERSEDES the form stated here and was ruled later the same day)** — **consent-gated** — ⚠ ruled **against** the recommendation, on the ground that an unnamed beneficiary reads as *the Trust diverting funds to its own account*. ⛔ The widening is a **matrix-governance change** owing its own decision clause, ⛔ not a code edit. **(2) → D2(a):** search ships **district + date range + pool code**; name search **deferred** on the `name_blind_index` trigger alone (its second condition **discharged** by D1(b), ⛔ not dropped). **(3) → D4(b):** `sahyog_drive_publication` is **MINTED**, declinable + revocable — ⭐ and D1(b) **VACATED** D4's first ruling (a), whose entire ground was *"no subject PII ⇒ no subject to gate"*; ⛔ **(a) did not become wrong, its QUESTION ceased to exist.** ⭐⛔ **AND IT IS A ONE-WAY DOOR:** consent is recordable only **pre-adjudication** and pools spawn **one per APPROVED claim** ⇒ by the time a pool exists the window has **shut**; every claim filed before the type exists is **permanently unaskable**. Pre-launch that cohort is **empty by construction**. **(4) unchanged** — counsel's hold stands, and ⚠ **D1(b) was taken WITH IT OPEN**, deliberately and recorded. **(5) → D5(a):** the POOL index; the UX table is 11b.2's components on 11b.3's host.
>
> ✅ **D10 (BigDev, 2026-08-24) — ⭐ THE DECEASED MEMBER'S *FULL NAME* RENDERS ON THIS SURFACE, ⛔ NOT first-name + last-initial. ⚠ PANEL RATIFICATION IS OWED and is ROUTED.** ⭐ **Why it is the coherent answer, ⛔ not the aggressive one:** (i) a shielded form **defeats D1(b)'s own ground** — the UX spec's Real Data Test (`:1252`) is built on *"duplicate **full names** disambiguated only by Member ID + HRMS lookup"*, so *"Sushil K."* on a district-level index is useless for the verification the name was admitted to serve; (ii) ⭐⭐ it is **strictly MORE protective than what is already authorised** — `/members` publishes **living** members' **full legal names by DEFAULT with ⛔ no opt-out** (`-135` cl.7(c)/`-136`), while this publishes a **deceased** member's full name **only on explicit revocable family consent**; (iii) `shielded_name` returns `''` — **omit** — for **every mononym** (`2026-08-21-145` cl.3), and mononyms are common in India, so the shield would leave a whole class of deceased members **unnamed** on the public record. ⛔⛔ **THE FENCE IS NARROW — D10 reaches ⛔ NOTHING but the deceased member's name on THIS surface:** ⛔ **11b.3 and 11b.6 are UNTOUCHED** (the matrix exception's `scope:` keeps them at first-name + last-initial; ⛔ their own rulings to seek) · ⛔ **contributor and donor name forms stay UNRULED** (the 2026-08-19 ruling is undisturbed) · ⛔ the **nominee/family identifier renders nowhere** (Deed 15(c) names *"nominee"*; `-157` cl.3(b) stands). ⭐ **BUILD CONSEQUENCE:** resolve through `resolvePublicMemberName(mode, storedName)` — ⛔ **NEVER** `resolvePoolIdentity()`, which **hard-codes** `splitFirstNameLastInitial` and can only ever return the shielded form; and ⛔ never a literal, since `-136` cl.1 fails *"a build in which the public name form cannot be changed without a code change"*. ⚠ **Observational, ⛔ not fixed:** the **public** page now shows **more** than the **member app** does for the same pool (8.6/8.7/8.8 shield it) — binds **11b.2** and **11b.3**.
>
> ⭐⭐ **AND A DEED FINDING THAT REACHES FURTHER THAN THIS STORY, ⛔ PREVIOUSLY UNRECORDED — read it before authoring 11b.3 or 11b.6.** **Trust Deed Clause 15(c)**: *"Public rendering of any **member, contributor, nominee, or verifier** information (including **memorial or contributor-list surfaces**) shall be effected **only with explicit, revocable, purpose-specific consent and never on a default opt-in basis**."* ⇒ ⭐ **the consent posture of EVERY Epic 11b public surface is fixed by the DEED, ⛔ not by the Niyamavali and ⛔ not by C-5** — and Deed cl.20(a) forbids amending the rulebook into inconsistency with it (the Deed prevails, cl.28). ⇒ ⛔ **no 11b surface may publish member, contributor, nominee or verifier information on any basis other than explicit revocable purpose-specific consent**, and ⛔ a Niyamavali amendment cannot create one. ⚠ ⭐ **THE PENDING 2026-08-21 DIRECTORY AMENDMENT CONTRADICTS IT** — it would have §4.4 read that the directory is *"rendered on the authority of the Trustee Panel and **not on consent**"*. ⛔ **Do not ratify as drafted**; cost today is **zero** (unratified). ⚠ The exposure from `-156` cl.2's default full-name authorisation is **PROSPECTIVE, ⛔ not actual** — `/members` is ⛔ not live. ⭐ The Deed is **UNEXECUTED and in active redraft**, and v4 ⛔ does **not** reach cl.15(c) (⚠ v4's *"15"* is **Object 15**, a different section) ⇒ this is a **drafting instruction to counsel**, ⛔ not an amendment. Routed: `trustee-panel-routing-note-2026-08-24-drive-record-publication-basis.md` + `niyamavali-amendment-draft-2026-08-24-drive-record-consent.md`.
>
> ⭐⚠ **THE PROCESS FINDING, RECORDED BECAUSE IT IS THE POINT OF AI-11a-1(b):** this AC has now been reconciled **three times** — 2026-08-19 (attribute model), 2026-08-23 (`members.state` + anti-enumeration), 2026-08-23 AI-11a-2 (SD-2). ⛔ **None of the three asked whether the fields it was ruling on could be DECLARED IN THE MATRIX AT ALL.** ⇒ ⭐ **a reconciliation pass inherits the question it was pointed at; only an AUTHORING pass reads the substrate end-to-end.** That is exactly why vehicle (b) exists, and this block is its first instance.
...
> ⛔ **RECONCILED 2026-08-30 (AI-11a-1(b), Story 11b.2 authoring + three validation passes) — ⭐ THE AC ABOVE CONTAINS A SELF-CONTRADICTION ON TWO ADJACENT LINES, AND THE STORY IT NAMES HAS BEEN SPLIT THREE WAYS.** ⛔ Annotation only — ⛔ **no AC above is rewritten** ([[feedback_supersede_never_reinterpret]]).
>
> **(1) ⛔ THE STATUS-PILL CLAUSE IS A DEFECTIVE AC, AND IT IS RECONCILED AWAY** (`.decision-log.md#decision-2026-08-30-168` cl.2 = Story 11b.2's **D2(a)**). The clause *"lists render … + **status pill (Story 9.6)** per the matrix tier"* sits **one line above** *"never yellow-pill or attested-only states"*. ⚠ **The two cannot both be built.** The 8.3 wire contract gives a contributor row **no status field at all** — `pool-contributor-list.ts:39-40`, *"a row's mere presence means confirmed"*, with `.strict()` teeth at `:51` that exist to reject one. ⇒ a pill here would either (a) require widening the row with `status`, breaking the shape test **by design** and re-opening the yellow/attested door 8.3 and 9.5 closed **structurally**, or (b) be hard-coded `'green'` — a decoration asserting a fact nothing checked.
> ⇒ ⛔ **NO `status` field, ⛔ no `deriveStatusPillViewModel` call, ⛔ no `'green'` literal — and (b) was rejected BY NAME, so the pill may ⛔ not return as chrome at the render layer either.**
>
> **(2) ⭐ "VIRTUALIZED COMPONENTS … AS EXTENSIONS OF `packages/ui`" IS SATISFIED BY SPLITTING THE NOUN, ⛔ not by putting a virtualizer in the package.** `@twt/ui` is **headless by construction** — `dependencies` is exactly `{"@twt/contracts"}`, zero `.tsx`, and a pure `(input) → view-model` function has ⛔ no scroll container, ⛔ no viewport and ⛔ no mount lifecycle. **Virtualization is a render-layer property, always** (UX-DR80 says so itself: *"Native: FlatList tuning. Web: TanStack Virtual / react-virtuoso / react-window"*). ⇒ **11b.2 owns the row's CONTENT CONTRACT; the render layers own the WINDOWING.** ⚠ The presenter is nonetheless **virtualization-friendly BY SHAPE** — per-row and allocation-light (`deriveContributionRowViewModel(row)`), ⛔ never a list-mapping function.
> ⚠ **And the 50k/10k figures bind BEHAVIOUR UNDER LOAD, ⛔ not a page's row budget** — `ux-design-specification.md:2165` calls it *"a **performance contract**, not an implementation specification"*, and a public surface tops out at **10,000 rows by construction** (50-row page cap × page 200; the literals live at `packages/contracts/src/_common/pagination.ts:26` and `public-pages/directory.ts:48`).
>
> **(3) ⭐⭐ STORY 11b.2 IS SPLIT THREE WAYS — three risk classes, three reviewers, three revert units. ⛔ A future `sprint-planning` run must NOT regenerate a single ghost story from this section.** The sprint-status keys are named here so it cannot:
> · **`11b-2-contribution-list-components-table-mobile-row`** `[PRIMITIVE]` — the `@twt/ui` `contribution-list` presenter + its teeth + the governance writes. ⛔ Ships no pixel; ⛔ touches no `apps/`.
> · **`11b-2a-contributor-name-resolution-defect`** `[DEFECT]` — the **live, user-visible** RTBF contributor-name defect + the `N+1` decrypt bound. ⭐ Runs in **parallel**; ship first if capacity is scarce.
> · **`11b-2b-contributor-list-mobile-render-layer`** `[SURFACE]` — the mobile render layer (`<PoolContributorList>` + the Nominee Console) + family-13 a11y. ⛔ Runs **after both**.
>
> **(4) ⚠⛔ THE `<StatCardStrip>` NAMED AT `:1788` IS ⛔ NOT IN SCOPE AND ⛔ MUST NOT BE STUBBED.** C-3 (`:4799`) records *"NO PRODUCER | No owner"*, and `2026-08-23-154` declined to settle it — *"a settled **shape** is ⛔ not a settled **source**."*
>
> **(5) ⏳ THE CONTRIBUTOR NAME FORM IS ⛔ STILL UNRULED, AND THIS BLOCK DOES ⛔ NOT RULE IT** (`-168` cl.4 = 11b.2's **D7-nameform(a)**). ⚠⛔ **THREE committed lines in THIS document assume first-name + last-initial for CONTRIBUTORS** — ⭐ `:3145` (Story 8.3's own *"I want"*, naming the audience as *"any pool member viewing the My Pool card **or any visitor on Sahyog Drive (Epic 11b)**"*), `:3238` (the receipt PDF), and the AC above. ⛔ **None is a ruling** — that is the whole reason the question survives — but the count is **three**, ⛔ not one. The question is routed to the Trustee Panel as `_bmad-output/planning-artifacts/trustee-panel-routing-note-2026-08-30-contributor-name-public-tier.md`: ⛔ **ROUTED, ⛔ nothing ratified, ⛔ nothing applied.** ⇒ the presenter emits name **PARTS** and ⛔ never joins them (`-168` cl.6), because a join would decide the form in `presenter.ts`.
>
> **(6) ⚠ THE UX-SPEC COLUMN INVENTORY THIS STORY WOULD RENDER IS ⛔ NOT BUILDABLE, and the amendment is written at this story** (`-168` cl.3 = **D6-uxspec(a)**; `deferred-work.md` item **(f)** (`:296-313` after Story 11b.2's own section was prepended), whose trigger *"Story 11b.2 or 11b.3 authoring, whichever comes first"* **has fired**). Of the ten columns at `ux-design-specification.md:1158`: **three have no substrate** (`Donation ID`, `HRMS`, `Member ID` — the last **refused in terms** on a public wire as an enumeration primitive), **two labels are microcopy-PROHIBITED** (`microcopy.yaml:42` *"Donor Name"*, `:48` *"Late Teacher"*, both `member_only: true`), and **`School` / `Block` are separately ineligible or gated** (`2026-08-19-133` cl.1, `-132` cl.3, `2026-08-19-137` cl.7). ⛔ The amendment **annotates** the spec at five anchors; it ⛔ never rewrites it.
>
...
> ⭐⛔ **ANNOTATED 2026-09-04 (Story 11b.10 Task 0, before the first line of code). ⛔ ANNOTATION ONLY — ⛔ no AC above is rewritten, and ⛔ the 2026-08-23 / 2026-08-29 / 2026-09-02 blocks are neither deleted nor re-worded ([[feedback_supersede_never_reinterpret]]).**
>
> ⛔⛔ **A STORY EXISTS THAT IS ⛔ NOT IN THIS FILE'S STORY LIST — `11b-10-sahyog-vivran-unguessable-address-and-inbound-path` `[SURFACE]`.** It was created by a **Trustee-ratified decision**, ⛔ not by epic decomposition — exactly as `11b-3a` was. The sprint-status key is named here so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**.
>
> **(i) ⭐ WHY IT EXISTS.** [`2026-09-03-184`](../../.decision-log.md#decision-2026-09-03-184) (Trustee-ratified — Dhiraj Rahul, Kalpana Bharti) answered Story 11b.3a's enumeration bound: **(A) YES**, a `live` drive's page **should be publicly reachable**, and **(B) MAKE THE ADDRESS UNGUESSABLE** (option **(c)**). ⚠⛔ The Panel directed **(c)**, ⛔ **not** (b) ⇒ the **rate-limit tier is NOT changed**; `limits.search` stays exactly as it is, and 11b.3a's **AC2** — that tightening it as an authoring act is what may not happen — did ⛔ **not** expire when the note was answered.
>
> **(ii) ⛔⛔ ITS LOAD-BEARING PROPERTY: THE TOKEN AND THE INBOUND PATH ARE ⛔ ONE DELIVERABLE IN TWO PARTS.** `-184` **cl.4**, widened by [`2026-09-04-185`](../../.decision-log.md#decision-2026-09-04-185) **cl.3**. ⚠ The token is the **easy, satisfying** half. ⛔ **Landing it alone makes the ENTIRE Sahyog Vivran surface reachable by NOBODY** — silently converting the Panel's ratified **(A)** into *"nobody can"*, ⛔ **while passing every gate and looking like a security win**. ⛔ **A partial landing is ⛔ not a milestone, and this story may ⛔ not be split to ship the token first.**
>
> **(iii) ⭐⭐ THE PREMISE `-184` WAS RATIFIED ON WAS FALSE, AND `-185` CORRECTS IT — ⛔ the answers stand.** The routing note's §6(c) told the Panel that `closed`/`settled` drives were *"already discoverable through a published link"*. ⛔ **They are not.** ⭐ Verified: `apps/public/src/lib/sahyog-render.ts` produces **exactly TWO** `href`s — `page-1` (`:308`) and `page+1` (`:315`) — and there is ⛔ **no link to any drive's detail page from anywhere in `apps/public/src`, in ANY state**. The index **LISTS** drives; it ⛔ **never LINKS** to them. ⇒ ⛔ **all three states need an inbound path built**, ⛔ not only `live`. ⚠ The Panel was **notified and its standing is UNCHANGED** — recorded as notified-and-affirmed, ⛔ **NOT** as a second ratification vote ([[feedback_record_unattested_no_backfill]]); `-184` is ⛔ **not** edited in place.
>
> **(iv) ⛔⛔ IT IS ⛔ NEITHER 11b.3's NOR 11b.3a's, AND IT IS ⛔ NOT A FOURTH CHILD OF THE SPLIT.** `-184`'s follow-ups record it in terms: it touches **11b.3's** surface (the drive page and its addressing) **and** **11b.3a's** (the four Tier-1 fields that make the address matter) ⇒ ⛔ neither may absorb it retrospectively. ⚠ Item **(ii)** above rules a **THREE-WAY** split ([`2026-09-02-182`](../../.decision-log.md#decision-2026-09-02-182) **D6(b)**) — ⛔ **do NOT renumber this story `11b-3c`**, which would misrepresent that ruling.
>
> **(v) ⛔⛔ IT IS A DEPLOYMENT BLOCKER FOR EPIC 11b.** `deferred-work.md` carries it as **BLOCKING ON DEPLOYMENT**. ⛔ **The Sahyog Vivran surface does not ship until it lands** — and it closes **"Closed by [edit]"**, ⛔ never *"resolved via deferral"* ([[feedback_closure_language_precision]]).
>
> **(vi) ⭐ FOUR DECISIONS, ⛔ ZERO BLOCKING** (ruled by BigDev in the story file, 2026-09-04 / -04b — ⛔ no decision-log entry, matching D1–D3's convention):
> · **D1 — an OPEN LINK.** The page answers **200 to anyone presenting a valid address** — ⛔ no member session, ⛔ no new auth surface, ⛔ **no branch on the reader's membership standing** of any kind. ⭐ The token bounds **DISCOVERY**, ⛔ not **AUTHORISATION**. ⚠ Its price is part of the ruling: a forwarded link is permanent public access **until the token is rotated**.
> · **D2 — the token is RANDOM · STORED · ROTATABLE**, 128 bits CSPRNG under a unique index, minted at pool spawn, ⛔ **never derived** from `pool_id` or any pool fact. ⚠ **Rotatability is what decides it, and only because D1 went the way it did.** ⛔ The migration **backfills every existing pool** — a visible pool with a NULL token is a drive whose page **404s**.
> · **D3 — the `/sahyog` per-row link is IMPLIED by (A)+(B)**, ⛔ not a fresh exposure decision. ⚠⛔ **But it is RECORDED, ⛔ not waved through:** every listed drive becomes **ONE CLICK** from four Tier-1 fields under `D8-default` **FAIL-OPEN**. ⭐ A reviewer must meet that sentence in **prose**, ⛔ never discover it in a diff.
> · **D4 — the `live` path is a MEMBER-APP ENTRY on Tab 1 `(tabs)/index.tsx` ("My Pool")**, beside `<ActiveContributionCard />` in `<ViewContributorsEntry />`'s ruled shape (Story 8.3 **D8**), self-suppressing in lock-step. ⛔ **No notification**, ⛔ no 8th FR-71 push category, ⛔ no new deep-link `resource`, ⛔ no new route group. ⚠⛔ **⛔ NOT the Shradhanjali tab despite its name** — `(tabs)/shradhanjali.tsx` renders `sample-data` with ⛔ **zero API wiring** (a P0-5 measurement prototype), so choosing it on its name would silently re-scope the story into building the memorial surface's data layer.
>
> **(vii) ⚠ WHAT IT DOES ⛔ NOT TOUCH — stated so it is ⛔ not re-opened in review.** ⛔ `D8-default` **FAIL-OPEN** is unchanged ([`2026-09-02-179`](../../.decision-log.md#decision-2026-09-02-179) cl.1; `-184` **cl.5** records that option (d) was **disclosure only** and the Panel did ⛔ not direct it) · ⛔ the **rate-limit tier** is unchanged · ⛔ [`2026-08-28-160`](../../.decision-log.md#decision-2026-08-28-160) **cl.10(a)** is unchanged · ⛔ the **masking schedule, its knob and its predicate** are untouched · ⛔ 11b.3a's **four Tier-1 allowlist entries** and the matrix are unchanged — this story adds ⛔ **no** field and changes ⛔ **no** tier. ⚠ The **canonical identifier `P-YYYY-MM-###` is RETAINED** as the operational/audit key (`-184` **cl.2**): the token governs the **public address only**, and ⛔ a route accepting **either** form has ⛔ not closed the walk.

> ⭐⛔ **ANNOTATED 2026-09-05 (Story 11b.11 Task 0, before the first line of code). ⛔ ANNOTATION ONLY — ⛔ no AC above is rewritten, and ⛔ the 2026-08-23 / 2026-08-29 / 2026-09-02 / 2026-09-04 blocks are neither deleted nor re-worded ([[feedback_supersede_never_reinterpret]]).**
>
> ⛔⛔ **A SECOND STORY EXISTS THAT IS ⛔ NOT IN THIS FILE'S STORY LIST — `11b-11-nominee-banking-coordinates-withdrawn-from-public` `[SURFACE]`.** Like `11b-3a` and `11b-10` it was created by a **Trustee-ratified decision**, ⛔ not by epic decomposition. The sprint-status key is named here so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**.
>
> **(i) ⭐ WHY IT EXISTS.** [`2026-09-04-190`](../../.decision-log.md#decision-2026-09-04-190) (Trustee-ratified — Dhiraj Rahul, Kalpana Bharti) **cl.1** removes the banking coordinates from the public surface, and **cl.2** keeps the nominee's name there under the public wording **"Nominee Name"**. ⇒ this story **implements** `-190` cl.1–2 (plus cl.4's retention duty), with [`2026-09-04-191`](../../.decision-log.md#decision-2026-09-04-191) **cl.1** supplying the **fifth** field (`nominee_vpa`, ⛔ NOT named in `-190` cl.1) and **cl.2** ruling that the masked projection **RETAINS** the nominee name.
>
> **(ii) ⭐⭐ IT IS STORY **A** OF A SIX-STORY SPLIT.** [`2026-09-04-195`](../../.decision-log.md#decision-2026-09-04-195) **cl.3** scoped the ruled programme **out of Story 11b.10** into six stories — ⚠⛔ **CORRECTED 2026-09-08 (`#decision-2026-09-08-208` cl.6): the six are `11b-11` … `11b-15` PLUS `11b-17`, and ⛔ story F is `11b-17`, ⛔ NOT `11b-16`.** ⭐ A seventh (`11b-16`, story G) was added at `-197`/`-198` and is now **WITHDRAWN** (`-208` cl.1) — which restores the COUNT to six while leaving the original contiguous range `11b-11` … `11b-16` wrong on its KEYS. ⚠ `11b-11:818` — a `done`, committed story — repeats the old mis-keying; ⭐ **recorded, ⛔ not edited** — so 11b.10 could close on its own terms. ⛔ None of the six is a child of the 11b.3 three-way split ([`2026-09-02-182`](../../.decision-log.md#decision-2026-09-02-182) D6(b)) and ⛔ **none may be renumbered `11b-3c`/`-3d`**, which would misrepresent that ruling.
>
> **(iii) ⛔⛔ IT SHIPS FIRST AND ALONE.** Of the six it is the ⛔ **ONLY** one that **REDUCES** public exposure, and it depends on ⛔ nothing. ⛔ Do ⛔ not bundle it with B–F *"since they touch the same files"* — that would gate a withdrawal of live PII behind feature work.
>
> **(iv) ⭐ WHAT IT SUPERSEDES, AND WHERE THE SUPERSESSION LIVES.** [`2026-08-28-165`](../../.decision-log.md#decision-2026-08-28-165) **cl.1–2** are superseded **IN PART** — its **FOUR** ruled `(surface, field)` Tier-1 pairs on `sahyog-vivran` become **ONE** (`nominee_account_holder_name`, re-keyed to `-190` cl.2). ⛔ `-165` is Trustee-ratified and is ⛔ **NOT edited in place**; its **cl.3–4** — *masking is a presentation/projection policy; the underlying fields stay Tier-1 in every state* — ⛔ **STAND UNCHANGED**, and this story depends on them. ⚠ The 2026-09-02 block's item **(i)** third bullet, which reads *"ruled **four** named `(surface, field)` Tier-1 pairs"*, is **true as of its own date and superseded as of 2026-09-04** — ⛔ read it as the record, ⛔ not as live.
>
> **(v) ⚠⛔ THE MASKING MACHINERY IS RETAINED AND ITS STATUS CHANGES.** `-190` **cl.4** forbids deleting `isNomineeBankMasked`, `pariwar_nominee_bank_masking_schedule`, its permission key or its tests — *"we may use it in future"*. ⛔ **But after this story it has ⛔ no public consumer**, and it may ⛔ **not** be described as a live safeguard on any Trustee-facing material until it has one.
>
> **(vi) ⚠ WHAT IT DOES ⛔ NOT TOUCH — stated so it is ⛔ not re-opened in review.** ⛔ The **rate-limit tier** (`limits.search`) is unchanged · ⛔ [`2026-09-02-179`](../../.decision-log.md#decision-2026-09-02-179) cl.1's `D8-default` **FAIL-OPEN** is unchanged · ⛔ 11b.10's opaque `publicToken`, the index, the stage vocabulary and the meter are untouched (stories **B**/**C**/**D** own those) · ⛔ the **deceased-name publication basis** is untouched · ⛔ **no member surface changes** — `contracts/src/contributions/nominee-accounts.ts` keeps returning the member's payment coordinates, and `-190` **cl.3**'s *complete banking information for a FINISHED drive* is story **F**'s, ⛔ not this one's.

> ⭐⛔ **ANNOTATED 2026-09-06 (Story 11b.12 Task 0, before the first line of code). ⛔ ANNOTATION ONLY — ⛔ no AC above is rewritten, and ⛔ the 2026-08-23 / 2026-08-29 / 2026-09-02 / 2026-09-04 / 2026-09-05 blocks are neither deleted nor re-worded ([[feedback_supersede_never_reinterpret]]).**
>
> ⛔⛔ **A THIRD STORY EXISTS THAT IS ⛔ NOT IN THIS FILE'S STORY LIST — `11b-12-stage-vocabulary-and-info-affordance` `[SURFACE]`.** It is **story B** of the same six-story split as `11b-11` ([`2026-09-04-195`](../../.decision-log.md#decision-2026-09-04-195) **cl.3**), created by **Trustee-ratified decision**, ⛔ not by epic decomposition. The sprint-status key is named here so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**.
>
> **(i) ⭐ WHAT IT IMPLEMENTS.** [`2026-09-04-190`](../../.decision-log.md#decision-2026-09-04-190) **cl.5** (the three stage words, and why *"Collecting"* is retired — the **register** ground, *"like Trust is collector"*, ⛔ not a synonym swap) · [`-191`](../../.decision-log.md#decision-2026-09-04-191) **cl.3** · [`-192`](../../.decision-log.md#decision-2026-09-04-192) **cl.1** (*Verified* = reconciled) and **cl.3** (the info affordance) · [`-193`](../../.decision-log.md#decision-2026-09-04-193) **cl.1** and **cl.3** (⭐ **ONE** shared copy source, web and app). ⇒ the three public stages are **Live · Closed · Verified**, in one keyed set in `packages/i18n`.
>
> **(ii) ⭐⭐ IT REMOVES THREE **FALSE** SENTENCES FROM A LIVE PUBLIC PAGE — ⛔ this is ⛔ not a copy pass.** `locales/{en,hi}/sahyog-drive.json` `page.intro`, `section.active.help` and `section.archive.help` each state or imply that **the trust pays the family**. [`-192`](../../.decision-log.md#decision-2026-09-04-192) establishes that the trust **⛔ NEVER disburses** — members pay the nominee's VPA directly — so a family is paid **THROUGHOUT** a drive, ⛔ never at a payout step. ⚠ *"has not yet been paid"* has been rendered to the public for months.
>
> **(iii) ⚠⛔ THE WIRE TOKENS MOVE TOO, AND THAT COLLIDES WITH A SHIPPED ANTI-LEAK RULE.** Story 11b.12 **D1(b)** (BigDev, 2026-09-04) aligns the wire to the ruled words — index `['closed','verified']` (⛔ **TWO** members; `live` on the index is story **D**) and drive page `['live','closed','verified']` — and re-shapes every anti-leak assertion from a **DENY-list** to an **ALLOW-list**. ⚠ `live` and `closed` are simultaneously the ruled **public** words and **internal** `pools.current_state` names, and **nine** shipped sites assert the internal names never cross ([`2026-08-21-144`](../../.decision-log.md#decision-2026-08-21-144) **cl.8**). ⭐ All nine are amended **and each names cl.8**, so the overlap reads as **ruled**, ⛔ never as the `/members` `lock-in` leak recurring. ⛔ `spawned` remains a **pure deny**.
>
> **(iv) ⭐ [`-194`](../../.decision-log.md#decision-2026-09-04-194) **cl.1** IS RECORDED **SATISFIED BY CONSTRUCTION**, ⛔ NOT BUILT.** `sahyog.astro:461`/`:514` already guard both sections with `.length > 0`, so the rename inherits the behaviour. ⛔ No second guard is added; ⭐ a test **pins** it so a future refactor cannot drop it silently.
>
> **(v) ⭐⭐ D2 IS **CLOSED** (Trustee Panel — Dhiraj Rahul + Kalpana Bharti, 2026-09-05), AND ITS THREE RULINGS ARE **ROUTED**, ⛔ not held.** The routing note is `_bmad-output/planning-artifacts/trustee-panel-routing-note-2026-09-05-11b12-under-funded-commitment-claim.md`. ⭐ **Ruling 3** (*an absent token **drops its clause**; ⛔ no combinatorial variants*) is **11b.12's**, as its **AC9 / Task 2b** — 11b.12 **authors** the Panel-ratified index line with `{amount}` **and** `{nominee_name}` **dark**, and ⛔ renders neither. ⛔ **Ruling 1** (mechanize the approver duty, so *"nominee of"* is true **because it was checked**) became **Story 6.18**, created 2026-09-05. ⛔ **Ruling 2** (the nominee name **on the index** — §9.4's bulk-harvest property was put to the Panel and **ACCEPTED**) **landed in story D** as `11b-14` **AC7 + Task 8**, whose sub-clause (a) owes a **decision-log entry and a matrix row before any code**. ⚠⛔ **`outcome.under_funded` (`sahyog-drive.json:44`) is ⛔ NOT touched by 11b.12** — closing D2 settled **who authors what**, ⛔ not *"the copy story may rewrite a statement of the trust's obligation"*.
>
> **(vi) ⭐⭐ THE MEMBER-APP RENDER IS STORY **E**'s (`11b-15` AC4 / Task 4) — ⛔ IT IS ⛔ NOT DEFERRED WORK.** ⭐ Verified: `driveStatus` appears **⛔ nowhere** in `apps/mobile`, and `SahyogVivranEntry.tsx` is a **link-out card** with ⛔ no stage — ⇒ there is **nowhere in the app for a stage word to go**. ⛔ 11b.12 changes **no** `apps/mobile` file; it ships the **source**, and E builds the first surface that consumes it. ⚠ Recorded from **both** sides, because B and E each read as though the other does it and ⛔ no per-story pass can see the loop ([[feedback_circular_deferral_between_sibling_stories]]).
>
> **(vii) ⚠ WHAT IT DOES ⛔ NOT TOUCH — stated so it is ⛔ not re-opened in review.** ⛔ ⛔ No `live` drive is added to the **public index** and ⛔ no third member to its enum (story **D**) · ⛔ no field tier, listing predicate, state machine, rate limit, masking behaviour or bank field moves · ⛔ no JSON **key name** and ⛔ no TypeScript **label field** is renamed (**D3** — the ban is on **rendered values**, ⛔ not on identifiers a member never reads) · ⛔ `locales/*/members.json`'s *"Active"* is the **ratified member-lifecycle** label and is **correct** — ⛔ the copy assertion is scoped to the two sahyog namespaces and ⛔ must not be widened · ⛔ `pool.settled` is ⛔ **still** unreachable in production, which is why the affordance must explain a stage ⛔ nothing can currently be in.

> ⭐⛔ **ANNOTATED 2026-09-06 (Story 11b.13 Task 0, before the first line of code). ⛔ ANNOTATION ONLY — ⛔ no AC above is rewritten, and ⛔ the 2026-08-23 / 2026-08-29 / 2026-09-02 / 2026-09-04 / 2026-09-05 / 2026-09-06 (11b.12) blocks are neither deleted nor re-worded ([[feedback_supersede_never_reinterpret]]).**
>
> ⛔⛔ **A FOURTH STORY EXISTS THAT IS ⛔ NOT IN THIS FILE'S STORY LIST — `11b-13-per-pariwar-drive-target-substrate` `[SUBSTRATE]`.** It is **story C** of the same six-story split as `11b-11` and `11b-12` ([`2026-09-04-195`](../../.decision-log.md#decision-2026-09-04-195) **cl.3**), created by **Trustee-ratified decision**, ⛔ not by epic decomposition. The sprint-status key is named here so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**.
>
> **(i) ⭐ WHAT IT IMPLEMENTS.** [`2026-09-04-190`](../../.decision-log.md#decision-2026-09-04-190) **cl.7** — a **Pariwar Admin** records what a drive in their Pariwar needs to raise (**cl.7(a)**), the figure is **invisible to members and the public** (**cl.7(b)**), and only a **Super Admin** may reveal it, to members and/or publicly, on **two independent switches** (**cl.7(c)**) — with [`-191`](../../.decision-log.md#decision-2026-09-04-191) **cl.4** making it a **RUPEE** figure. ⭐ [`-189`](../../.decision-log.md#decision-2026-09-04-189) **cl.3**'s *member ≥ public* is satisfied **symmetrically** (hidden from both) and is **ENFORCED** in the reveal, ⛔ not documented: public-revealed-while-member-hidden is **refused**.
>
> **(ii) ⭐⭐ IT OPENED WITH A GOVERNANCE DECISION, AND ⛔ NOT A BUILD TASK.** `-195` **cl.2** rules that minting a permission key gets its **own** decision. ⇒ [`2026-09-06-203`](../../.decision-log.md#decision-2026-09-06-203) mints **TWO** keys — `pariwar.manage_drive_target` (`pariwar_admin` + `super_admin`) and `pariwar.manage_drive_target_visibility` (⛔ `super_admin` **ONLY**) — and moves `PERMISSION_CATALOG_VERSION` **39 → 41** (+1 per key; **read live**, ⛔ not transcribed — **Story 6.18** bumps the same counter).
>
> **(iii) ⛔⛔ IT GRANTS `pariwar_admin` ON A NEW PARIWAR-DIMENSION KEY, AND THAT IS ⛔ NOT THE MOVE THE NEIGHBOURING KEY FORECLOSES.** `pariwar.manage_nominee_bank_masking`'s doc-block names *"granting it for symmetry"* as the **"reverse a ratified ruling by way of a catalog edit"** move, with **a Panel ruling** as its acceptance condition. ⭐ `-190` cl.7(a) **is** one — but the real ground is narrower and is what `-203` **cl.3** records: the foreclosing precedents govern **DISCLOSURE**, and **SETTING the target discloses ⛔ NOTHING** (cl.7(b) hides it from everyone). **REVEALING** it **IS** a disclosure act, and cl.7(c) keeps that `super_admin`-only. ⇒ [`2026-08-19-136`](../../.decision-log.md#decision-2026-08-19-136) **cl.3**'s two-axis rule is **FOLLOWED**: per-Pariwar in **SCOPE**, Pariwar-Admin in **OPERATIONAL** authority, **central in DISCLOSURE** authority. ⛔ [`2026-09-02-178`](../../.decision-log.md#decision-2026-09-02-178) and the masking key's foreclosure **STAND UNTOUCHED**.
>
> **(iv) ⭐ THE AUTHORITY SPLIT IS STRUCTURAL TWICE OVER — in the CATALOG and in the SUBSTRATE.** **D1** (BigDev, 2026-09-04) ruled **two keys** rather than one-key-plus-a-role-check-in-a-handler, so an auditor can see the split **where they would look for it**. **D2** (BigDev, 2026-09-06) ruled **two records** — `pariwar_drive_target_schedule` (versioned, `pariwar_admin`-written) **+** `pariwar_drive_target_visibility` (`super_admin`-written, both flags) — because under the append-only shape a shared row would make a **Pariwar Admin's** write the act that **re-states a `super_admin`-only reveal on every change**. ⇒ *"a target change leaves both flags byte-unchanged"* is **true by construction**, ⛔ not a test of discipline.
>
> **(v) ⚠⛔ IT SHIPS A CONTROL WITH ⛔ NO VISIBLE OUTPUT, AND THAT IS CORRECT.** The target is rendered **nowhere** (cl.7(b)); the meter that consumes it is story **D** (`11b-14`), **server-side only**. ⇒ ⛔ do ⛔ not "finish the job" by rendering it or adding a preview — the story's own tests are its **only** proof it works.
>
> **(vi) ⚠⛔ ITS NAMED PRECEDENT HAS A RULED, ⛔ UNFIXED CONCURRENCY DEFECT — and this story does ⛔ NOT inherit it.** [`2026-09-05-201`](../../.decision-log.md#decision-2026-09-05-201) was ruled **after** this story's baseline and is ⛔ still unbuilt in `apps/api/src/modules/nominee-bank-masking/`: the masking PUT's advisory lock **removed the collision that was the only thing preventing a silent overwrite**. ⇒ this story's **new** write path carries `-201`'s two controls **in order** — `Idempotency-Key` **FIRST**, `expectedVersion` (REQUIRED, `number | null`) **SECOND**, with a **registered** 409. ⚠ ⛔ It does ⛔ **not** owe 11b.3a's patch; reaching into the masking module would be scope drift.
>
> **(vii) ⚠ WHAT IT DOES ⛔ NOT TOUCH — stated so it is ⛔ not re-opened in review.** ⛔ No stage vocabulary (story **B**) · ⛔ no bank field (story **A**) · ⛔ no listing predicate, meter, rate limit or masking behaviour · ⛔ `pools.fixed_amount` and **everything a member owes** are untouched — the target is a **presentation denominator** and introduces ⛔ **no predicate that gates a member's access to a benefit** (AI-10-1) · ⛔ **no per-drive override** — one target per Pariwar, ⛔ not a column, ⛔ not a nullable field, ⛔ not a seam · ⛔ **D3 is ⛔ not decided here** — the meter recovers the hidden target by arithmetic from **two published figures** (`target ≈ amountRaisedInr ÷ confirmedPercentage`), and that is **ROUTED to story D** (`11b-14` AC2 + Task 3) with the question **OPEN**, ⛔ not pre-ruled and ⛔ not escalated.

> ⭐⛔ **ANNOTATED 2026-09-07 (Story 11b.14 Task 0, before the first line of code). ⛔ ANNOTATION ONLY — ⛔ no AC above is rewritten, and ⛔ the 2026-08-23 / 2026-08-29 / 2026-09-02 / 2026-09-04 / 2026-09-05 / 2026-09-06 (11b.12) / 2026-09-06 (11b.13) blocks are neither deleted nor re-worded ([[feedback_supersede_never_reinterpret]]).**
>
> ⛔⛔ **A FIFTH STORY EXISTS THAT IS ⛔ NOT IN THIS FILE'S STORY LIST — `11b-14-live-drives-listed-and-the-progress-meter` `[SURFACE]`.** It is **story D** of the same six-story split as `11b-11`, `11b-12` and `11b-13` ([`2026-09-04-195`](../../.decision-log.md#decision-2026-09-04-195) **cl.3**), created by **Trustee-ratified decision**, ⛔ not by epic decomposition. The sprint-status key is named here so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**.
>
> **(i) ⭐⭐ THIS IS THE STORY THAT RESTORES FR-76 — ⛔ AND THAT IS THE REPAIR OF A FIVE-MONTH GAP, ⛔ NOT A FEATURE REQUEST.** **FR-76** (`epics.md:160`) — *"Sahyog Drive — Active + Archive. **Active page near-real-time during live alert.**"* — has been a **standing, un-superseded requirement since the PRD**, and it is cited in ⛔ **ZERO** implementation records until [`2026-09-04-187`](../../.decision-log.md#decision-2026-09-04-187) found it, and only because BigDev asked *"who decided a collecting drive is a solicitation?"*. ⇒ the public index has shipped for months **excluding** the very drives FR-76 named as its live case.
>
> **(ii) ⛔⛔ THIS STORY'S OWN AC PARENTHETICAL, AT `epics.md:4872`, IS **SUPERSEDED** — ⭐ named, ⛔ never edited in place.** It reads *"Active drive shows currently-live pools **(closed but not yet settled)**"*. ⚠ That gloss equates *"currently-live"* with the internal `pools.current_state` value **`closed`**, and it is **superseded by** [`2026-09-04-189`](../../.decision-log.md#decision-2026-09-04-189) **cl.2** (*"(Q2) — **YES: A COLLECTING DRIVE IS LISTED**"*), which admits the internal state **`live`** to the public index for the first time. ⭐ Read the parenthetical as **the record of what shipped**, ⛔ not as live. ⚠ ⛔ `spawned` remains excluded, ⛔ unchanged.
>
> **(iii) ⭐ WHAT IT IMPLEMENTS.** [`-189`](../../.decision-log.md#decision-2026-09-04-189) **cl.2** (a collecting drive is listed; **cl.2(b)** each row carries a progress bar; **cl.2(e)** the amount **and** the contributor count, wording delegated) · [`-190`](../../.decision-log.md#decision-2026-09-04-190) **cl.6** (the participation-first headline) and **cl.7(b)/(c)** (the target is invisible by default; ⛔ **ONLY a Superadmin** may reveal it) · [`-196`](../../.decision-log.md#decision-2026-09-04-196) **cl.8** (*"**Story D is its first consumer, server-side**"*) · [`-200`](../../.decision-log.md#decision-2026-09-05-200) (Trustee-ratified: a collecting drive's page **should** be reachable by the public) · and **ruling 2** of the 2026-09-05 Panel note on 11b.12 **D2** — the **nominee name on the index** (**AC7 + Task 8**).
>
> **(iv) ⚠⛔⛔ THREE TRUSTEE-RATIFIED CLAUSES ARE SUPERSEDED BY THE **2026-09-07** PANEL RULINGS — ⭐ and they are recorded in [`2026-09-07-204`](../../.decision-log.md#decision-2026-09-07-204), ⛔ not left in a story file.** ⛔ [`-191`](../../.decision-log.md#decision-2026-09-04-191) **cl.4** (*"THE PROGRESS BAR FILLS AGAINST A **RUPEE** TARGET"*) is **SUPERSEDED** — the bar measures **contributors**, `confirmedCount ÷ rosterSize`, which is ⭐ exactly what `packages/ui/src/pool-progress` already ships. ⛔ `-189` **cl.2(d)** (*"Superadmin-settable, **PER-PARIWAR** … the **SAME** target for every drive"*) is **SUPERSEDED as to the VALUE** — लक्ष्य is the **derived** per-drive total `assignedCount × pools.fixed_amount`; ⭐ its *"enable switch"* half **survives**. ⛔ `-190` **cl.7(a)** (*"the **PARIWAR ADMIN** sets the target, from day 1"*) is **SUPERSEDED** — ⭐ there is ⛔ **no setter**. ⭐ **cl.7(b) and cl.7(c) STAND, undisturbed** — the ruling replaced the value's **SOURCE**, ⛔ never the **visibility rule**.
>
> **(v) ⚠⛔ HALF OF STORY C IS NOW UNCONSUMED — ⭐ RETAINED, ⛔ NOT DELETED, ⭐ AND ITS STATUS IS STATED.** Per the [`-190`](../../.decision-log.md#decision-2026-09-04-190) **cl.4** masking-window precedent: `pariwar_drive_target_schedule`, `resolveEffectiveDriveTargetInr`, `setDriveTargetSchedule` and the admin **target** form have ⛔ **no consumer** after 2026-09-07, and ⛔ may **not** be described as live controls on any Trustee-facing material until they have one. ⭐ `pariwar_drive_target_visibility`, its `member_ge_public` CHECK, its RLS and `pariwar.manage_drive_target_visibility` are **LIVE** — they gate लक्ष्य. ⛔⛔ **The retirement of `pariwar.manage_drive_target` is ⛔ NOT this story's** — it is a **catalog act** and it has its own vehicle, **`11b-18-drive-target-write-authority-retired`**, on its own branch.
>
> **(vi) ⚠⛔ IT REPAIRS SIX SHIPPED SITES THAT NAME **THIS STORY BY NUMBER** AND DESCRIBE A RULING THAT ⛔ NO LONGER HOLDS — ⛔ one of them the committed, client-facing API spec.** Story **C** shipped while `-191` cl.4 still ruled a rupee denominator and wrote that ruling into its code *"Story 11b.14's meter is `amountRaisedInr / target`"*. ⇒ `drive-target.ts:57`/`:75` · `drive-target-ceiling-sync.test.ts` (a **test NAME**) · `emit-openapi.ts` ×3 · ⛔⛔ **`openapi/v1.yaml`** are **amended and NAMED**, ⛔ never deleted, and ⛔ **NO behaviour moves with them** — `.positive()`, the ₹ ceiling and the `configured` flag all **stay**; ⛔ only the stated **ground** moves. ⭐ The spec is **re-emitted** from `emit-openapi.ts`, ⛔ never hand-edited. ⛔⛔ **And `11b-13` is ⛔ NOT amended retrospectively and stays `done`** — ⛔ it was ⛔ not wrong when it shipped ⇒ ⭐ a **supersession**, ⛔ never a defect.
>
> **(vii) ⚠⛔ THE DRIVE-PAGE AMOUNT IS ⛔ NOT THIS STORY'S — ⭐ IT IS `11b-3b`'s, BY A RULING THREE DAYS OLDER.** 11b.14 **D2** rules the amount renders on **both** the index row and the drive page; ⛔ but **11b.3 D1(b)** ([`2026-09-02-176`](../../.decision-log.md#decision-2026-09-02-176)) already **moved** the drive-page figure to `11b-3b-sahyog-vivran-named-identity-render-layer` (`ready-for-dev`), whose **AC3b** consumes `derivePoolProgressCardViewModel(...).amountRaisedInr` **unchanged** and lifts the `@twt/ui` fence **there**. ⇒ ⭐ D widens the **INDEX** row only, **amends** the drive page's *prose*, and renders ⛔ **nothing** on it. ⛔ D does ⛔ not lift the `@twt/ui` fence — ⭐ `apps/public` has ⛔ **no** `@twt/ui` dependency, and that is a `package.json` fact, ⛔ not prose.
>
> **(viii) ⚠⛔ ONE DIVERGENCE WITH `11b-3b` IS **RECORDED**, ⛔ NOT SILENTLY TAKEN — ⭐ and neither story knew of the other.** 11b.14 **AC2** ships a **server-computed `confirmedPercentage`** on the public **index** wire, which is squarely `-189` **cl.2(b)**'s ratified bar. ⚠ `11b-3b` **AC3b** records, for the **drive page**: *"⛔⛔ **ONLY `amountRaisedInr` IS AUTHORISED** … the presenter also emits **`confirmedPercentage`** … ⛔ **not** authorised by `D1(b)` … ⛔ **It needs its own decision**."* ⇒ ⭐ the two stories hold **opposite postures on adjacent contracts**. ⛔ D does ⛔ **not** decide `11b-3b`'s question and ⛔ does not edit its file; ⭐ it names the divergence from both sides so ⛔ no per-story pass can miss it ([[feedback_circular_deferral_between_sibling_stories]]).
>
> **(ix) ⚠ WHAT IT DOES ⛔ NOT TOUCH — stated so it is ⛔ not re-opened in review.** ⛔ No stage word is invented (story **B** owns the vocabulary; D **composes** `sahyog-shared:stage.live`, ⛔ never restates it) · ⛔ no target authority, key, table or admin surface (story **C**) · ⛔ no bank field beyond the **one** ruled `nominee_account_holder_name` (story **A** owns the rest; the keys are **ABSENT**, ⛔ never `null`) · ⛔ **no member surface** (stories **E**/**F**) · ⛔ **no rate tier and no cache policy** — `/sahyog` stays `s-maxage=300`, and FR-76's *"near-real-time"* is met by that bound, **stated**, ⛔ never silently shortened · ⛔ no `spawned` · ⛔ **no ordering, ranking, "most-supported" view or cross-drive comparison** — 11b.1 **AC5**'s surviving half stays **enforced**; the amendment **narrows** the *"never a sum of amounts"* sentence, ⛔ it does ⛔ not repeal AC5 · ⛔ **no written pitch** (`-190` cl.8) — ⭐ the arithmetic is **SHOWN**, ⛔ never **ASSERTED** · ⛔ **no `PERMISSION_CATALOG_VERSION` bump** (AC6; the catalog act is `11b-18`'s) · ⛔ **no predicate that gates a member's access to a benefit** (AI-10-1) — it widens a **LISTING** predicate, which governs what a **stranger may see**, ⛔ not what a member may do.
...
> ⭐⛔ **SECTIONED 2026-09-09 (Story 11b.15 Task 0, before the first line of code).** This story had **no `epics.md` entry** until it created its own — the **Story 8.16 precedent** (`epics.md:3400`), itself following **Story 7.11** (`epics.md:3041`) — so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**. ⚠ The sprint key `11b-15-member-drive-list-fourth-tab` was already *named* at `:3410` (8.16's sequencing), `:5189` (the corrected six-story range) and `:5213` (11b.12's annotation assigning the member-app render here); ⇒ this discharges the missing **section**, ⛔ not a first mention.
>
> ⚠ **WHY A SECTION AND ⛔ NOT AN ANNOTATION.** Its four shipped siblings — `11b-11` (**A**), `11b-12` (**B**), `11b-13` (**C**), `11b-14` (**D**) — each annotated **Story 11b.3**, because each **amended** the public Sahyog surface that story owns. ⛔ This one amends nothing on that surface: it creates a **new member-app surface** with ⛔ no parent in this file.
>
> ⭐ **IT IS STORY E OF THE SIX-STORY SPLIT** ([`2026-09-04-195`](../../.decision-log.md#decision-2026-09-04-195) **cl.3**; the six are `11b-11` … `11b-15` **plus `11b-17`**, corrected at [`-208`](../../.decision-log.md#decision-2026-09-08-208) cl.6), following Trustee-ratified [`-193`](../../.decision-log.md#decision-2026-09-04-193) **cl.2** and BigDev's [`-194`](../../.decision-log.md#decision-2026-09-04-194) **cl.2** / [`-196`](../../.decision-log.md#decision-2026-09-04-196). ⭐ **Story F (`11b-17`) depends on it.**
>
> ⭐⭐ **IT IS A NEW SURFACE, ⛔ NOT A FILTER CHANGE.** `apps/mobile/app/(tabs)/` holds **three** tabs and ⛔ **no member-facing drive list exists** — `SahyogVivranEntry` is a single link-out on the My Pool card. ⇒ new tab, new route, new read, its own family-13 pass.

As a **member of a Pariwar**,
I want to see **every drive my Pariwar has run** — the one collecting now, the ones finished, and the ones fully checked —
So that I can see what my contributions have added up to, in one place inside the app.

**Depends on:** Story 11b.12 (**B** — the ⭐ one shared stage-copy source in `packages/i18n`, namespace `sahyog-shared`), Story 11b.13 (**C** — the drive-target substrate + `resolveDriveTargetVisibility`), Story 11b.14 (**D** — the public index whose fields are this list's floor), Story 8.16 (the mode-resolved member name form — **sequenced before this story** by [`2026-09-08-208`](../../.decision-log.md#decision-2026-09-08-208) cl.3). **Blocks:** Story 11b.17 (**F**).

**Acceptance Criteria:**

**Given** [`2026-09-04-194`](../../.decision-log.md#decision-2026-09-04-194) **cl.2** (the list is a **peer of My Pool**, ⛔ not a route buried inside it)
**When** the member drive list ships
**Then** a **fourth** `Tabs.Screen` joins `index` · `shradhanjali` · `panchayat`
**And** ⚠⛔ it **SUPERSEDES A RECORDED REJECTION, WHICH IS NAMED RATHER THAN OVERWRITTEN** ([[feedback_supersede_never_reinterpret]]): Story **10.15** (`done`) records the rejected alternative *"A dedicated 4th bottom tab → the tab bar is at three and the UX spec does not add one. Enter from Panchayat."* (`10-15-survey-poll.md:134`). ⭐ `-194` cl.2 is the ruling that changes it, and 10.15's ground was **negative evidence only** — `ux-design-specification.md` fixes ⛔ no tab count (its single *"tab bar"* mention, `:481`, is unrelated). ⇒ the supersession is recorded **by name in the `_layout.tsx` doc-block**, so the next author does ⛔ not read 10.15 and revert the tab
**And** ⭐ **THE TAB TITLE IS `t()`-RESOLVED — a deliberate, recorded departure from its three siblings** (BigDev, 2026-09-09). `_layout.tsx:42,49,56` carry `'My Pool'` · `'Shradhanjali'` · `'Panchayat'` as **hardcoded English literals** in a bilingual app, and the file imports ⛔ no `@twt/i18n` at all. ⛔ That is **PRE-EXISTING** and ⛔ **not swept here**; ⭐ but new member-facing copy is ⛔ never minted untranslated, so the fourth tab is **the one translated title among four** — **Sahyog Drives / सहयोग अभियान**

**Given** [`-196`](../../.decision-log.md#decision-2026-09-04-196) (the three visible states)
**When** the list resolves which drives a member sees
**Then** it lists every drive in the member's **own Pariwar** in `live` · `closed` · `settled`, scoped **from the session**, ⛔ never from a client-supplied Pariwar id
**And** ⛔ `spawned` is **EXCLUDED, and the ground is ⛔ not tidiness**: a `spawned` pool follows an **approved claim**, before contributions open ⇒ listing it would disclose **a death and its claim approval to the whole Pariwar, earlier than any surface does today**. ⛔ A **disclosure change**, ⛔ not a filter widening — and the ground lives **in the code**
**And** ⚠⛔ the predicate is this surface's **OWN named fragment**, ⛔ never the public tuple — ⭐ **even though the two now read identically.** `SAHYOG_DRIVE_VISIBLE_POOL_STATES` (`packages/domain/src/pool/public-read.ts:126`) became `['live','closed','settled']` at 11b.14 AC1; ⇒ the coincidence is a **TRAP**, ⛔ not a licence to import it — the two tuples agree *today* by accident of two separate rulings ([`-189`](../../.decision-log.md#decision-2026-09-04-189) cl.2(a) for the public, `-196` here) and **either may move alone**. This follows `public-read.ts:283-284`'s standing rule and the `sahyog-vivran-read.ts:124` precedent; ⛔ the new fragment is **not** added to that constant's five-artefact fan-out list (`public-read.ts:119-125`)

**Given** [`-189`](../../.decision-log.md#decision-2026-09-04-189) **cl.3** (*a member must see MORE than the public, and never less*), **scoped** by [`-195`](../../.decision-log.md#decision-2026-09-04-195) cl.1 to the **drive data class** and these six stories — ⛔ it is ⛔ not a universal invariant
**When** the list's field set is fixed
**Then** for every drive the **public index** shows, this list shows **at least as much** — the floor being `SAHYOG_DRIVE_ROW_FIELD_IDS` (`apps/public/src/lib/surface-fields.ts:401-428`), ⭐ **14 mapped field ids** (15 keys, of which `driveLinkA11yLabel → null` is an a11y annotation, ⛔ not a field), ⭐ measured against the index **as it stands**, ⛔ not as it stood at authoring
**And** ⚠⛔ **THE COUNT IS 14, ⛔ NOT THE *"13"* THE STORY FILE'S PROSE STATES** — ⭐ a story-file defect found at Task 2 (2026-09-09) and **recorded, ⛔ not silently absorbed**. The story's own **enumeration** lists **14** ids and is **correct**; only its count WORD was wrong, which is why two `validate` passes re-checking *"the enumeration, exact match, order and all"* both passed. ⇒ ⭐ **the test enumerates `SAHYOG_DRIVE_ROW_FIELD_IDS` PROGRAMMATICALLY and ⛔ never a transcribed number** — a hand-copied count is the same failure mode as the hand-listed subset below
**And** ⚠ **five of those did ⛔ not exist at authoring** — story D added the meter's three governed values (`drive_progress_percentage`, `drive_participation_line`, `drive_target`; all `pii_tier: 3`), `drive_index_line`, and ⛔⛔ **the nominee's FULL name** (`nominee_account_holder_name`, `pii_tier: 1`, **Trustee-ratified** [`2026-09-07-205`](../../.decision-log.md#decision-2026-09-07-205) cl.1). ⭐ A nominee's **NAME** is ⛔ **not** a banking coordinate ⇒ the `-190` cl.3 exclusion does ⛔ not reach it
**And** ⚠ one field is **nullable by ruling** — [`-207`](../../.decision-log.md#decision-2026-09-08-207) cl.1 drops `drive_progress_percentage` from the wire on `closed` and `settled` rows ⇒ the comparison treats a public `null` as *"nothing to match"*, ⛔ never as a missing member field
**And** the comparison **enumerates every id in the live map and fails on any the member read omits** — ⛔ never a hand-listed subset, ⭐ which is how [`-188`](../../.decision-log.md#decision-2026-09-04-188) survived
**And** ⛔⛔ **the test SEEDS THE PUBLICATION BASIS, or it proves nothing.** The basis is **inert** — `niy.public-disclosure.member-information` has exactly **one** repo site, its own definition (`public-read.ts:261`) ⇒ the public read returns `deceased_member_name: null` on **every** un-seeded drive and *member ⊇ public* degenerates to a null-check. ⭐ The real chain is seeded (`terms_and_conditions_versions` → `clause_versions` → `terms_and_conditions_pinned_clauses` → `consent_records`); ⛔ a fixture touching `sahyog_drive_publication` proves nothing (`-160` cl.5 de-authorised it, **read-never**)

**Given** [`-193`](../../.decision-log.md#decision-2026-09-04-193) **cl.3** (⭐ **ONE** shared copy source, web and app) and Story 11b.12's shipped `sahyog-shared` namespace
**When** a drive's stage renders
**Then** the words are **Live · Closed · Verified**, resolved from **B's keys by name** — ⛔ this story may ⛔ **not** mint its own, and a **live assertion** aimed at it says so (`apps/mobile/tests/unit/sahyog-stage-copy-resolves.test.ts:66-73`)
**And** they agree with `PUBLIC_STATUS_BY_POOL_STATE` (`public-read.ts:169-173`), whose totality-asserting accessor is `publicStatusForPoolState` (`:180`) — ⭐ `POOL_LIFECYCLE_STATES` is exactly four values and the machine is strictly linear, so ⛔ no cancelled/void state is silently dropped
**And** the **info affordance** is a **real focusable control** with a **tap** handler and an accessible name — ⛔ never hover-only
**And** ⭐⭐ **THIS IS THE FIRST PLACE IN THE APP WHERE A STAGE EXISTS AT ALL.** ⛔ B renders **nothing** in `apps/mobile`, **by ruling** — it shipped the **source only**. ⚠ Recorded from **both** sides deliberately: B and E each read as though the other renders it, and ⛔ no per-story pass can see that loop ([[feedback_circular_deferral_between_sibling_stories]])

**Given** [`2026-09-09-211`](../../.decision-log.md#decision-2026-09-09-211) — ⭐⭐ **the FIRST production consumer [`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.7(c)'s MEMBER arm has ever had**
**When** a drive's expected figure (**लक्ष्य**) is resolved
**Then** it renders where, and ⛔ **only** where, `resolveDriveTargetVisibility` (`packages/domain/src/pool/drive-target-policy.ts:233`) reports **`revealToMembers`**
**And** ⚠⛔ **the gate is `reveal_to_members`, ⛔ NOT `reveal_to_public` — ⭐ a CHOICE, ⛔ not a transcription** (`-211` **cl.2**). The 2026-09-07 routing note's §13.3 said *"on that same condition"*, which read literally means the **public** axis; `-211` cl.2 reads the **member** axis on three recorded grounds — cl.7(c) authorises the axes **separately**; the DB CHECK `NOT (reveal_to_public AND NOT reveal_to_members)` (`schema/pariwar_drive_target_visibility.ts:96`) makes public-on **imply** member-on, so `-189` cl.3 holds either way; and reading the public axis would ship an **INERT** member switch. ⛔ Do ⛔ not reverse it silently
**And** the default is **fail-closed** — an absent row is hidden from **everyone** (`-190` cl.7(b)) ⇒ ⛔ **nothing renders at launch, for any Pariwar**, and ⭐ that is correct
**And** ⛔ it gets its **own resolver** — `resolveDriveTargetForMembers` beside `resolveDriveTargetForPublic` (`public-read.ts:652`); ⛔ ⛔ **no parameter is bolted onto the public one** (`-211` cl.4)
**And** the number form is [`-206`](../../.decision-log.md#decision-2026-09-07-206) **cl.3**, unchanged: short-form at and above ₹1,00,000 (*"₹50 lakh"*, ⛔ never *"₹50.00 lakh"*), **exact below ₹1 lakh**, crore joining at ₹99.99 lakh. ⚠ The ₹10-lakh cut-off governs the **contributed** amount and does ⛔ **not** apply to the target

**Given** **UX-DR80** (`epics.md:509`) — *"10k mobile / 50k desktop virtualization contracts on **Sahyog List components**"*
**When** the list renders
**Then** it is virtualized via **`@shopify/flash-list`**, ⛔ **not `FlatList`** — the repo's recorded threshold split reserves FlashList for exactly this case (`YogdaanBahi.tsx:23`; `PoolContributorList.tsx:26-28`)
**And** ⚠ **empty · loading · error render OUTSIDE the list**, ⛔ never as a `ListEmptyComponent` swapped in place — New-Arch FlashList **and** FlatList both red-box crossing **empty → populated in place**, so the rule is ⛔ **primitive-independent** ([[project_fabric_flatlist_empty_populated_crash]]), and *"loading, then rows"* is this surface's **normal** path
**And** the read is **PAGINATED** — ⛔ a Pariwar's whole history is ⛔ not one response. ⚠⛔ ⛔ **No member route paginates today** ⇒ the **public Sahyog Drive** shape is copied (`page` + `limit` on the contract; `.limit(clampLimit(...)).offset(...)` in the domain)
**And** ⛔⛔ **`clampLimit` is MANDATORY** (`packages/domain/src/pagination.ts:34`) — the `domain-accessor-invariants` CI gate fails any dynamic `.limit()` without it, and `Math.min(limit, cap)` does ⛔ **not** satisfy it ([[project_domain_limit_clamp_and_savepoint_retry]])

**Given** family 13 and the tamagui finding recorded by Story 11b.10's review and ⛔ **not** discharged by B
**When** any labelled or interactive element renders
**Then** every container carrying `accessibilityLabel` is **explicitly `accessible={true}`** — ⚠ a tamagui `<Button>` is `styled(View)` and `@tamagui/web` sets `accessible` **nowhere** (verified at the installed **2.1.0**)
**And** a role implying interaction has a **real handler**, and every state the ACs ratify as reachable is **ANNOUNCED**, ⛔ not merely reflected in a prop
**And** ⭐ the precedent is **cited, ⛔ not re-derived**: `components/sahyog-vivran/SahyogVivranEntry.tsx:108-128` for a pressable, `contributor-list/PoolContributorList.tsx:246` or `panchayat/PinnedItem.tsx:107` for a non-pressable labelled container

**Given** [`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.3's *"complete banking information"* is a **per-drive** view and belongs to story **F**
**Then** ⛔ **nothing else moves**: ⛔ no banking coordinates on this list · ⛔ no contributor names · ⛔ no per-member amounts · ⛔ no `spawned` rows · ⛔ no change to `active-contribution` or anything a member owes · ⛔ no public surface touched
**And** ⚠⛔ **the two STALE post-8.16 code comments are RECORDED, ⛔ not edited** — `contracts/src/public-pages/sahyog-drive.ts:128` and `apps/api/src/modules/public-pages/handlers.ts:518-520` both still say `resolvePoolIdentity` *"hard-codes"* the shielded form. ⭐ The **fence** they express still holds; ⛔ their stated **reason** is false at HEAD. ⇒ `deferred-work.md`, ⛔ not a public-surface edit here
**And** ⚠⛔ **the RETIRED JUSTIFICATION is ⛔ not available here either** ([`-209`](../../.decision-log.md#decision-2026-09-08-209) **cl.3**): ⛔ do ⛔ not write, paraphrase or offer a member *"the same name anyone can already see on the public page"* or *"until now the app showed you less than the public page did"*. ⭐ Both were **false for every drive**, and 8.16 was re-grounded away from them

**Dev Notes / guardrails:**
- ⛔ **NO PREDICATE THAT GATES A MEMBER'S ACCESS TO A BENEFIT** is introduced or changed (AI-10-1). ⭐ The story **adds a read**: ⛔ no eligibility, ⛔ no assignment, ⛔ no obligation, ⛔ no amount owed. ⚠ **TWO VISIBILITY predicates are introduced and neither gates a benefit** — *which drives appear* (`-196`) and *whether the expected figure appears* (`revealToMembers`, `-190` cl.7(c)); ⭐ checked against the Niyamavali, which requires and forbids ⛔ neither.
- ⭐ **THE LOAD-BEARING TEST IS A COMPARISON, AND ⛔ IT ALREADY EXISTS.** `-189` cl.3 is a **relational** invariant — a test checking this list's fields **in isolation cannot see a violation**. `apps/api/tests/integration/contributions/member-name-form-parity.spec.ts` (Story 8.16) already drives **both real routes** over live Postgres; ⇒ this list is added as the **THIRD** surface in that comparison, reusing its basis-seeding block. ⛔ Do ⛔ not reinvent the harness.
- ⚠ Its concurrency is **1** and **load-bearing**; assert **membership and explicit values**, ⛔ never counts over the shared fixture ([[project_live_db_test_gotchas]], [[project_ci_local_concurrency_oversubscription]]).
- ⭐ The name field calls the **same resolver** as every other member surface — `resolvePoolIdentity` stays the **ONE join site** (8.16 AC2b), reading the Pariwar's stored `public_name_presentation_mode` once per request. ⛔ It ⛔ never re-derives a name form, and ⛔ never reuses `resolvePublicMemberName` (the public omits a mononym under `shielded_name`; the member shows it).
- ⚠⛔ **THE RESIDUAL NAME ASYMMETRY IS ⛔ A DECISION, ⛔ NOT A DEFECT** (`-209` cl.2): on a Pariwar in the default `full_name` mode a member sees a full legal name that ⛔ **nobody** can see publicly, because the public gate is inert. ⛔ Do ⛔ not "correct" it. ⭐ What survives is the **structural** reason to keep the surfaces aligned (`-209` cl.4) — the moment the basis is provisioned, the public rises to the configured form.
- ⚠ Trap 5 is a **FLOOR, ⛔ not a licence**: `-189` cl.3 sets a minimum and does ⛔ **not** authorise showing a member anything the Panel has ⛔ not ruled public **or** member-visible.

**FRs:** ⛔ **None amended.** This story introduces no FR and edits none — it is a **read surface** over drives the Trust already runs. **UX-DR anchors:** UX-DR80 (list virtualization — the *"Sahyog List components"* clause). **Decisions:** [`2026-09-04-193`](../../.decision-log.md#decision-2026-09-04-193) cl.2 · [`-194`](../../.decision-log.md#decision-2026-09-04-194) cl.2 (the peer tab) · [`-195`](../../.decision-log.md#decision-2026-09-04-195) cl.1 + cl.3 (the split, and cl.3's scope bound) · [`-196`](../../.decision-log.md#decision-2026-09-04-196) (the three visible states) · [`-189`](../../.decision-log.md#decision-2026-09-04-189) cl.3 (*member ≥ public*) · [`-197`](../../.decision-log.md#decision-2026-09-04-197) + [`-198`](../../.decision-log.md#decision-2026-09-04-198) cl.1 (the configured **FORM**, ⛔ not the publication **BASIS**) · [`-205`](../../.decision-log.md#decision-2026-09-07-205) cl.1 (the nominee's full name, Trustee-ratified) · [`-206`](../../.decision-log.md#decision-2026-09-07-206) cl.3 (the number form) · [`-207`](../../.decision-log.md#decision-2026-09-08-207) cl.1 (the nullable percentage) · [`-208`](../../.decision-log.md#decision-2026-09-08-208) cl.3 + cl.6 · [`-209`](../../.decision-log.md#decision-2026-09-08-209) cl.2–cl.4 · **[`-211`](../../.decision-log.md#decision-2026-09-09-211)** (⭐ AC8b in full — ⚠ read **cl.2** before building it).

---

### Story 11b.17: The Member's View of ONE Drive — Carrying What the Public Page No Longer Does `[SURFACE]`

> ⭐⛔ **SECTIONED 2026-09-13 (Story 11b.17 Task 1, before the first line of code).** This story had **no `epics.md` entry** until it created its own — the **Story 11b.19 precedent** (its `SECTIONED 2026-09-12` annotation, which itself follows **11b.15**, **8.16** and **7.11**) — so a future `sprint-planning` run can ⛔ **neither drop it nor regenerate a ghost**. ⚠ The sprint key `11b-17-member-drive-detail-unredacted` was already *named* in `sprint-status.yaml` (row + ledger) and in 11b.15's own section; ⇒ this discharges the missing **section**, ⛔ not a first mention.
>
> ⚠ **WHY A SECTION AND ⛔ NOT AN ANNOTATION.** Its four shipped siblings — `11b-11` (**A**), `11b-12` (**B**), `11b-13` (**C**), `11b-14` (**D**) — each annotated **Story 11b.3**, because each **amended** the public Sahyog surface that story owns. ⛔ This one amends nothing there: like **E** (`11b-15`) it creates a **new member-app surface** with ⛔ no parent in this file. ⛔ **NOT** the `11b-13` shape, which owes an **ANNOTATION**.
>
> ⭐ **IT IS STORY F OF THE SIX-STORY SPLIT** ([`2026-09-04-195`](../../.decision-log.md#decision-2026-09-04-195) **cl.3**; ⚠ **one of SIX**, ⛔ not *"the last of seven"* — [`-208`](../../.decision-log.md#decision-2026-09-08-208) **cl.6** re-counted the set when **G** (`11b-16`) was withdrawn). ⭐ It **depends on E** (`11b-15`, `done`) and on **A** (`11b-11`, `done`); its copy source is **`11b-19`** (`done`).
>
> ⭐⭐ **THIS IS THE STORY THAT MAKES [`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.3 TRUE** — *"a logged-in member sees the complete banking information."* ⚠⛔ And ⛔ **NOT** because cl.3 forces it: story **A** removed the banking coordinates from the **public** surface entirely, so *member ≥ public* is satisfied on banking **by construction** whatever this story does ([`-189`](../../.decision-log.md#decision-2026-09-04-189) cl.3). ⇒ ⭐ **the scope of member banking access is a FREE, SEPARATE decision**, and it was made at [`-199`](../../.decision-log.md#decision-2026-09-04-199): **option (a)**, scope **(i)** — the member's OWN Pariwar.

As **a member looking at a drive in my Pariwar**,
I want to open it and see what the trust actually holds about it — including the account the money went to —
So that the people whose contributions funded it can see **where it went**, instead of that record existing **nowhere a member can reach**.

**Depends on:** Story 11b.11 (**A**, `done`) · Story 11b.15 (**E**, `done` — this detail is reached from its fourth tab) · Story 11b.19 (`done` — the ratified message block this story renders). **Blocks:** ⛔ nothing.

**Acceptance Criteria:**

**Given** [`-199`](../../.decision-log.md#decision-2026-09-04-199) — *"any authenticated member may access the member-facing Sahyog Vivran detail for any Trust Pariwar drive, including its full nominee banking coordinates, subject to the drive's lifecycle and member-surface access controls"* (option **(a)**, scope **(i)**)
**When** a member opens ONE drive from their Pariwar's drive list
**Then** a **detail read** resolves it by its **opaque public address token** and ⛔ by nothing else — ⛔ never by the walkable `P-YYYY-MM-###` ([`-184`](../../.decision-log.md#decision-2026-09-03-184) **(B)**)
**And** its visible-state tuple is **`live` · `closed` · `settled`** (⛔ never `spawned`, [`-196`](../../.decision-log.md#decision-2026-09-04-196)), declared as this surface's **OWN named fragment** — ⛔ never shared with, and ⛔ never parameterised onto, the list's ([`-196`](../../.decision-log.md#decision-2026-09-04-196) Consequence 2)
**And** ⚠⛔ **the two vocabularies are ⛔ NEVER mixed in an assertion**: `live`/`closed`/**`settled`** are **POOL STATES**, `live`/`closed`/**`verified`** are **WIRE TOKENS**

**Given** [`-189`](../../.decision-log.md#decision-2026-09-04-189) cl.3, scoped by [`-195`](../../.decision-log.md#decision-2026-09-04-195) cl.1 to the drive data class
**When** the member detail renders
**Then** it shows **at least everything the PUBLIC drive page shows** for the same drive, proven by a **comparison test** that reads the floor **programmatically** from `SAHYOG_VIVRAN_FIELD_IDS` **∪** `SAHYOG_VIVRAN_NOMINEE_ACCOUNT_FIELD_IDS` — ⛔ never from a hard-coded list and ⛔ never asserting a **COUNT**
**And** ⛔ **NOT** against `SAHYOG_DRIVE_ROW_FIELD_IDS`, which is the public **INDEX** map — copying it proves ⛔ nothing about this surface
**And** ⚠ **exactly THREE exclusions**: लक्ष्य (a member-side **ADDITION**, ⛔ not a superset claim), the **DECEASED** member's name (its public gate is **provisioning-inert**, [`-209`](../../.decision-log.md#decision-2026-09-08-209) cl.2), and ⛔ **nothing else** — ⛔⛔ the **NOMINEE's** name is ⛔ **not** an exclusion: it carries ⛔ no gate and that is RULED ([`-205`](../../.decision-log.md#decision-2026-09-07-205) cl.9), so the two surfaces are **SYMMETRIC** and a carve-out there would **suppress a real failure**

**Given** [`-212`](../../.decision-log.md#decision-2026-09-10-212) **cl.1** (Trustee-ratified, DR + KB, 2026-09-10) — the detail renders **लक्ष्य** on a **`live`** drive **ONLY**
**When** the drive is `live` **and** a `super_admin` has switched **`reveal_to_members`** ON for that Pariwar
**Then** the figure renders in [`-204`](../../.decision-log.md#decision-2026-09-07-204) **cl.12**'s ruled slot, **DERIVED** as `assignedCount × pools.fixed_amount` (cl.2 — ⛔ there is ⛔ no setter), formatted per [`-206`](../../.decision-log.md#decision-2026-09-07-206) **cl.3**, and consumed from **`sahyog-shared`**'s `drive_target` **BY NAME** ([`-193`](../../.decision-log.md#decision-2026-09-04-193) cl.3)
**And** ⛔ the key is **ABSENT** — ⛔ never `null` — on the **WIRE TOKENS** `closed`/`verified`
**And** ⚠⛔⛔ the gate is resolved through **`resolveDriveTargetVisibility`**, ⛔ **NEVER** `getDriveTargetVisibilityRow` — *"a caller interpreting it is exactly how a fail-closed default becomes fail-open"* ([`-211`](../../.decision-log.md#decision-2026-09-09-211) cl.3). ⚠ A regression here is a **DISCLOSURE defect**, ⛔ not a UI defect
**And** ⭐ ⛔ **NO Pariwar has a row** ⇒ ⛔ **NOTHING renders** on the day this ships — ⭐ that is **CORRECT**, ⛔ never a bug to "fix"

**Given** [`-199`](../../.decision-log.md#decision-2026-09-04-199)'s scope **(i)** — the member's **OWN** Pariwar
**When** the coordinates are served
**Then** the boundary is enforced **SERVER-SIDE** — ⛔ never by hiding a field the response already carried
**And** a member of **another** Pariwar requesting the drive gets a **404** (member routes carry ⛔ no `:pariwarId`; `claim_nominee_bank_accounts` runs **RLS FORCE**d under `SET LOCAL app.pariwar_id`)
**And** the **CONTRACT's** coordinate keys are structurally **ABSENT**, ⛔ never `null` (the 11b.11 shape, [`-205`](../../.decision-log.md#decision-2026-09-07-205) cl.9) — ⚠⛔ asserted against the **schema**, ⛔ never as `expect(body).not.toHaveProperty(…)` against a 404, which passes **vacuously**

**Given** [`-213`](../../.decision-log.md#decision-2026-09-10-213) **cl.1** and Trap 4 (*"a masked account# cannot be transferred to"*)
**When** the coordinates appear
**Then** **BOTH** accounts render — ⛔ no primary/secondary, ⛔ no ordering implied (6.8 **D1**, 9.9) — each with **FIVE** fields: **account number · IFSC · holder name · bank · branch**, **UNMASKED** and **COMPLETE**
**And** ⛔⛔ the **UPI ID is ⛔ NOT on this wire**, on ⛔ any drive, in ⛔ any stage — [`-212`](../../.decision-log.md#decision-2026-09-10-212) **cl.2** ruled it onto the **PAYMENT screen** instead, and **`8-17`** (`done`) shipped that half
**And** ⚠ two **differing** holder names are a **LEGITIMATE state** (the schema carries ⛔ no FK, ⛔ no `nominee_rank`, ⛔ no match rule) ⇒ **surface both, pick neither**, exactly as [`-213`](../../.decision-log.md#decision-2026-09-10-213) **cl.2** orders
**And** ⚠⛔ the holder LABEL is the ruled **"Nominee Name"** ([`-190`](../../.decision-log.md#decision-2026-09-04-190) **cl.2**) via `label.account_holder` — ⛔ **never** `nominee.label`, which renders *"Nominee"*, and ⛔ never `upi_intent.account_holder_label`, which is literally the forbidden string
**And** ⛔ the list's and the pay screen's **one-decrypt** behaviour is **CORRECT** and is ⛔ **not** touched

**Given** this surface hands **Tier-1 payment coordinates to a human** on the ordinary browsing path
**When** a member opens one drive detail
**Then** **EXACTLY ONE** audit line is written **per DETAIL OPEN** — ⛔ **not** one per coordinate read — naming the **member**, the **drive** (by its **canonical identifier**, ⛔ never a token or an account number) and the **instant**
**And** ⚠⛔ it is written as a **NAMED DEPARTURE** from the anonymous public precedent (`writeAppealReversalDisclosureAudit` writes `actorId: null` and says *"⛔ do not widen this to log every request"*), ⛔ never as inherited posture
**And** ⭐ the **write volume is SIZED before shipping** ([`-199`](../../.decision-log.md#decision-2026-09-04-199) Consequence 2) — ⚠ the real unit is **≈5–8 global-lock acquisitions per open**, ⛔ not one: `envelope.ts` fires `auditHook('decryptDek')` on **every** `decryptTier1` with ⛔ **no DEK cache**, so **seven of the eight are INVISIBLE**. ⭐ A **PRE-EXISTING** condition this story **AMPLIFIES** (story E already decrypts two names per row) — ⛔ not one it introduces, and ⛔ **never** "optimised" by dropping the KMS hook, which is the **FR-47** record of which key opened which field

**Given** the routed obligations from story **E**'s THIRD code-review pass (2026-09-09, BigDev — author-committed, ⛔ not Trustee-ratified)
**When** the detail ships
**Then** **`pool_canonical_identifier`** and **`drive_href`** (which **IS** `publicToken` on the member wire — ⛔ there is ⛔ no key literally named `drive_href`) both **RENDER here**, and the affordance is a **REAL focusable control** with a real handler, an accessible name, and an `accessibilityRole` **matching what it does**
**And** ⚠⛔ **discharging it turns a GREEN test RED BY DESIGN** — `apps/mobile/tests/unit/drive-list-render.test.ts`'s *"the ROW declares `text`, ⛔ never `button`/`link`"* — ⇒ **amend E's AC5 record and that test BY NAME**, ⛔ never delete either quietly
**And** ⚠ **the ZERO-DAY row renders the RATIFIED copy** — `zero_line.full` / `zero_line.no_family` from **`sahyog-shared`** BY NAME ([`-206`](../../.decision-log.md#decision-2026-09-07-206) cl.4), the variant chosen on **nullability** (`t()` **THROWS** on an unsupplied token), the **percentage ⛔ NOT suppressed at zero**, and the **accessible name** carrying the same softening
**And** ⚠ [`-207`](../../.decision-log.md#decision-2026-09-08-207) **cl.2**: where a drive's amount is **₹0** on the wire tokens `closed`/`verified`, the *"About this drive"* sentence renders **NOTHING** — ⛔ no placeholder, ⛔ no marker, ⛔ no partial sentence

**Given** [`-214`](../../.decision-log.md#decision-2026-09-11-214) **cl.4(b)** — *"`11b-17` carries the MEMBER render — AC10 / Task 5d — and ⛔ nothing more"*
**When** the message block renders
**Then** the **eight** `message_block.*` keys shipped by **`11b-19`** are consumed **BY NAME** from `sahyog-shared`, in **both** locales — ⛔ never authored, re-derived or translated at a render site; ⚠ a missing key is a **STOP**, ⛔ not a licence
**And** a **two-column table — `Nominee full name` | `District` — sits ABOVE the message**, on the **MEMBER** view (⛔ the public half is **`11b-20`**, `ready-for-dev` — **HOMED, ⛔ not built**)
**And** ⭐ **FOUR ruled render properties**: `{amount}` arrives **ALREADY FORMATTED** and carries its own ₹ (⛔ a literal ₹ ships **₹₹**) · the **headline variant** is chosen on `{family_name}` **nullability** and ⛔ **only the headline** varies · there is ⛔ **NO `no_amount` variant** and where the amount is unavailable the block renders **NOTHING** ([`-207`](../../.decision-log.md#decision-2026-09-08-207) cl.2) · an absent **`district` DROPS ITS COLUMN** ([`-214`](../../.decision-log.md#decision-2026-09-11-214) Consequence 6) — ⛔ never *"Not recorded"*, ⛔ never a placeholder
**And** ⛔ **NO key may assert the NOMINEE RELATIONSHIP** ([`-214`](../../.decision-log.md#decision-2026-09-11-214) Consequence 8) — the value behind *"Nominee full name"* is `account_holder_name_ciphertext`, and **the SCHEMA is the authority**; ⭐ they are **LABELS**, ⛔ not assertions
**And** ⚠⛔⛔ **the dark-copy fence is NARROWED, ⛔ never deleted and ⛔ never appended to.** `packages/i18n/tests/sahyog-shared-dark-copy.test.ts` holds `AUTHORISED: string[] = []` with a resolver regex **deliberately hardened** to catch a runtime-built `` `message_block.headline.${variant}` `` ⇒ ⛔ there is ⛔ **no way to build this without meeting it**. ⭐ Narrow it per its own author's written instruction — *"⛔ never resolved without every token supplied"* — exactly as story **D** narrowed the `index_line.*` fence

**Given** the load-bearing-invariant checklist's **family 13**
**When** the screen ships
**Then** every **control** carries `accessible={true}` + `accessibilityRole` + an accessible name, and is a **SIBLING** — ⛔ never a descendant — of a labelled container
**And** ⚠⛔ **the checklist's own 13(a) is WRONG and has ⛔ not been swept there**: *"a container carrying `accessibilityLabel` is explicitly `accessible={true}`"* — ⭐ E's THIRD pass **disproved** it, because an `accessible` container **COLLAPSES its subtree** and would have made this story's own control **UNREACHABLE**. ⇒ ⭐ **label the LEAVES**
**And** ⛔⛔ **`ANONYMIZED_SENTINEL` ⛔ NEVER RENDERS** — `anonymizeMember` overwrites `name_ciphertext` **in place** with an *encrypted* sentinel and **RETAINS** the row, so the decrypt **SUCCEEDS**. ⭐ This surface's remedy is the **drive list's** (keep the drive, drop the name), ⛔ **not** the contributor list's (omit the row)

**Given** the six-story split's fence
**When** this story ships
**Then** ⛔ **nothing else moves**: ⛔ no public surface · ⛔ no masking behaviour (⭐ dormant, [`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.4) · ⛔ no change to the 9.9 donor path's own gate · ⛔ no contributor names · ⛔ no `spawned` · ⛔ no `vpa` on any wire · ⛔ no widening of `contribution-history` (its *"DELIBERATELY … NO nominee/bank data"* exclusion is a **shipped ruling**; widening it would be a **REVERSAL to NAME**, ⛔ never to absorb)
**And** ⛔⛔ **`SahyogVivranEntry` is ⛔ NOT touched** — **Trustee-ratified** ([`-200`](../../.decision-log.md#decision-2026-09-05-200) **cl.4**, *"Phone-app member should reach the page"*). ⚠ After this story a member has **TWO** views of one drive — this one unredacted, that one carrying no coordinates. ⭐ That divergence is **expected and recorded**, ⛔ not a defect to "fix"
**And** ⭐ AC7 is discharged by a **FENCE TEST**, ⛔ not by assertion — ⚠⛔ modelled on `sahyog-shared-dark-copy.test.ts` (a repo walk · a **non-vacuity** assertion · **self-exclusion by REAL PATH** · an `AUTHORISED` list that is ⛔ **not a waiver**), ⛔ **not** on E's AC8, which is a prose enumeration that scans ⛔ nothing and was breached by E's own dev

**Dev Notes / guardrails:**
- ⛔ **NO PREDICATE THAT GATES A MEMBER'S ACCESS TO A BENEFIT** is introduced or changed (AI-10-1): ⛔ no eligibility, ⛔ no assignment, ⛔ no obligation, ⛔ no amount owed. ⚠ **ONE VISIBILITY predicate is introduced and it gates ⛔ no benefit** — *which member may read a drive's banking coordinates*. ⭐ In the member's own terms: **"Any member of your Pariwar can see the bank account details of every family your Pariwar has supported — ⛔ not only the drives you were asked to contribute to, and ⛔ not only while a drive is still collecting."** ⚠ A **second** visibility predicate (`reveal_to_members`) is **CONSUMED, ⛔ not introduced** — this story is its **THIRD** consumer.
- ⚠⛔⛔ **TRAP 2, RECORDED RATHER THAN GLOSSED.** Read literally, [`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.3 puts **every family's account number in every pocket in the Pariwar** — on phones, screenshottable, forwardable, **persisting on devices where it cannot be taken back**. ⭐ That may be a **larger exposure than the one this programme just removed** (the public page was revocable by changing the page; this is not). ⚠⛔ **The scale figure is PER PARIWAR and ⛔ NO ratified per-Pariwar membership figure exists — ⛔ invent none**; the *"43,000"* is the Panel's own **ILLUSTRATIVE** number and [`-199`](../../.decision-log.md#decision-2026-09-04-199) says to ignore its arithmetic. ⭐ **BigDev's call, and it is made** — the Panel disclosure note was written and committed (`468d43c1`).
- ⚠⛔ **`resolveMemberFacingDeceasedName`, ⛔ NEVER `resolvePublicMemberName`.** The two share the stored mode and the form rule and differ ⛔ only in **ABSENCE** behaviour — the public **omits** a mononym under `shielded_name`, the member **SHOWS** it ⇒ reusing the public one **DROPS A MEMBER'S OWN DRIVE**. ⭐ The warning lives in the **DOMAIN** file (`packages/domain/src/pool/member-drive-list.ts`), ⛔ **not** the contracts file of the same basename.
- ⚠⛔ **PREFER A NEW CONTRACT OVER AN ADDITIVE FIELD ON `MemberDriveListEntry`.** It is `.strict()` and `api-client`'s `call` throws ⇒ one added field **BLANKS THE WHOLE TAB** for every member on an installed build older than the API release. ⭐ **`8-17`** fired that trigger first and discharged it with a written **deployment order** (mobile to devices FIRST, API second); this story avoids it entirely.
- ⚠ Any new `packages/domain` accessor taking a caller-supplied limit routes through **`clampLimit`** — the `domain-accessor-invariants` gate scans all of `packages/domain/src` and `Math.min` does ⛔ **NOT** satisfy it.
- ⚠⛔ **THERE IS ⛔ NO RN MOUNT HARNESS.** ⛔ No `@testing-library/react-native`; `MemberDriveList.tsx` **cannot be imported**. ⭐ The two shipped idioms are a **source scan** driven by the real catalog/contract, and **pure logic extracted into a plain `.ts`**. ⚠ Live-DB for the read, the boundary and the audit; assert **membership and explicit values**, ⛔ never counts over the shared fixture.
- ⚠ **THE ASYMMETRY WORTH NOTICING:** this programme spent six stories **narrowing** what a stranger can see; **D1(a) widens**, in one step, what a whole Pariwar can see — under a clause written to **close** an inversion.

**FRs:** ⛔ **None amended.** This story introduces no FR and edits none — it is a **read surface** over a drive the Trust already ran. **UX-DR anchors:** UX family 13 (semantic accessibility). **Decisions:** **[`-199`](../../.decision-log.md#decision-2026-09-04-199)** (⭐ **D1** — the scope, author-committed) · **[`-212`](../../.decision-log.md#decision-2026-09-10-212)** cl.1 + cl.2 + Consequences 2–6 (⭐ **D2**/**D3**, Trustee-ratified DR + KB) · **[`-213`](../../.decision-log.md#decision-2026-09-10-213)** cl.1 + cl.2 (both accounts; surface both names, pick neither) · **[`-215`](../../.decision-log.md#decision-2026-09-13-215)** (⭐ the §8.4(ii) successor entry this story owed) · [`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.1–cl.4, **cl.7(b)/(c)** · [`-189`](../../.decision-log.md#decision-2026-09-04-189) cl.3 · [`-193`](../../.decision-log.md#decision-2026-09-04-193) **cl.3** · [`-195`](../../.decision-log.md#decision-2026-09-04-195) cl.1 + cl.3 · [`-196`](../../.decision-log.md#decision-2026-09-04-196) + Consequence 2 · [`-200`](../../.decision-log.md#decision-2026-09-05-200) cl.4 · [`-204`](../../.decision-log.md#decision-2026-09-07-204) cl.2 + **cl.12** · [`-205`](../../.decision-log.md#decision-2026-09-07-205) **cl.1**, cl.9 · [`-206`](../../.decision-log.md#decision-2026-09-07-206) cl.2, **cl.3**, cl.4 · [`-207`](../../.decision-log.md#decision-2026-09-08-207) cl.2 · [`-208`](../../.decision-log.md#decision-2026-09-08-208) cl.4 + cl.6 · [`-209`](../../.decision-log.md#decision-2026-09-08-209) cl.2 + cl.3 · **[`-211`](../../.decision-log.md#decision-2026-09-09-211)** cl.1, cl.2, cl.3, cl.5 + Consequences 2 + 4 · **[`-214`](../../.decision-log.md#decision-2026-09-11-214)** cl.4(b) + Consequences 3, 6, 8. **Routing notes:** `trustee-panel-routing-note-2026-09-10-11b17-member-drive-detail-two-questions.md` (**Q1** = D2, **Q2** = D3) · `trustee-panel-routing-note-2026-09-05-11b12-under-funded-commitment-claim.md` **§8.1** + **§9.1 row 3**, with **§8.3(2)** for the PAGE-shape constraint only (⛔ **NOT** §8.3(3) / §8.5 row 3 — ⭐ §8.5 is *"What we propose"* and §8.3(3) is a **sequencing statement**; ⛔ neither is a ruling) · `trustee-panel-disclosure-note-2026-09-04-member-banking-access-quantification.md` (⛔ for information; ⛔ nothing waits on it).
