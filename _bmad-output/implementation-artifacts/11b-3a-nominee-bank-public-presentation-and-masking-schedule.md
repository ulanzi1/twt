...
real — **unless your Pariwar has chosen to hide it permanently, which hides it even while the drive is
still collecting.** After the drive closes, the Trust Admin for your Pariwar decides how long that
stays visible — hidden immediately, hidden after N days, or hidden permanently — and after that the
public sees only the last four digits, the bank, the branch **and the IFSC code**."*
...
⚠⛔ **THIS PASS IS CHUNKED, AND ⛔ NOT YET COMPLETE — read this before treating it as closure**
([[feedback_record_unattested_no_backfill]], family 10). The diff is **61 files / +7,996**, far past
the review workflow's 3,000-line guidance, so it was split into four groups and reviewed group by
group. **G1** (`packages/domain`, 17 files / +2,213), **G2** (`apps/api` + `packages/contracts` + `openapi`,
19 files / +2,288) and **G3** (`apps/public` + `apps/admin` + `packages/i18n` + `scripts`, 19 files /
+1,998) have each been reviewed by all three layers — their findings are in the three sub-sections
below. ⛔ **G4** (governance docs — the decision log, the routing note, `sprint-status.yaml` and
`friction-budget.md`; 6 files / +1,497) is ⛔ **NOT REVIEWED.**
⭐⭐ **G3 VINDICATED THE CHUNKING DECISION AND INDICTED ITS OWN EARLIER FIX.** It carried checklist
family **13** — un-mechanized BY RULING, so the Auditor is the ⛔ only thing that reads it — and it
found the **third instance** of a defect class this pass had already "closed" twice: the family-11
falsehood corrected in the **spec** on 2026-09-04 was still live in the **shipped operator copy**,
where ⛔ nothing had looked. ⇒ ⛔ **do not read a chunk's `done` as the story's closure.** ⇒ ⛔ **Do not read this section as
a whole-story clearance.** ⭐ The chunk boundary is itself load-bearing — see the first dismissal
below, where a HIGH finding collapsed purely because the test that refutes it lives in G2.
