...
export interface DirectoryFetchOptions {
  ...
  limit: number;
  /**
   * The visitor's address (`Astro.clientAddress`) plus any inbound `X-Forwarded-For` chain.
 ...
 *
 * ⚠ APPENDS to the inbound chain rather than replacing it — that is the standard's own semantics,
 * and dropping upstream hops would discard exactly the information a future operator needs. When
 * there is no inbound chain, the visitor's address IS the chain.
 *
 * PURE — exported so it is testable without a server.
 */
export function buildForwardedFor(
  inboundChain: string | null | undefined,
  ...
  if (client.length > 0 && parts[parts.length - 1] !== client) parts.push(client);
  return parts.join(', ');
}
