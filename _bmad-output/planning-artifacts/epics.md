...
>
> **(6) ⚠ THE UX-SPEC COLUMN INVENTORY THIS STORY WOULD RENDER IS ⛔ NOT BUILDABLE, and the amendment is written at this story** (`-168` cl.3 = **D6-uxspec(a)**; `deferred-work.md:104-121` item **(f)**, whose trigger *"Story 11b.2 or 11b.3 authoring, whichever comes first"* **has fired**). Of the ten columns at `ux-design-specification.md:1158`: **three have no substrate** (`Donation ID`, `HRMS`, `Member ID` — the last **refused in terms** on a public wire as an enumeration primitive), **two labels are microcopy-PROHIBITED** (`microcopy.yaml:42` *"Donor Name"*, `:48` *"Late Teacher"*, both `member_only: true`), and **`School` / `Block` are separately ineligible or gated** (`2026-08-19-133` cl.1, `-132` cl.3, `2026-08-19-137` cl.7). ⛔ The amendment **annotates** the spec at five anchors; it ⛔ never rewrites it.
>
