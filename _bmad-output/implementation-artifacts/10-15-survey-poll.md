...
> **2026-08-17: code review complete (4/4 chunked passes) AND live-DB verification complete.**
> All 34 patch findings from the four review passes are fixed and verified — one of them (`publish()`'s
> concurrency guard) required a same-day CORRECTION after live-DB verification caught it failing every
> publish call; see below. No Docker daemon was available in the review environment, so a
> local equivalent of `twt-test-pg` was built by hand — a fresh Postgres role/database, migrated via
> the project's own `pnpm db:migrate` against the real, checked-in migration history (not a
> hand-crafted schema) — and every live-DB suite the runbook names was run and confirmed green:
> `packages/domain` (2985/2986, 1 pre-existing unrelated skip), `apps/api` (1018/1019, 1 pre-existing
> unrelated skip, includes the 17-test `surveys.spec.ts`), `apps/jobs`, `@twt/events`, `@twt/queue`,
> `@twt/niyamavali-engine`, `@twt/validity-service`, `@twt/channels` — every package the runbook's
> `integration-tests` CI job names. The newly-wired `survey-advisory-invariant` CI job was exercised
> via a full `pnpm ci:local` run and passed. Two unrelated live-DB test failures (a Story 10.23
> UTC-session-timezone gate; a Story-AI-4-1 p95 latency benchmark) and two unrelated build failures
> (`@twt/ui`, `@twt/channels`) surfaced only under full concurrent load and were confirmed as pure
> environment/resource-contention artifacts, not regressions, by re-running each in isolation — none
> touch survey/poll code.
>
> **⭐ Live-DB verification found and fixed one real regression from this review's own Pass-1 fix:**
> `publish()`'s optimistic-concurrency guard (`eq(surveys.updatedAt, survey.updatedAt)`, added to close
> a TOCTOU race) failed on EVERY publish call — `updated_at` is stored at microsecond precision but a
> JS `Date` only holds milliseconds, so the echoed comparison value could never match the stored one.
> Fixed via SQL-side `date_trunc('milliseconds', …)` truncation on both sides. This is exactly the
> class of bug DB-free unit tests structurally cannot catch, and exactly why this story was not moved
> to `done` until live-DB verification actually ran — see `packages/domain/src/surveys/write.ts`.
...
**Scope note:** this pass covered the Domain + Contracts + API layers only (diff `5b469b3..37b9661`, restricted to `packages/domain`, `packages/contracts`, `apps/api`, `packages/queue`). Jobs, Admin, Mobile, the invariant script, and generated/i18n files are a follow-up pass.

- [x] [Review][Patch] `survey.responses_viewed` audit line never carries a COUNT despite three docstrings promising it, and the shared audit primitive can't recoverably carry one (`requestPayloadHash` is a one-way SHA-256 digest) — **Fixed 2026-08-17: corrected all four docstring copies to drop the count claim.** — `apps/api/src/modules/surveys/handlers.ts:45-46,136,315,350-352`; `apps/api/src/modules/surveys/routes.ts:153`; `packages/domain/src/surveys/read.ts:352`
- [x] [Review][Patch] Member survey list (`MemberSurveyListResponse = { items: [] }`) has no `offset`/`next_offset`, unlike the admin list, while nothing caps how many surveys can be concurrently open and in-audience for one member — surveys beyond the internal page limit are silently and permanently unreachable with no signal more exist. **Fixed 2026-08-17: added `offset`/`next_offset` mirroring the admin list; `listOpenSurveysForMember` now returns `{candidates, hasMore, consumed}`; also fixed a latent missing tie-break in `listOpenSurveysForPariwar`'s `ORDER BY` (needed once offset pagination sits on top of it) and added a regression test.** — `packages/contracts/src/surveys/dto.ts:275`; `packages/domain/src/surveys/read.ts` (`listOpenSurveysForPariwar`, `listOpenSurveysForMember`); `apps/api/src/modules/surveys/member-handlers.ts`, `member-routes.ts`
- [x] [Review][Patch] AC6 violation: submitting a response to a `draft` survey returns 409, not the 404 AC6 requires — **Fixed 2026-08-17: `recordResponse` now throws `SurveyNotFoundError` for a draft survey before reaching the open/closed check; split the integration test into a draft→404 case and a closed→409 case.** — `packages/domain/src/surveys/write.ts` (`recordResponse`); `packages/domain/tests/integration/surveys/surveys.spec.ts`
- [x] [Review][Patch] `FROZEN_AFTER_PUBLISH` under-reports the fields `updateSurvey` actually freezes post-publish (missing `title`, `body`, `title_hi`, `body_hi`, `valid_from`) — **Fixed 2026-08-17: constant completed to all nine fields, in the guard's own order.** — `packages/domain/src/surveys/write.ts`
- [x] [Review][Patch] A PATCH supplying `audience_scope_value` while omitting `audience_scope` bypasses validation at both the wire refinement and the domain guard, letting a meaningless value persist alongside `members-all`/`public` — **Fixed 2026-08-17: `assertAudienceAuthorable` (called with the MERGED/resulting state in every caller) now rejects a `members-all` scope carrying a non-null `audience_scope_value`, closing the gap regardless of which field the PATCH omitted.** — `packages/domain/src/surveys/write.ts`
- [x] [Review][Patch] `publish()`'s UPDATE guards only on `status='draft'`, not `updated_at` — a concurrent `updateSurvey` PATCH between publish's read and its UPDATE can leave the tone-review `contentHash` attesting to stale content — **Fixed 2026-08-17: added `eq(surveys.updatedAt, survey.updatedAt)` to the UPDATE's WHERE clause; a race now falls into the existing "changed state before publish" 409 instead of publishing silently.** ⚠ **CORRECTED 2026-08-17, same day, by live-DB verification: the fix as first written FAILED EVERY publish call** — `updated_at` is stored at microsecond precision, a JS `Date` only holds milliseconds, so the echoed comparison value could never match. Re-fixed via SQL-side `date_trunc('milliseconds', …)` on both sides of the comparison; see the Live-DB Verification section below. — `packages/domain/src/surveys/write.ts`
- [x] [Review][Patch] `hasMemberResponded` is documented as "the member route's idempotency/replay precondition" but has zero call sites outside its own declaration/re-export — dead code or a dropped precheck — **Fixed 2026-08-17: removed (confirmed 100% dead — `listOpenSurveysForMember` already computes `answered` via its own batched query, and the composite PK is the actual invariant per LBD-6).** — `packages/domain/src/surveys/read.ts`, `index.ts`
- [x] [Review][Patch] Member submit handler releases the idempotency claim in `finally` even when `recordResponse` already committed and only `idempotencyStore.recordResult()` failed afterward — a client's honest retry then hits the domain's composite-PK 409 instead of the original 201 — **Fixed 2026-08-17: tracks `responseRecorded` separately from `claimSettled`; the claim is only released when the domain write itself never succeeded.** — `apps/api/src/modules/surveys/member-handlers.ts`
- [x] [Review][Patch] `listFreeTextAnswers` applies LIMIT/OFFSET across all responses, then filters to this question's answers in JS afterward — corrupts `hasMore`/`next_offset` and can skip real answers across pages — **Fixed 2026-08-17: the "answered this question" condition moved into the SQL WHERE clause (reusing the same extraction expression), before LIMIT/OFFSET.** — `packages/domain/src/surveys/read.ts`
- [x] [Review][Patch] A corrupted/schema-incompatible stored idempotency payload fails `safeParse` silently and is reported as "still in progress" (409) rather than a distinguishable failure — **Fixed 2026-08-17: logged distinctly (`request.log.error`) before falling through to the same 409, so a real storage/shape problem is visible in logs instead of reading as ordinary contention.** — `apps/api/src/modules/surveys/member-handlers.ts`
- [x] [Review][Defer] `PublishSurveyRequest`/`CloseSurveyRequest` are `z.object({}).strict()` (no `.optional()`) while the generated OpenAPI doc marks the request body `required: false`; untested whether an entirely-absent body validates — deferred, needs a runtime check rather than a blind fix — [`packages/contracts/src/surveys/dto.ts:187-192`]

**Verification:** `tsc --noEmit` and `eslint` clean on `@twt/domain`, `@twt/contracts`, `@twt/api`; all DB-free unit tests pass (domain 1880/1880, contracts 941/941, api 331/331). ⚠ Live-DB integration suites (`packages/domain/tests/integration/surveys/surveys.spec.ts`, `apps/api/tests/integration/surveys/surveys.spec.ts`) could **not** be run in this environment — no local `twt-test-pg`/Docker daemon available — and were only reasoned through by inspection. **Un-attested — run `pnpm ci:local` with `DATABASE_URL` set (or CI) before merging, per [[feedback_record_unattested_no_backfill]].**

**Out of scope for this pass (noted for follow-up):** `openapi/v1.yaml` was not regenerated to reflect the `MemberSurveyListResponse`/`MemberListQuery` and audit-docstring changes above — run `emit-openapi.ts` before merging. `apps/mobile`'s consumption of `MemberSurveyListResponse` (an additive `next_offset` field) was not reviewed or touched — Jobs/Admin/Mobile/invariant-script/generated files remain a separate chunked pass per the scope note above.

### Review Findings — Pass 2 (Jobs + Admin)

**Scope note:** covered `apps/jobs/src/scheduler/survey-publish.ts` (+ `boot.ts` registration) and `apps/admin/src/modules/surveys/*` + `apps/admin/src/api/{client,hooks}.ts` (diff `5b469b3..37b9661`). Mobile/invariant-script/generated files remain a follow-up pass. Patches applied directly (continuing Pass 1's "apply every patch" preference; not re-confirmed per-chunk this time — flagged to the user).

- [x] [Review][Patch] A 5th, uncorrected copy of the false "`survey.responses_viewed` carries a COUNT" claim, in a file Pass 1 didn't cover — **Fixed 2026-08-17: corrected `listSurveyFreeText`'s docstring.** — `apps/admin/src/api/client.ts`
- [x] [Review][Patch] Admin editor could never clear `audience_scope_value` when switching a draft's audience away from `state` — the PATCH omitted the key rather than sending `null`, so the OLD value persisted into the merged state Pass 1's `assertAudienceAuthorable` fix now correctly rejects, making the edit unreachable — **Fixed 2026-08-17: `save()` now always sends `audience_scope_value` explicitly (`null` when unused); `SurveyEditor`'s audience `<select>` also clears local state on change.** — `apps/admin/src/modules/surveys/SurveysPage.tsx`, `SurveyEditor.tsx`
- [x] [Review][Patch] `selected` survey was derived PURELY from the current page's `items` — changing the state filter or offset while the editor stayed open (both remain interactive during an edit) could drop the open survey off the page, silently flipping `save()` from UPDATE to CREATE with no warning (a real edit became a silent duplicate) — **Fixed 2026-08-17: added an `openedSurvey` cache, refreshed on open/save-success, used as `selected`'s fallback.** — `apps/admin/src/modules/surveys/SurveysPage.tsx`
- [x] [Review][Patch] `ErrorBanner` never reset `publish`/`closeSurvey` mutation errors (only `create`/`update` were `.reset()`), and `actionError`'s fixed `??` precedence could hide a newer failure behind an older stale one — **Fixed 2026-08-17: replaced the derived `actionError` with an explicit `useState`, set via each mutation's `onError` and cleared on New/Cancel/Open.** — `apps/admin/src/modules/surveys/SurveysPage.tsx`
- [x] [Review][Patch] `save()` silently no-op'd on an empty/invalid window, and a non-numeric `responseThreshold` silently became `null` via `NaN` — both with zero admin feedback — **Fixed 2026-08-17: both now set a visible validation error instead of failing silently.** — `apps/admin/src/modules/surveys/SurveysPage.tsx`, `i18n-en.ts`
- [x] [Review][Patch] The per-option "remove" button had no floor — an admin could remove down to 0/1 options, deferred to a server-side rejection at Save with no earlier signal — **Fixed 2026-08-17: disabled at `MIN_OPTIONS_PER_CHOICE_QUESTION`.** — `apps/admin/src/modules/surveys/SurveyEditor.tsx`
- [x] [Review][Patch] Switching a question's type away from a choice type and back destroyed previously authored option text (regenerated two blanks) — **Fixed 2026-08-17: options preserved per `question_id` in a ref across the round trip.** — `apps/admin/src/modules/surveys/SurveyEditor.tsx`
- [x] [Review][Patch] No `isPending` guard on Save/Publish/Close — a double-click could fire duplicate requests; Save also stayed enabled over an effectively no-op patch on a `closed` (terminal) survey, unlike the editor's own stated disable-not-409 design — **Fixed 2026-08-17: all three buttons now disable while their mutation is in flight; Save also disables when `editableFields(status) === 'none'`.** — `apps/admin/src/modules/surveys/SurveysPage.tsx`
- [x] [Review][Patch] `listSurveys`'s `displayState` parameter was typed as plain `string`, not `SurveyDisplayState` — a typo'd filter value would compile and silently mis-filter — **Fixed 2026-08-17: retyped through `client.ts` → `hooks.ts` → `SurveysPage.tsx`'s `stateFilter`.** — `apps/admin/src/api/client.ts`, `hooks.ts`, `apps/admin/src/modules/surveys/SurveysPage.tsx`
- [x] [Review][Patch] `SurveyResults` fetched `/aggregate` for ANY selected survey, including an untouched `draft` (never published, no responses possible) — wasted request / spurious error state — **Fixed 2026-08-17: the aggregate query is now gated off for `draft`.** — `apps/admin/src/modules/surveys/SurveysPage.tsx`
- [x] [Review][Patch] The page size `50` was duplicated as an independent magic number in both `client.ts`'s default `limit` and `SurveysPage`'s Previous-button offset math — **Fixed 2026-08-17: extracted `SURVEY_LIST_PAGE_SIZE`, used by both.** — `apps/admin/src/api/client.ts`, `apps/admin/src/modules/surveys/SurveysPage.tsx`
- [x] [Review][Defer] No `try`/`catch` around `runSurveyPublish` in the pg-boss job loop, a claim-loop throw mid-iteration can leak earlier per-member claims for the full TTL, and the fan-out is a fully sequential/unbatched loop with no child-job chunking for a stated "4L-member" scale — **verified IDENTICAL to the pre-existing `news-publish.ts` pattern this file explicitly cites as its precedent** (same unguarded job loop, same unguarded claim loop, same inline sequential fan-out with no chunking despite citing `runContributionNotifyChild` by name). Real, but a cross-cutting job-harness gap shared by ≥2 workers, not something to patch in one file in isolation — logged for a dedicated fix touching every affected worker. — `apps/jobs/src/scheduler/survey-publish.ts`, `news-publish.ts`
- [x] [Review][Defer] `SurveyResults` has no "load more" pagination UI for free-text answers, despite the client/API already supporting `limit`/`offset` — real completeness gap, but sizable new UI surface (pagination controls + state) rather than a bug-fix-sized patch. — `apps/admin/src/modules/surveys/SurveyResults.tsx`

**Dismissed as noise (6), with reasons — verified false or genuinely non-issues:**
- "Hand-rolled UUIDv5 instead of a vetted library" — verified: an IDENTICAL helper (same bit-twiddling, same structure) already exists in `news-publish.ts` under its own pinned namespace UUID — an established, deliberate per-worker convention, not an oversight.
- "`isEditable()`/`UNSAVED_DRAFT_ID` exported but unused in production" (2 findings) — harmless leftover exports, zero functional risk, not worth a change during this pass.
- "`now`/`clock` prop is dead" — verified true (display state is server-derived, nothing in this component does client-side date math) but harmless; doc comment corrected in place rather than removing a public prop 14+ tests already inject.
- "Missing '(not auto-retried)' qualifier on the release-for-re-run comment" — trivial wording nit matching a pattern already present (if incompletely worded) in the cited `news-publish.ts` precedent; not worth a standalone change.
- Save button not disabled for a `closed` survey — folded into the isPending/editableFields fix above rather than tracked separately.

**Verification:** `tsc --noEmit` + `eslint` clean on `@twt/admin` and `@twt/jobs`; DB-free unit tests all green (admin 355/355 incl. `surveys-page.test.tsx` + `surveys-derive.test.ts`; jobs 247/247 incl. `survey-publish.test.ts`). No live-DB suites in this chunk's scope.

### Review Findings — Pass 3 (Mobile)

**Scope note:** covered `apps/mobile/app/(polls)/*`, `apps/mobile/components/polls/*`, `apps/mobile/lib/poll-api.ts`, `apps/mobile/lib/poll-i18n.ts`, `apps/mobile/components/panchayat/PanchayatNoticeboard.tsx`, `apps/mobile/tests/unit/polls-copy.test.ts`. Also touched, as necessary support for fixes below (not a full review of either): `packages/api-client/src/index.ts` (the member-survey SDK `list()` mobile consumes — not assigned to any earlier chunk) and `packages/i18n/locales/{en,hi}/polls.json` (two new copy keys). The invariant-script/generated-files group remains a final follow-up pass. Patches applied directly, continuing the established "apply every patch" pattern without a fresh per-chunk confirmation.

- [x] [Review][Patch] **Mobile reopened the exact pagination gap Pass 1 closed server-side** — `createMemberSurveyClient().list()` took no `limit`/`offset`, and `usePollsQuery` was a single `useQuery`, so a member with more open, in-audience polls than one server page could never reach the rest — worse, the answer screen (`[surveyId].tsx`) searches only what's loaded, so a poll reached via a deep link/notification past page 1 would report "not found" even though it exists. **Fixed 2026-08-17: `list()` now takes `{limit, offset}`; `usePollsQuery` uses `useInfiniteQuery` + the server's `next_offset`; `index.tsx` wires `onEndReached`; `[surveyId].tsx` keeps fetching further pages while the target survey isn't found yet.** — `packages/api-client/src/index.ts`; `apps/mobile/components/polls/usePollQueries.ts`, `PollsEntry.tsx`; `apps/mobile/app/(polls)/index.tsx`, `[surveyId].tsx`
- [x] [Review][Patch] The submission success notice was invisible — `setNotice(t('submitted')); router.back()` fired back-to-back, unmounting the screen before the member could ever see the one confirmation LBD-6 most wants seen (a final, un-undoable answer) — **Fixed 2026-08-17: a `submitted` flag now gates a static confirmation view (mirroring the existing `alreadyAnswered` branch) with its own explicit Back button, instead of auto-navigating away.** — `apps/mobile/app/(polls)/[surveyId].tsx`
- [x] [Review][Patch] `usePollsQuery`'s `enabled: !!pariwarId` meant `isLoading` was FALSE while the session was still resolving `pariwarId` — both screens briefly showed a false "couldn't load"/"nothing to answer" instead of a loading state during ordinary session hydration — **Fixed 2026-08-17: both screens now branch on `pending = isLoading || !pariwarId`.** — `apps/mobile/app/(polls)/index.tsx`, `[surveyId].tsx`
- [x] [Review][Patch] The list screen's error-state button called `refetch()` but displayed the `back` copy key — a "Back" label on a button that actually retries — **Fixed 2026-08-17: new `retry` copy key (en+hi).** — `apps/mobile/app/(polls)/index.tsx`, `packages/i18n/locales/{en,hi}/polls.json`
- [x] [Review][Patch] A pull-to-refresh failure while the list already had items was completely silent — `RefreshControl` just stopped spinning with no error indication — **Fixed 2026-08-17: a small inline error line now renders above the list on `isError`; new `refresh_error` copy key (en+hi).** — `apps/mobile/app/(polls)/index.tsx`, `packages/i18n/locales/{en,hi}/polls.json`
- [x] [Review][Patch] A member could tap Back while `onSubmit`'s async flow (`getTurnstileToken` + `submit.mutateAsync`) was still in flight (only Submit was gated on `isPending`, Back never was), unmounting the screen before the promise settled — its continuation then set state against an unmounted component — **Fixed 2026-08-17: a `mountedRef` guards every post-await `setNotice`/`setSubmitted` call.** — `apps/mobile/app/(polls)/[surveyId].tsx`
- [x] [Review][Patch] `PollsEntry`'s badge doc comment claimed the count "includes ones the member has already answered" unconditionally — the code actually shows the unanswered count first, falling back to the total only once nothing remains unanswered — **Fixed 2026-08-17: comment corrected to match shipped behavior** (no behavior change — the existing formula was judged the more useful one). — `apps/mobile/components/polls/PollsEntry.tsx`
- [ ] [Review][Defer] `components/polls/copy.ts` reimplements the identical Hindi-first fallback pattern three times (`selectSurveyCopy`/`selectQuestionText`/`selectOptionText`), in a file whose own stated premise is "every non-trivial decision lives here, tested once" — real DRY gap, not applied this pass (budget), logged for a follow-up.
- [ ] [Review][Defer] `PollsEntry` ignores `usePollsQuery`'s `isError`/`isLoading` — a PERSISTENT (not transient — TanStack Query's default retry already self-heals a transient one) fetch failure hides the ONLY entry point into the polls surface (no bottom tab; declared deliberately per the file's own header) indefinitely, indistinguishable from "nothing to answer." Deferred rather than patched: the right fix depends on a product call this quiet-noticeboard widget's design didn't make — show a subtle error state (against its stated "renders nothing" premise) or accept the silent gap.

**Dismissed as noise (7), with reasons — verified false or genuinely non-issues:**
- "No locale catalog for the ~20 new `t('...')` keys — every screen would crash on first render" — verified FALSE: `packages/i18n/locales/{en,hi}/polls.json` exists, is correctly wired into the `polls` namespace in `catalog.ts`, and covers every key the diff's blind reviewer (correctly, given it couldn't see this file) couldn't confirm.
- "`_layout.tsx`'s 'DECLARED SUBSTITUTION (Escalation 3)' comment self-certifies an architecture deviation with no corroborating artifact" — verified FALSE: Escalation 3 is genuinely documented in the story spec's own `⚠ Escalations` section, predating this code, describing exactly this substitution.
- "The closing date uses `toLocaleDateString()` with no locale arg, following the device OS locale instead of the app's `locale`" — verified TRUE but matches the PREVAILING convention across most other member-app screens (`helpdesk`, `membership` do the same; only `ActiveContributionCard`/`RenewalStatusWidget` bother with a locale-aware variant, and even those use a different tag — `'hi-IN-u-nu-latn'` — than what a polls-specific fix would invent). A pre-existing, codebase-wide inconsistency, not something 10.15 introduced; fixing polls alone would add a THIRD convention rather than align with an established one.
- "Submit button stays enabled regardless of `canSubmit`, only discovered via post-tap missing-question highlights" — deliberate, per the code's own comment: surfacing exactly which question is missing (an affordance) rather than a disabled button with no explanation.
- "`onSubmit` never guards against `pariwarId` becoming undefined mid-flight" — narrow session-loss-mid-answer race, effectively subsumed by the `pending` gate fix above (the screen no longer renders the form at all once `pariwarId` is absent going forward).
- "`questions.length === 0` leaves Submit permanently disabled with no feedback" — not chased: a published survey with zero questions is not evidenced as reachable (`validateQuestionnaire`'s domain-level minimum), and this diff doesn't need to re-verify a Pass-1-owned invariant.
- "`newIdempotencyKey`'s non-crypto fallback branch is untested" — test-coverage nit (the fallback for environments without `crypto.randomUUID`), not a defect in the code itself.

**Verification:** `tsc --noEmit` + `eslint` clean on `@twt/mobile`, `@twt/api-client`, `@twt/i18n`; DB-free unit tests all green (mobile 284/284 incl. `polls-copy.test.ts`; api-client 1/1 smoke; i18n 57/57 incl. the en/hi key-`parity.test.ts` sync-guard, which caught nothing wrong with the two new keys). No live-DB or RN-render suites exist for this surface (the mobile harness is pure Vitest with no RN mount renderer, per this module's own stated constraint) — screen-level behavior (loading/error/pagination/submission flows) was verified by inspection and typecheck only, not exercised by a test.

### Review Findings — Pass 4 (invariant script + generated/misc — FINAL chunk)

**Scope note:** covered `scripts/survey-advisory-invariant/*` (the LBD-1 mechanized gate), `friction-budget.md`, root `package.json`, `packages/i18n/src/catalog.ts`, plus `.github/workflows/ci.yml`/`scripts/ci-local.sh` (wiring fix) and `packages/contracts/scripts/emit-openapi.ts`/`openapi/v1.yaml` (regeneration + a drift fix surfaced by that regeneration) as necessary support for findings below. This is the last of the four chunked passes — every file this story touches has now been reviewed. Patches applied directly, continuing the established pattern without a fresh per-chunk confirmation.

- [x] [Review][Patch] **CRITICAL: the LBD-1 gate existed only as `package.json` scripts and was never wired into CI or `ci:local`.** Every sibling invariant gate (`helpdesk-state-invariant`, `governance-boundary`, `custom-field-governance`, …) has an explicit `run "<name>" "pnpm x:test && pnpm x:check"` line in `ci-local.sh` and a matching job in `ci.yml`; `survey-advisory` had neither. A reintroduced `quorum`, or a deleted/renamed `SCAN_PATHS` entry, got a green `pnpm ci:local` and a green CI run regardless — exactly the "by eye" enforcement Task 11 explicitly rejects, contradicting the story's own "MECHANIZED, NOT MERELY ASSERTED" claim in its Completion Notes / sprint-status ledger. **Fixed 2026-08-17: added the matching `run` line to `ci-local.sh` and a full job to `ci.yml`, mirroring the sibling gates exactly; verified `pnpm survey-advisory:test`/`:check` both actually pass.** — `scripts/ci-local.sh`, `.github/workflows/ci.yml`
- [x] [Review][Patch] **CRITICAL: `SCAN_PATHS` was incomplete** — asserted as "every path Story 10.15 adds" but 10 real files this story modifies (not adds, but touches with survey-owned identifiers/comments/routes) were absent: `packages/api-client/src/index.ts`, `packages/queue/src/index.ts`, `packages/domain/src/rbac/{permissions,roles}.ts` + its test, `apps/mobile/components/panchayat/PanchayatNoticeboard.tsx`, `apps/api/src/{context,server}.ts`, `apps/jobs/src/boot.ts`, `apps/admin/src/router.tsx`. Per this exact codebase's own [[feedback_gate_scope_semantic_coverage]] discipline (which this gate's own header comment cites), a green run over an unlisted surface proves nothing about it. **Fixed 2026-08-17: added all 10, each verified `grep`-clean of "quorum" first; gate re-run confirms `50 files across 24 declared paths`, still green.** `openapi/v1.yaml` deliberately left uncovered — it's a mechanical derivative of the already-scanned `packages/contracts/src/surveys` source (Zod descriptions copy verbatim into it) and extending the scanner to a new file format is out of proportion to a patch. — `scripts/survey-advisory-invariant/check.ts`
- [x] [Review][Patch] **A 6th and 7th uncorrected copy of the "carries a COUNT" audit-docstring claim** — surfaced only because this pass regenerated the stale `openapi/v1.yaml` (flagged as an outstanding action item since Pass 1): `packages/contracts/scripts/emit-openapi.ts`'s hand-written route descriptions for the admin free-text read and the member survey list still made the false claim. **Fixed 2026-08-17.** — `packages/contracts/scripts/emit-openapi.ts`
- [x] [Review][Patch] **`openapi/v1.yaml` was stale** (flagged since Pass 1/2, never actioned) — regenerated via `pnpm contracts:emit-openapi`; determinism re-verified via `contracts:check-openapi-determinism`. This surfaced a real, separate gap: the emitter hand-duplicates every route's query-parameter shape rather than importing the route's actual Zod schema, so Pass 1's `offset` addition to the member survey list route (`member-routes.ts`'s `MemberListQuery`) had silently NOT propagated to the OpenAPI doc — an external API consumer reading the spec would not know pagination was possible. **Fixed 2026-08-17: added the missing `offset` param to `emit-openapi.ts`'s `memberSurveyListQuery`; the doc-vs-route drift risk itself (two hand-synchronized authorities) is a pre-existing pattern across the whole file, not introduced by this story — not restructured here.** — `packages/contracts/scripts/emit-openapi.ts`, `openapi/v1.yaml`
- [x] [Review][Patch] `BANNED_COPY_PHRASES`'s doc-comment claimed the phrases were scoped to "the i18n/label files) specifically" and "Deliberately NOT applied to all code" — the scanner actually applies both word lists identically to every `SCAN_PATHS` file regardless of type (domain/SQL included); the real scoping mechanism is `SCAN_PATHS` itself, not a file-type branch inside the scanner. **Fixed 2026-08-17: comment corrected; no behavior change** (the implementation was already correct — over-scoped in the safe direction, not under). — `scripts/survey-advisory-invariant/lib.ts`
- [x] [Review][Patch] The `'reached quorum'` entry in `BANNED_COPY_PHRASES` was redundant (contains `quorum`, so `BANNED_WORDS` already catches every line it would) and untested; a test's own comment claimed "one finding per banned word" (implying 2) while asserting only `toBeGreaterThanOrEqual(1)`, masking that the scanner records one finding per (line, word) pair regardless of occurrence count. **Fixed 2026-08-17: removed the redundant phrase; tightened the assertion to `toHaveLength(1)` with a corrected comment.** — `scripts/survey-advisory-invariant/lib.ts`, `lib.test.ts`
- [ ] [Review][Defer] The gate's own self-described "real weakness" (`SCAN_PATHS` completeness — the missing-path-fails-the-build branch, and the recursive `collectFiles` traversal) has zero test coverage; `lib.test.ts` only exercises the pure scanning functions. Not applied this pass: `check.ts`'s `main()` calls `process.exit()` at module-load time and isn't currently structured as an importable/testable function — closing this gap needs a small refactor (extract a pure, injectable-fs function `main()` can wrap), not just a test file, which crosses out of a bug-fix-sized patch. Logged for a follow-up.

**Dismissed as noise (9), with reasons — verified false, pre-existing, or accepted trade-offs:**
- `stripComments`' same-line `//`/`--`-inside-a-string-literal false negative — this is an ALREADY deliberately disclosed, reasoned trade-off (the code's own comment explicitly argues the alternative "is a parser this gate does not need"), not an oversight. Left as a known, accepted residual risk rather than overridden.
- "Survey/poll naming inconsistency" (backend says `survey`, mobile says `poll`) — deliberate, established distinction, already verified compliant by this same review's Pass-3 Acceptance Auditor.
- friction-budget.md's claim that the migration withholds `UPDATE` on `survey_responses` from `twt_app` — verified TRUE, live, against `packages/domain/migrations/0109_survey-poll.sql`.
- `KNOWN_NAMESPACES`/`catalogs` dual-list sync gap in `packages/i18n/src/catalog.ts` — a pre-existing pattern across every namespace already in that file (not introduced by adding `polls`), not this story's problem to fix.
- No word-boundary substring matching in the scanner — low practical collision risk for a word like "quorum," and boundary-aware matching would weaken camelCase-identifier detection (e.g. `quorumThreshold`) rather than help it.
- Crash-safety on ELOOP/EACCES/unreadable files, duplicate-scan on overlapping `SCAN_PATHS` entries, silent-pass on a path contributing zero scannable files — none currently reachable (no symlinks, no overlapping declared paths, every declared path resolves to `.ts`/`.tsx`/`.sql`/`.json` files); an uncaught exception in this CI-invoked script already produces a non-zero exit and a FAILING run, satisfying "fail loud, never silently pass" without further hardening.

**Verification:** `pnpm survey-advisory:test` (13/13) and `pnpm survey-advisory:check` (50 files, 24 paths, 0 violations) both run and pass directly. `tsc --noEmit` clean across all 19 packages in the dependency graph touched by this whole review (`turbo run typecheck` over domain/contracts/api/admin/jobs/mobile/api-client/i18n/queue and their transitive deps). `contracts:check-openapi-determinism` passes. `@twt/contracts` full suite re-run: 941/941. ⚠ The CI-wiring fix itself (the `ci.yml` job, the `ci-local.sh` line) could not be exercised by actually running GitHub Actions or a full `pnpm ci:local` in this environment — verified by direct structural comparison against sibling gates and by running the underlying `pnpm survey-advisory:*` commands standalone, not by a live CI run. **Un-attested in that specific sense — confirm on the next real CI run before treating this as fully closed, per [[feedback_record_unattested_no_backfill]].**

---

## Chunked review complete (4/4 passes)

All eight file groups originally scoped for this story's diff (`5b469b3..37b9661`) have now been reviewed: Domain+Contracts+API (Pass 1), Jobs+Admin (Pass 2), Mobile (Pass 3), invariant-script+generated/misc (Pass 4). Total across all passes: **2 decision-needed** (both resolved with the user, folded into the patch count below), **34 patch findings** (all fixed and verified), **6 deferred** (logged in `deferred-work.md`, none blocking), and **28 dismissed** as noise (each with a stated, verifiable reason — several were blind-review artifacts caught only because this workflow verifies every finding against source before triaging it, per [[feedback_verify_before_committing_governance_claims]]).

---

## Live-DB Verification (2026-08-17, same day)

No Docker daemon was available in the review environment, so `twt-test-pg` was reconstructed by hand rather than skipped: a fresh Postgres role (`twt_dev_app`, superuser, matching the official `postgres` image's `POSTGRES_USER` behavior per `integration-setup.ts`'s own documented bootstrap) + database (`twt_dev`), migrated via the project's own `pnpm db:migrate` against the real, checked-in migration history — not a hand-crafted schema. This is the same schema-defining source `twt-test-pg` itself is built from; only the container/port differ, and `hasDatabase`/test behavior is verified port-agnostic (`Boolean(DATABASE_URL)`).

**Every live-DB suite the test runbook's `integration-tests` CI job names was run and is green**, each confirmed in isolation where a full concurrent run first showed noise:
- `packages/domain` — 2985/2986 (1 pre-existing, unrelated skip)
- `apps/api` — 1018/1019 (1 pre-existing, unrelated skip; includes the 17-test `tests/integration/surveys/surveys.spec.ts`)
- `apps/jobs` — 346/346 (after correcting a local-environment gap, not a code issue — see below)
- `@twt/events`, `@twt/queue`, `@twt/niyamavali-engine` — green in the full concurrent run, no failures reported
- `@twt/validity-service` — 284/284 in isolation
- `@twt/channels` — 204/204 in isolation
- A full `pnpm ci:local` run (all 27 jobs) exercised the newly-wired `survey-advisory-invariant` CI job end-to-end — **passed**.

**One real regression found and fixed** (see the corrected Pass-1 `publish()` bullet above): the `updated_at` optimistic-concurrency guard added in Pass 1 compared a microsecond-precision DB value against its own millisecond-truncated JS-`Date` echo, so it failed on every publish call. This is exactly the class of bug DB-free unit tests structurally cannot see — confirming why this story stayed `in-progress` rather than `done` until this step actually ran, per [[feedback_record_unattested_no_backfill]]. Fixed via SQL-side `date_trunc('milliseconds', …)` truncation; domain + api survey suites re-run and green.

**Four failures surfaced only under full concurrent load and were confirmed as environment/resource-contention artifacts, not code regressions**, by re-running each in isolation:
- `tests/restoration-discipline-production-path-live.test.ts` (Story 10.23) — the LOCAL Postgres instance defaulted to `Asia/Kolkata`, not UTC; this test's own AC4 gate explicitly requires and checks for a UTC session timezone (with a documented remediation in its own error message). Fixed by appending `&options=-c%20timezone%3DUTC` to `DATABASE_URL`, exactly as instructed. Unrelated to surveys.
- `tests/integration/measured-validation-fr12a.spec.ts` (Story AI-4-1, `@twt/validity-service`) — a p95-latency-under-concurrent-load benchmark measured 2456ms against a 2000ms budget while 8 packages' test suites ran simultaneously on one non-dedicated machine; re-run alone, p95 was 108.5ms. A wall-clock benchmark, not a correctness test. Unrelated to surveys.
- `@twt/ui#build` and `@twt/channels#build` — both failed with no emitted error text under the same full concurrent load (consistent with resource exhaustion, not a compile error); both build clean in isolation. Neither package was touched by this review.
