...
> ⛔ **AMENDED 2026-08-30 (Story 11b.2, D6-uxspec(a) · `.decision-log.md#decision-2026-08-30-168` cl.3) — ⭐ THE `Public column inventory` IN THE PRIMITIVE ABOVE IS ⛔ NOT BUILDABLE AS WRITTEN. ⛔ ANNOTATION ONLY — the primitive above is ⛔ NOT rewritten** ([[feedback_supersede_never_reinterpret]]).
>
> ⭐ **The full finding, with its grounds, is written at §"Public Column Inventory — Sahyog List" (`:1287-1298`) — the section literally titled for it, and the first document the next author opens.** In brief: of the ten columns, **three have no substrate** (`Donation ID` · `HRMS` · `Member ID`), **two labels are microcopy-PROHIBITED** (`Donor Name`, `Late Teacher`), and **`School` / `Block` are separately ineligible or gated**. ⚠ The **identifier semantics** paragraph above (*Donation ID* / *Member ID* / *HRMS*, and the per-Pariwar HRMS label) describes **fields that do not exist**; it is annotated, ⛔ not deleted, because the semantics remain correct **if** the fields are ever built.
> ⚠ ⛔ **And the `aggregate stat-cards row above the table` has ⛔ NO PRODUCER and ⛔ NO OWNER** (`epics.md:4799`, C-3). ⛔ Do ⛔ not stub it.
>
> ⭐⭐ **AND THE BUILDABLE HALF IS NOW WRITTEN (2026-09-15, Story `11b-3b` AC6)** — at the same canonical section, §"Public Column Inventory — Sahyog List", under **"THE BUILDABLE INVENTORY"**. ⭐ **FIVE of the ten are buildable** (`Donor Name` · `District` · `Pool` · `Late Teacher` · `Date`), ⚠⛔ **but ⛔ NOT AS ONE TABLE:** the inventory's **ROW GRAIN** — one row per CONTRIBUTION — exists on ⛔ no shipped surface and may ⛔ not, so the ten-column table was **SPLIT BY GRAIN** across `/sahyog` (one row per DRIVE) and `/sahyog-vivran/[driveToken]` (one row per CONTRIBUTOR, carrying the name and ⛔ nothing else). ⚠ Two of the five keep **microcopy-fenced LABELS** and one (`Late Teacher`) is **INERT by design**. ⛔ Cite the canonical section by its TITLE, ⛔ not by line.
...
> ⛔ **AMENDED 2026-08-30 (Story 11b.2, D6-uxspec(a) · `-168` cl.3) — ⭐ THE REAL DATA TEST'S DISAMBIGUATION SCENARIOS DEPEND ON THREE COLUMNS THAT HAVE ⛔ NO SUBSTRATE, SO THE TEST AS WRITTEN CANNOT RUN.** ⛔ Annotation, ⛔ not a rewrite — ⚠ and the **gate is ⛔ not weakened**: the disambiguation question is real and still owed; what is amended is the **means**.
>
> The scenarios *"duplicate full names disambiguated only by **Member ID + HRMS** lookup"*, *"multiple Late Teacher entries with the same surname requiring **Member ID** lookup"* and *"**HRMS**-near-collisions"* all rest on `Member ID` and `HRMS`. ⛔ **Neither exists**: no HRMS field on any member table, and a public `Member ID` is **refused in terms** by Story 11a.3's handler as an enumeration primitive. ⛔ `Donation ID` likewise has no substrate.
> ⚠⛔ **AND `Member ID` IS ⛔ NOT A PENDING FEATURE THAT WILL ARRIVE** — a membership number is a **confirmed requirement that is deliberately absent from the substrate** ([[project_membership_number_deferred_feature]]); ⛔ do not "fix" this by inventing a `member_number` column.
> ⇒ ⭐ **The disambiguation question must be re-posed against the fields that DO exist** (first-name + last-initial, pool letter code, district, date) before the gate can be run — ⛔ and re-posing it is ⛔ **not** this amendment's to do. ⚠ It is recorded here so the next author does not schedule a test whose fixtures cannot be built.
>
> ⭐⭐ **AND THE BUILDABLE HALF IS NOW WRITTEN (2026-09-15, Story `11b-3b` AC6)** — at the same canonical section, §"Public Column Inventory — Sahyog List", under **"THE BUILDABLE INVENTORY"**. ⭐ **FIVE of the ten are buildable** (`Donor Name` · `District` · `Pool` · `Late Teacher` · `Date`), ⚠⛔ **but ⛔ NOT AS ONE TABLE:** the inventory's **ROW GRAIN** — one row per CONTRIBUTION — exists on ⛔ no shipped surface and may ⛔ not, so the ten-column table was **SPLIT BY GRAIN** across `/sahyog` (one row per DRIVE) and `/sahyog-vivran/[driveToken]` (one row per CONTRIBUTOR, carrying the name and ⛔ nothing else). ⚠ Two of the five keep **microcopy-fenced LABELS** and one (`Late Teacher`) is **INERT by design**. ⛔ Cite the canonical section by its TITLE, ⛔ not by line. ⚠⛔⛔ **AND THE RE-POSING ABOVE NEEDS ONE CORRECTION BEFORE IT IS USED:** it lists *"first-name + last-initial"* among *"the fields that DO exist"*. ⛔ **That is the MEMBER form.** [`-174`](../../.decision-log.md#decision-2026-09-02-174) ruled the **public** contributor name at the **FULL NAME**, unconditional per [`-175`](../../.decision-log.md#decision-2026-09-02-175) ⇒ ⭐ a public disambiguation fixture must be built on **full names**, which collide **differently** — ⛔ the shielded form was never the public one and a test fixtured on it would measure the wrong surface.
...
> · **`Donation ID`** — there is ⛔ **no `donation_id`** anywhere under `packages/`. The identifier the spec describes as *"the permanent sequential contribution reference"* does not exist in any schema, contract or event payload.
> · **`HRMS`** — there is ⛔ **no HRMS field on any member table**. The per-Pariwar label configurability described above (*"eHRMS" / "PF Number" / "Railway ID" / "Employee Code"*) is a semantics commitment over a field that was never built.
> · **`Member ID`** — ⛔ **refused in terms on a public wire.** Story 11a.3's handler treats a per-member public permalink as an **enumeration primitive** and declines it. ⚠⛔ **And this is ⛔ NOT a pending feature awaiting arrival** — a membership number is a **confirmed requirement deliberately absent from the substrate** ([[project_membership_number_deferred_feature]]). ⛔ **Nobody may "fix" this column by inventing a `member_number` column.**
>
> **TWO LABELS ARE MICROCOPY-PROHIBITED — and they now BITE:**
> · **`Donor Name`** — `microcopy.yaml:42`, `member_only: true`. ⚠ §7's *"never 'donor'"* stance is already recorded above for **`Member ID`**'s vocabulary; the **column label itself** was missed.
> · **`Late Teacher`** — `microcopy.yaml:48`, `member_only: true`.
> ⇒ ⛔ neither string may render as a public column header.
>
> **TWO COLUMNS ARE PARIWAR-CONDITIONAL OR GATED — ⛔ not a fixed column set:**
> · **`School`** and **`District`**/**`Block`** are **Pariwar-selected directory attributes** (`2026-08-19-132` R1/R7; `-133` cl.1). A Pariwar that selects neither `school` nor `block` renders neither, ⇒ **the table must degrade without them.**
> · **`Block`** is additionally gated on `2026-08-19-137` **cl.7**'s member-aware publish + member-choice migration mechanism, ⛔ neither of which exists.
>
> ⚠⛔ **AND THE INVENTORY IS RESTATED IN FOUR OTHER PLACES, ALL NOW ANNOTATED — ⛔ THE COUNT IS ⛔ NOT THE REQUIREMENT, THE LIST IS:** `:1158` (the layout primitive) · `:1252` (the Real Data Test's 10-column restatement) · `:1788` (desktop anatomy) / `:1798` (mobile row anatomy) · `:2161`+`:2165` (the performance contract).
> ⚠ ⭐ **SIX FURTHER SITES *REFER* TO THE INVENTORY WITHOUT RESTATING IT, AND ARE ⛔ NOT IN SCOPE HERE** — `:1317`, `:1319`, `:1321`, `:1330`, `:1787`, and ⭐ **`:2581`, in a breakpoint table no one annotating §8/§10/§11 would open.** ⛔ **Do not read the annotated set as exhaustive.**
>
> ⭐⛔ **WHAT IS ⛔ NOT DECIDED HERE, AND ⛔ MUST NOT BE READ AS DECIDED:**
> · ⛔ **The contributor NAME FORM is UNRULED.** Three committed `epics.md` lines assume first-name + last-initial for contributors (`:3145` · `:3238` · `:4931`) and ⛔ **none is a ruling**. The question is routed to the Trustee Panel as `_bmad-output/planning-artifacts/trustee-panel-routing-note-2026-08-30-contributor-name-public-tier.md` — ⛔ **ROUTED, ⛔ nothing ratified, ⛔ nothing applied** (`-168` cl.4).
> · ⛔ **No column is deleted from this spec, and no replacement inventory is authored.** ⭐ Naming the buildable inventory is **11b.3's**, at the point it has a host — `apps/public/src/pages/` holds ⛔ no contributor route today.
>
> ⚠⛔⛔ **AMENDED 2026-09-15 (Story `11b-3b`, AC11(e)) — ⛔ TWO SENTENCES ABOVE ARE SPENT. ⭐ Both are KEPT as the record and ⛔ neither is rewritten** ([[feedback_supersede_never_reinterpret]]).
> · ⛔ **"The contributor NAME FORM is UNRULED … ⛔ ROUTED, ⛔ nothing ratified, ⛔ nothing applied" is ⛔ NO LONGER TRUE.** ⭐ The routing note it names was **ANSWERED**: [`2026-09-02-174`](../../.decision-log.md#decision-2026-09-02-174) (**Trustee Panel**) ruled the contributor name renders at the `public` tier in the **FULL NAME** form, and [`-175`](../../.decision-log.md#decision-2026-09-02-175) made it **UNCONDITIONAL**. ⇒ ⛔ the three `epics.md` lines this bullet cites are **annotated as superseded**, ⛔ not still open.
> · ⚠⛔ **THE BUILDABLE INVENTORY IS ⛔ NOT `11b-3`'s ANY MORE.** `11b-3` is **`done`** and ⛔ did ⛔ not author it; the work moved to **`11b-3b`** in the **`D6(b)`** three-way split ([`2026-09-02-182`](../../.decision-log.md#decision-2026-09-02-182)) and is its **AC6 / Task 5**. ⇒ ⭐ a reader following the sentence above lands on a **finished** story. ⛔ The host now exists: `apps/public/src/pages/sahyog-vivran/[driveToken].astro`.
> ⭐ **⛔ STILL UNCHANGED:** ⛔ no column is deleted from this spec by that story either, and the *"⛔ THE COUNT IS ⛔ NOT THE REQUIREMENT, THE LIST IS"* rule above governs how the buildable half is written.
>
> ⭐⭐ **THE BUILDABLE INVENTORY, WRITTEN 2026-09-15 — Story `11b-3b`, AC6 / Task 5.** ⭐ The
> **not-buildable** half above already names all ten dispositions; ⛔ this adds ⛔ only the half that
> was owed. ⛔ **ANNOTATION, ⛔ NEVER A REWRITE** — ⛔ no column is deleted and ⛔ no replacement
> inventory is authored ([[feedback_supersede_never_reinterpret]]).
>
> ⚠⛔⛔ **AND THE FIRST FINDING IS ⛔ NOT ABOUT COLUMNS AT ALL — IT IS THE ROW GRAIN, AND ⛔ NO COLUMN
> COUNT CAN FIX IT.** The inventory describes **ONE ROW PER CONTRIBUTION** — a row carrying
> `Donor Name` **and** `Late Teacher` **and** `Pool` **and** `Date` together is one contribution
> joined to its drive. ⛔ **⛔ NO SHIPPED PUBLIC SURFACE HAS THAT GRAIN, AND ⛔ NONE MAY:** the public
> wire carries ⛔ no per-contribution identifier, ⛔ no per-contribution date and ⛔ no per-contributor
> amount, so the five buildable columns below ⛔ **cannot be assembled into a single row of this
> table** even though each exists. ⇒ ⭐ the ten-column table was ⛔ not narrowed — it was **SPLIT
> ACROSS TWO SURFACES BY ROW GRAIN**, and ⛔ that split is structural, ⛔ not a build-order accident:
> · **`/sahyog`** (Story `11b-14`) — **ONE ROW PER DRIVE.** Carries `District` · `Pool` ·
>   `Late Teacher` · `Date`, ⛔ never a contributor.
> · **`/sahyog-vivran/[driveToken]`** (Story `11b-3b`) — **ONE DRIVE, then ONE ROW PER CONTRIBUTOR.**
>   The contributor row carries the name and **⛔ NOTHING ELSE**.
>
> ⭐ **FIVE OF THE TEN ARE BUILDABLE, AND ⛔ THE COUNT IS ⛔ NOT THE REQUIREMENT — THE LIST IS:**
> · ⭐ **`Donor Name`** — ✅ **BUILDABLE, and ⛔ NO LONGER "CONDITIONAL".** [`2026-09-02-174`](../../.decision-log.md#decision-2026-09-02-174) (**Trustee Panel**) ruled it at the **FULL NAME**; [`-175`](../../.decision-log.md#decision-2026-09-02-175) made it **UNCONDITIONAL**. ⭐ Shipped as `contributor_name` on `/sahyog-vivran`. ⚠⛔ **THE COLUMN *LABEL* STAYS FENCED** — `microcopy.yaml:42` keeps `donor` `member_only: true`, so the DATA renders and the WORD does ⛔ not. ⇒ ⭐ the fence moved from the value to the header, ⛔ it did ⛔ not lift.
> · ⭐ **`District`** — ✅ **BUILDABLE, ⚠ and PARIWAR-CONDITIONAL, which is ⛔ not the same as gated.** [`2026-08-19-132`](../../.decision-log.md#decision-2026-08-19-132) **cl.1 (R1/R7)**, with [`-133`](../../.decision-log.md#decision-2026-08-19-133) **cl.1** as co-ground — ⛔ **not** `-132` cl.3, which is the RBAC eligibility-class axis and ⛔ answers a different question. ⇒ ⭐ a Pariwar that does ⛔ not select it renders ⛔ no column, and **the table must degrade without it.**
> · ⭐ **`Pool`** — ✅ **BUILDABLE.** The letter code only (F, D, J, H, B…), per the Mahabharata-naming drop below. Shipped as `pool_letter_code` on both surfaces.
> · ⭐ **`Late Teacher`** — ✅ **BUILDABLE**, [`2026-09-02-173`](../../.decision-log.md#decision-2026-09-02-173) (**Trustee Panel**), at the **FULL NAME**. ⚠⛔⛔ **AND IT RENDERS ⛔ NOTHING TODAY, BY DESIGN:** publication is gated on a pinned `clause_versions` row for `niy.public-disclosure.member-information`, counsel's written clause is still **owed**, and there is ⛔ no migration, ⛔ no seed and ⛔ no writer for it ⇒ the gate is **FAIL-CLOSED for every member**. ⭐ INERT, ⛔ not unbuilt ([`-209`](../../.decision-log.md#decision-2026-09-08-209) cl.2) — ⛔⛔ and ⛔ **no placeholder row may be seeded to make it look alive**. ⚠⛔ **ITS LABEL IS ALSO FENCED** — `microcopy.yaml:48`, `member_only: true`; the canonical term is **"Deceased Member"**.
> · ⭐ **`Date`** — ✅ **BUILDABLE**, ⚠ **as the DRIVE's close/settle instant, ⛔ NOT a contribution date.** Shipped as `drive_closed_at`, sourced from the `pool.closed` / `pool.settled` stream. ⛔ It is **`null` while a drive is still collecting** and the surface then renders **nothing** rather than estimating — ⛔ do ⛔ not read this column as always-present.
>
> ⚠⛔⛔ **AND A SIXTH FIGURE THE ANATOMIES BELOW ASSUME IS REFUSED BY RULING, ⛔ NOT MISSING FOR WANT OF SUBSTRATE — ⭐ THIS IS A FINDING THE 2026-08-30 PASS DID ⛔ NOT MAKE.** `<ContributionListMobileRow>`'s **primary line** is specified as *"donor name + ₹ amount"*. ⛔ **There is ⛔ NO per-contributor amount on any public surface and there must ⛔ not be one:** Story 11b.1 **AC5** forbids leaderboards, rankings, gamification and social-performance metrics, and a per-person rupee figure is the shortest path to all four; `D10-rowkey`(a) ([`-177`](../../.decision-log.md#decision-2026-09-02-177) cl.3) additionally refuses any per-row identifier beside the name. ⇒ ⭐ the shipped contributor row carries **exactly one field**, and `.strict()` makes an amount, a rank or a key a **parse error** rather than a convention. ⚠ The DRIVE-level `amountRaisedInr` ([`-190`](../../.decision-log.md#decision-2026-09-04-190) cl.6) is a **whole-drive aggregate** and is ⛔ **not** this figure — ⛔ do ⛔ not satisfy the primary line by dividing it.
>
> ⭐ **WHAT THIS BLOCK DOES ⛔ NOT DO:** ⛔ it deletes ⛔ no column, ⛔ authors ⛔ no replacement inventory, ⛔ lifts ⛔ neither microcopy fence, and ⛔ resolves ⛔ nothing about `School`, `Block`, `Donation ID`, `Member ID` or `HRMS` — all five keep the dispositions recorded above. ⛔ The `<StatCardStrip>` stays unowned.
> · ⛔ **The `<StatCardStrip>` above the table stays unowned** — C-3, `epics.md:4799`, *"NO PRODUCER | No owner"*; `2026-08-23-154` declined to settle it. ⛔ Do not stub it.
> · ⛔ **The status-pill clause in Story 11b.2's epic AC is reconciled away separately**, in `epics.md`'s Story 11b.2 RECONCILED block (`-168` cl.2): the 8.3 wire row carries ⛔ **no status field by design**, so a pill here would either break the `.strict()` shape test or assert a fact nothing checked.
...
> ⛔ **AMENDED 2026-08-30 (Story 11b.2, D6-uxspec(a) · `-168` cl.3) — ⭐ BOTH ANATOMIES ABOVE ENUMERATE COLUMNS THAT DO ⛔ NOT EXIST, AND ONE NAMES A COMPONENT WITH ⛔ NO PRODUCER.** ⛔ Annotation, ⛔ not a rewrite.
>
> · **`Donation ID` · `HRMS` · `Member ID`** appear in the desktop *identity/metadata* line and the mobile *identity/metadata* line. ⛔ **All three are unbuildable today** — there is no `donation_id` anywhere in `packages/`, no HRMS field on any member table, and a public `Member ID` is what Story 11a.3's handler **refuses in terms** (a per-member permalink is an enumeration primitive). ⚠ The mobile row's *"monospace identity line"* is therefore currently **empty of substrate except the date**.
> · **`donor name`** (desktop header + the mobile primary line) is a **microcopy-PROHIBITED label** (`microcopy.yaml:42`, `member_only: true`); **`Late Teacher`** likewise (`:48`). ⛔ Neither may be rendered as a column label.
> · **`school` · `block`** are **Pariwar-selected directory attributes**, ⛔ not a fixed column set (`2026-08-19-132` R1/R7), and `block` is additionally gated on `2026-08-19-137` cl.7's unbuilt migration mechanism. ⇒ the anatomy must **degrade** without them.
> · ⛔ **The `stat-cards strip above table` has ⛔ NO PRODUCER and ⛔ NO OWNER** (`epics.md:4799`, C-3; `2026-08-23-154` declined to settle it — *"a settled **shape** is ⛔ not a settled **source**"*). ⛔ It must ⛔ not be stubbed to satisfy this anatomy.
>
> ⇒ ⭐ **What Story 11b.2's presenter actually emits for a row is: the contributor's name PARTS (⛔ never joined — the name FORM is UNRULED and routed to the Trustee Panel), the pool LETTER code, and an a11y i18n ref.** ⛔ No status pill (the epic AC clause that ordered one is reconciled away at `epics.md`'s Story 11b.2 RECONCILED block, `-168` cl.2 — the 8.3 wire row has no status field **by design**).
>
> ⭐⭐ **AND THE BUILDABLE HALF IS NOW WRITTEN (2026-09-15, Story `11b-3b` AC6)** — at the same canonical section, §"Public Column Inventory — Sahyog List", under **"THE BUILDABLE INVENTORY"**. ⭐ **FIVE of the ten are buildable** (`Donor Name` · `District` · `Pool` · `Late Teacher` · `Date`), ⚠⛔ **but ⛔ NOT AS ONE TABLE:** the inventory's **ROW GRAIN** — one row per CONTRIBUTION — exists on ⛔ no shipped surface and may ⛔ not, so the ten-column table was **SPLIT BY GRAIN** across `/sahyog` (one row per DRIVE) and `/sahyog-vivran/[driveToken]` (one row per CONTRIBUTOR, carrying the name and ⛔ nothing else). ⚠ Two of the five keep **microcopy-fenced LABELS** and one (`Late Teacher`) is **INERT by design**. ⛔ Cite the canonical section by its TITLE, ⛔ not by line.
>
> ⚠⛔⛔ **AND TWO SENTENCES IN THIS ANNOTATION ARE NOW SPENT — ⭐ KEPT, ⛔ NOT REWRITTEN** ([[feedback_supersede_never_reinterpret]]):
> · ⛔ *"the name FORM is UNRULED and routed to the Trustee Panel"* — ⭐ **the Panel RULED it**: [`-174`](../../.decision-log.md#decision-2026-09-02-174) at the **FULL NAME**, unconditional per [`-175`](../../.decision-log.md#decision-2026-09-02-175). ⚠⛔ **The PARTS shape is ⛔ NOT thereby wrong** — it is the **MEMBER** surface's form and it STANDS there; what changed is that the **PUBLIC** surface is a different form, and `11b-3b` therefore consumes the shared presenter for its **⛔ NON-name outputs only**. ⛔ Do ⛔ not "align" the two by joining the parts.
> · ⚠⛔ **THE MOBILE PRIMARY LINE'S `₹ amount` IS REFUSED BY RULING, ⛔ not missing for want of substrate** — a finding this 2026-08-30 pass did ⛔ not make, because it flagged only the identity/metadata line. Story 11b.1 **AC5** forbids leaderboards, rankings and social-performance metrics, and `D10-rowkey`(a) ([`-177`](../../.decision-log.md#decision-2026-09-02-177) cl.3) refuses any per-row identifier beside the name. ⇒ ⭐ the shipped public contributor row carries **exactly one field**, `.strict()`-enforced.
...
> ⛔ **AMENDED 2026-08-30 (Story 11b.2, D6-uxspec(a) · `.decision-log.md#decision-2026-08-30-168` cl.3) — ⭐ THE PERFORMANCE CONTRACT IS SOUND AND IS ⛔ NOT AMENDED; WHAT IS AMENDED IS THE ASSUMPTION THAT `<ContributionListTable>` IS ONE BUILDABLE COMPONENT.** ⛔ Annotation, ⛔ not a rewrite — the paragraph above stands verbatim, and its own sentence (*"a **performance contract**, not an implementation specification"*) is what makes the split below legal.
>
> **(1) ⭐ THE 50k/10k FIGURES BIND BEHAVIOUR UNDER LOAD, ⛔ NOT A PAGE'S ROW BUDGET.** A public surface tops out at **10,000 rows by construction**: the 50-row page cap (`packages/contracts/src/_common/pagination.ts:26`) × page 200 (`public-pages/directory.ts:48`), stated in the code itself at `apps/public/src/lib/pagination.ts:57` — *"At the 50-row cap, page 200 is row 10,000."* ⚠ ⛔ Do ⛔ not read the desktop 50k figure as authorising a 50,000-row response.
>
> **(2) ⛔ VIRTUALIZATION IS A RENDER-LAYER PROPERTY AND CANNOT LIVE IN `packages/ui`.** `@twt/ui` is **headless by construction** — `dependencies` is exactly `{"@twt/contracts"}`, zero `.tsx` — and a pure `(input) → view-model` function has ⛔ no scroll container, ⛔ no viewport, ⛔ no mount lifecycle. ⇒ **the noun splits**: Story 11b.2 owns the row's **content contract** (`deriveContributionRowViewModel`, per-row and allocation-light so a windowing layer may call it once per visible row per scroll frame); **11b.2b** and a future Astro layer own the **windowing**. ⭐ UX-DR80 already says exactly this — *"Native: FlatList tuning. Web: TanStack Virtual / react-virtuoso / react-window"*.
>
> ⚠ **The column inventory this contract is measured against is ⛔ NOT buildable as written — see the amendment at §"Public Column Inventory — Sahyog List" (`:1287-1298`), which carries the full finding.**
>
> ⭐⭐ **AND THE BUILDABLE HALF IS NOW WRITTEN (2026-09-15, Story `11b-3b` AC6)** — at the same canonical section, §"Public Column Inventory — Sahyog List", under **"THE BUILDABLE INVENTORY"**. ⭐ **FIVE of the ten are buildable** (`Donor Name` · `District` · `Pool` · `Late Teacher` · `Date`), ⚠⛔ **but ⛔ NOT AS ONE TABLE:** the inventory's **ROW GRAIN** — one row per CONTRIBUTION — exists on ⛔ no shipped surface and may ⛔ not, so the ten-column table was **SPLIT BY GRAIN** across `/sahyog` (one row per DRIVE) and `/sahyog-vivran/[driveToken]` (one row per CONTRIBUTOR, carrying the name and ⛔ nothing else). ⚠ Two of the five keep **microcopy-fenced LABELS** and one (`Late Teacher`) is **INERT by design**. ⛔ Cite the canonical section by its TITLE, ⛔ not by line. ⚠⛔ **AND THIS CONTRACT GAINS A SECOND CORRECTION FROM IT:** the mobile row's primary line assumes a **per-contributor ₹ amount**, which is **REFUSED BY RULING** (11b.1 AC5), ⛔ not merely unbuilt — so ⛔ no virtualization budget may be sized against it.
