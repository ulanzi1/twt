...
_Code review — three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor + load-bearing-invariant checklist) against `git diff 31153962 HEAD`. 0 decision-needed, 5 patch, 3 deferred, 7 dismissed as noise/resolved._

- [x] [Review][Patch] `sahyog-copy.test.ts:31` comment points to a test file that doesn't exist (`sahyog-shared-copy.test.ts`) [apps/public/tests/sahyog-copy.test.ts:31] — fixed: now points to `sahyog-stage-vocabulary.test.ts`
- [x] [Review][Patch] Change Log table rows out of order — `1.0` and `0.5` (both dated 2026-09-06) are placed before `0.4` (dated 2026-09-05), breaking both date and version-number order [_bmad-output/implementation-artifacts/11b-12-stage-vocabulary-and-info-affordance.md:1196-1198] — fixed: reordered to `0.4 → 0.5 → 1.0`, new `1.1` row records this review's patches
- [x] [Review][Patch] `sahyog-shared-dark-copy.test.ts` assertion failure message still says "t() interpolates an unsupplied token to NOTHING" — contradicts the file's own header, which correctly establishes `t()` THROWS [packages/i18n/tests/sahyog-shared-dark-copy.test.ts] — fixed: message now says THROWS / 500, cites `resolver.ts:36-42`
- [x] [Review][Patch] AC1's Hindi positive-pin is incomplete: `RETIRED_HI` omits `संग्रह` (the actual pre-story value of both `section.archive.title` and `status.archive` in `hi/sahyog-drive.json`), and the `.includes()` substring check has no word-boundary safeguard the way the English check does — a regression reintroducing `संग्रह` at `status.archive` would pass silently [apps/public/tests/sahyog-stage-vocabulary.test.ts:93,97] — fixed: added `संग्रह` to `RETIRED_HI`, with an explicit, documented exclusion for the two keys (`empty.body`, `collecting.body`) that legitimately use it as ordinary prose, not as the archive stage label
- [x] [Review][Patch] D3's ruled exact wording for `collecting.body` was changed beyond what D3 authorized — shipped copy reads "the contribution window closes" (en) / "योगदान की अवधि" (hi) where D3's ruling table specifies "the collection window closes"; the Dev Agent Record's own correction (2) is now internally inconsistent with the shipped text (it argues the word-boundary test is needed because a substring `collect` would false-fail against D3's *retained* "collection" phrase, but that phrase is no longer present) [packages/i18n/locales/en/sahyog-vivran.json:16, packages/i18n/locales/hi/sahyog-vivran.json:16] — fixed: reverted both locales to D3's ruled wording ("collection window closes" / "संग्रह की अवधि"); all 6 affected test suites (185 + 87 tests) re-run green
- [x] [Review][Defer] Hand-listed literal-set runtime guards in `sahyog.server.ts`/`sahyog-vivran.server.ts` stay independently duplicated from the Zod contract enums rather than deriving from `PublicSahyogDriveStatus`/`PublicSahyogVivranStatus` at runtime — the exact drift class this diff spent significant effort explaining could recur at the next rename [apps/public/src/lib/sahyog.server.ts, apps/public/src/lib/sahyog-vivran.server.ts] — deferred, pre-existing pattern, follow-up refactor not blocking
- [x] [Review][Defer] AC9's four dark-copy variants have no defined behavior for simultaneous multi-token absence (e.g. nominee AND district both absent) — Ruling 3 rules out combinatorial variants but doesn't establish precedence for the double-absence case; nothing renders these tokens in this diff, so not reachable/decidable here — flag explicitly for Story D (11b-14 AC7/Task 8), the consuming story, before it wires up variant selection [packages/i18n/locales/en/sahyog-shared.json:12-15] — deferred, next story's scope
- [x] [Review][Defer] Stale doc comment atop `sahyog.astro` still reads "Active (window closed, family not yet paid) and Archive (paid out)" — the exact retired vocabulary and false-payout narrative this story removes from user-facing copy, sitting in a developer comment one page above the code this story touched; predates this diff's hunks, never rendered [apps/public/src/pages/sahyog.astro:10-11] — deferred, pre-existing, out of this diff's touched hunks

Dismissed as noise or resolved on verification (7): possibly-unresolved `pool` identifier in `sahyog-drive.spec.ts` (verified: pre-existing import, outside diff hunks); inconsistent em-dash escaping (verified: file has zero `—` escapes anywhere, no inconsistency exists); magic-number `>80` threshold in the non-vacuous copy-scan guard (trivial test nitpick); the "three corrections" narrative repeated across story file/sprint-status/completion notes (matches this project's established verbose cross-referencing convention); the `$comment`-prefix exclusion convention being unenforced by lint/schema (narrow test-only convention, no current violation); the "verbatim from routing note" claim being unverifiable from the diff alone (resolved: Acceptance Auditor independently checked it against the actual routing note file and confirmed an exact match); brittle `indexOf`-based string-position test standing in for real DOM nesting verification (test-quality nitpick, no currently-reachable defect).
...
| 2026-09-06 | 1.1 | 🔧 **Code review — 5 patches applied.** (1) Fixed a dangling comment reference to a nonexistent `sahyog-shared-copy.test.ts` in `sahyog-copy.test.ts:31` — the correct file is `sahyog-stage-vocabulary.test.ts`. (2) **This table's own row order was broken** — rows `1.0` and `0.5` (dated 2026-09-06) were sitting before `0.4` (dated 2026-09-05); reordered to `0.4 → 0.5 → 1.0`. (3) Fixed `sahyog-shared-dark-copy.test.ts`'s assertion failure message, which still said `t()` "interpolates an unsupplied token to NOTHING" after the file's own header had already corrected that to THROWS. (4) **AC1 gap**: `RETIRED_HI` omitted `संग्रह` — the actual pre-story value of `hi/sahyog-drive.json`'s `status.archive`/`section.archive.title` — leaving the positive Hindi pin blind to a regression reintroducing it; added. (5) **D3 deviation**: `collecting.body` shipped as *"once the **contribution** window closes"* (en) / *"जब **योगदान** की अवधि"* (hi) where D3's ruling table specifies the AFTER value keeps *"the **collection** window closes"* unchanged, only prepending a sentence; reverted both locales to D3's ruled wording — consistent with `empty.body`'s established "collection window" phrasing, which the AC1 test's word-boundary rationale was already built around. | Claude (code review) |
