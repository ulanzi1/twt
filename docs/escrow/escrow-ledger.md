...
Authority: Story 0.2 (epics.md) AC-1 / AC-2 / AC-3. Stored alongside the rest of the escrow framework under the trustee-accessible repo. Mirror-coverage discipline inherited from `docs/runbooks/operational-readiness-ledger.md` — primary git repo is trustee-accessible for v1; Story 0.3 (Code Escrow Auto-Mirror Pipeline) extends coverage to a trustee-controlled mirror destination on every release-branch push. **As of 2026-06-05 per `.decision-log.md` Decision 2026-06-05-019 + [ADR-0002 code-escrow-mirror-destination](../adr/adr-0002-code-escrow-mirror-destination.md):** mirror destination ratified = GitLab.com under trustee-owned foundation account; credential model = SSH deploy key (push-only scope; whitelisted for branch-protection-exempt force-push per ADR-0002 body item 3 amended via Decision 2026-06-05-037); workflow at `.github/workflows/code-escrow-mirror.yml`. Substantive provisioning + secret wiring + first push + ≥2-trustee read-access verification + restoration drill + bus-factor switch-to-mirror exercise are deferred per Story 0.3 Tasks 8-11 (see `docs/escrow/code-escrow/code-escrow-ledger.md` for substantive event log; per Decision 2026-06-05-035 Phase-0 provisional closure meta-entry). The mirror-coverage stance flips from "pending" to "ratified; substantive execution in flight".
