...
> ⚠ **ANNOTATED (Code review, 2026-08-26) — "call `selectCloseOfCycleFraming()`" reaches the ENUM
> decision only, ⛔ not the currency-bearing template contract.** `selectCloseOfCycleFraming()`'s
> `requiredParams` includes `amount` (a formatted delivered-total figure), which this surface must
> never carry — it counts confirmations, ⛔ never sums amounts (this AC's first clause, and
> `SahyogDriveEntry.confirmedContributionCount`'s own doc comment). The delegation this AC requires
> is satisfied by `classifyCycleOutcome` (called, correctly) producing the opaque enum;
> `sahyog-render.ts`'s `framingFor()` then maps that enum to three bespoke, amount-free strings —
> which is the correct shape for a count-only consumer, ⛔ not the re-implementation this AC warns
> against. ⛔ **Never rewritten** — the clause stands as written for `selectCloseOfCycleFraming()`'s
> OTHER consumers (Panchayat Noticeboard, Sahyog Vivran), which do carry an `amount`.
...
Three-layer adversarial review (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against `main..HEAD` (52 files, +7689/-79), triaged 2026-08-26. All high-severity claims below were independently verified by reading the named source, not taken on the subagent's word.

- [x] [Review][Patch] Archive "permanent record" district can silently drift after publication [packages/domain/src/pool/public-read.ts `DECEASED_DISTRICT`] — resolves the deceased member's LATEST posting as of query time (`now`), never as of the drive's close/settle date. **Resolved (user, 2026-08-26): freeze district as of the drive's close/settle instant** (join `DECEASED_DISTRICT` against `driveClosedAt` instead of `now`), so a previously-published Archive row's district never changes after publication.
- [x] [Review][Dismiss] `sahyogDrivePublication` is required inside a `.strict()` object with no rollout shim [packages/contracts/src/claims/dpdpa-consent.ts] — **withdrawn (user, 2026-08-26): this code is not in production.** There is no pre-update mobile client in the field to break — this surface hasn't reached its launch gate (see the story's own "may be BUILT; may NOT be PUBLISHED" posture). The field stays required, as originally built: the adjacent comment already documents why (an optional field would let a client that never rendered the box submit a body indistinguishable from an explicit decline) — a real concern only in a live-rollout scenario that does not exist here.
- [x] [Review][Defer] `classifyCycleOutcome` classifies a zero-assignee pool (`expectedTotal=0`) as `'fully_funded'` by vacuous comparison [packages/domain/src/close-of-cycle/framing.ts] — deferred, pre-existing shape, unreachable in practice. **Resolved (user, 2026-08-26): deferred as unreachable** — `member_pool_assignments` rows are written at pool-spawn time, well before a pool can reach `closed`/`settled`, so this state can't occur without an independent data-integrity bug elsewhere; recorded rather than guarded against speculatively.
- [x] [Review][Patch] Filter form is broken for realistic use, not just edge cases [apps/public/src/pages/sahyog.astro:160-178, apps/public/src/lib/sahyog.server.ts:96-99, packages/contracts/src/public-pages/sahyog-drive.ts:150-156] — `Astro.url.searchParams.get(...) ?? undefined` never catches an empty string, so a filter form submitted with any field left blank forwards `field=''` rather than omitting it, tripping the API's `.min(1)` validation; separately, the native `<input type="date">` sends bare `YYYY-MM-DD` forwarded as-is against a `z.string().datetime()` schema that requires a full ISO timestamp. Both make ordinary partial-filter use 400, and `sahyog.server.ts`'s `!res.ok` handling collapses that 400 into a generic "outage" state rather than a fixable-input message.
- [x] [Review][Patch] Pool-code search never matches the letter code [packages/domain/src/pool/public-read.ts:289-308] — `sahyogDrivePredicate()`'s poolCode conjunct only compares `pools.poolCanonicalIdentifier`, despite the comment directly above it, the wire contract's own `poolLetterCode` field (`packages/contracts/src/public-pages/sahyog-drive.ts:100`), and AC3's explicit "(letter code or canonical identifier)" requirement all describing letter-code matching as supported. No test (domain or API integration) exercises a letter-code lookup.
- [x] [Review][Amend] AC4's "call `selectCloseOfCycleFraming()`" clause conflicts with its own no-amount rule for this consumer [apps/public/src/lib/sahyog-render.ts:128 `framingFor()`, packages/domain/src/close-of-cycle/framing.ts] — **resolved (user, 2026-08-26) as a discovered spec-internal tension, not a code defect.** `selectCloseOfCycleFraming()`'s template contract requires FOUR interpolation params, including `amount` (a formatted currency figure — the actual delivered total, per Pool-Reality #1). This surface's own `SahyogDriveEntry.confirmedContributionCount` is explicitly "a count, never a sum, never a score," and AC4's own first clause forbids "never a sum of per-event amounts" — so satisfying AC4's literal instruction would require computing a currency figure this surface is elsewhere forbidden from carrying. `framingFor()`'s three bespoke, amount-free strings (one per `CycleFundingOutcome` member) are the correct shape for THIS consumer: the outcome ENUM → copy-family mapping, without the full 4-param template contract. AC4 is annotated here to record that "never a re-implementation" reaches only the ENUM decision (`classifyCycleOutcome`, correctly delegated) and not the currency-bearing template contract, which does not fit a count-only surface. Not extended to `selectCloseOfCycleFraming()` itself — that function is shared with the Panchayat Noticeboard and Sahyog Vivran, and widening its contract is out of this story's scope.
- [x] [Review][Patch] District filter has no trim/case-fold normalization [packages/domain/src/pool/public-read.ts `sahyogDrivePredicate`, apps/api/.../handlers.ts rendered `row.district?.trim()`] — the rendered value is trimmed for display but the filter predicate does a raw `=` against the untrimmed, uncased stored value with no normalization on the user-supplied side either. A district that displays correctly cannot reliably be found by filtering on it.
- [x] [Review][Patch] `DRIVE_CLOSED_AT` sort has no explicit `NULLS LAST` [packages/domain/src/pool/public-read.ts:364] — `.orderBy(desc(DRIVE_CLOSED_AT(now)), desc(pools.poolId))` relies on Postgres's `DESC` default of `NULLS FIRST`. Since the predicate already restricts to `closed`/`settled` pools, a null `driveClosedAt` here is a data anomaly, and the current ordering sorts it to the very top instead of the bottom.
- [x] [Review][Dismiss] `evaluateDirectoryAbuse(...)` call is unguarded fire-and-forget in the new `sahyogDrive` handler [apps/api/src/modules/public-pages/handlers.ts] — **verified false positive.** `evaluateDirectoryAbuse` (`abuse-rules.ts:175`) is fully synchronous (not async — no rejection is possible), wraps its entire body in `try`/`catch`, and its own docstring guarantees "never throws into the request path... a detection surface that could 500 the page it watches would be a worse outage than the abuse it detects." The bare, unguarded call exactly mirrors the pre-existing `/members` call site (`handlers.ts:167`, untouched by this diff) — the established, deliberate, safe pattern for this function. No code change.
- [x] [Review][Patch] Orphaned unused i18n keys shipped in both locale files — `page.noindex_note`, `filter.district.any`, `filter.reset`, `pagination.status`, `pagination.total` in `packages/i18n/locales/{en,hi}/sahyog-drive.json` are referenced by neither `sahyog.astro` nor `sahyog-copy.test.ts`'s key-completeness list.
- [x] [Review][Patch] Test-hygiene nitpicks — `apps/api/tests/integration/public-pages/sahyog-drive.spec.ts` hardcodes `limit: 25` as a bare literal in one assertion despite the file's own comment warning against exactly that anti-pattern (citing the 11a.2 defect) elsewhere in the same file; `apps/api/tests/unit/dpdpa-consent-record-atomicity.test.ts` crams `sahyogDrivePublication: false` onto the same line as its neighbor instead of its own line, unlike every other call site touched by this diff.
- [x] [Review][Dismiss] Comment/test citation to "D4(c), rejected on the record" — **verified false positive.** D4's full options list (story file lines 878-880) DOES include a `(c) Reuse sahyog_vivran_publication. Rejected: it was captured for a different publication and reusing it would silently widen what a family agreed to` — an exact match to the test citations in `apps/api/tests/integration/claims/dpdpa-consent.spec.ts` and `packages/domain/tests/integration/pool/sahyog-drive-public-read.spec.ts`. The Blind Hunter layer ran diff-only with no project access and could not see the full decision log's options list. No code change.
- [x] [Review][Dismiss] Vocabulary-leak test coverage "duplicated across two suites" — **verified false positive.** `apps/public/tests/integration/public-pages/scrape-test.spec.ts` contains no assertion about `spawned`/`live`/`closed`/`settled` tokens at all (checked; its tier-leak tests guard an unrelated `terms`/legal-reviewer surface). `apps/public/tests/sahyog-render.test.ts`'s fixtures only ever use the already-public `'active'`/`'archive'` tokens — structurally correct, since `sahyog-render.ts` consumes the wire response AFTER the API has already translated pool state to the public vocabulary, so it never has access to the internal tokens to leak in the first place. The ONE test asserting this (`apps/api/tests/integration/public-pages/sahyog-drive.spec.ts:409-427`) is architecturally the only place such a leak could occur. No code change.
- [x] [Review][Defer] `/sahyog.astro` itself does not reject an unknown query parameter (e.g. `?format=csv`) [apps/public/src/pages/sahyog.astro:160-178] — deferred, pre-existing. AC3's "unknown query parameter is a refusal" holds at the `apps/api` route's `.strict()` schema (tested), but the public page never forwards more than its four named params, so a visitor hitting the page directly with an extra param gets a silent 200 with the param ignored, not a 400. Same pattern already exists on `/members` (11a.3); not introduced by this diff.

### Review Findings — SECOND PASS (2026-08-27)

Three-layer adversarial review (Blind Hunter, Edge Case Hunter, Acceptance Auditor) re-run against `main...story/11b.1-sahyog-drive` at `c02dae2` (53 files, +7809/−78; the 6053-line code half, governance/docs excluded by agreement). All layers returned; none failed. Every high-severity claim below was independently verified by reading the named source or reproducing against Postgres — ⛔ none is taken on a subagent's word.

⚠ **THREE OF THESE ARE RE-OPENINGS, ⛔ NOT NEW FINDINGS.** The first pass named them; the closure did not hold. They are marked ⟲ and state what the first pass concluded and why that conclusion does not survive.

> ✅ **ALL 19 PATCHES APPLIED AND VERIFIED (2026-08-27).** ⛔ Nothing below is "written up but unbuilt".
> · `turbo run typecheck` — **20/20 packages, 0 errors** · `turbo run lint` — **20/20 clean**
> · `turbo run test` — **all suites green** (domain 129 files · contracts 62 · public 15/331 tests ·
>   mobile 27/342 · channels 15 · ui 8 · i18n 10, others green; `@twt/api` runs no `test` task under
>   turbo, so its unit suite was run directly: **42 files / 348 passed**)
> · **DB-gated specs run live** against `twt-test-pg`:
>   `pool/sahyog-drive-public-read.spec.ts` **22/22**, `public-pages/sahyog-drive.spec.ts` **16/16**,
>   `claims/dpdpa-consent.spec.ts` **12/12**.
>
> ⭐⛔ **AND THE ZERO-EXPECTATION FINDING CONFIRMED ITSELF WHILE BEING FIXED.** On first re-run the
> AC4 quarantine test FAILED with *".toMatch() expects to receive a string, but got object"* — i.e.
> `fundingOutcome` came back `null`. The reason: **`seedDrive` has never written
> `member_pool_assignments`**, so ⛔ **EVERY drive in that entire spec had zero assignees** and, under
> the pre-fix code, was being classified **`fully_funded`**. ⚠ The only assertion covering it was
> `toMatch(/^(fully_funded|under_funded|partial)$/)` — which passes for every member of the union and
> so asserted **nothing**. ⇒ the fixture was ⛔ not an edge case; it was the ordinary case, green.
> ⭐ Replaced with an EXACT assertion on a drive seeded with **3 assignees** (`under_funded`) plus a
> new test pinning **0 assignees ⇒ `null`**, and `seedDrive` gained an `assignedMembers` option.

**⚖️ Decision needed**

- [x] [Review][Dismiss] ⭐ **The kill-switch default-open behaviour — ✅ RULED KEEP (BigDev, 2026-08-27). ⛔ NOT A DEFECT.** [apps/api/src/modules/public-pages/handlers.ts:322-330 · packages/domain/src/member/directory-publication.ts:74-79] — the review raised that §Launch posture:169 (*"⛔ Do not enable the surface for any Pariwar"*) has no mechanism behind it, because D3(a) reuses the Member Directory switch and `resolveDirectoryPublicationEnabled` returns `row?.enabled ?? true`. ✅ **RULED (BigDev, 2026-08-27): KEEP, and the reading was wrong.** The existing directory-publication switch is an **EMERGENCY / OFF control**, ⛔ not a launch gate. A missing Pariwar row resolving to `enabled` is **by design**; 11b.1 reuses that behaviour unchanged. ⛔ **No new opt-in flag and ⛔ no fail-closed default is introduced.** ⚠ §Launch posture:169 must ⛔ **NOT** be read as requiring a new default-off mechanism — it is an instruction to the operator, ⛔ not a specification of a code control. The switch stays available to disable publication for a Pariwar when required. ⭐ **The consequence, recorded openly rather than left implicit** ([[feedback_record_unattested_no_backfill]]): with this ruling the controls keeping `/sahyog` dark are **deployment and the counsel/Trustee-Panel process**, ⛔ **not a code mechanism** — so ⛔ nobody may later cite the publication switch as the technical launch gate for this surface. ⚠ §Launch posture is left **byte-unchanged** ([[feedback_supersede_never_reinterpret]]); this entry annotates it and is the superseding reading. ⇒ ⛔ **Do not re-raise this finding in a later review pass** — it is ruled, not open.
- [x] [Review][Patch] ⟲ **`fully_funded` for a zero-expectation pool — ✅ RESOLVED (BigDev, 2026-08-27): the outcome goes NULLABLE and the row SAYS NOTHING.** ⭐ Ruled fix: `SahyogDriveEntry.fundingOutcome` becomes `CycleFundingOutcome | null`; `public-read.ts` returns `null` when `assignedCount === 0` instead of calling `classifyCycleOutcome`; `framingFor` returns `''` for `null` — the SAME *"null ⇒ NOTHING, ⛔ no placeholder"* discipline the surface already applies to a withheld name. ⛔ The shared `CYCLE_FUNDING_OUTCOMES` union is NOT widened (it is shared with the Panchayat Noticeboard + Sahyog Vivran and its ordering is provenance-stable) and ⛔ `classifyCycleOutcome` is NOT changed — the prior pass declined that widening and the decline STANDS. ⛔ `partial` was considered and REJECTED: its shipped copy (*"Reconciliation is still in progress"*) is ⛔ not true of a drive that had nobody assigned — that trades a false statement for a misleading one. ⛔ Excluding the pool from the index was also rejected: it cuts against *"the index degrades PER-POOL, ⛔ never per-page"* and hides an operational signal. ⚠ Owes: the contract change (`sahyog-drive.ts`), the `framingFor` null branch, and a test that pins `assignedCount === 0 ⇒ fundingOutcome === null` — ⛔ the existing `toMatch(/^(fully_funded|under_funded|partial)$/)` assertion passes for all three and asserts nothing. **Original finding follows.** ⟲ **The first pass's deferral ground is FALSIFIED** [packages/domain/src/pool/public-read.ts:405-424 · packages/domain/src/close-of-cycle/framing.ts] — **First pass (2026-08-26) deferred this as "unreachable"**, on the stated ground that *"`member_pool_assignments` rows are written at pool-spawn time, well before a pool can reach `closed`/`settled`."* ⛔ **That ground does not hold.** `packages/domain/src/pool/assign.ts:147-155` returns an empty assignment on `if (m === 0)` — its own comment calls this *"empty roster → empty assignment (**the common (B)-scope case**)"* — and `capacity[i] = floor(m/n) + (i < m % n)` assigns **0** to the trailing `n − m` pools whenever approved claims outnumber the assignable roster. Pools spawn one per approved claim, independently of roster size. ⇒ `expectedTotal = 0 * fixedAmount = 0`, `deliveredTotal = 0`, and `classifyCycleOutcome` returns `fully_funded` on `0 >= 0` (verified: the function's last line is `return deliveredTotal >= expectedTotal ? 'fully_funded' : 'under_funded'`). The row publishes **"0 confirmed"** beside **"The cycle closed with the support it needed."** / "इस चक्र को आवश्यक सहयोग मिला", edge-cached 300s. ⚠ The only test on this path asserts `toMatch(/^(fully_funded|under_funded|partial)$/)` — which passes for all three, i.e. asserts nothing. ⛔ The fix is ⛔ not unambiguous: what a zero-expectation drive should say publicly is a product/governance call, not a code call.
- [x] [Review][Patch] ⟲ **Every API 4xx renders as an OUTAGE at HTTP 503 — ✅ RESOLVED (BigDev, 2026-08-27): ⭐ AC7 GOVERNS. Split the reason.** ⭐ Ruled fix: `fetchSahyogDrive` distinguishes an INPUT REFUSAL from a TRANSPORT FAILURE — `reason: 'rejected'` for a 4xx, `'bad_response'` for everything else — and `sahyog.astro` routes `'rejected'` to the EXISTING `buildSahyogRejectionView` (400 + `no-store` + fixable-input copy), leaving the outage path (503 + `Retry-After: 60`) for genuine upstream failure. ⭐⛔ **THIS CLOSES THE SHAPE, ⛔ NOT THE TRIGGERS** — the first pass patched two triggering inputs and the collapse survived to be re-found; a third pass must ⛔ not find it again. ⚠ **AC8 is ANNOTATED IN PLACE, ⛔ never rewritten** ([[feedback_supersede_never_reinterpret]]): its *"`ok:false` presented as an OUTAGE"* is recorded as having meant **transport failure**, ⛔ not input refusal — AC7's four-state contract is the governing one and the two are ⛔ no longer read as interchangeable. ⚠ Owes: the `reason` widening in `sahyog.server.ts`, the branch in `sahyog.astro`, the AC8 annotation, and a test per state — ⛔ `apps/public/tests/` currently covers ⛔ NO malformed-filter path at all. **Original finding follows.** ⟲ **The first pass patched the TRIGGERS, ⛔ not the COLLAPSE** [apps/public/src/lib/sahyog.server.ts:114 · apps/public/src/pages/sahyog.astro:243-246] — the first-pass item at line 732 named this verbatim (*"collapses that 400 into a generic 'outage' state"*) and fixed two triggering inputs (blank fields, bare `YYYY-MM-DD`). ⛔ The collapse itself is untouched: `if (!res.ok) return { ok:false, reason:'bad_response' }` makes a 400, a 403, a **429** and a 502 the same value, and every one becomes `apiUnavailable` ⇒ 503 + `Retry-After: 60` + *"This is a problem on our side, **not a statement about the trust's activity**."* Still reachable from the page's own form: a district of only spaces (`|| undefined` treats `'   '` as present), district > 120 chars, drive code > 64 chars, or any browser rendering `<input type="date">` as free text. ⚠ **AC7 and AC8 pull against each other here** — AC7 owes FOUR distinct states incl. a 400 rejection; AC8 says `ok:false` is presented as an OUTAGE, and the code follows AC8's letter. ⛔ Which AC governs is BigDev's call.
- [x] [Review][Patch] **A subject with >200 accumulated grant rows cannot fully revoke — ✅ RESOLVED (BigDev, 2026-08-27): LOOP UNTIL EXHAUSTED.** ⭐ Ruled fix: wrap the existing `listConsents(limit: 200)` + per-row revoke in a loop that repeats until it returns ZERO active grants, with a `MAX_REVOKE_PAGES` sanity guard. ⛔ **No schema change**, ⛔ Story 2.7's frozen `listConsents` primitive is NOT touched (⛔ no cursor/offset added), and ⭐ the per-row audit/event pair family 8 requires is PRESERVED. ⚠ **A unique index was considered and is ⛔ NOT AVAILABLE** — `recordConsent`'s own docstring rules the multi-row model deliberate (*"re-recording after a revoke is a fresh row by design (AC3; there is no unique constraint on `(subject_id, consent_type)`)"*), because a revoked row must survive so `consentExists(..., pastTimestamp)` stays true. ⭐ Grant history is the FEATURE; the ceiling was the bug. ⛔ A set-based single `UPDATE` was rejected: it changes the audit/event shape (N events or one?) and that is a family-8 contract question this story ⛔ does not own. ⚠ Note the handler ALREADY knew — it deliberately raised the cap 50→200 and documented why; ⭐ the defect is that a CEILING was chosen where the loop should have been UNBOUNDED. **Original finding follows.** [apps/api/src/modules/claims/claims.dpdpa-consent.handlers.ts:300-303 · packages/domain/src/pool/public-read.ts:167-175] — `recordConsent` is a plain INSERT with ⛔ no upsert and ⛔ no unique index on `(pariwar_id, subject_id, consent_type)`; the claim-time handler loops over checked boxes on **every** submission, so each pre-adjudication re-submit appends another live grant. Revoke resolves active grants with a hard `limit: 200` (the handler's own comment at :294-299 names the ceiling as known), while `NAME_CONSENT_GRANTED` is an **uncapped** `EXISTS` ⇒ one surviving row keeps the verdict `true`. ⇒ **a DPDPA withdrawal reports success while the deceased member's full legal name keeps publishing on an unauthenticated, edge-cached page.** ⚠ Pre-existing from Story 6.9 — but ⭐ **this diff is what makes it govern a public name publication**, which is why it is escalated rather than deferred. Fix needs a schema call: unique index + upsert, vs cursor-paginating the revoke.

**🔧 Patch**

- [x] [Review][Patch] ⭐ **A 7-letter drive code 500s the unauthenticated route (int4 overflow)** [packages/domain/src/pool/naming.ts:102-112 · packages/domain/src/pool/public-read.ts:327-334]
- [x] [Review][Patch] ⟲ The letter-code branch ships with ⛔ ZERO tests at any layer, and `poolIndexFromLetterCode` has ⛔ no test anywhere — the first pass named this gap in terms and the row is checked `[x]` [packages/domain/tests/integration/pool/sahyog-drive-public-read.spec.ts · apps/api/tests/integration/public-pages/sahyog-drive.spec.ts]
- [x] [Review][Patch] The RN consent row announces `accessibilityRole="checkbox"` + label + checked state on a Tamagui `XStack` with ⛔ no `accessible={true}` — **REAL GAP, load-bearing-invariant family 13(a)+(d)** [apps/mobile/app/(claim)/consent.tsx:51-59]
- [x] [Review][Patch] The anti-enumeration counter merges BOTH public surfaces on one IP key with ⛔ no surface discriminator; every emitted line reads `directory:`; the 60s dedupe can suppress a Sahyog crawl signal entirely [apps/api/src/modules/public-pages/handlers.ts:343-350 · abuse-rules.ts:159,260]
- [x] [Review][Patch] `splitSections` partitions rows by comparing **localised display strings**, and `drive_status` is matrix-declared with ⛔ no `visibleSahyogColumns` entry ⇒ ⛔ no `<MatrixField>` enforcement point [apps/public/src/lib/sahyog-render.ts:269-277 · apps/public/src/lib/surface-fields.ts:323]
- [x] [Review][Patch] The tier-leak scrape test **rebuilds its own HTML** and does ⛔ not mirror the page it claims to mirror (`''` ⇒ the page emits `<td></td>`; the test emits ⛔ no cell) — ⛔ nothing in the suite reads `sahyog.astro` [apps/public/tests/integration/public-pages/scrape-test.spec.ts:2632-2647 · apps/public/src/pages/sahyog.astro:375,404]
- [x] [Review][Patch] The handler defaults `limit` from the **directory's** constant; the domain's `SAHYOG_DRIVE_PAGE_SIZE_DEFAULT` is dead code and the test asserts the unreached one — they pass only because the numbers currently coincide [apps/api/src/modules/public-pages/handlers.ts:294 · packages/domain/src/pool/public-read.ts:119]
- [x] [Review][Patch] `isSahyogDriveResponse` validates `fundingOutcome` as a bare `string` while the adjacent `status` check is validated against its literals — any string then reaches `framingFor`, whose `default:` **throws**, defeating the module's own "⛔ NEVER THROWS" guarantee one function later [apps/public/src/lib/sahyog.server.ts:136-159 · sahyog-render.ts:128-146]
- [x] [Review][Patch] `isSahyogDriveResponse` accepts an empty-string name against a `z.string().min(1).nullable()` contract — `''` is truthy-distinct from `null` and renders as a present-but-blank name cell [apps/public/src/lib/sahyog.server.ts:149]
- [x] [Review][Patch] ⟲ Unknown query params render a normal 200 and mint unbounded edge-cache keys — and `buildSahyogRejectionView`'s doc-block **falsely asserts** that `?format=csv` produces byte-identical rejection output. The behaviour was deferred by the first pass (line 742); ⭐ **the false doc claim is new** and is a family-10 closure-honesty nick [apps/public/src/lib/sahyog-render.ts:363-370 · apps/public/src/pages/sahyog.astro:152-153]
- [x] [Review][Patch] `page.intro` claims *"Every drive this trust has run"* / *"सभी अभियान"* while the predicate deliberately excludes `spawned` and `live` — an overclaiming headline on the one surface whose whole premise is that its statements can be checked [packages/i18n/locales/{en,hi}/sahyog-drive.json:2]
- [x] [Review][Patch] The table captions hard-code a four-item column list that omits the **name** column, the letter-code column and the close-of-cycle column — the caption is the table's accessible name (AC10) and is frozen copy while the columns are computed from the matrix [packages/i18n/locales/{en,hi}/sahyog-drive.json `table.caption.*`]
- [x] [Review][Patch] The filter form carries ⛔ no `lang` input, so a GET submit drops the visitor's locale and can flip the page to English mid-task [apps/public/src/pages/sahyog.astro:286-308]
- [x] [Review][Patch] Dates are formatted **and filtered** in UTC on an India-facing surface — a drive closed 00:00–05:30 IST renders as the previous day and falls out of its own date filter. ⚠ The repo has an established IST convention (`packages/validity-service/src/producer.ts:261` scopes facts to "the current IST calendar year") [apps/public/src/lib/sahyog-render.ts:155-163 · apps/public/src/pages/sahyog.astro:179-180]
- [x] [Review][Patch] The matrix entry omits `renders: true`, which AC7 and Task 4 both name explicitly — behaviourally inert (`matrix.ts:221` defaults it) but the shipped YAML and the story's own test fixture disagree on the surface's shape [packages/contracts/public-pages/public-vs-private-matrix.yaml:427-446]
- [x] [Review][Patch] The handler comment claims `apps/public` *"provably CANNOT decrypt"* — ⛔ overstating what the gate proves. `no-kms-in-public.test.ts`'s own header says it *"says nothing about what `@twt/domain`'s OTHER namespaces transitively contain. What it proves is that no module in this app ASKS for encryption."* ⭐ The gate is honestly self-documented; the handler comment is not [apps/api/src/modules/public-pages/handlers.ts:285-288]

**⏸️ Deferred**

- [x] [Review][Defer] The `member_kyc_profiles` LEFT JOIN carries ⛔ no explicit `pariwar_id` predicate, unlike every correlated subquery in the same module, and it is the join producing Tier-1 ciphertext [packages/domain/src/pool/public-read.ts:389] — deferred, matches the `/members` precedent (`member/directory-read.ts:279,321`); a wrong-tenant ciphertext fails the decrypt closed and RLS covers the read. Revisit if `member_kyc_profiles` is ever re-keyed `(pariwar_id, member_id)`.
- [x] [Review][Defer] `ORDER BY` on a correlated `events_log` subquery forces evaluation across the whole candidate set before `LIMIT/OFFSET`; `DECEASED_DISTRICT` re-nests it and the district filter nests it again [packages/domain/src/pool/public-read.ts:178-206,395] — deferred, pre-existing shape, needs measurement before a fix. ⚠ The module's D7(a) section is meticulous about two other N+1 doors and ⛔ does not mention this one.
- [x] [Review][Defer] App-clock `now` vs DB-clock `occurred_at`: under clock skew a just-closed pool gets a null date **and** drops out of every date-filtered index, silently and consistently across page and total [packages/domain/src/pool/public-read.ts:178-186,313-318] — deferred, mechanism certain but the trigger (NTP drift/VM suspend) is infrastructural and unproven here.
- [x] [Review][Defer] `ASSIGNED_MEMBER_COUNT` is the one fragment with ⛔ no `now` bound, so an archived drive's published `fundingOutcome` can flip after the fact on a surface whose copy calls it a permanent record [packages/domain/src/pool/public-read.ts:209-214] — deferred, the missing bound is certain but ⛔ no production writer appends to `member_pool_assignments` after close was found; reachability unproven ([[feedback_trace_reachability_before_escalating]]).
...
|---|---|---|---|
| 2026-08-26 | 2.1 | **CODE REVIEW — three-layer adversarial pass (Blind Hunter, Edge Case Hunter, Acceptance Auditor), all findings verified against source before acting.** ⭐ **Seven real defects patched:** the pool-code filter's letter-code half was declared (comment + wire contract's `poolLetterCode` field + AC3) but never wired into the SQL predicate — fixed with a new `poolIndexFromLetterCode` inverse (bijective base-26) in `pool/naming.ts`; the district filter had no trim/case-fold, so a correctly-DISPLAYED district was unfindable by filtering on it; `DRIVE_CLOSED_AT`'s `DESC` order had no `NULLS LAST`, sorting a data-anomaly row to the top instead of the bottom; the Archive "permanent record" district resolved the deceased member's LATEST posting at query time rather than as of the drive's close/settle instant, letting a later correction retroactively rewrite a published row (now frozen via `COALESCE(DRIVE_CLOSED_AT, now)`); the filter FORM was broken for ordinary use — `?? undefined` doesn't catch an empty string, so any blank field 400'd, and the native date picker's bare `YYYY-MM-DD` never matched the API's `z.string().datetime()`; five orphaned i18n keys (a `<select>`/"Clear filters" UI that never shipped) removed from both locales; two test-hygiene nits (a hardcoded `25` literal against the file's own anti-pattern warning, a cramped two-fields-one-line edit) cleaned up. ⭐⛔ **One genuine spec-internal tension found and resolved by annotation, ⛔ not code:** AC4's "call `selectCloseOfCycleFraming()`" clause requires that function's `amount` interpolation param — a currency figure this count-only surface is elsewhere forbidden from computing (AC4's own first clause, and `SahyogDriveEntry`'s "never a sum" contract) — so `framingFor()`'s enum-only mapping is the correct shape here, not the re-implementation the clause warns against; AC4 annotated in place, `selectCloseOfCycleFraming()` itself left untouched (shared with Noticeboard + Sahyog Vivran, which DO carry an amount). ⚠ **One patch withdrawn after a user correction:** making `sahyogDrivePublication` optional/defaulted to survive a mobile-rollout gap would have recorded pre-update clients as an explicit DECLINE rather than "never asked" — a false DPDPA consent record — and the premise was moot anyway, since **this code is not in production**; the field stays required, as originally built and documented. ⭐ **Four findings verified as false positives** (all from layers without full project access) and dismissed with the evidence recorded in place: the `evaluateDirectoryAbuse` call is a deliberately-safe bare fire-and-forget (fully synchronous, self-catching, matches the untouched `/members` precedent); D4's decision log DOES carry a `(c)` option (the "D4(c)" test citations were correct); the "duplicated vocabulary-leak coverage" claim named a test that does not exist — only one such test exists, and only one place such a leak structurally could occur. One item deferred as unreachable-by-construction (`classifyCycleOutcome`'s zero-assignee vacuous-`fully_funded` case — assignment rows are written at spawn, before a pool can close/settle). Full diff re-verified clean: `tsc --noEmit`, `eslint`, and all non-DB unit suites (`sahyog-render`, `sahyog-copy`, `sahyog-invariant`, `pool/naming`, `dpdpa-consent-record-atomicity`) pass. Status → `done`. | Claude (code review) |
