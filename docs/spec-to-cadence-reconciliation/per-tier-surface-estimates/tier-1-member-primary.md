...
_**`<TO-BE-AUTHORED-BY-SOLO-BUILDER>`** — Task 7. Note: Solo Builder may choose to decompose this row into per-surface granularity at Task 7 (per Open Question 4 in Story 0.12 file). If decomposed, each surface gets its own row; this aggregate row becomes a derived view. See `estimation-worksheet.md §4` discussion._

| Field | Value |
|---|---|
| `engineer_month_floor` | `<TO-BE-AUTHORED-BY-SOLO-BUILDER>` |
| `engineer_month_ceiling` | `<TO-BE-AUTHORED-BY-SOLO-BUILDER>` |
| `confidence_band` | `pending-Task-7` (expected: mixed — `medium` for signup/contribution loop; `low` for memorial + Sahyog Drive; `medium` for nominee console pending P0-2b synthesis) |
