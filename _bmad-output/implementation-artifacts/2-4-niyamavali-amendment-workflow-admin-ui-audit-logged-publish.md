...
**AC4 — Publish without tone-review sign-off → 409 `tone_review.required`.**
Given a publish attempt without a recorded (resource-bound, non-author, content-current) tone-review sign-off, when the publish endpoint is called, then the request is rejected with a **409** indicating `tone_review.required` (the existing `ToneReviewRequiredError` → 409 mapping); the admin UI surfaces the rejection clearly with the path to resolution (submit for review / await a non-author sign-off).
...
- [x] **Task 6 — Admin UI module `apps/admin/src/modules/niyamavali-admin/` + route + client + hooks (AC1, AC4).**
  ...
  - [x] **AC4 UI:** on a `409 tone_review.required` from publish, surface a clear, non-author-friendly message + the resolution path (submit / await sign-off). The `ApiError` carries `status` + `code` for branching. [Source: apps/admin/src/api/client.ts ApiError L]
...
*(`/bmad-code-review`, 2026-06-21 — second independent pass, after the Codex/`bmad-story-automator-review` pass already recorded under "Senior Developer Review (AI)" below. Layers: Blind Hunter ✓ · Edge Case Hunter ✓ · Acceptance Auditor ✓. 2 decision-needed · 5 patches · 7 deferred · 7 dismissed as noise.)*

- [x] [Review][Patch] **(resolved from Decision 1, option chosen: add a row lock)** Concurrent publish requests on the same draft have no row lock — possible duplicate clause_versions + orphan audit line beyond the scope ADR-0021 documents as accepted — `loadDraftForPublish` does a plain `SELECT` (no `FOR UPDATE`) before `publishGate` runs; two simultaneous publish calls for the same `signed_off` draft can both pass the gate, both call `audit.writeAuditEntry` (on `deps.servicePool`, a separate connection from the scope tx) and both attempt `createClause`/`amendClause`, with only one `markDraftPublished` conditional UPDATE succeeding — the loser's `clause_versions` insert rolls back via the scope tx, but its audit line has already committed on the separate connection. Fix: add `SELECT ... FOR UPDATE` on the draft row in `loadDraftForPublish` to close the race. [`apps/api/src/modules/rules/index.ts:453-461`; `packages/domain/src/niyamavali/drafts.ts:369-405`]
- [x] [Review][Patch] **(resolved from Decision 2, option chosen: correct the wording, doc-only)** AC5's "the NOT-NULL invariant is enforced here" is not backed by a database-level constraint — `clause_versions.audit_id` is declared `uuid('audit_id').references(...)` with no `.notNull()`, and migration 0015 contains no `ALTER COLUMN audit_id SET NOT NULL`. The invariant holds only because the publish route always supplies a pre-generated `auditId`; any non-route write path (a script, a future domain-direct call, a backfill) could still insert a null-`audit_id` row without the database objecting. Fix (doc-only, no schema change): amend AC5's Completion Notes / ADR-0021 to state explicitly that the invariant is app-level-enforced (the publish route always supplies `auditId`), not a structural DB guarantee — do not leave the "enforced here" claim unqualified. [`packages/domain/src/schema/clause_versions.ts:147`; `packages/domain/migrations/0015_strong_cerise.sql`; `docs/adr/ADR-0021-niyamavali-draft-publish-workflow.md`]
- [x] [Review][Patch] `GET /clauses`, `GET /clauses/:clauseId/versions`, and `GET /amendments` are gated to `niyamavali.amend` only, contradicting the route module's own header comment ("reads accept niyamavali.amend OR niyamavali.review — the non-author reviewer must load content") and AC1(d). A trustee holding only `niyamavali.review` cannot browse the published registry, a clause's version history, or the amendment ledger while reviewing a pending amendment — this is the same class of bug the prior review pass already fixed for `GET /clauses/drafts`, just not applied to these three routes. Fix: swap `preHandler: [session, scope, amend]` → `[session, scope, read]` on all three (matching `GET /clauses/drafts` and `GET /clauses/drafts/:draftId`, which already use the `read` gate). [`apps/api/src/modules/rules/index.ts:165,185,213` vs `:233,312`]
- [x] [Review][Patch] The orphan-audit window for amend-publish is needlessly widened by a validation that runs after the audit write, and is reachable via an ordinary edit (not just a race) — `updateDraft` lets a caller clear `affectedMemberScope` to `null` on an *amend* draft with no operation-aware guard (the `null` "passes through" by design for clearing). If that draft is then submitted, signed off, and published, the publish handler's `if (!scopeDecl) throw DraftStateError(...)` check only fires *after* `audit.writeAuditEntry` has already committed — a 100%-reproducible orphan audit line, no concurrency required. Fix: (a) reject (or require re-supplying) a null `affectedMemberScope` when `updateDraft`'s target is an amend-operation draft, and/or (b) move the `!scopeDecl` guard in the publish handler before the audit write. [`apps/api/src/modules/rules/index.ts:507-543`; `packages/domain/src/niyamavali/drafts.ts:196-199`]
- [x] [Review][Patch] `GET /clauses/:clauseId/versions` accepts an unvalidated `clauseId` path param — `ClauseParam` declares `clauseId: z.string()` with no format constraint, and the handler does an unchecked `clauseId as ReturnType<typeof ids.clauseId>` cast, unlike body-validated routes elsewhere that go through the branded `ClauseIdSchema` regex. A malformed clause id silently returns an empty array instead of a clear 400. Fix: tighten `ClauseParam.clauseId` to reuse the same branded schema/regex used for `ClauseId` elsewhere in the contracts package. [`apps/api/src/modules/rules/index.ts:72,194`]
- [x] [Review][Patch] `requireNiyamavaliReadAccess` duplicates the `authz.denied` audit-emission shape inline instead of reusing the existing pattern already used by `requirePermissionHook` (`apps/api/src/modules/rbac/index.ts:93-95,179-181`) — a maintenance/consistency risk if the standard authz-denial audit-event shape changes elsewhere and this hand-rolled duplicate silently drifts out of sync. [`apps/api/src/modules/rules/index.ts:114-133`]
- [x] [Review][Patch] `buildDraftBody` in the admin derive layer uses `as CreateDraftBody` type-assertion casts on both the create and amend return arms, silencing the compiler's ability to catch a missing/extra field on either arm of the discriminated union (e.g. a future required field added to `CreateClauseDraftRequest` would only surface as a runtime 400 from the server's `.strict()` Zod schema, not a build-time error). Fix: restructure so each branch is independently structurally assignable to its member type, or use a discriminated-union-safe builder helper. [`apps/admin/src/modules/niyamavali-admin/derive.ts:99,107`]
- [x] [Review][Defer] No client-side permission gate on the Sign-off / Publish buttons — rendered for every trustee regardless of `niyamavali.review`/`niyamavali.amend`, relying entirely on the server's 403/409 to reject. The route's own comment states this is intentional ("server is authoritative"); flagged as a UX/audit-trail follow-up (hide/disable client-side based on grants, matching the status-based `isPublishable`/`awaitsSignoff` gating already present), not a security defect. [`apps/admin/src/modules/niyamavali-admin/NiyamavaliPage.tsx`]
- [x] [Review][Defer] `ToneReviewSignoffRequest.confirm: z.literal(true)` provides no genuine assurance — the admin client always hardcodes `{ confirm: true }` with no checkbox/checklist UI affordance, so the field satisfies the schema without capturing a real acknowledgment that the reviewer applied the tone-review checklist. No AC violation (AC4 is satisfied); follow-up UX work to add a real confirmation affordance. [`packages/contracts/src/rules/clause.ts`; `apps/admin/src/api/client.ts`]
- [x] [Review][Defer] `listClauses`/`listAmendments` use a flat `LIMIT` with no cursor/offset or `hasMore` signal — once a Pariwar exceeds the cap, the registry/amendment admin views silently truncate with no "load more" affordance and no way for a trustee to know more rows exist. Not an AC violation (pagination not specified); out of this story's stated scope ("create+amend+publish UI only"). [`packages/domain/src/niyamavali/read.ts`; `apps/admin/src/modules/niyamavali-admin/NiyamavaliPage.tsx`]
- [x] [Review][Defer] Dense `as unknown as` casts at the DTO boundary (a dozen+ instances across the publish/response-mapping code) — justified by an existing code comment as an "established convention" across the codebase. A real type mismatch between a domain brand and a contract brand would be silently accepted by the compiler. Pre-existing pattern, not introduced by this story; broader refactor (e.g. consistent `Schema.parse()` at boundaries) is out of scope here. [`apps/api/src/modules/rules/responses.ts`; `apps/api/src/modules/rules/index.ts`]
- [x] [Review][Defer] `recordDraftSignoff`'s `reviewedAt` is supplied by the app-server clock (`deps.clock()`) rather than DB-authoritative time, unlike every other timestamp this story is careful to leave DB-default (`createdAt`/`updatedAt`, the deliberately-left-to-default `resolveByClauseId` asOf). No AC violation; low-severity discipline inconsistency that could drift under app/DB clock skew. [`apps/api/src/modules/rules/index.ts:437`]
- [x] [Review][Defer] Admin UI diff-panel / selected-draft staleness — `selectedDraftId` is not cleared when the selected draft transitions state via another action, and `useSubmitForReview`/`useSignoffDraft` invalidate the clause/draft list queries but not the diff query for the affected draft, so the diff panel can keep showing a stale or orphaned query after a lifecycle transition. Display-only staleness, no data-integrity impact; a page refresh or re-selection clears it. [`apps/admin/src/modules/niyamavali-admin/NiyamavaliPage.tsx`; `apps/admin/src/api/hooks.ts`]
- [x] [Review][Defer] `NiyamavaliPage`'s `pariwarId` prop falls back to an empty string when route params lack it (`params.pariwarId ?? ''`), which would issue API calls against a malformed empty path segment and surface a confusing generic 404 rather than a clear routing error. Low likelihood given TanStack Router's typed params should guarantee presence at this route; low-priority hardening. [`apps/admin/src/routes/NiyamavaliRoute.tsx`]

**Dismissed as noise (7):** (1) Blind Hunter's "missing `await` on `recordToneReviewSignoff`" — false positive, the function is explicitly documented + typed as synchronous, fire-and-forget, never-throw by design (Story 2.2's seam contract), not part of this diff. (2) Migration 0015's De2 trigger `SECURITY INVOKER` residual gap — already documented in the migration comments, ADR-0021, and deferred-work.md as a named re-trigger condition; not a new actionable finding. (3) Create-draft clause-id-conflict pre-check race ("no conflict" at draft-create time can be stale by publish time) — both reviewers who raised it concede it's acceptable per the route's own comment ("publish re-checks authoritatively"), and the real data-integrity race is correctly caught by the DB partial-unique index. (4) AC5 publish-sequencing deviation from the Dev Notes' described order — the Acceptance Auditor itself confirms this is "not itself a defect," an intentional and arguably better sequencing already documented in ADR-0021 §4 and the Dev Agent Record. (5) Notification-hook async-rejection guard — the hook is an intentionally synchronous no-op placeholder per AC3 ("ships the seam + call site, NOT delivery"); guarding against a hypothetical future async implementation is premature. (6) `buildDraftPatch`'s form-vs-stored-`operation` divergence concern — verified unreachable: `DraftForm`'s operation `<select>` is `disabled={isEditing}` and is populated from `editingDraft.operation` via `draftToFormFields` on every `editingDraft` change, so it cannot diverge through the UI. (7) Duplicate `editingDraft` guard between `DraftForm` and `NiyamavaliPage` — the Edge Case Hunter itself notes this is "not possible via UI," flags only a duplicated implicit contract with no reachable gap.
...
- **Audit-or-throw publish (AC2/AC5):** the append-only amendment ledger cannot back-fill `audit_id`, so the publish writes the audit line FIRST and passes it (plus a PRE-GENERATED `clause_version_id`) into `createClause`/`amendClause` so BOTH the version row AND the amendment row carry `audit_id` non-null at INSERT, and the single audit line's hash commits the full provenance (diff + reviewer + clause_id + clause_version_id). `createClause`/`amendClause` gained an optional, backward-compatible `clauseVersionId` param to enable this. Orphan-audit-on-rollback is the accepted edge (documented in ADR-0021). **Correction (code review, 2026-06-21):** AC5's "the NOT-NULL invariant is enforced here" is **app-level-enforced only** — the publish route always supplies a pre-generated `auditId`, but `clause_versions.audit_id` has no DB-level `NOT NULL` constraint (left nullable in migration 0015, since pre-2.4 domain-direct creates / the structural seed may predate this audited route). This is not a structural DB guarantee the way AC7's immutability trigger is; a future non-route write path could still insert a null-`audit_id` row without the database objecting.
