...
group. **G1 (`packages/domain`, 17 files / +2,213) and G2 (`apps/api` + `packages/contracts` + `openapi`,
19 files / +2,288) have each been reviewed by all three layers** — their findings are in the two
sub-sections below. **G3** (`apps/public` + `apps/admin` + `packages/i18n` + `scripts`, 19 files /
+1,998) and **G4** (governance docs, 6 files / +1,497) are ⛔ **NOT REVIEWED** — ⛔ 4,286 lines and
⛔ 25 files of this story have had ⛔ no third-pass review at all, including the ⭐ **admin console**
and the ⭐ **public Astro render layer**, where checklist family **13** (semantic accessibility, ⛔ un-mechanized BY RULING) lives. ⇒ ⛔ **Do not read this section as
...
**Outcome (G1 only):** 27 raw findings across three layers → **3 merged duplicates → 24 surviving,
3 dismissed.** **1 decision-needed** · **16 patch** (⛔ none applied — all are action items) ·
