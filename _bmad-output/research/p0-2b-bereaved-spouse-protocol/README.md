...
**90-day window (D-05):** If no trustee-mediated path identifies a willing candidate within 90 days of the first `approved-for-recruitment` trustee-review-log row, a formal escalation meeting with the Trustee Panel is required per ethics-protocol §3.0. **Broad approval scope (D-06):** A single Trustee Panel `approved-for-recruitment` verdict covers all four enumerated paths generically — a separate per-path approval is not required unless the Trustee Panel explicitly restricts scope in the `sign-off_note`.
...
2. **Informed consent before recording-or-notes.** The interview MUST have informed consent (per ethics-protocol §2 (a)-(h) + §2-bis re-consent-for-quotation checkbox) BEFORE any recording or note-taking activates. Verbal reconfirmation of recording consent is required at the start of every recorded session. **Verbal-only consent (D-03):** if the spouse declines to sign but confirms verbally, researcher explicitly confirms verbal consent aloud before any note-taking begins; conversation is treated as informational only (notes destroyed immediately; not cited as research in synthesis). **Principal-only (D-04):** only the bereaved spouse may consent — no guardian proxy.
...
8. **Pattern 4 evaluation section must populate per-sample verdict.** The Pattern 4 evaluation §4 of synthesis must populate the per-sample-copy verdict for **each of the 8 sample-copy rows** in UX spec §12 Pattern 4 table at lines 2349-2360 + the 7 cross-cutting grief-grammar rows OR explicitly mark `not-evaluated-due-to-spouse-non-engagement` with rationale (Pattern 4 sample-copy review is opt-in mid-interview per ethics-protocol §3.7). Honest non-engagement is **anticipated as a possible outcome and is explicitly permitted** — it is not a framework failure. When non-engagement occurs, the divergence-log records the gap per pattern-4-evaluation-worksheet.md §6-§7 non-engagement protocol, and the assumption-inventory rows for affected assumptions move to `not-evaluated-due-to-spouse-non-engagement` (P-08 coverage-gap acknowledgment). Silent omission (no verdict AND no non-engagement marker) is the only forbidden state.
...
- **Emergency approval extension (D-02):** If the 30-day chair-only emergency approval window is about to expire without a second trustee being available for re-review (e.g., second trustee travel, illness), the Trustee Panel chair may log an `emergency_approval_expiry_date` extension in the trustee-review-log row + a `second_trustee_re_review_status` field noting the reason for delay. The extension is valid for a single additional 30-day period only; after 60 days from chair sign-off, a full Trustee Panel re-convening is required regardless.
- **Rejected-pending-rework scope (P-23):** If post-synthesis verdict is `rejected-pending-rework`, the trustee-review-log `revision_list` column must specify whether the rework requires: (a) synthesis revision only (Tasks 8-9 cycle), (b) framework revision (Tasks 1-6 cycle), or (c) full re-recruitment (Tasks 7-9 cycle). This scope specification prevents ambiguity about how far back the rework cycle must reach.
