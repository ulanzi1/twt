...
3. **Branch protection at mirror** — enforced per Q3.5 ratification. GitLab project settings: no force-push on `main` + `release/*` branches; signed-commits required; ≥1 trustee approval for any direct write to protected branches. The auto-mirror workflow's push from GitHub Actions is the sole permitted automated write path; manual writes by trustees are gated on branch protection.
