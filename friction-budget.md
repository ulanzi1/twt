...
| member (a fresh `nominee_change` step-up OTP on EVERY nominee RE-declaration once they have left the signup wizard — on the signup route `POST /member/nominees` as well as the Life Events route; the first-ever declaration and the wizard's own back-navigation re-declares are exempt) | ⭐ Conformance to AR-24, which already names "nominee change" among the step-up operations — the signup route accepted a re-declaration on the member session ALONE, so any active member could skip the Life Events step-up (Story 6.20 D9, T1). What it protects: the period BEFORE a claim, when the declaration is still freely changeable (`2026-09-20-234` V) and the step-up is the only guard. ⚠ It NARROWS, ⛔ does not close, the Ravi-mode window — the filer's session IS the deceased's, and which phone receives that OTP is recorded as untraced (Story 6.20) | forced |
| family / member (asking for a CORRECTION to a locked nominee declaration — the nominee's details re-entered, a note saying what was entered incorrectly, a `nominee_change` step-up, then a wait for TWO approvals: the District Admin, then the Pariwar Admin) | The as-at-death rule's integrity: once a claim is filed the declaration is LOCKED (`2026-09-20-233`, `-234` V), and a correction INHERITS the corrected version's position in time — so it could back-date a DIFFERENT person into a pre-death slot, and the two human approvals, with the target's relationship shown beside the proposal, are the ONLY defence (`-236` Z; T15). ⚠ A nominee declared as "Other" can ⛔ never be corrected (`-237` cl.2): the member is told so AT THE PICKER, before they choose, rather than discovering it after a death (Story 6.20) | optional |
