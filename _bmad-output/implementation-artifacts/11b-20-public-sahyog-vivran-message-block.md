...
_Code review 2026-09-19 (`bmad-code-review`, scope: full branch diff `main...HEAD`, 13 files, governance included; Blind Hunter + Edge Case Hunter + Acceptance Auditor, all three completed). ⭐ The Auditor found ⛔ no AC violation and ⛔ no REAL GAP in any load-bearing-invariant family the diff touches (families 3, 6, 9, 10, 11, 13 walked; 1, 2, 4, 5, 7, 8, 12 untouched). 0 `decision-needed`, 3 `patch` (⭐ all three APPLIED the same day, see the Change Log), 4 `defer`, 15 raw findings dismissed. ⚠ `ci:local` and the Dev Record's planted-defect "teeth" were ⛔ NOT re-run by this review._

- [x] [Review][Patch] An invisible-only or bidi-laden nominee holder name builds a visually BLANK "Nominee full name" column [apps/public/src/lib/sahyog-vivran-message-block.ts:88] — the selector's own rule is "an absent value drops its column, never a blank" (`-214` Consequence 6), but `selectColumns` tests `nomineeName !== null` only. The API maps a nominee's `accountHolderName` to `null` only for the decrypt sentinel or `length === 0` — ⛔ it is never run through `normalisePublicName` (which the deceased name and the district ARE, `handlers.ts:920`, `:1209`). Intake is `z.string().trim().min(1)`; `trim()` keeps U+200B, so a zero-width-only name is reachable. Same defect class the API comment at `handlers.ts:1118-1120` already names ("an omission that ANNOUNCES itself"). Fix: strip invisibles/bidi in the selector and drop the column when nothing visible remains; add a test with `"\u200b"` and an RLO-wrapped name. ⚠ The API-side half is deferred (see the Defer below).
- [x] [Review][Patch] Stale test title asserts a falsehood [packages/i18n/tests/sahyog-shared-dark-copy.test.ts:514] — `describe('⛔⛔ AC6 — ⛔ NOTHING RENDERS THE MESSAGE BLOCK. The copy exists; the render may ⛔ not.')` still reads as the pre-`11b-17`/`11b-20` state; its body and the `$comment.message_block` strings were rewritten to authorise TWO render sites (`AUTHORISED` at :620). Retitle to the surviving property (a `message_block.*` key is resolved only at the two authorised sites, only where every token is supplied). Line 180's title is about the INDEX line's tokens, ⛔ not this block — leave it.
- [x] [Review][Patch] The new section's accessible-name link is untested, and its table is unnamed [apps/public/src/pages/sahyog-vivran/[driveToken].astro:931-950] — Family 13(a) is covered by construction only: the contributors section pins its `aria-labelledby`/`id` pair (`sahyog-vivran-a11y.test.ts:59-60`), the new pair (`sv-message-headline`) has no pin, and the checklist rules family 13 un-mechanized, so this decays silently. Also the `<table>` has no name (`<caption>` or `aria-labelledby`) and sits BEFORE its labelling `<p>`. Fix: add the same two-regex pin for `sv-message-headline`, and name the table with `aria-labelledby="sv-message-headline"` (⛔ no new copy is authored). ⚠ Judgment call left open: the headline is a `<p>`, ⛔ not an `<h2>` like the other sections, so heading navigation skips the block. Not a WCAG failure (the region is named, `<th scope="col">` is present); change it only if you want it in the heading outline.
- [x] [Review][Defer] The public drive LIST still renders `sahyog-drive:consent.note` ("A family may choose whether their relative is named here…"), which `-160` cl.6 made false [apps/public/src/pages/sahyog.astro:898] — deferred, pre-existing. Recorded in `-223` Consequence 3 and this story's Trap 1 as "found, NOT addressed", ⛔ but ⛔ not in `deferred-work.md` and ⛔ owned by no story or routing note. It sits on the page one click from the new block. ⚠ What the Trust tells families about their own disclosure is the Panel's/BigDev's to route, ⛔ not this review's.
- [x] [Review][Defer] `message_block.table.nominee_name` / `.district` Hindi labels reach the PUBLIC for the first time here [packages/i18n/locales/hi/sahyog-shared.json] — deferred, pre-existing. §8.1 gave ⛔ no Hindi for the table; `11b-19` lifted the Panel's own nouns from §9.2's ratified HI index line and recorded the gap ("a future routing note's confirmation") ONLY in the `$comment`; ⛔ no `deferred-work.md` entry carries it. This diff is the first public render.
- [x] [Review][Defer] The public API never passes `accountHolderName` through `normalisePublicName` [apps/api/src/modules/public-pages/handlers.ts:965] — deferred, pre-existing. The root of the first Patch: the existing bank group renders the same value the same way, so an invisible-only name is a blank cell there too. Changing it moves the wire contract and another surface, ⛔ not this story's diff.
- [x] [Review][Defer] A 200-character unspaced holder name overflows the new table at phone width [apps/public/src/pages/sahyog-vivran/[driveToken].astro:933] — deferred, low. `<table class="mb-4 w-full …">` with `<td class="pr-4">`; intake caps at 200 with no word-break rule. Layout only, and needs a real CSS choice (`overflow-wrap:anywhere` — Tailwind's `break-words` does ⛔ not shrink a table cell's min-content).
...
|---|---|---|---|
| 2026-09-19 | 1.1 | **Code review (`bmad-code-review`, full branch diff): 0 decision-needed, 3 patches APPLIED, 4 deferred; Status → `done`.** **(1)** `selectColumns` now drops the Nominee column for an invisible-only or bidi-only holder name (`visibleHolderName`: the strip is a TEST on a copy, ⛔ the joiners stay, bidi controls come off), with two tests, one of which pins the Devanagari joiner. **(2)** The `AC6` dark-copy `describe` retitled to the surviving property (two authorised sites). **(3)** The message-block table is named by the headline (`aria-labelledby`, ⛔ no new copy) and the section/table/headline id trio is pinned. ⭐ Teeth: with `visibleHolderName` bypassed, exactly the two new holder-name tests fail. ⭐ Re-run: `apps/public` 670/670 (29 files), i18n dark-copy 19/19, financial-truth gate + its 36 tests, `astro check` 0/0/0, `eslint` clean. ⚠ `ci:local` was ⛔ NOT re-run after the patches. Four defers recorded in `deferred-work.md` (the `consent.note` on `/sahyog`, the Hindi table labels, the API never normalising the holder name, a long-name overflow). | BigDev + Claude |
