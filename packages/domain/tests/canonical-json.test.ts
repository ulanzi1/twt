// Authoritative canonical-JSON suite — co-located with the implementation after
// the DD-1 / Story 1.10 move from @twt/events. These RFC 8785 JCS cases are
// load-bearing: the audit-log hash chain (computeAuditHash / verifyChainSegment)
// and event-replay determinism both depend on byte-stable output. The mirror
// suite in packages/events/tests/canonical-json.test.ts exercises the @twt/events
// re-export shim (proving the backward-compat surface is intact).
...
describe('canonicalJsonStringify', () => {
  ...
  it('throws on BigInt (not representable in JSON / RFC 8785)', () => {
    expect(() => canonicalJsonStringify(10n)).toThrow(TypeError);
  });

  it('throws on undefined and on Date (must be ISO-stringified first)', () => {
