...
// graduated `push` (fcm/apns → real firebase-admin), so the REMAINING stubs are whatsapp-business / sms-dlt
// / telegram (real integration lands in 5.3–5.6). Each implements the `ChannelProvider` port with NO real
// SDK and NO network; swapping a stub for a real implementation of the SAME interface never changes the
// dispatcher.
//
...
// (firebase-app.ts — firebase-admin manages the service-account JWT→access-token refresh internally); the
// remaining stubs (whatsapp-business / sms-dlt / telegram) still need theirs:
//   • Auth-lifecycle refresh — WA partner JWT / telephony tokens must be auto-refreshed + verified. The
//     real provider plugs a token-refresh hook in here.
