...
#
# Runs every CI job that needs no external service (14 jobs), plus the live-DB
# `integration-tests` job (the 15th) when DATABASE_URL is set. Stopgap while
...
# Usage:
#   pnpm ci:local
#       → 14 static jobs (lint, typecheck, build, unit test, + all gates)
