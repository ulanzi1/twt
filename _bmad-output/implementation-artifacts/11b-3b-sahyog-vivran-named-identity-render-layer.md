...
<!--
⭐⭐ **CLOSED 2026-09-17.** ⚠⛔ **AND THE BAR WAS SET BY THIS STORY'S OWN HISTORY:** it went `done` once
before, on 2026-09-15, over a **RED TREE** whose repair was sitting uncommitted. ⇒ ⭐ this closure is
stated against evidence, ⛔ not against a narrative.
  · ⭐ **`ci:local` PASSED — 34/34**, on the PLAIN `DATABASE_URL` (⛔ no override).
  · ⭐ **Every AC and every task is checked**; ⛔ zero `- [ ]` items remain in this file.
  · ⭐ **`-219` cl.1/cl.3/cl.4, cl.5(a) and cl.6 are BUILT**, and cl.2/cl.5(b) are record clauses that
    needed no code. ⭐ `-220` discharges the Hindi follow-up.
  · ⭐ **A THIRD, ADVERSARIAL pass ran against this session's OWN patches** (2026-09-17) and found 12
    defects — including a gate blind to the `.astro` template half, a normaliser that CORRUPTED
    Devanagari legal names, and a systemic-failure path that published as lawful mass erasure. ⭐ All
...
⚠⛔⛔ **WHAT `done` DOES ⛔ NOT MEAN HERE — ⛔ do ⛔ not read these as closed:**
  ...
  · ⚠ The gate is a **TRIPWIRE**, ⛔ not a proof: an aliased const, a cross-statement product, a
    computed literal and any third-module indirection remain uncaught, and it fires ⛔ only on
    `renderPath: true` files. ⛔ Never cite it as coverage of D1(c)'s *"anywhere in this app"*.
-->
...
⛔ it is ⛔ no longer *"awaiting a ruling"* — it is ⭐ **IMPLEMENTATION OWED BY THE RULING**:
  · ✅ **cl.1/cl.3/cl.4 — DONE 2026-09-16.** ⭐ **THE HINDI IS `एक सहकर्मी`** — which UNBLOCKED this.
    ⚠⛔⛔ **CORRECTED 2026-09-17: THIS READ *"THE HINDI WAS RULED"*, WHICH READS AS A PANEL ACT.**
    ⛔ It was ⛔ not. It is a **BigDev author-commit**, recorded a day LATE as
    `#decision-2026-09-17-220`; ⛔ no Panel saw the word, and `-219`'s follow-up still said
    *"⛔ NOT RULED"* while it shipped. ⭐ The COPY CALL was BigDev's to make; ⛔ the attribution was
    wrong ([[feedback_verify_before_committing_governance_claims]]). ⚠⛔ **`सहकर्मी` is ⛔ NOT a new word:** it is the SHIPPED Hindi for *colleague* on
    ...
    **`sahyog-vivran.value.contributor_unnamed`**. ⚠⛔ **THE NAMESPACE IS A FENCE BY CONVENTION, ⛔ NOT BY STRUCTURE** — corrected 2026-09-17, having
    first been written as *"can ⛔ not reach this key"*. `t()` takes the namespace as a RUNTIME
    STRING and `catalog.ts` registers every namespace globally, so any consumer *could* resolve it.
    ⭐ What IS structural is narrower: `packages/ui/src/contribution-list/view-model.ts` types its
    refs as `namespace: 'contribution'`, so the PRESENTER cannot emit a `sahyog-vivran` ref. ⭐ And
    the scope claim holds — this work touches ⛔ no `packages/ui` or `apps/mobile` file.
    ...
    row holding ⛔ no name.
    ⭐ **Three API tests INVERTED, ⛔ none deleted** — ⚠ TWO were re-titled and the third kept its title
    (this said *"by name"* of all three); ⭐ all three QUOTE the assertion they replace, each quoting the superseded assertion.
  ...
  · ✅ **cl.5(a) — DONE 2026-09-16.** `isAmountDerivation` gained a SECOND leg: a `COUNT_OPERAND`
    times a **numeric literal**, symmetric on operand order, parenthesis-tolerant. ⭐ **Four new tests** (the suite went 20 → 24; ⚠ this said THREE), **and the ISOLATION is proven, ⛔ not asserted:** reverted to `HEAD`'s `lib.ts` the two
