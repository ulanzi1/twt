...
- [x] [Review][Defer] Fail-soft design allows a temporary race between disclosure rendering and user interaction during validity loading [apps/mobile/app/(contribution)/pay.tsx:120-121] — deferred, accepted architectural trade-off. `disclosure` stays `null` while `useMemberValidityQuery` is in flight (no `isLoading` is destructured), and Task 3 explicitly forbids gating the pay CTA on this read. **Reason for deferring:** Task 3 intentionally separates informational disclosure from payment authorization. A brief load-time race is accepted because delaying or gating payment on validity retrieval would change the story's architectural contract rather than fix a defect. The only test touching this (`pay-screen-disclosure-render.test.ts:98-105`) asserts the absence of an `isLoading` destructure that was never introduced in the first place—a hollow guard rather than a meaningful regression check. Tightening that test remains worthwhile in a future pass even though the underlying race is an accepted design trade-off.
