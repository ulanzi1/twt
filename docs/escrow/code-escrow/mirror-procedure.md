...
- **Mirror destination ADR-ratified.** [ADR-0002 code-escrow-mirror-destination](../../adr/adr-0002-code-escrow-mirror-destination.md) ratified 2026-06-05 per `.decision-log.md` Decisions 2026-06-05-019 + 2026-06-05-034. Destination platform = GitLab.com under trustee-owned foundation account; credential model = SSH deploy key (push-only scope); replication mechanism = `git push --mirror` from GitHub Actions; branch-protection rules = no force-push + signed-commits required + ≥1 trustee approval for write on `main` + `release/*`.
