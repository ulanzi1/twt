...
⚠ **ANNOTATED at code review, 2026-09-19 (⛔ the original text is KEPT):** the `"Sunita ."` clause is ⛔ **not
what shipped, and ⛔ not what D3 rules.** Under `shielded_name` the public form of `"Sunita ."` is `null`
(one token survives cleaning ⇒ a mononym ⇒ withheld), and the member sees `Sunita` — the ONE named place the
member sees MORE than the public (D3, the mononym arm; the same divergence as `Ravi` below). Under `full_name`
the two are equal (`Sunita`). The live-DB parity loop names the two `shielded_name` exceptions in an explicit
set (`MEMBER_SEES_MORE`) instead of skipping every null public form.
...
- [x] **Task 4 — API handler** (D3, D4, D7, AC1, AC2, AC7, AC10)
  ...
  - [x] Keep logs member-attributed and **name-free** ([[project_anonymous_diagnostic_log_convention]]).
        ...
        ⚠ ANNOTATED at code review, 2026-09-19 (⛔ the text above is KEPT): the FAULT arms (decrypt failure, resolver failure) still log per row and member-attributed. The LAWFUL withheld arms (no profile, erasure sentinel, empty-after-normalise) are now COUNTED and logged ONCE per request with ⛔ no member id (Round 2 finding 2) — a per-row line at the default `info` level named an erased member on every poll of every pool member. ⛔ No cause-per-row line reaches a client on either path.
...
- [x] **Task 9 — Verify** (AC4, AC9, AC10)
  ...
  - [x] Then `pnpm ci:local`, which runs on push. Report the job count.
        ⚠ Satisfied for `b5aec519` only — ⛔ not re-run after the code-review patches (see Verification's SCOPE note).

---

### Review Findings

`bmad-code-review` 2026-09-19 · full branch diff `a44aef23..b5aec519` · layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor (all three completed; none mutated the tree) · **0 decision-needed, 7 patch (ALL 7 APPLIED same day, 2026-09-19), 4 defer, 18 dismissed.**
⭐ Panel-routing gate (§0) applied: no finding is the Panel's. The two disclosure-shaped items (Nominee Console, mononym) were already recorded in this story and are carried as defers.

- [x] [Review][Patch] The outage / bad-KEK consequence is ⛔ not re-marked for the MEMBER route — Two `deferred-work.md` items ("Behavioural cost of classify-by-error", "UN-ATTESTED: Cloud KMS's status … INVALID_ARGUMENT") were written for the PUBLIC route and were ⛔ not extended, though only the timing item was. On the member route the same facts now mean: ONE row wrapped under a DISABLED/DESTROYED KEK version, or (if the un-attested premise is wrong) ONE corrupt envelope, sends the whole list to `{ assigned:false }` for every member of the pool, which the client renders as *"You have no live pool right now."* (`contributor_list.no_pool`) to a member who IS assigned. Before this story that row was silently dropped. Re-mark both items to cover the member route and state the member-facing text; ⛔ do not claim it closed. (blind+edge+auditor F2) [`_bmad-output/implementation-artifacts/deferred-work.md` ~L46-47]
- [x] [Review][Patch] The name resolver runs OUTSIDE any `try` on the member route — `publicNameTokens` / `resolveMemberFacingDeceasedName` / `normalisePublicName` sit after the decrypt `catch`, so a throw on one pathological stored name takes the whole list to `{ assigned:false }`. The public route wraps the identical call in a per-row `try` and records "the asymmetry was not defensible either way round" (second pass); the contributor arm there returns `{ name:null }`. Wrap the resolver+normalise call, log by action name (member id only), return `{ name: null }` — the row stays (`-222` cl.1) and it is ⛔ never an outage. Add a live-DB or unit leg that forces a throw. (edge) [`apps/api/src/modules/member-pool/handlers.ts:1209-1214`]
- [x] [Review][Patch] The AC5 "three rendered rows" test re-implements the component instead of exercising it — `contributor-list-render.test.ts` `Story 11b.21 — a valid payload renders EVERY row` rebuilds `label = vm.displayName.kind === 'name' ? … : t(…)` and the a11y string inline; the only tie to `PoolContributorList.tsx` is source-regex pins. A regression that maps a row to `null` or drops the placeholder arm while the strings stay would leave it green. "A stub is a second source; it must call, never transcribe." Extract the derivation from `renderableRows` to an exported pure helper the component AND the test both call. (auditor F1; checklist 13(d) rests on it) [`apps/mobile/tests/unit/contributor-list-render.test.ts:727-749`, `apps/mobile/components/contributor-list/PoolContributorList.tsx:128-160`]
- [x] [Review][Patch] Stale prose whose reasoning is now reversed — the per-row `catch` comment still says an erased contributor "is omitted, 11b.2a's D5" so a render failure is "indistinguishable from a LAWFUL ERASURE" (erasure is now a placeholder row); and `sahyog-vivran-message-block.ts` still cites the API's `INVISIBLE_CHARS` at `handlers.ts:1257-1269` (moved to `kyc/name-render.ts`). Annotate the first as SUPERSEDED (quoted, not deleted — the file's convention) and repoint the second. (blind+edge+auditor) [`apps/mobile/components/contributor-list/PoolContributorList.tsx:144-151`, `apps/public/src/lib/sahyog-vivran-message-block.ts:73`]
- [x] [Review][Patch] AC1 is ⛔ not amended in place — AC1's text says `"Sunita ."` returns "the same as public", but the live-DB test asserts member `Sunita` against public `null` under `shielded_name` (the D3 mononym arm), disclosed only in Completion Notes; the parity loop also skips any case where the public form is null. Annotate AC1 in place (the story's own lesson), and name the skipped cases in the test. (auditor F3) [story AC1 `:364-377`; `apps/api/tests/integration/contributions/pool-contributors-rtbf.spec.ts`]
- [x] [Review][Patch] Rewritten tests missing the AC10 "Was:" note — `actually invoked: a Devanagari name passes through…`, `actually invoked: whitespace is neither trimmed…` (contributor-list-render.test.ts); `the ROW presenter emits row_a11y (and, for an unnamed row, the placeholder)…` and the `(d) runtime keys` assertion (presenter.test.ts). (auditor F5) [`apps/mobile/tests/unit/contributor-list-render.test.ts`, `packages/ui/tests/contribution-list/presenter.test.ts`]
- [x] [Review][Patch] One `warn` per withheld row per member per 60 s poll, naming the erased member — erasure and "no profile" are lawful states, not warnings, and the removed pre-filter used to drop erased members silently; now each poll of each pool member logs a `warn` with `memberId` for every erased row (log volume ∝ members × withheld rows; a cause-attributed, member-attributed record of an RTBF subject). ⛔ Nothing reaches a client, so `-222` cl.2 holds. Downgrade the lawful arms (erasure sentinel, no profile, empty-after-normalise) to `info`/`debug` or emit one per-request counts line; keep `warn` for decrypt failure. (blind+edge) [`apps/api/src/modules/member-pool/handlers.ts:1173,1184,1194,1216`]
- [x] [Review][Defer] Any non-INVALID_ARGUMENT KMS error blanks the whole member list to `assigned:false` (false "no live pool" for an assigned member; 60 s poll can flip good data to that view) — deferred: ruled by `-224` D4 / AC7 and mirrors the public route's outage precedent; the fail-soft shape is the member routes' standing contract. Changing it needs an outage-distinguishing wire state AND new copy — a product call (D5: "no new copy"), not this story's. The record gap is the first patch above. (blind+edge) [`apps/api/src/modules/member-pool/handlers.ts:1181`, `PoolContributorList.tsx:226`]
- [x] [Review][Defer] Mononym under `shielded_name` is shown to the member and omitted on the public page (D3) — deferred: already flagged to BigDev in this story ("Question for BigDev"), ships the "show" default, and is unreachable in production (the mode has no writer). Two ratified clauses pull apart (`-189` cl.3 vs `-222` cl.2); ⛔ trigger: any writer for `public_name_presentation_mode`. (auditor P1) [`apps/api/src/modules/member-pool/handlers.ts:1209-1214`]
- [x] [Review][Defer] Nominee Console — a nominee acting as the deceased sees full legal names of the drive's contributors — deferred: recorded in AC5 / Completion Notes, not ruled. No exposure beyond the public drive page's own full-name tier under `full_name` (the member list is `≥` public by `-189` cl.3); ⛔ trigger: any surface where a nominee session can reach a name the public page withholds (the mononym arm, above). (blind+auditor P2) [`apps/mobile/components/contributor-list/PoolContributorList.tsx`]
- [x] [Review][Defer] Kill-switched Pariwar — the member list keeps showing names when the public page is off, so it becomes the only disclosure point — deferred: already carried as a residual; `-222` ruled without addressing it. (auditor P3)

#### Round 2 — the patch delta (uncommitted, on `b5aec519`), 2026-09-19

`bmad-code-review` re-run over the round-1 patches · Blind Hunter, Edge Case Hunter, Acceptance Auditor (all completed; tree unchanged) · **0 decision-needed, 6 patch (ALL 6 APPLIED same day), 0 defer, 12 dismissed.** ⛔ No new defer: the one Panel-shaped item the Auditor raised (the outage copy) is round 1's already-carried defer, ⛔ not new. ⚠ Round 1's *"every fence that scanned that logic follows it"* was ⛔ **false** — the first finding below is the correction.

- [x] [Review][Patch] Seven fences still scan `component` only, and their subject moved into `derive-contributor-row.ts` — a re-introduced `member.anonymousMember` (the ruled-out placeholder key), a retired `'unknown'` / `anonymized` arm, a `contributorLabel` / `ConfirmedRow` shadow, an inline `item.name` composition, or a relative import into `packages/contracts` would leave every test green in the new file. Retarget each to `component` + `derive` (the WIRE_READ property: `derive` must contain no `item.name` read). MEDIUM — a gate that lost its subject by the very patch that moved the code ([[feedback_gate_scope_semantic_coverage]]). (blind+edge+auditor F1) [`apps/mobile/tests/unit/contributor-list-render.test.ts` ~L378, 386, 515, 519, 525, 680-701, 1288]
- [x] [Review][Patch] Round-1 patch 7 changed a LEVEL, ⛔ not the problem — the logger defaults to `info` (`server.ts:90`), so every pool member's 60 s poll still writes one `{ memberId }` line per withheld row: same volume, same erased-member attribution. Replace the three per-row lawful-arm lines with ONE per-request `info` line of counts by cause (no member id); keep the per-row `warn` for the faults (decrypt failure, resolver failure). Add a test that pins the aggregate line, its level, and the absence of `memberId` — the level was previously unpinned, so reverting it broke nothing. (blind+edge+auditor F2) [`apps/api/src/modules/member-pool/handlers.ts:1175,1197,1231`, `apps/api/tests/unit/pool-contributors.test.ts`]
- [x] [Review][Patch] The new resolver-fault `warn` logs the whole `err` (message + stack) from code that has just handled the DECRYPTED stored name — an error message that echoes its input would put a person's name in a log, where every other arm logs by action name. Log the error's NAME only. (blind) [`apps/api/src/modules/member-pool/handlers.ts:1227`]
- [x] [Review][Patch] Five tests retargeted from `component` to `derive` carry no "Was:" note (AC10): `calls the presenter in the memoized derivation block`, `renders the PLACEHOLDER arm through t()`, `the row a11y label resolves the PRESENTER REF`, `row_a11y is called WITH a {name} param`, and the AC7 `a name row` entry. (auditor F4) [`apps/mobile/tests/unit/contributor-list-render.test.ts`]
- [x] [Review][Patch] The AC5 / family-13(d) coupling is half closed — the derivation is CALLED, but the binding of `renderable.ariaLabel` and `renderable.label` into the row JSX, and the memo's routing through `deriveContributorRow`, are unpinned (the generic "every `accessibilityLabel` is `accessible`" floor is the only cover). Pin the binding in `renderItem`. (auditor F3) [`apps/mobile/tests/unit/contributor-list-render.test.ts`, `PoolContributorList.tsx` `renderItem`]
- [x] [Review][Patch] Closure honesty in this file — (a) Completion Notes' "Deviation, stated (AC1's `Sunita .`)" still describes the OLD mechanism (*"asserts `member === public` wherever the public form is non-null"*) and the Verification block still reads "ci:local … 34 jobs green" for the pre-patch tree, with Task 9's `[x]` beside it; annotate both, ⛔ keep the text. (b) Change Log 1.1 says "both new tests mutation-proven" — three tests changed, two were mutation-checked (the resolver unit test against the restored old handler; the three-rows test by breaking `isPlaceholder`), the parity loop was ⛔ not, and no mutation record was written. State exactly that. (auditor F7 + §3) [story Completion Notes / Change Log]

#### Round 3 — the round-2 patch delta (uncommitted, on `b5aec519`), 2026-09-19

`bmad-code-review` re-run over the round-2 patches only (delta rebuilt against the state round 2 reviewed) · Blind Hunter, Edge Case Hunter, Acceptance Auditor (all completed; tree unchanged) · **0 decision-needed, 3 patch (ALL 3 APPLIED same day), 0 defer, 15 dismissed.** No high or medium code finding: the counters, the single `info` line and the retargeted fences held under trace (Edge Case Hunter: increments are synchronous and call-local; `confirmed.length` is the pending-math value; causes are mutually exclusive; no other reader of the removed per-row messages in the repo; no plaintext channel left in the resolver-fault log). ⛔ Nothing here is a Panel question (the weak one — does `-222` cl.2 reach OPERATOR logs — is a server-side, non-identifying counts line; ⛔ no client channel).

- [x] [Review][Patch] The new log test's teeth are thinner than its comment says — it claims *"reverting to a per-row line, to `warn`, or re-adding `memberId` fails HERE"*, but: (a) `infoCalls` is filtered to the `counts by cause` message, so a per-row `info` line WITHOUT a member id passes; (b) the negative message regex runs over the `warn` calls while the removed lines were `info`, so it cannot fire for them; (c) nothing asserts the `> 0` guard — a healthy pool must log NO counts line; (d) the resolver-fault payload check `not.toContain('Rajesh')` cannot fail because the thrown message does not contain the name, and re-adding the error under another key would pass. Tighten: assert the request's `info` calls are exactly the one counts line; run the cause-message regex over the `info` messages; add a healthy-pool test; make the thrown message echo the name and pin the fault payload's KEYS to exactly `['errorName','memberId']`; fix the stale *"logged by ACTION (with the error)"* comment. (blind+edge+auditor F4) [`apps/api/tests/unit/pool-contributors.test.ts` ~L86-96, L121-137, L194-195]
- [x] [Review][Patch] The counts line reads as complete but is not — fault rows (decrypt failure, resolver failure) return `{ name: null }` and are in NO counter, so `noProfile + erased + emptyAfterNormalise` does not reconcile with the unnamed rows whenever a fault exists, and the line is silent when the only unnamed rows are faults (the per-row `warn` covers them). By design, but the message *"confirmed rows rendered unnamed (counts by cause)"* claims more than it holds. Reword it to say it counts the LAWFUL causes and that faults log per row; say so at the counters; and state at the decrypt-failure `catch` why logging `err` is safe there (it fails before any plaintext exists), so the asymmetry with the resolver arm is a recorded choice, ⛔ not an accident. (blind+edge+auditor F7/F6) [`apps/api/src/modules/member-pool/handlers.ts` ~L1155, L1191, L1243-1252]
- [x] [Review][Patch] Closure honesty in the round-2 records — (a) the ledger entry `2026-09-19j` says *"`done` stands"*, but `done` has ⛔ never been committed: HEAD carries `review`, and entry `i` (`review → done`) is itself in this uncommitted diff; state the transition THIS commit makes, `review → done`. (b) The SCOPE note, Change Log 1.2 and ledger `j` list `friction:check` among things re-verified on the final tree — AC9 itself says it diffs `${baseRef}...HEAD` and *"passes vacuously until you commit"*; say ⛔ not meaningful until committed. (c) The SCOPE note lists typecheck for api/mobile/ui but ⛔ not `astro check` for `apps/public` (round 1 edited a comment there) and words the six-package unit run loosely — run it and state exactly what ran. (d) Task 4's *"Keep logs member-attributed and name-free"* is unannotated though the lawful arms are now counted with no member id; annotate, ⛔ keep the text. (auditor F1/F2/F3/F5) [story Task 4 / Completion Notes SCOPE / Change Log 1.2; `sprint-status.yaml` ledger `2026-09-19j`]
...
**Verification (Task 9), all run 2026-09-19:** `pnpm typecheck` 20/20 · `pnpm lint` 20/20 (after the irregular-whitespace fix in the Debug Log) · `env -u DATABASE_URL pnpm test` 37/37 tasks · `pnpm i18n:check` green · `astro check` (`apps/public`) 0 errors / 0 warnings · ⭐ the live-DB specs RUN AGAINST `:5433` (they self-skip in `turbo test`): `apps/api` `tests/integration/{contributions,public-pages}` + `tests/unit` → 53 files, 523 passed / 1 skipped · `pnpm friction:check` run AFTER the feat commit `2255f909` → "declaration affirmed with no new row (11b.21)", passed · ⭐ **`pnpm ci:local` (with `DATABASE_URL`) → PASSED, 34 jobs green, `integration-tests` included.**
  ⚠ **SCOPE, stated at code review 2026-09-19 (⛔ the result above is KEPT):** that run was against the tree at `b5aec519`. The two code-review rounds then changed code and tests (see Change Log 1.1 / 1.2). ⛔ `ci:local` has **NOT** been re-run on the final tree. Re-verified instead, on the final tree (round 3, 2026-09-19): `tsc --noEmit` in `apps/api`, `apps/mobile` and `packages/ui`; `astro check` in `apps/public` (0 errors / 0 warnings / 0 hints — round 1 edited a comment there); eslint on the touched files; the unit suites of `apps/api` (423), `apps/mobile` (541), `apps/public` (670) and `packages/ui` (257) — ⚠ `packages/contracts` (1135) and `packages/i18n` (110) were last run in round 2 and are untouched by the round-2/3 patches; the live-DB `contributions` + `public-pages` specs on `:5433` (135/135); and `microcopy:check` / `domain-invariants:check` / `i18n:check-parity`. ⛔ **`friction:check` is NOT meaningful until the work is committed** — AC9: it diffs `${baseRef}...HEAD`, so on an uncommitted tree it passes vacuously; an earlier note listing it as re-verified overstated. ⭐ Run `pnpm friction:check` after committing, and `pnpm ci:local` before merge.
...
⚠ **SUPERSEDED at code review, 2026-09-19 (⛔ the sentence above is KEPT as the record):** *"wherever the public form is non-null"* was the loop's `if (pub !== null)` **skip**, which also skipped every case where BOTH forms should be `null`. The loop now asserts `member === public` for EVERY case except an explicit `MEMBER_SEES_MORE` set (`shielded_name|Sunita .`, `shielded_name|Ravi`), where it asserts the public form is `null` and the member's is not.
...
- `apps/mobile/components/contributor-list/derive-contributor-row.ts` (new — code review 2026-09-19: the ONE per-row derivation the component and its test both call)
...
| 2026-09-19 | 1.1 | `bmad-code-review` (Blind Hunter, Edge Case Hunter, Acceptance Auditor; full branch diff): 0 decision-needed, 7 patch, 4 defer, 18 dismissed — all 7 patches applied. Name resolver moved inside a per-row `try` (a throw is that row's `{ name: null }`, ⛔ never a whole-list `assigned:false`); the lawful withheld arms (no profile, erasure sentinel, empty-after-normalise) log at `info`, decrypt/resolver faults stay `warn`; the per-row derivation extracted to `derive-contributor-row.ts` so the AC5 test CALLS it (⛔ no longer a transcription) and every fence that scanned that logic follows it; stale prose annotated; AC1 annotated in place for the `"Sunita ."` mononym arm and the parity loop's skip replaced by an explicit `MEMBER_SEES_MORE` set; missing "Was:" notes added; the member-route outage / un-attested-KMS cost re-marked in `deferred-work.md`. Verified: typecheck + lint clean; unit suites green (api 421, mobile 540, public 670, ui 257, contracts 1135, i18n 110); live-DB (`twt-test-pg` :5433) contributions + public-pages 135/135; both new tests mutation-proven (fail against the pre-patch code). ⚠ `ci:local` NOT re-run after the patches. Status → `done`. ⚠ **CORRECTED by 1.2 (text kept):** *"every fence that scanned that logic follows it"* was FALSE (seven did not); the log change was a LEVEL change only; and *"both new tests mutation-proven"* miscounted — three tests changed, two were mutation-checked, the parity loop was not. | BigDev + Claude |
| 2026-09-19 | 1.2 | `bmad-code-review` ROUND 2 over the round-1 patch delta (Blind Hunter, Edge Case Hunter, Acceptance Auditor): 0 decision-needed, 6 patch, 0 defer, 12 dismissed — all 6 applied. ⭐ **(1)** the seven fences that still scanned `component` only (`contributorLabel`, `ConfirmedRow`, `anonymized`, `anonymousMember`, `'unknown'`, the `item.name` wire-read floor, the `packages/contracts` path) now scan `derive-contributor-row.ts` too. **(2)** the lawful withheld arms (no profile, erasure sentinel, empty-after-normalise) are COUNTED and logged ONCE per request at `info` with ⛔ no member id — round 1's `warn`→`info` had changed only a level (the logger defaults to `info`), leaving one `{ memberId }` line per withheld row per member per 60 s poll; the faults keep a per-row `warn`. **(3)** the resolver-fault `warn` logs the error's NAME only, ⛔ never `err`. **(4)** five retargeted tests gained "Was:" notes. **(5)** a new pin for the memo → `deriveContributorRow(item, poolLetterCode, t)` call and the row's `renderable.ariaLabel` / `renderable.label` binding. **(6)** these record corrections. Mutation-proven this round, each planted then reverted: a per-row erasure line with a member id → the new log test fails; an `'unknown'` arm, an `anonymousMember` string and an `item.name` read in `derive` → three fences fail; swapped `deriveContributorRow` arguments and a dropped `accessibilityLabel` binding → the new pin fails. ⛔ The parity loop and the derive three-rows test are NOT mutation-recorded beyond round 1's `isPlaceholder` check. Verified on the final tree: typecheck (api, mobile, ui) clean; eslint clean on the touched files; unit suites api 422, mobile 541, public 670, ui 257, contracts 1135, i18n 110; live-DB `contributions` + `public-pages` 135/135 on `:5433`; `friction:check`, `microcopy:check`, `domain-invariants:check`, `i18n:check-parity` pass. ⚠ `pnpm ci:local` NOT re-run. ⚠ **CORRECTED by 1.3 (text kept):** `friction:check` on an uncommitted tree passes vacuously (AC9) and is ⛔ not evidence; `astro check` for `apps/public` had not been run. | BigDev + Claude |
| 2026-09-19 | 1.3 | `bmad-code-review` ROUND 3 over the round-2 patch delta: 0 decision-needed, 3 patch, 0 defer, 15 dismissed — all 3 applied. ⭐ **(1)** the log tests now have teeth: the request's `info` calls about a withheld row must be exactly the one counts line (with or without a member id), no lawful arm may `warn` at any wording, a healthy pool logs NO counts line (new test), and the resolver-fault payload's keys are pinned to `['errorName','memberId']` with a thrown message that echoes the decrypted name. **(2)** the counts message now says it counts the LAWFUL causes and that faults log per row; the counters and the decrypt-arm `err` logging (safe: it fails before any plaintext exists) are commented as recorded choices. **(3)** record corrections: `friction:check` is not evidence on an uncommitted tree; `done` has ⛔ never been committed (HEAD carries `review`); Task 4 annotated; the SCOPE note lists exactly what ran. Mutation-proven this round, each planted then reverted (handler restored byte-identical): a per-row `info` with no member id, a per-row `warn` in new wording, the `> 0` guard removed, and the error re-added under another key → each fails its test. Verified on the final tree: `tsc` clean (api, mobile, ui); `astro check` (public) 0 errors; eslint clean on the touched files; unit suites api 423, mobile 541, public 670, ui 257; live-DB `contributions` + `public-pages` 135/135 on `:5433`; `microcopy:check` / `domain-invariants:check` / `i18n:check-parity` pass. ⚠ `ci:local` NOT re-run; `friction:check` NOT meaningful until committed. | BigDev + Claude |
