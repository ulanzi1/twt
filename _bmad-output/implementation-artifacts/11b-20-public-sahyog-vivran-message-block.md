...
| **`11b-3b`** | ⭐⭐ **TWO things** — the **deceased member's name** AND `fixedAmount` / `rosterSize` | `ready-for-dev` — ⛔ not built |

⚠⛔ **⛔ THE `11b-3b` DEPENDENCY IS ⛔ NOT ONE THING, AND THAT IS THE POINT.** ⭐ Most readers will
assume it is only the name. `sahyog-vivran.ts:52-56` says otherwise: the **amount** *"MOVES to
**11b.3b**, which adds that dependency … 11b.3b will need `rosterSize` and `fixedAmount` to feed the
presenter."* ⇒ ⛔ **both of the headline's two tokens are 11b-3b's**, ⛔ not one.
...
⇒ ⭐ **the honest posture is to ⛔ not ship until `11b-3b` lands.** ⛔ That is what the Preflight says.
...
⇒ ⭐ `fixedAmount` / `rosterSize` arrive **with `11b-3b`**, ⛔ not ahead of it, and ⛔ not from here.
...
- [ ] ⛔⛔ **Task 0 — THE PREFLIGHT GATE.** ⛔ Do ⛔ not start until **`11b-19`** AND **`11b-3b`** are
      `done` **and merged**. ⚠ Re-read this story's Traps at that point — ⭐ `11b-3b` will have changed
      this page.
...
- ⭐ `packages/contracts/src/public-pages/sahyog-vivran.ts:25-31` (⛔ no deceased name), `:44-58` (⛔ no rupee figure; `11b-3b` adds `fixedAmount`/`rosterSize`; ⛔ do not pre-add), `:287` (`accountHolderName`), `:344` (`district`, nullable), `:383` (`confirmedContributionCount`)
