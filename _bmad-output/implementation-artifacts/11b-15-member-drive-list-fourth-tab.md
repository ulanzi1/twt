...
- [x] [Review][Decision] ✅ **RULED 2026-09-09 (BigDev): DEFER THE LINK TO STORY F; AC3's FLOOR IS AMENDED BY NAME.** ⛔ No code change. `drive_href` and `pool_canonical_identifier` are discharged at the **DATA layer** for this story — both ride the wire — and the RENDER belongs to `11b-17`, which owns the per-drive detail view and is `ready-for-dev`. ⭐ Amended EXPLICITLY rather than left silently unmet ([[feedback_supersede_never_reinterpret]]); see the AC3 amendment note. ⚠ This is what keeps AC5's `accessibilityRole="text"` ruling intact — a row that LOOKED tappable and did nothing is the failure AC5 forbids. **`drive_href` and `pool_canonical_identifier` ride the member wire and are ⛔ NEVER rendered**

> ⭐⭐ **AMENDED BY NAME BY STORY `11b-17` (2026-09-14) — ⛔ THE SENTENCE ABOVE IS KEPT, ⛔ NOT DELETED**
> ([[feedback_supersede_never_reinterpret]], [[feedback_record_unattested_no_backfill]]).
> ⛔⛔ **BOTH HALVES OF THE BOLD CLAIM ARE ⛔ FALSE AT HEAD**, and `11b-17` AC8 is what made them false:
> **(1)** the row is ⛔ no longer `accessibilityRole="text"` — `DriveRow` now carries
> `accessibilityRole="button"` **with a real `onPress`** that pushes `/(contribution)/drive/[driveToken]`;
> **(2)** `drive_href` (as `publicToken`) and `pool_canonical_identifier` **ARE** rendered — on
> `11b-17`'s per-drive detail screen, which AC8 ratifies.
> ⭐ **THE AC5 CONCERN ITSELF IS DISCHARGED, ⛔ NOT OVERRIDDEN.** AC5 forbade *"a row that LOOKED
> tappable and did nothing"* — ⛔ the defect was ⛔ never the ROLE, it was the **ABSENT DESTINATION**.
> `11b-17` built the destination, so the row may now truthfully announce itself as a control. ⇒ this is
> AC5's condition **MET**, ⛔ not waived.
> ⚠ The paired fence moved with it: `apps/mobile/tests/unit/drive-list-render.test.ts` previously
> asserted the row had ⛔ no `onPress` and ⛔ no `button`/`link` role; it now asserts the **opposite**,
> amended BY NAME in the same commit rather than deleted quietly.
> ⚠⛔ **WHY THIS NOTE EXISTS AT ALL:** `11b-17` AC8 ordered *"amend E's AC5 record **AND** that test BY
> NAME — ⛔ never delete either quietly."* ⭐ The TEST half shipped with the story; ⛔ **this RECORD half
> did ⛔ not**, and was ⛔ not routed either — found by the 2026-09-14 Group-C code review, which traced
> `grep` hits for this filename in `11b-17`'s story file and in `deferred-work.md` and got **ZERO in
> both**. ⇒ the honest word for the interval was **NOT ADDRESSED** ([[feedback_closure_language_precision]]). — `publicToken`'s own contract doc-block says it is on the wire *"because AC3's floor includes `drive_href`"*, yet its ⛔ only use is `keyExtractor` (`MemberDriveList.tsx:298`); `poolCanonicalIdentifier` appears ⛔ nowhere in `apps/mobile`. The public index links every row. ⚠⛔ **AND THE TWO ACs PULL OPPOSITE WAYS:** AC3 puts `drive_href` in the floor, while **AC5 rules the row `accessibilityRole="text"`, ⛔ never `button`/`link`, because story F (`11b-17`) owns the per-drive detail view.** ⇒ rendering a link here may be story F's scope, ⛔ or AC3's floor may be unmet. [`apps/mobile/components/drive-list/MemberDriveList.tsx`; `packages/contracts/src/contributions/member-drive-list.ts`]
