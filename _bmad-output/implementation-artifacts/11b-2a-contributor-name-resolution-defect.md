...
- [x] [Review][Patch] `mapWithConcurrency` doesn't stop sibling worker loops when one `fn` call rejects — `Promise.all` rejects immediately but other workers keep pulling from the shared counter and keep executing (e.g. further KMS calls) after the batch is already treated as failed [apps/api/src/modules/kyc/bounded-decrypt.ts] — ✅ FIXED: a `stopped` flag set on any worker's rejection is checked at the top of every worker's loop.
- [x] [Review][Patch] Anti-duplication regex guard in the bounded-decrypt drift test only matches unindented, non-exported, `function`-keyword declarations at start-of-line — an indented, `export`-prefixed, `var`, or arrow-function re-declaration of `DIRECTORY_DECRYPT_CONCURRENCY`/`mapWithConcurrency` would slip past the exact guard the test exists to enforce [apps/api/tests/unit/bounded-decrypt.test.ts:102-103] — ✅ FIXED: regex widened to cover leading whitespace, `export`, `var`, and the arrow-function form.
- [x] [Review][Patch] Test asserts `MEMBER_STATE_REPLAY_CHUNK_SIZE > 8` (a hardcoded magic number) instead of importing and comparing against the actual `DIRECTORY_DECRYPT_CONCURRENCY` constant it's meant to stay decoupled from — if that constant is later raised, the test keeps passing even if the two "must never collapse" values converge [apps/api/tests/unit/bounded-decrypt.test.ts] — ⚠ APPLIED WITH A CORRECTION: `@twt/domain` cannot import from `apps/api` (one-directional workspace dependency; the reverse would be a layering violation and would re-couple the two deliberately-independent quantities). Fixed instead by naming the literal `KNOWN_DECRYPT_CONCURRENCY_AT_TIME_OF_WRITING` with a comment explaining the boundary and flagging that it needs manual sync if `DIRECTORY_DECRYPT_CONCURRENCY` changes [packages/domain/tests/member/batched-member-states.test.ts].
- [x] [Review][Patch] Test asserts `DIRECTORY_DECRYPT_CONCURRENCY < 50` (hardcoded magic number for "the public directory's max page size") instead of importing `PUBLIC_DIRECTORY_PAGE_SIZE_DEFAULT` — a future page-size change wouldn't be caught [apps/api/tests/unit/bounded-decrypt.test.ts] — ✅ FIXED: imports `PUBLIC_SURFACE_PAGE_SIZE_CAP` (50) from `@twt/contracts` directly (the actual cap the comment meant, not the unrelated `_DEFAULT` of 25) — same package tree as the test, no layering issue.
- [x] [Review][Defer] Raw-JSON leak assertions (`.not.toContain('anonymized')` / `'anonymousMember'`) are coarse substring checks that would false-fail against any future, legitimately-named field or value containing those substrings [apps/api/tests/integration/contributions/pool-contributors-rtbf.spec.ts] — deferred, pre-existing test-robustness pattern class, low risk today
- [x] [Review][Dismiss] `sprint-status.yaml` "new" `last_updated_prior:` key — verified against the file's pre-existing rotation convention already used by the 11b-2/11b-2b entries before this diff (`git show` on the prior commit shows the same rotation); not a new schema introduced here
- [x] [Review][Dismiss] Sibling-story claim ("11b-2 shipped `done` with the anonymized arm dropped") called unverifiable by a no-context reviewer — verified true via `git show 6028581`: `member.anonymousMember` is explicitly commented "DELIBERATELY ABSENT... 11b.2a's D6(a) removed it"
- [x] [Review][Dismiss] RTBF test fixture forces every member through `active → withdrawal_completed → rtbf_anonymized` — the domain state machine (`packages/domain/src/member/state.ts:196`) also allows a direct `active → anonymized` transition, but the code under test only branches on the final resolved state, not the transition path, so this is immaterial to correctness
...
_Independent 3-layer adversarial review over the **whole stacked range `80e0d12..HEAD`** — all three stories at once. ⭐ Its subject is the SEAMS BETWEEN THE STORIES, ⛔ not a re-run of the three passes that closed this row. **2 decision-needed, 12 patch, 2 deferred, 1 dismissed.** ⛔ NO ROW IS FLIPPED by the writing of this section._

**Findings owned by 11b.2a (the RTBF defect + the decrypt bound). The full cross-story list lives in all three story files, routed by owner.**

- [x] [Review][Decision] **The erasure guarantee ENDS AT THE WIRE — an RTBF'd contributor keeps rendering from the MMKV-persisted query cache for up to 7 days, in plaintext, and survives sign-out**
  — `usePoolContributorsQuery` sets no `gcTime`, so it inherits `apps/mobile/lib/query-client.ts:18` (`gcTime: 7d`); `apps/mobile/components/Provider.tsx:18` persists via `PersistQueryClientProvider` with `maxAge: 7d`; the persister is MMKV `twt-p0-5-cache` over `createMMKV({ id: 'twt-p0-5' })` with ⛔ **no `encryptionKey`** (`apps/mobile/lib/mmkv.ts:14-16`) — plaintext at rest; `signOut()` (`apps/mobile/lib/session-context.tsx:80-90`) clears the session and member drafts and ⛔ **never** calls `queryClient.clear()`/`removeQueries` nor touches the cache key. ⚠ **Verified live, all five call sites.** ⭐ **Honest severity bounding:** online, the 60s poll replaces the cached list on the next tick, so the practical window is short; the two real exposures are (a) an **offline / unopened** device, where the erased member's `firstName + lastInitial` renders for up to a week, and (b) **account switch on a shared device**, where the cache is not scoped to the member who filled it. ⛔ This substrate is **PRE-EXISTING** (P0-5 / Story 8.3) and 11b.2a's scope was the API — so this is ⛔ **not** a regression this story introduced. ⇒ **The decision is whether 11b.2a's erasure guarantee is RULED to end at the wire, or to extend to the device.** Three candidate dispositions: (i) rule the wire is the boundary and RECORD it in the story + `.decision-log.md` (the guarantee is currently silent on this, which is the actual gap); (ii) set a short `gcTime` on this one query; (iii) clear the query cache on `signOut`. ⛔ Do not treat (i) as a no-op — an unrecorded boundary is what let three review passes miss it.
- [x] [Review][Patch] **The new batched lifecycle read at step (6a) is the ONE step in the pipeline with no degradation path — any failure tells an assigned member "you have no live pool"** [`apps/api/src/modules/member-pool/handlers.ts:319-322`]
  — `await memberDomain.getCurrentMemberStates(tx, confirmed.map(c => c.memberId))` sits outside every try/catch in `resolveContributorList`; the only catch above it (`:163-167`) converts **any** throw into `CONTRIBUTOR_LIST_UNASSIGNED`. ⭐⭐ **The diff identifies this exact consequence one layer down and guards against it** — `:355-357`: *"Unguarded, this rejected `mapWithConcurrency` and collapsed the whole surface into CONTRIBUTOR_LIST_UNASSIGNED — telling an assigned member 'you have no live pool'."* ⇒ **the per-row guard this story added is bypassed by the batch read the same story added.** ⚠ **Reachability, traced honestly** ([[feedback_trace_reachability_before_escalating]]): the malformed-UUID trigger is **REFUTED** — `ContributionConfirmedPayloadSchema` pins `memberId: z.string().uuid()` (`packages/domain/src/contribution/events.ts:150`), so `22P02` on the `uuid` column is unreachable. What remains is a statement timeout / cancellation / connection fault on a query that is an unprojected `SELECT *` over every confirmed contributor's **entire** stream, chunked at 500, on a surface polled every 60s per active member. ⇒ **The fix is unambiguous and SAFE:** catch, log, and fall through with the set unfiltered — because the `ANONYMIZED_SENTINEL` plaintext check (`:389`) is snapshot-independent by ruling (Decision `2026-08-31-170` cl.1) and still omits every erased row on its own. ⛔ Do NOT fail the response closed; ⛔ do NOT add a per-row state re-check (Trap 1 / AC2 forbid it by name).
  ⚠⚠ **RESOLVED DIFFERENTLY — THE PROPOSED FIX WAS TRACED AND FOUND TO BE THEATRE (2026-09-01).** ⛔ ⭐ **No try/catch was added, deliberately.** Two of the three review layers proposed one; tracing it showed it cannot fire and cannot help: (a) `replayMemberState` is a **TOTAL reducer** — *"never throws on a well-formed event"* (`packages/domain/src/member/state.ts:15`), `.safeParse` throughout — so there is ⛔ no in-process throw to rescue; (b) every remaining failure mode is a **SQL/connection error, which ABORTS the scope tx**, so a catch here only lets execution reach the next statement, which returns `25P02`, and the surface collapses anyway. ⇒ ⭐ **The finding was REAL and the remedy was WRONG.** It is resolved by the sibling patch below (`isAbortedTransaction`) plus a corrected coverage comment — and the unguarded call now carries a ⛔-block explaining why hardening it would be theatre, so the next reviewer does not re-file it ([[feedback_trace_reachability_before_escalating]]).
- [x] [Review][Patch] **The per-row `getMemberKycProfile` catch cannot distinguish a transaction-wide abort from a per-row anomaly — one transient fault logs N FALSE integrity anomalies against real members** [`apps/api/src/modules/member-pool/handlers.ts:349-359`]
  — `mapWithConcurrency` at concurrency 8 now issues up to 8 concurrent reads on the **single pinned pg client** the scope tx holds (`apps/api/src/modules/multi-tenant/scope-tx.ts:34-46` — `pool.connect()` + `BEGIN`); node-postgres queues them, so ⛔ no corruption — but once any one errors, Postgres aborts the tx and every subsequent statement returns `25P02`, each caught at `:352` and logged as a name-integrity anomaly **with that member's id**. ⚠ **This burst is NEW:** pre-diff the loop was serial and unguarded, so the first fault propagated once; the new per-row catch converts one fault into N false attributions. ⭐ It matters more than a log-noise issue normally would, because this diff **elevates that same log to the only signal distinguishing a render failure from a lawful erasure** (`PoolContributorList.tsx:125-133`) — so operators auditing erasure completeness are pointed at the wrong members. Fix: detect the aborted-transaction class (`err.code === '25P02'`) and rethrow rather than mis-attributing.
- [x] [Review][Patch] **The RTBF integration spec HARD-CODES the erasure sentinel instead of importing it — the exact drift class this story's own AC3 forbids, in the one test that proves the leak is closed** [`apps/api/tests/integration/contributions/pool-contributors-rtbf.spec.ts:42`]
  — `const ANONYMIZED_SENTINEL = '[anonymized]'` re-types a constant the handler compares against as `memberDomain.ANONYMIZED_SENTINEL`, in a file that **already imports `memberDomain`**. ⚠ `bounded-decrypt.ts`'s own header argues at length that *"Sharing ONLY the constant while re-implementing the helper is the drift class AC3 exists to forbid"* — here the constant itself is the re-typed half. ⇒ if the sentinel is ever changed in `@twt/domain`, both `not.toContain('[anonymized]')` assertions pass **vacuously** against a string that is no longer the sentinel, and the leak test reports green over a live leak.
- [x] [Review][Patch] **Two stacked JSDoc blocks on `anonymizeCiphertextOnly`; the first describes the OPPOSITE function and contradicts the code beneath it** [`apps/api/tests/integration/contributions/pool-contributors-rtbf.spec.ts:255-272`]
  — the first block ends *"…plus the `member.rtbf_anonymized` projection"* (that is `reallyAnonymize`, defined ~20 lines later at `:287`); the second, on the same function, says it *"⛔ deliberately does NOT append `member.rtbf_anonymized`"*. ⇒ a later editor who trusts the first block and "fixes" the function silently destroys the **only TOCTOU coverage in the suite**; the premise probe would then fail and the natural repair is to delete the probe.
- [x] [Review][Patch] **`spawns no more workers than there are items` is TAUTOLOGICAL — it cannot fail** [`apps/api/tests/unit/bounded-decrypt.test.ts:47-58`]
  — it calls `mapWithConcurrency([1, 2], 8, fn)` and asserts `peak <= 2`. With only two items, ⛔ no implementation can ever exceed 2 — including one that spawns all 8 workers. Delete the `Math.min(concurrency, items.length)` clamp and write `Array.from({ length: concurrency }, …)`: six workers spawn, immediately return, `peak` is still 2, the test stays **green**. The property the name claims is not observable through `fn`; instrument worker STARTS to make it real.

**Cross-cutting — the `contribution.json` tone pass (`bb77a53`), which rides inside this review range but belongs to ⛔ NONE of the three stories.** It was reviewed by BigDev as a non-author (`08b57f2`) and that record is correct and does ⛔ not claim to discharge AC8. These two items are ⛔ not a re-litigation of that review; they are findings the Blind Hunter raised against the strings themselves, with ⛔ no spec access.

- [x] [Review][Decision] **`missed_cycle.state_a11y` was re-worded to assert NON-CONSEQUENCE, which the system's own contribution-discipline model contradicts** [`packages/i18n/locales/{en,hi}/contribution.json`]
  — en: *"This stays open, and it takes nothing away from you."* → *"This stays open, and it **doesn't affect you**."* · hi: *"इससे आपका कुछ कम नहीं होता"* → *"इससे आप पर कोई प्रभाव नहीं पड़ेगा"* (= *"this will have no effect on you"*). ⚠ The original was a statement about **standing not being diminished**; the replacement is a blanket claim of **no effect**. ⛔ A missed cycle is precisely an input to R7 gap facts ([[project_contribution_discipline_opportunity_not_elapsed_time]] — R7 counts periods where an assigned cycle closed), and the same file's `out_of_band.helpline.clarify.script` states the R7 contribution-discipline rules apply. The sibling `missed_cycle.section_note` still says *"nothing here is settled or closed"*, which the new line overreaches past. ⚠ The Hindi drifts **further** than the English. ⛔ No test in this diff covers this key. ⇒ **BigDev's call as the author of the tone pass** — is the intended promise "your standing is not diminished" (the original, and true) or "this has no effect" (the new text, and not true of R7)? A screen-reader user gets whichever one ships.
- [x] [Review][Defer] **The tone pass made two EN/HI-ASYMMETRIC edits, one of which strengthens a money-handling promise in Hindi only** [`packages/i18n/locales/{en,hi}/contribution.json`] — deferred, trigger-bound
  — `out_of_band.helpline.boundary.title` edited in **en only** (*"do not voice this"* → *"do not say this"*; operator-facing, harmless). ⚠ `amount_mismatch_over.body` edited in **hi only**: *"सुलझा देगी"* (will help resolve) → *"सही कर देगी"* (will set it right), while the untouched en string still says the helpdesk will *work with you to sort it out* — ⇒ hi-locale members reading an **overpayment** message are now told the trust will *correct* it, a firmer commitment than the en text, and the parallel sibling `amount_mismatch.body` still says *"सुलझा देगी"* in Hindi, so the two now diverge where English keeps them parallel. ⛔ Deferred rather than patched: this is a copy/tone judgement on strings BigDev authored, and a **qualified non-author sweep is already scheduled** — carried onto `CR-11b.2a-AC8-TONE`'s existing trigger (BigDev's pre-production tone sweep over the `copy_globs` namespaces) rather than minting a second one.

**✅ RESOLUTION OF THE TWO `decision-needed` ITEMS (BigDev, 2026-09-01).**

· ⭐⭐ **RTBF SCOPE → (i) THE WIRE IS THE BOUNDARY, AND IT IS RECORDED.** Minted as Decision
  **`2026-09-01-172`**. The erasure guarantee of `2026-08-30-169` cl.1 / `2026-08-31-170` cl.1 is a
  guarantee about **what the API emits**; a response already delivered to a device is outside it.
  ⛔ **No code change lands on this ruling** — both candidate mitigations touch shared app substrate
  outside all three stories' scope and neither closes both exposures. ⚠ **The residual is carried OPEN**
  as `CR-11b.2a-COMBINED-W3` with a three-way trigger, and cl.2 states explicitly that the ruling is a
  statement of **SCOPE** and may ⛔ never be cited as evidence the residual was weighed and dismissed.
  ⭐ **The gap this closes is the SILENCE, ⛔ not the cache** — three prior passes verified the erasure
  at the API and none asked where it stopped. ⚠ cl.4 routes the DPDPA disclosure question to engaged
  counsel and ⛔ does not answer it.

· ⭐ **`missed_cycle.state_a11y` → REVERT TO THE STANDING-NOT-DIMINISHED SENSE, both locales.**
  *"it doesn't affect you"* / *"इससे आप पर कोई प्रभाव नहीं पड़ेगा"* overclaims: a missed cycle **is** an
  input to R7 gap facts ([[project_contribution_discipline_opportunity_not_elapsed_time]]), and the
  sibling `missed_cycle.section_note` still says *"nothing here is settled or closed"*. The original
  sense — **your standing is not diminished** — is both true and consistent. ⇒ reclassified from
  `decision-needed` to **`patch`** and applied. ⛔ The en/hi **asymmetry** finding stays **DEFERRED**
  onto `CR-11b.2a-AC8-TONE`'s existing sweep — ⛔ no second trigger minted for the same sweep.
