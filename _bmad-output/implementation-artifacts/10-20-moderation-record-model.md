...
Code review complete across all 5 chunks (domain-core, API layer, contracts+OpenAPI, admin UI,
governance/docs) — see Review Findings below. Migration 0100 applied and live-DB verification
complete (see "Live-DB verification" note at the end of the chunk-5 findings) — it caught and fixed
two real regressions the review's own earlier patches had introduced. All live-DB + unit suites
green as of this session. All decision-needed and patch findings resolved; remaining items are
explicitly deferred with reasoning, recorded in `deferred-work.md`.
...
### Review Findings (domain-core diff — chunk 1 of 5, `packages/domain/**` only)

⚠ **Partial review.** The full branch diff vs `baseline_commit` is 58 files / ~11,245 lines —
over the chunking threshold. This pass covers only `packages/domain/**` (30 files). Remaining
chunks not yet reviewed: **API layer** (`apps/api/**`), **contracts + OpenAPI**, **admin UI**
(`apps/admin/**`), **governance/docs**. Do not treat this section as a complete code review of
Story 10.20 until the remaining chunks are run.

- [x] [Review][Decision RESOLVED — dismiss] Post-RTBF audit-signal asymmetry: `member_moderation_grounds.note_ciphertext` scrubs to `NULL` (indistinguishable from "never had a note"), while sibling `member_moderation_actions.decision_note_ciphertext` scrubs to a `NOT NULL` sentinel (preserves "a note existed here") — `packages/domain/src/member/anonymize.ts:216-227`. Resolved: each column's scrub value is correct for its own NULL-ability/semantics (grounds notes are optional so NULL means "none"; actions notes are mandatory so a sentinel is the only non-fabricating choice) — not a defect.
- [x] [Review][Decision RESOLVED — deferred] `evidence_refs` RTBF exemption ("a reference is an identifier, not prose") has no evidenced legal/DPO sign-off in the diff; a quasi-identifying ref (e.g. an external order/complaint number) may still be traceable to the member — `packages/domain/src/member/moderation/evidence-refs.ts`. Reason: technical rationale (structural non-prose enforcement) is sound but whether it satisfies DPDPA RTBF for quasi-identifying references is a legal call, not a code call — needs DPO/legal sign-off before this is closed.

- [x] [Review][Patch] RTBF scrub unconditionally sets `escalationInadequacyCiphertext`/`escalationProportionalityCiphertext` on EVERY moderation row for a member, which violates `member_moderation_actions_escalation_iff_terminate` on any non-`terminate` row (suspend/restore) and aborts the entire RTBF scrub with a `23514` — not caught by `rtbf-anonymize.test.ts` because that suite mocks the update capture rather than hitting a real schema [packages/domain/src/member/anonymize.ts:204-212]. **Fixed**: escalation columns now written via a per-row `CASE WHEN action = 'terminate' THEN sentinel ELSE NULL END`, in the same single statement.
- [x] [Review][Patch RESOLVED from Decision 1] Legality check runs after escalation/evidence shape checks in `moderateMember`, so a doomed `terminate` (not legally terminable AND missing escalation) surfaces a 422 `escalation_required` instead of the 409 `invalid_state` that `errors.ts`'s own stated intent implies should take priority. [packages/domain/src/member/moderation/write.ts:181-225]. **Fixed**: legality (2) now runs immediately after the reason-code guard (1), ahead of the escalation-presence backstop (now 2c) and evidence-refs check (now 2d).
- [x] [Review][Patch] `ESCALATION_PART_MAX_CHARS` (4000) is declared but never enforced — neither `assertPart` nor `assertImmediateTerminationReason` checks an upper bound, so unbounded plaintext reaches encryption/storage [packages/domain/src/member/moderation/escalation.ts:47-96,152-172]. **Fixed**: both now throw `ModerationEscalationRequiredError(part, 'too_long', ESCALATION_PART_MAX_CHARS)`; `errors.ts` gained the `too_long` reason and a `max_chars` details key.
- [x] [Review][Patch] `min_chars` is inconsistent for the same error code + reason (`missing`): `write.ts`'s backstop throws with `min_chars: 0` while `escalation.ts`'s route-level check throws with `min_chars: 40` [packages/domain/src/member/moderation/write.ts:196,199]. **Fixed**: both `missing` throws in `write.ts` now pass `ESCALATION_PART_MIN_CHARS`.
- [x] [Review][Patch] `supersedes_ground_id` has no DB-level guard against pointing at a different action's ground, or at an `is_primary = true` row — enforcement is app-code only in `appendModerationGround`, unlike every other invariant in migration 0099 [packages/domain/migrations/0099_moderation-record-model.sql:186; packages/domain/src/member/moderation/grounds.ts:192-211]. **Fixed**: new migration `0100_moderation-grounds-supersede-integrity.sql` adds a `BEFORE INSERT` trigger validating same-action + non-primary. ⚠ Not yet run against a live DB in this session — verify with the live-DB moderation-grounds specs before merge.
- [x] [Review][Patch RESCOPED to defer] `member_moderation_grounds.code` ↔ action-type applicability (`assertReasonCodeAppliesTo`) has no DB-level CHECK/trigger — app-code only, inconsistent with the migration's stated defense-in-depth philosophy [packages/domain/src/member/moderation/grounds.ts]. Rescoped during patch application: enforcing this at the DB layer means replicating the reason-code registry's applicability rules in SQL (an IMMUTABLE function, mirroring `moderation_evidence_refs_valid`'s pattern) — that's registry-design work, not a mechanical patch. Moved to Deferred below.
- [x] [Review][Patch, partial] `r7a_restorations_used_snapshot` (no bounds CHECK) and `dwell_policy_version` (no FK to the clause registry) get zero DB-level validation, while sibling `evidence_refs` in the same migration gets three CHECKs plus an immutable validator function [packages/domain/migrations/0099_moderation-record-model.sql:124-125]. **Fixed**: migration 0100 adds a `>= 0` CHECK on `r7a_restorations_used_snapshot`. The `dwell_policy_version` FK is deferred below — it needs the clause-registry table's exact versioning shape confirmed before a target is picked.
- [x] [Review][Patch] Duplicate `// (2)` comment-numbering in `appendModerationGround` (registry guard and supersede-target-exists steps both labeled `(2)`, then jumps to `(3)(4)(5)`) [packages/domain/src/member/moderation/grounds.ts:179-215]. **Fixed**: the supersede-target step is now labeled `(2b)`.

- [x] [Review][Defer] `resolveModerationDwellPolicy` collapses "unprovisioned" and "malformed payload" into the same `null`/503 outcome, so an admin sees "not provisioned" even for a corrupted-but-present clause [packages/domain/src/member/moderation/dwell.ts] — deferred, pre-existing class of ambiguous-error-surface issue, low severity
- [x] [Review][Defer] `getProducingSuspensionActedAt` orders by the app-injected `acted_at`, not a monotonic/DB-generated sequence — theoretical clock-skew edge case across writer instances [packages/domain/src/member/moderation/dwell.ts:153] — deferred, no evidence of multi-writer clock skew in current deployment
- [x] [Review][Defer] `moderation-dwell.test.ts`'s DST-boundary test uses only UTC (`Z`-suffixed) timestamps and doesn't exercise an actual local-timezone DST transition despite its name [packages/domain/tests/member/moderation-dwell.test.ts] — deferred, test-quality gap not a runtime defect
- [x] [Review][Defer] Live-DB negative-case CHECK coverage (evidence-shape checks) is tested only for `member_moderation_grounds`, not for the identical CHECKs installed on `member_moderation_actions` [packages/domain/tests/integration/rls/member-moderation-actions-policy-regression.spec.ts] — deferred, coverage gap not a runtime defect
- [x] [Review][Defer] Immediate-termination substance floor silently reuses `ESCALATION_PART_MIN_CHARS` from the two-part escalation test with no comment flagging the shared coupling at the reuse site [packages/domain/src/member/moderation/escalation.ts:152-172] — deferred, documentation hardening not a runtime defect
- [x] [Review][Defer] RTBF scrub for `member_moderation_grounds` filters only on `memberId` (no explicit `pariwarId` predicate), relying on RLS session scope alone for tenant isolation [packages/domain/src/member/anonymize.ts:216-227] — deferred, matches the pre-existing sibling `member_moderation_actions` scrub pattern, not a new regression
- [x] [Review][Defer, rescoped from patch] `member_moderation_grounds.code` ↔ action-type applicability has no DB-level CHECK/trigger, app-code only [packages/domain/src/member/moderation/grounds.ts] — deferred, needs a dedicated pass to replicate `assertReasonCodeAppliesTo`'s registry as a SQL IMMUTABLE function without drift risk between the two copies
- [x] [Review][Defer, rescoped from patch] `dwell_policy_version` has no FK to the clause registry [packages/domain/migrations/0099_moderation-record-model.sql:125] — deferred, needs the clause-registry table's exact versioning shape confirmed (which table/column `dwell_policy_version` should reference) before a target is picked

### Review Findings (API-layer diff — chunk 2 of 5, `apps/api/**` only)

⚠ **Partial review, continued.** This pass covers `apps/api/**` (10 files). Remaining chunks not
yet reviewed: **contracts + OpenAPI**, **admin UI** (`apps/admin/**`), **governance/docs**.

- [x] [Review][Decision RESOLVED — patch] `performAction`'s own plaintext pre-checks (`assertRationalePresent`, `assertEscalationJustification`, `assertEvidenceRefs`, `assertImmediateTerminationReason`) run BEFORE `openScopeTx`/`moderateMember` — i.e. before legality is checked ANYWHERE in the call chain, since legality only lives inside `moderateMember`. This means the chunk-1 domain-layer reorder fix (legality before escalation/evidence checks) is invisible to real HTTP callers: a `terminate` against a member who isn't even suspended, with missing escalation, still gets `422 escalation_required`, never `409 invalid_state`, because the API's own duplicate pre-check throws first [apps/api/src/modules/member-moderation/handlers.ts:187-268]. **Fixed**: an early, non-authoritative legality fast-fail (`getCurrentMemberModerationOverlay` + `nextModerationStatus`) added right after the existence check, on the ALREADY-OPEN `request.scopeTx.tx` (no extra connection). `moderateMember`'s own tx-scoped legality check remains the TOCTOU-safe authority for the actual write — this only decides which error a doomed request sees first.

- [x] [Review][Patch] `ModerationGroundAlreadySupersededError` gap (Edge Case Hunter, race condition): nothing stopped two supporting grounds from both naming the same `supersedes_ground_id` — either a concurrent `appendGround` race past the pre-check, or a stale-console caller reusing an already-superseded target [packages/domain/src/member/moderation/grounds.ts:192-224]. **Fixed**: new typed error `ModerationGroundAlreadySupersededError` (→ 409) in `errors.ts`, a pre-check in `grounds.ts`, wired into `apps/api/src/middleware/error-mapping/index.ts`, and a partial UNIQUE INDEX `member_moderation_grounds_supersedes_target_idx` in migration 0100 as the backstop. ⚠ Not yet run against a live DB.
- [x] [Review][Patch] AC12 gap: "the Decision Note and both escalation parts stay decrypt-on-demand, per-action" — but `ModerationRationaleResponse` (`packages/contracts/src/member-moderation/dto.ts:298-304`) was `.strict()` with only `moderation_action_id`/`rationale`, and no route anywhere decrypted `escalation_inadequacy_ciphertext`, `escalation_proportionality_ciphertext`, or `immediate_termination_reason_ciphertext` on demand. Confirmed by reading the CURRENT contracts DTO directly, not just the API diff. [apps/api/src/modules/member-moderation/handlers.ts (rationale handler), packages/contracts/src/member-moderation/dto.ts:298-304]. **Fixed**, spanning all three layers: `getModerationActionRationale` (domain `read.ts`) now selects all four ciphertext columns; `ModerationRationaleResponse` (contracts `dto.ts`) gained three nullable fields; the `rationale` handler decrypts all four fields concurrently, each independently fail-soft on a corrupt envelope. `openapi/v1.yaml` regenerated deterministically (`contracts:check-openapi-determinism` passes).
- [x] [Review][Patch] Sequential, not parallel, KMS round-trips on `terminate`: up to 3 sequential `await encryptModerationRationale(...)` calls (escalation inadequacy, proportionality, immediate reason) on the request's hot path, despite the adjacent comment implying they're "made here, together" [apps/api/src/modules/member-moderation/handlers.ts (performAction, escalation/immediate-reason encryption block)]. **Fixed**: all four encrypt calls (decision note + escalation × 2 + immediate reason) now run via `Promise.all`.
- [x] [Review][Patch] `GroundParam` duplicates `RationaleParam` byte-for-byte in `routes.ts` instead of aliasing it, despite a comment noting "the same shape the rationale read uses" [apps/api/src/modules/member-moderation/routes.ts]. **Fixed**: `GroundParam` now aliases `RationaleParam` directly.
- [x] [Review][Patch] `terminationAvailableAt`'s history read runs `resolveModerationDwellPolicy` and `getProducingSuspensionActedAt` sequentially — two independent lookups that could run concurrently [apps/api/src/modules/member-moderation/handlers.ts (history handler)]. **Fixed**: both now run via `Promise.all`.
- [x] [Review][Patch] Four copy-pasted, byte-identical error-mapping blocks for `ModerationEscalationRequiredError`/`ModerationEscalationNotApplicableError`/`ModerationEscalationRestatementError`/`ModerationEvidenceRefInvalidError` in `error-mapping/index.ts` — a code→status map would remove the duplication risk [apps/api/src/middleware/error-mapping/index.ts]. **Fixed**: consolidated into `MODERATION_RECORD_SHAPE_ERROR_CLASSES` + `isModerationRecordShapeError`, one block instead of four.

- [x] [Review][Defer] `appendGround`'s `note` has no domain-layer substance-floor backstop for a non-HTTP caller (unlike every sibling Tier-1 field) — HTTP callers are already protected by the contracts `DecisionNote` schema (`.trim().min(1)`) [apps/api/src/modules/member-moderation/handlers.ts (appendGround)] — deferred, HTTP surface already safe, only a non-HTTP-caller defense-in-depth gap
- [x] [Review][Defer] The 42501-grant revert-sanity test proves the one permitted `note_ciphertext` UPDATE doesn't throw, but never asserts `rowCount`/re-reads state, so a predicate typo matching zero rows would still pass [apps/api tests, grounds RLS revert-sanity] — deferred, test-quality gap not a runtime defect
- [x] [Review][Defer] The 403/`auth.step_up_required` test for a wrong-context elevation on `appendGround` checks only the status code, not that no ground was written, unlike its sibling refusal tests — deferred, test-quality gap not a runtime defect
- [x] [Review][Defer] No test asserts entry-level `evidence_refs` (from the action row) agrees with the primary ground's `evidence_refs` — the two lists can silently diverge — deferred, test-coverage gap not a runtime defect
- [x] [Review][Defer] `AVAILABLE_AT`/`JUST_BEFORE` in `moderation-dwell.spec.ts` are hand-computed literal `Date` strings rather than derived from `SUSPENDED_AT`/`DWELL_DAYS` — deferred, test-quality gap not a runtime defect
- [x] [Review][Defer] `moderation-dwell.spec.ts`'s cleanup omits `events_log`/`members` with no comment, unlike its sibling `moderation-escalation.spec.ts` which explicitly documents the same omission — deferred, documentation gap not a runtime defect
- [x] [Review][Defer] Test setup boilerplate (`authenticate`/`grant`/`elevate`/`seedActiveMember`/`errCode`) is duplicated near-verbatim (~60-70 lines each) across `moderation-dwell.spec.ts`/`moderation-escalation.spec.ts`/`moderation-grounds.spec.ts` instead of a shared fixture — deferred, test-maintainability gap not a runtime defect
- [x] [Review][Defer] `error-mapping/index.ts` carries roughly 60 lines of policy prose (Q4.1, Q4.2, D5, D6) for ~35 lines of logic, none of it living near the domain code it describes — deferred, stylistic/documentation-placement concern

Dismissed (with reasoning): `produceContributionFacts` throwing (vs. returning `null`) inside the open termination tx is correct fail-safe behavior, not a re-introduction of the rejected hard-gate — the rejected gate was specifically about an *unprovisioned* (null) fact, not a genuine infra fault, confirmed by reading `producer.ts:550` (`r7aRestorationsUsed: consecutiveRequired === null ? null : ...`, never a throw for "unprovisioned"). The new `apps/api → @twt/validity-service` coupling is a deliberate, extensively-documented, ratified design (Q5(a)), not an oversight. Edge Case Hunter's "no re-check of current member standing on `appendGround`" is out of model — grounds are historical annotations to a past decision, not a live-standing assertion. "Pre-existing termination tests now exercise only the exception path" is already mitigated by this same diff's new `moderation-dwell.spec.ts`, which exists specifically to cover the ordinary path, and the choice is explicitly justified in the test file's own header comment. "Not self-verifying as the API layer" (calls into domain functions not shown here) is a chunking artifact — the domain layer was already reviewed separately in chunk 1.

### Review Findings (contracts + OpenAPI diff — chunk 3 of 5, `packages/contracts/**` + `openapi/v1.yaml`)

⚠ **Partial review, continued.** This pass covers `packages/contracts/**` + `openapi/v1.yaml` (6 files). Remaining chunks not yet reviewed: **admin UI** (`apps/admin/**`), **governance/docs**.

- [x] [Review][Decision RESOLVED — dismiss] The Decision Note's wire field name is still `rationale` in both `ModerateMemberRequest` and `ModerationRationaleResponse` — AC12 lists `decision_note` as one of "every new field" requiring the domain-camelCase ↔ contracts-snake_case boundary walk. [packages/contracts/src/member-moderation/dto.ts (ModerateMemberRequest, ModerationRationaleResponse)]. Resolved: AC12's field list tracks the domain concept's naming, already satisfied by chunk 1's DB-column rename (`rationale_ciphertext` → `decision_note_ciphertext`); a wire-field rename would be a breaking API change AC12 doesn't clearly mandate, and the admin-UI-copy rename it separately describes ("the field is now the Decision Note") is chunk 4's concern.

- [x] [Review][Patch] AC6's minimum-substance floor (`MODERATION_ESCALATION_MIN_CHARS = 40`) was declared and drift-tested against `@twt/domain`'s `ESCALATION_PART_MIN_CHARS`, but `EscalationPart`'s Zod schema used `.min(1)`, not `.min(MODERATION_ESCALATION_MIN_CHARS)` — confirmed independently by all three review layers, and by reading the schema directly. A 1-character `"x"` passed contract validation and `openapi/v1.yaml` faithfully mirrored the bug (`minLength: 1`); only the domain's `assertPart`/`assertImmediateTerminationReason` backstop actually caught it. Same schema backs `escalation_inadequacy`, `escalation_proportionality`, AND `immediate_termination_reason` — all three were affected. [packages/contracts/src/member-moderation/dto.ts:59]. **Fixed**: `EscalationPart` now uses `.min(MODERATION_ESCALATION_MIN_CHARS)`; added a behavioral test (`member-moderation-evidence-refs.test.ts`) that actually exercises the floor via `ModerateMemberRequest.safeParse`, not just a numeric-equality pin; `openapi/v1.yaml` regenerated deterministically.
- [x] [Review][Patch] `AppendModerationGroundRequest`/`AppendModerationGroundResponse` — a REAL, live write endpoint (`POST .../moderation/:moderationActionId/grounds`, confirmed wired in `apps/api/src/modules/member-moderation/routes.ts` in chunk 2) — had ZERO representation in `openapi/v1.yaml`: `emit-openapi.ts` never imported either DTO and never called `registerPath` for the route. Confirmed by reading the emit script directly: it is a manually-maintained registry, not auto-discovered from Fastify routes, so a new route requires an explicit registration that was never added. [packages/contracts/scripts/emit-openapi.ts]. **Fixed**: both DTOs registered as components, a full `registerPath` block added (matching the route's actual params/step-up/error surface from chunk 2's handler), regenerated deterministically.
- [x] [Review][Patch] AC12/AC7 gap: two of AC5 item 7's three ruled governance-fact columns (`r7a_restorations_used_snapshot`, `dwell_policy_version`) were completely absent from the contracts/OpenAPI layer — not in any response DTO, not in any test, not in the OpenAPI spec. Confirmed the domain reader (`listModerationHistoryForMember`) didn't even SELECT these columns. Since both are non-PII (a bounded integer and a clause-version id, deliberately never RTBF-scrubbed per chunk 1's review), they don't need decrypt-on-demand gating and belong on the regular history list, same as `action`/`reason_code`. [packages/domain/src/member/moderation/read.ts, packages/contracts/src/member-moderation/dto.ts]. **Fixed**, spanning all three layers: the domain reader now selects both columns; `ModerationHistoryEntryDto` gained two nullable fields; the API `history` handler maps them through. `openapi/v1.yaml` regenerated deterministically.

- [x] [Review][Defer] `EVIDENCE_REF_PATTERN` (`^[A-Za-z0-9][A-Za-z0-9._/-]{0,63}$`) permits `.`/`/` combinations that could form traversal-shaped strings (e.g. `a/../../secret`) — flagged independently by Blind Hunter and Edge Case Hunter. Verified via `grep` that NOTHING in the current codebase resolves `evidence_refs.ref` against a filesystem or object store (it is stored and displayed as an opaque JSONB identifier only), so there is no live exploit path today. Deferred as defense-in-depth hardening for whenever a real storage/file resolver consumes this field — tightening the regex then requires synchronized changes to both the domain and contracts copies plus their drift test. `packages/domain/src/member/moderation/evidence-refs.ts`, `packages/contracts/src/member-moderation/evidence-refs.ts`.
- [x] [Review][Defer] Output-side `evidence_refs` arrays (`ModerationGroundDto.evidence_refs`, `ModerationHistoryEntryDto.evidence_refs`) use bare `z.array(EvidenceRefDto)` with no `.max()`, unlike the inbound `EvidenceRefsDto` (capped at `EVIDENCE_REFS_MAX`) — benign today since the DB-side CHECK constraints (migration 0099) bound what can ever be stored to 10, but the read DTOs don't structurally mirror that cap. `packages/contracts/src/member-moderation/dto.ts` (ModerationGroundDto, ModerationHistoryEntryDto).
- [x] [Review][Defer] `AppendModerationGroundResponse` echoes back only `ground_id`/`moderation_action_id`/`code`/`supersedes_ground_id`/`added_at` — omits `evidence_refs` and `has_note`, so a caller that just appended a ground with evidence references has no way to confirm what was persisted without a follow-up GET on history. `AppendGroundResult` (domain) doesn't carry those fields either, so fixing this spans three layers again (same shape as the two patches above) — deferred rather than compounding this chunk's scope further; a follow-up GET already exists as a workaround. `packages/domain/src/member/moderation/grounds.ts` (AppendGroundResult), `packages/contracts/src/member-moderation/dto.ts` (AppendModerationGroundResponse).
- [x] [Review][Defer] The AC12 drift-guard test (`member-moderation-evidence-refs.test.ts`) covers only the evidence-ref primitives and the (now-fixed) escalation floor — `is_primary`, `ordinarily_results_in`, `supersedes_ground_id`, `termination_available_at`, and `decision_note` have no drift assertion anywhere, despite AC12 naming all of them for the boundary walk. Test-coverage gap, not a runtime defect.
- [x] [Review][Defer] Response-side `grounds` array on a history entry has no upper bound — a heavily-disputed action with many append+supersede cycles ships an arbitrarily large list with no truncation signal. No domain-level cap on total grounds per action currently exists; capping it would be a domain-layer design decision (how many is too many?), not a contracts-only fix.
- [x] [Review][Defer] Submitting the same `{kind, ref}` pair twice in one `evidence_refs` array is not rejected — a data-quality nicety, not a correctness or security issue.

Dismissed (with reasoning): Edge Case Hunter's two suggested `.refine()`s ("both escalation parts or neither present," "immediate reason requires escalation") are deliberately NOT enforced at the DTO layer — the schema comment above `ModerateMemberRequest` explicitly documents why: the `iff` is enforced "where it can see the action" (the route handler's `assertEscalationJustification`, backed by the domain presence backstop and the DB `escalation_iff_terminate` CHECK), specifically BECAUSE one shared request schema serves suspend/terminate/restore and cannot see which action a given request targets. `added_by_display`/`actor_display` being an unbounded `z.string()` is a pre-existing pattern across the ENTIRE contracts package (not introduced by this diff) and the value is server-controlled (`users.display_name`, an admin-managed field), not attacker-influenced free input — out of this diff's scope to fix in isolation. The YAML anchor-renumbering churn Blind Hunter flagged as a "process risk" is standard, expected `zod-to-openapi`-generator behavior (confirmed `openapi/v1.yaml` is machine-generated by `emit-openapi.ts`, not hand-maintained) — not a defect. "Migration 0099 is leaned on for guarantees not in this diff" is a chunking artifact — already reviewed in chunk 1.

### Review Findings (admin UI diff — chunk 4 of 5, `apps/admin/**`)

⚠ **Partial review, continued.** This pass covers `apps/admin/**` (5 files). Remaining chunk not yet reviewed: **governance/docs**.

- [x] [Review][Patch] `confirm()` re-reads `escalationInadequacy`/`escalationProportionality`/`evidenceRefs` from live component state and sends them WITHOUT re-running the length/restatement/pattern checks `requestSubmit` performed before opening the modal — flagged independently by Blind Hunter and Edge Case Hunter, confirmed by reading `confirm()` directly. [apps/admin/src/modules/member-status/ModerationStrip.tsx:262-293]. **Fixed**: extracted `validateEscalationGroup()`, shared by `requestSubmit` and `confirm()`; a failure inside `confirm()` now closes the modal and shows the same group-level error `requestSubmit` would have shown.
- [x] [Review][Patch] `dwellOpen` fails OPEN on a malformed `termination_available_at`: `new Date(malformed).getTime()` is `NaN`, and `NaN > Date.now()` is `false` — silently treated as "dwell elapsed," skipping the re-confirmation/immediate-reason flow AC8 requires. [apps/admin/src/modules/member-status/ModerationStrip.tsx:188-190]. **Fixed**: explicit `Number.isNaN` guard, now fails toward showing the warning (treats unparseable as OPEN, not elapsed).
- [x] [Review][Patch] AC12's Decision Note rename is incomplete: `revealRationale: 'Show rationale'` and `rationaleUnreadable: 'This rationale cannot be read...'` were left untouched while sibling strings were renamed — confirmed directly. [apps/admin/src/modules/member-status/i18n-en.ts:174-176]. **Fixed**: copy updated to "Show Decision Note" / "This Decision Note cannot be read..." (key names unchanged — internal identifiers only).
- [x] [Review][Patch] The escalation-inadequacy, escalation-proportionality, and immediate-termination-reason textareas all reused `MODERATION_DECISION_NOTE_MAX_CHARS` — confirmed no dedicated escalation-max constant existed in contracts, only harmless today because the two numbers happen to be equal. [apps/admin/src/modules/member-status/ModerationStrip.tsx:412,462,485,654]. **Fixed**: added `MODERATION_ESCALATION_MAX_CHARS` to contracts (drift-tested against domain's `ESCALATION_PART_MAX_CHARS`), wired into all three fields; `openapi/v1.yaml` regenerated (byte-identical, value unchanged).
- [x] [Review][Patch] No duplicate-evidence-reference guard — confirmed no dedupe check existed. [apps/admin/src/modules/member-status/ModerationStrip.tsx]. **Fixed**: `requestSubmit`/`validateEscalationGroup` now rejects a repeated `(kind, ref)` pair with a new `evidenceDuplicateError` message.
- [x] [Review][Patch] The form-level error box was gated by translated-string identity instead of an error code/enum — confirmed. [apps/admin/src/modules/member-status/ModerationStrip.tsx:578-580]. **Fixed**: `validationError` is now `{ field: 'reasonCode' | 'rationale' | 'group'; message: string } | null`, and all three render sites match on `.field`, not string identity.
- [x] [Review][Patch] Evidence rows were keyed by array index (`key={i}`) — confirmed. [apps/admin/src/modules/member-status/ModerationStrip.tsx:507-508]. **Fixed**: each row now carries a client-only stable `key` (a ref-based counter), stripped before the submit payload is built.
- [x] [Review][Patch] Inconsistent evidence-reference rendering: the per-ground list dropped `kind`, the action-level block showed the raw untranslated kind slug. **Fixed**: both now share a `formatEvidenceRef` helper rendering `<translated kind>: <ref>`, using the same `t.evidenceKinds` lookup the form's own `<select>` uses.

- [x] [Review][Defer] Unsafe cast on the evidence "kind" `<select>` (`e.target.value as EvidenceRefDto['kind']`) with no runtime check — technically unsound, but constrained in practice since the `<select>`'s own `<option>`s are generated exhaustively from `EVIDENCE_REF_KINDS`, so no actually-invalid value can reach it via normal UI interaction. Type-safety hygiene, not a functional defect. `apps/admin/src/modules/member-status/ModerationStrip.tsx:519`.
- [x] [Review][Defer] Required-field asterisks are `aria-hidden` with no compensating `aria-required`/`required` attribute on the associated textareas (escalation-inadequacy, escalation-proportionality, immediate-reason) — screen-reader users get no non-visual signal these fields are mandatory before hitting a validation error. Accessibility gap, not a runtime defect.
- [x] [Review][Defer] `t.dwellOpenBody.replace('{date}', ...)` uses `String.prototype.replace` with a string argument, which only replaces the FIRST occurrence — fine today (one `{date}` placeholder), fragile if a future translation needs it twice.
- [x] [Review][Defer] Test coverage is asymmetric between the two 40-character floors: there's a dedicated test for the escalation-parts length floor, but no equivalent test for the immediate-termination-reason floor's below-floor-but-nonempty case (only the fully-empty case is tested).
- [x] [Review][Defer] No test exercises the server-rejection (422) paths for the four new conditional fields (`escalation_inadequacy`, `escalation_proportionality`, `evidence_refs`, `immediate_termination_reason`) to verify the UI surfaces that failure sensibly.
- [x] [Review][Defer] No per-row error for an invalid evidence reference — with multiple rows, the operator has no indication of WHICH row is invalid, only a single generic form-level error.

Dismissed (with reasoning): Edge Case Hunter's claim that a `restore`-guidance code renders "Not moderated" as if contradicting its own meaning is FALSE — verified by reading `statusForGuidance`: `restore` correctly maps to `'none'` because a restore action's resulting standing genuinely IS "none" (unmoderated); the function's own doc comment says exactly this ("the standing it produces"). The supporting-ground picker AC12 lists as missing from the terminate form is NOT a silent gap — the story's own Task 10 notes explicitly name it, explain why (no atomic contract field / can't make two requests atomic with the action), and point to where it's recorded (`docs/moderation-record-model.md`), matching this project's established [[feedback_record_unattested_no_backfill]] discipline. The Cancel button not resetting `confirmError`/`immediateReason` is not an actual bug — both persist TOGETHER, so a shown error remains factually accurate for the still-present (also-persisted) input; there is no inconsistency, just retained state. The admin UI's anti-restatement check being "cosmetically strict but substantively weak" is the SAME deliberate, documented tradeoff already reviewed and dismissed at the domain layer in chunk 1 (the admin UI's own comment says it "mirrors the domain's `normalizeEscalationPart`" on purpose). Comments citing `epics.md:NNNN` line anchors are this codebase's established, consistent citation convention throughout — not a defect introduced by this diff. `guidanceFor` being called twice per render is a trivial style nit against a ~10-item list, not worth tracking.

### Review Findings (governance/docs diff — chunk 5 of 5, `.decision-log.md` + routing note + `docs/moderation-record-model.md`)

⚠ **Final chunk.** Scoped to the three files that are genuine story-authored governance content. The full `git diff` for "governance/docs" also touches this story file, `deferred-work.md`, and `sprint-status.yaml` — excluded here because they are now dominated by THIS review session's own additions (findings, patches-applied notes, ledger entries), and adversarially reviewing my own just-written review notes would not serve the review's purpose.

- [x] [Review][Decision RESOLVED — patch] `.decision-log.md` Decision `2026-08-12-099` clause 8.1 asserted "✅ **CONFIRMED by the Trustee Panel, 2026-08-12**: brief principle 3 (*Proportionality*) STANDS as §8.6 clause 3. The reading above is **ratified, not merely author-committed**, and this sub-clause is **closed**" — but the SAME decision's outer Status line says clause 8 in its entirety is "**Author-committed**... neither is presented as ratified." Confirmed independently by Blind Hunter AND the Acceptance Auditor; verified further against the commit history (`f92699c` frames the reasoning as author-side, needing "no new Panel ruling because it changes no substance") and the routing note (no trace of a second Panel session anywhere). [.decision-log.md:110,193,205]. **Resolved**: no real second confirmation occurred — this was a drafting error. Per this project's own rule ("decisions, once ratified, are never edited in place"), Decision `099`'s text is left untouched and a new **Decision `2026-08-12-101`** supersedes the erroneous status claim, correcting it to author-committed while leaving the underlying reasoning (which changes no substance) intact.

- [x] [Review][Patch] AC2 requires recording the ABSENCE of an `APPENDIX A — RULE INDEX` entry when a Panel-ruled Niyamavali addition isn't an indexable `R`-rule. Decision `2026-08-12-099` never includes this disposition anywhere in its full content — while every comparable precedent decision in this SAME log includes it verbatim. [.decision-log.md, Decision 2026-08-12-099]. **Fixed**: recorded as an Addendum to the new Decision `2026-08-12-101` (not edited into `099`'s original text, per this log's own "never edit in place" rule).
- [x] [Review][Patch] Migration `0100` (added during THIS code-review session) and its live-DB-unverified status were not mentioned anywhere in `docs/moderation-record-model.md` §8 "What this model does NOT close." [docs/moderation-record-model.md §8]. **Fixed**: added as row 8.
- [x] [Review][Patch] `docs/moderation-record-model.md` §7's flat "safe... to keep out of the RTBF scrub" claim didn't carry the DPO/legal-sign-off caveat this session's own chunk-1 review already recorded. [docs/moderation-record-model.md §7, §8]. **Fixed**: §7 now explicitly separates the engineering argument from the open legal question, cross-referencing new §8 row 9.

- [x] [Review][Defer] Decision `100` Clause 2 files "Prior sanction required — the dwell it named as missing is now enforced" under a clause TITLED "What did NOT change" — reads as contradictory on a skim, even though the underlying logic (continuity of which row needed fixing, vs. a new fact layered onto it) is defensible once read closely. Wording-clarity issue, not a factual error.
- [x] [Review][Defer] The dwell boundary's exact inclusive/exclusive comparison (`now >= suspended_at + 7 days` vs `>`) is stated in code (verified correct in chunk 1) but never spelled out in the governance text — a documentation-completeness gap, not a code defect.
- [x] [Review][Defer] Whether an individual Pariwar's dwell registry clause may diverge from the ratified "7 days" (per-Pariwar override vs. Trust-wide constant) is not addressed anywhere in the governance text.
- [x] [Review][Defer] `docs/moderation-record-model.md` §8 row 3 justifies `NOT VALID`'s grandfathering with "the table is append-only" — but append-only describes `member_moderation_grounds`, not `member_moderation_actions` (the table the CHECK is actually on). The real justification (GRANT posture / no UPDATE path on that table) is stated correctly elsewhere in the same doc but not cross-referenced from this row.
- [x] [Review][Defer] Decision `100` invokes "Decision `2026-08-12-099` clause 10" as the standing rule authorizing future §8.4a disclosure-row flips, but clause 10 is itself labeled `[Author-finding]`, not Trustee-ratified — an authority-chain question (can an author-finding rule govern future updates without itself being ratified?) worth a dedicated look, not urgent.
- [x] [Review][Defer] Symbol vocabulary (⛔/⚠/✅) carries several distinct meanings throughout with no legend — consistent with this project's established (if informal) documentation style, low priority.

Dismissed (with reasoning): Blind Hunter's "provenance contradiction" (the routing note appears as a `new file` in the diff despite Decision 099 claiming it was "committed alone") is FALSE — verified via `git log --follow`: the file genuinely WAS committed alone first at `5ea5213` ("route seven questions to the Trustee Panel — the note ALONE, before any code"), then amended by three later commits that added the ruling and subsequent reconciliations. A branch-vs-baseline diff correctly shows it as "new" relative to the pre-branch state; this is fully consistent with the claim, not a contradiction. The "obligation queue only ever grows" observation reflects this project's deliberate governance-debt-tracking design ([[feedback_record_unattested_no_backfill]]), not a defect. The undefined ⛔/⚠/✅ symbol vocabulary is the established convention across the entire codebase's documentation, not specific to this diff. The "grammatical inference" critique of the "in particular... emphasis not enumeration" reading is commentary on argumentation style, already counterbalanced by the decision's own four-pronged (a)-(d) justification — not a factual defect. The Q4/Q4.4 "numbering drift" between the routing note's table and Decision 099's collapsed bullet is cosmetic; both describe the same ratified fact (7 days, versioned registry) without contradiction.

### Live-DB verification (post-review, all 5 chunks)

Migration `0100` applied cleanly against the live test DB (`twt-test-pg`, `:5433`) — trigger,
partial unique index, and CHECK constraint all confirmed present via `\d`. Running the full live-DB
integration suite then caught **two real regressions this review's own patches had introduced**,
both fixed and re-verified:

1. **Check-order regression (from the chunk-2 legality pre-read patch).** The API's new early
   legality fast-fail ran before EVERYTHING, including the reason-code applicability check that only
   happened inside the domain — so a request invalid on both axes (illegal transition AND an
   inapplicable reason code) now got `409` instead of the `422` a live-DB regression test
   (`member-moderation.spec.ts`, "pins the check ORDER") explicitly pins as correct, matching
   `moderateMember`'s own established priority (reason-code vocabulary objection before legality).
   **Fixed**: added a synchronous `assertReasonCodeAppliesTo` pre-check, ordered before the legality
   fast-fail, mirroring the domain's own (1)-before-(2) priority.
2. **Escalation-floor regression (from the chunk-3 Zod `.min()` patch).** Wiring
   `MODERATION_ESCALATION_MIN_CHARS` into `EscalationPart`'s `.min()` made Fastify reject a
   too-short value with a generic `400` before the domain's `assertPart` ever ran — but two live-DB
   specs (`moderation-escalation.spec.ts`, `moderation-dwell.spec.ts`) pin the domain's SPECIFIC
   typed `422` (`member_moderation.escalation_required`, with a `reason`/`min_chars` detail) for
   exactly this case. **Fixed**: reverted `EscalationPart` to `.min(1)` — the schema now rejects only
   the truly-empty case; the substance floor is intentionally the domain's job, one layer in. The
   `MODERATION_ESCALATION_MIN_CHARS` constant stays exported for the admin console's client-side hint
   and the drift-guard test, just not wired into this `.min()`.

Both fixes were caught by tests already present in the diff (not new tests added for this) — the
original story author had anticipated exactly these scenarios. After both fixes: domain live-DB (22)
+ API live-DB (100, across all 7 moderation/trustee-lite integration spec files) + domain unit (202)
+ contracts (28) + admin (35) all green; typecheck/lint clean across domain, contracts, api, admin;
`openapi/v1.yaml` regenerated and deterministic.

**This is the intended lesson of running live-DB verification as a distinct, later step**: two fixes
that looked correct under typecheck, lint, and every DB-free unit test were wrong in ways only a real
database and real HTTP round-trip could reveal.
