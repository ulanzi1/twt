// Local-filesystem SelfVerifyScreenshotStorage — Story 9.7 (Task 2, Decision D1).
//
// The dev/CI fallback when `SELF_VERIFY_SCREENSHOT_BUCKET` is unset. Persists bytes to a SHARED directory
// on disk so separate local processes (apps/api's upload + a future Story 9.8 reviewer reading the blob)
// resolve to the SAME object — the same reasoning as the 6.5 / 9.3 local-fs adapters (an in-process Map is
// invisible across processes). `signedReadUrl` returns a deterministic non-secret `file://` URL. Local dev
// / CI only; production always sets `SELF_VERIFY_SCREENSHOT_BUCKET` (the live GCS adapter).
...
export interface LocalFsSelfVerifyScreenshotStorageOpts {
...
const DEFAULT_ROOT_DIR = join(tmpdir(), 'twt-self-verify-screenshots-dev');
...
/**
 * Construct a filesystem-backed `SelfVerifyScreenshotStorage`. Object keys (always caller-minted, opaque
 ...
 */
export function createLocalFsSelfVerifyScreenshotStorage(
  opts: LocalFsSelfVerifyScreenshotStorageOpts = {},
  ...
  function resolveObjectPath(key: string): string {
    ...
    if (p !== rootDir && !p.startsWith(rootDir + '/')) {
      ...
      throw new Error(`[local-fs-self-verify-screenshot-storage] key escapes root: '${key}'`);
    }
    return p;
  }
  ...
  return {
    ...
    async getBytes(key) {
      ...
      try {
        ...
        if (code === 'ENOENT') {
          throw new Error(`[local-fs-self-verify-screenshot-storage] no object at key '${key}'`);
        }
        throw err;
      }
    },
