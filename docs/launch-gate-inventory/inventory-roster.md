...
- **current_status:** `conditional-escalation-pending-predicate` <annotated 2026-08-01 (Story 10.8 Review Pass 5): tooling DELIVERED and the predicate's second conjunct now false — see cross_story_discharge_path — but the row status is NOT flipped on a per-Story author-commit. Ratification of ADR-0036 (`drafted` → `ratified`) is the event that disposes this row. Corrects a 2026-07-31 flip to `predicate-foreclosed-pending-trustee-disposition`, a token absent from this row's own closure_criteria and from `closure-criteria-rubric.md`'s vocabulary; the Row 15 precedent (Story 7.9, identical shape — tooling delivered, ratification pending) preserved the prior status and annotated instead.>
...
### Row 17 — `directory-kill-switch-admin-ui`

- **gate_name:** Kill-switch administrative UI required before the public Member Directory goes live
- **architecture_source_line:** N/A — **Trustee-ratified decision-authored** (Decision `2026-08-21-147` cl.1); not an architecture-verbatim row and not PRD/epics-authored. ⚠ The FIRST row in this roster originating from a **code-review ratification** rather than from architecture, PRD or epics — recorded as such rather than back-fitted to a source line it does not have ([[feedback_record_unattested_no_backfill]]).
- **owner:** Trustee Panel (gate); BigDev + John (discharge path)
- **support:** Amelia (implementation)
- **closure_criteria:** Substantive: an administrative UI for the per-Pariwar directory-publication kill switch (`pariwar.manage_directory_publication`, super_admin-only) is **shipped and operable by a human without database access**, AND a Decision supersession entry records ≥2-trustee ratification that the switch may now be treated as an **operational control**. Testable signal: the Epic 10 story is `done`; a super_admin can disable and re-enable a Pariwar's directory **through the console**, both directions, with the required rationale + actor snapshot + §1.5 audit line written by that path; and this row's `closure_evidence_link` resolves to the ratifying Decision. ⛔ **A `done` story alone does NOT close this row** — per this roster's closure-status-aggregation discipline, closure needs the ≥2-trustee ratification leg too.
- **target_date:** before the public Member Directory is exposed to the internet (relative-to-fact: the gate IS the exposure event — the directory may not go live first, so there is no date independent of it).
- **current_status:** `open` — ⛔ **and the gated surface is BUILT, which is what makes this row load-bearing rather than anticipatory.** Story 11a.3 is `done`: `/members` renders real members' full legal names and the kill switch exists and is enforced in code. What does ⛔ **not** exist is any way to operate the switch without direct database manipulation, which Decision `2026-08-21-146` cl.5(b) forbids describing as normal operation and `2026-08-21-147` cl.1 forbids relying on as a fallback.
- **closure_evidence_link:** (empty — populated on closure) · Authority: [Decision 2026-08-21-147](../../.decision-log.md#decision-2026-08-21-147) (cl.1 — the launch-gate designation; cl.2 — Epic 10 ownership) + [Decision 2026-08-21-146](../../.decision-log.md#decision-2026-08-21-146) (cl.5 — "NOT an operational control until a dedicated admin UI ships"; direct DB manipulation ⛔ must not be described as normal manual operation) + [Decision 2026-08-21-145](../../.decision-log.md#decision-2026-08-21-145) (cl.5 — the kill-switch substrate, disclosed as having shipped implementation-first)
- **missed_target_escalation_log:** (empty)
- **cross_story_discharge_path:** The **Epic 10** kill-switch admin-UI story — **`_bmad-output/implementation-artifacts/10-30-directory-publication-kill-switch-admin-ui.md`** (`10-30-directory-publication-kill-switch-admin-ui`), authored 2026-08-21 and implemented under [Decision 2026-08-21-148](../../.decision-log.md#decision-2026-08-21-148). ⚠ **The story existing and being implemented walks the FIRST of this row's two closure legs ONLY** — ⛔ `current_status` stays `open` and `closure_evidence_link` stays empty until the ≥​2-trustee ratification leg lands, per this row's own closure-status-aggregation discipline ("⛔ A `done` story alone does NOT close this row"). ⚠ **Epic 11a carries the DEPENDENCY, ⛔ not the implementation** (`2026-08-21-147` cl.2): the UI belongs to the admin-console surface, not the public-surface epic. ⛔ **NOT** bundled into 11a.4 (phone/email obfuscation). Discharge is (a) the story ships → ≥2-trustee ratification → row `closed`; OR (b) the Panel disposes the row `accepted-risk` / `deferred-per-named-criteria` at a monthly review, which would ⛔ require an explicit ruling that the directory may go live with no operable pull-lever.
- **notes:** ⭐ **A `done` story and a LIVE surface are different facts, and this row is the seam between them.** 11a.3 built the directory correctly and closed cleanly; this gates its **exposure**. ⚠ **The fallback this row removes was implicit, not written:** before `2026-08-21-147` the honest answer to *"how do we pull a Pariwar today?"* was hand-run SQL — that answer is now **withdrawn**, so there is ⛔ **no** operational pull-lever for the directory at all, which is the whole reason the surface may not go live. ⚠ **AND THE GATE DOES NOT END AT THE UI:** even once it ships the control is ⛔ **not immediate** — `/members` is declared `cache_policy: edge_cacheable` with `s-maxage=300`, so a pulled Pariwar keeps being served real member names from every warm PoP, **per page number**, until those entries expire (`2026-08-21-145` cl.5(e)). ⛔ The word *"immediate"* may not be used about this control, and the Epic 10 story ⛔ must not imply otherwise. ⚠ **Interaction with Row 3 / `2026-08-19-136` cl.5 (DPDPA):** the directory publishes full legal names on an unauthenticated page while DPDPA counsel is ⛔ not engaged; this gate and that question now bind the **same event** (going live) but ⛔ neither discharges the other.
