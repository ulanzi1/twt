...
| State | Design | Source |
| --- | --- | --- |
| **Not yet published (no member has ever appeared)** | Dignified not-yet-published card: `not_published_title` ("The member directory is not published yet") + `not_published_body`, which says plainly that **nothing about the reader's membership changes** and that **no member details are shown on this page today**. The branded shell (header + language toggle + footer) still renders. ⚠ **CORRECTED at Story 11a.3** — this row previously read *"the primary state today"* and *"there is not a placeholder standing in for a failed read — **there is no read**"*. ⛔ Both are now FALSE: there IS a read (the `apps/api` hop), a failed read has its own dedicated row below, and the populated row is the primary state. | `members.astro`; `lib/members-render.ts` (`buildMembersView`, `hasMembers: false`, `apiUnavailable: false`, `pastEnd: false`); keys `not_published_title` / `not_published_body` |
...
| **⭐ API unavailable (the upstream read failed)** | A `role="alert"` card: `unavailable_title` ("The directory could not be loaded") + `unavailable_body`, which says plainly that this is **a problem on our side**, that it does **⛔ not** mean the directory is empty, and that **nothing about anyone's membership has changed**. ⚠ **CORRECTED at Story 11a.3 (`2026-08-21-145`)**: the page returns **503 + `Retry-After: 60` + `Cache-Control: no-store`**, ⛔ no longer 200. It previously returned 200 under `public, max-age=60, s-maxage=300`, which pinned "could not be loaded" into any shared cache for five minutes for **every** visitor, long after the upstream recovered — a page that says it failed must not be cached, and must not tell a crawler or an uptime check that it succeeded. The branded shell still renders. ⛔ One attempt, no retry: a retry storm on a public page turns a slow upstream into a self-inflicted outage. | `lib/directory.server.ts` (`fetchMemberDirectory` → `ok: false`); `lib/members-render.ts` (`apiUnavailable: true`); keys `unavailable_title` / `unavailable_body` |
