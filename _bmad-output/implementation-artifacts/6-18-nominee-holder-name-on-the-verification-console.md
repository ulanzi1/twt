...
- [ ] **Task 7 — Tests (AC1–AC12, incl. **AC10**)** ⚠ **v1.1: the header range read `(AC1–AC10)`, which excluded **AC11** and **AC12** — the very ACs this task's own subtasks test, and AC11 is named in its next sentence. **AC10** was an ORPHAN AC: ⛔ no task tagged it, and the only `AC10` match in the task list was this range notation, ⛔ not a tag.** ⚠ **RE-OPENED by code review 2026-09-20 (chunk 4):** AC7's note has no end-to-end test, AC11's `effectiveOutcome()` arm has none at any layer, checklist family 2 (two-connection races) is unproven for every new write, the P1/P3/P4 gate matrix is mostly absent, and several ticked lines exercise the D1-reversed window or the un-built halves of Tasks 5–6. The Dev Agent Record's test counts (domain 3427 · api 1317 · contracts 1149 · events 36 · 7,046) are ⚠ stale after the D1/D3/D4 patches and are ⛔ not verified by this review (the reviewers are read-only and the specs need the live DB). (AC1–AC10) — execute on `twt-test-pg` `:5433`; "written but not run" is ⛔ not
  ...
  - [ ] No-comparison fence over the read, the write and the gates (Trap 1, AC4).
        ⚠⚠ **v1.1 — UN-TICKED: false for "the gates".** `FENCED_FILES` covers the check, the persist
  ...
  - [ ] Both bank-write windows exclude the freeze states (AC4).
        ⚠⚠ **v1.1 — UN-TICKED: the test asserts a property the code does ⛔ NOT have.** The spec titled
...
> ⚠⚠ **v1.1 — THIS TOTAL IS UN-ATTESTED, AND THE STORY CARRIES TWO OF THEM.** This table and the sprint
> ledger say **api 1,333 / total 7,122**; the Change Log v1.0 row says **api 1,327 / total 7,116**. Both
> arithmetics are internally consistent, so one figure is simply wrong — and ⛔ **nothing in the repository
> attests either**: the commits carry no run output. ⚠ A 2026-09-21 attempt to settle it FAILED for an
> instructive reason: the runner's `DATABASE_URL` pointed at the **dev** database on `:5432`, which lacks
> migration `0116`, so every live-DB spec died on `42703` (undefined_column) — **80 false failures**. The
> mirror-image trap is this repo's own unit gate, which runs `env -u DATABASE_URL` and **skips live specs
> SILENTLY**. ⇒ a green and a red here can both be artefacts of one environment variable.
> ⭐ **Record it as un-attested rather than repeat it** ([[feedback_record_unattested_no_backfill]]); re-earn
> it on `twt-test-pg :5433` with `0116`–`0118` applied, and state the DB in the claim.
