...
**Derivation:** Epic 9 (12 stories — 9.1 through 9.12): 3 medium + 5 complex + 4 very complex = 6+20+28 = 54 story-points × 4 hr/pt = 216 hr raw. Bank-parser format-discovery overhead: 5 banks × golden-test-file corpus review (+15%): 216 × 1.15 = 248 hr. CI/ADR overhead: 50% (bank PII FR-74 + every contribution status transition emits FR-100 + Story 1.10 audit-log entries; property-test coverage for bank-parser determinism per §3 + methodology §4(f) Epic 9 ladder entry) → 248 × 1.50 = 372 hr ÷ 346 hr/month = 1.07 months computed midpoint. **Assumption-catalogue adjustment (per Tasks 7+8 review P-03):** computed midpoint 1.07 → operational midpoint 0.94 on the grounds that (i) the 5-bank allowlist v1 scope is a property of the spec, not a parsing complexity multiplier — AI generates per-bank format adapters efficiently once a single canonical adapter pattern is established (the first bank takes ~15 hr, banks 2-5 take ~5 hr each, not 15 hr each); (ii) the +15% format-discovery premium already double-counts AI's structured-extraction speedup. The 0.94 midpoint reflects this efficiency without violating the round-number-forbidden discipline. Low-band asymmetric formula per methodology §3 (factor = 1.0): floor = 0.94 / 2 = 0.47; ceiling = 0.94 × 2 = 1.88. Low-band ratio 1.88 ÷ 0.47 = 4.0 ✓ — formula consequence per P-13 tautological-band disclaimer.
