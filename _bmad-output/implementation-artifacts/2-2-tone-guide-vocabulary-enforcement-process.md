...
- A structured `ToneReviewRequiredError` (with a `toErrorResponse(requestId)` projector) mirrors `AuthorizationDeniedError`; it maps to **HTTP 409** with a `tone-review-required` code via the error-mapping middleware (matches Story 2.4 AC L1490).
...
- Story 2.4 consumer contract (409 `tone-review-required`; tone-reviewer attribution + `clause_version_id` on publish) [Source: epics.md L1484–1490].
